...
public class TargetCompileTest extends CompileTestHelper {
	...
	public class ResizingHelper
	{
		...
		public void help()
		{
			...
			c = CWin.getWindowTarget().getWindow();
			Rectangle d = c.getBounds();
	        r.mousePress(c,d.width-2,d.height-2,MouseEvent.BUTTON1_MASK);
	        try {
				Thread.sleep(20);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
	        r.mouseMove(c,d.width+4,d.height+2);
		}
	}
	
	public static ResizingHelper instance;
	
	public void testResizing()
	{
		instance=new ResizingHelper();
		
        ClassLoader cl = compile("  program\n",
                "mywindow window('test window'),at(10,10,100,100),ratio('fixed'),system,resize,max,timer(1)\n",
                "   button('hello'),use(?b1),at(10,10,80,20),ratio(300,300,400,400)\n",
                "   button('world'),use(?b2),at(50,80,40,20),ratio('bottom right')\n",
                ".\n",
                "  code\n",
                "  open(mywindow)\n",
                "  accept\n",
                "		if event()="+Event.TIMER+"\n",
                "			@java-code 'org.jclarion.clarion.compile.TargetCompileTest.instance.help();\n",
                "       .\n",                
                "  .\n",
                "  close(mywindow)\n",
                "");
