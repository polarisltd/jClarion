...
public class ClassExprType extends ClassedExprType {
    ...
    public Expr field(Expr in, String field,Scope callingScope) {
        ...
        if (v!=null) {
            return new VariableExpr(new DecoratedExpr(JavaPrec.POSTFIX,v.getType(),null,in,"."+v.getJavaName()),v);
        }
    ...
    public Expr destroy(Expr in) {
        return method(in,"destruct",new Expr[0]);
    }

    @Override
    public boolean isDestroyable() {
        Procedure p = group.matchProcedureThisScopeOnly("destruct",new Expr[0]);
        return (p!=null);
    }
