...
public abstract class Variable implements JavaDependency,VariableUtiliser
{
    ...
    public VariableExpr getExpr(Scope callingScope)
    {
        ...
        if (callingScope!=getScope() && getScope()!=null) {
            ...
            if ((getScope() instanceof MainScope) || (getScope() instanceof ModuleScope) ) {
    ...
    /**
     * Get expression used to construct this variable. 
     * 
     * Result is 3 element array. 
     *  0 = construction expression
     *  1 = simple construction expression. used for expressions with an init component on declaration of the variable.
     *  2 = init expression to issue post construction. 
     * 
     * @return
     */
    ...
    public final void generateInitCapable(StringBuilder buffer)
    {
    	generateInitCapable(buffer,true);
    }

    public void generateInitCapable(StringBuilder buffer,boolean definition)
    {
    	if (definition) {
    		generateDefinition(buffer);
    	} else {
	...
	public void setRelocatedScope() {
		this.relocated=true;
	}
	
	public boolean isRelocatedScope()
	{
		return relocated;
	}
