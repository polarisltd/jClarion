...
public class AbstractParser 
{
    ...
    public AbstractParser(ClarionCompiler compiler,Lexer input)
    {
    	this.compiler=compiler;
        _lex=input;
    }
    
    public AbstractParser(ClarionCompiler compiler,Parser parser)
    {
    	this.compiler=compiler;
        this.parser=parser;
    }
    ...
    protected final void error(String errmsg)
    {
    	...
    	compiler.error().error(lexer(),l.lineNumber,l.charStart,l.charEnd,errmsg,null,false);
    }

    protected final void error(String errmsg,Throwable t)
    {
    	Lex l = lexer().lookahead();
    	compiler.error().error(lexer(),l.lineNumber,l.charStart,l.charEnd,errmsg,t,false);
    }

    protected final void error(String errmsg,Lex item)
    {
    	compiler.error().error(lexer(),item.lineNumber,item.charStart,item.charEnd,errmsg,null,false);
    }
    ...
    static Set<String> realTypes = GrammarHelper.list(
            "real","sreal" );

    public String getAliasType(String type)
    {
        String alias = compiler.getScope().getAlias(type);
    ...
    public String collapseType(String type) {
        ...
        if (realTypes.contains(type)) return "real";
    ...
    public boolean emptyLex()
    {
        ...
        if (la.value.equalsIgnoreCase("include")) {
            ...
            if (section!=null) {
                ...
                Lexer sub = compiler.source().getLexer(name);
    ...
    protected void include(String name) {
    	...
    	LocalIncludeCache lic = compiler.getLocalIncludeCache(name);
    	
        if (compiler.getScope().isIncludedAlready(name,false)) {
            lic.alsoInclude(compiler,name);
            return;
        }
        compiler.getScope().addInclude(name);
        compiler.collator().associate(compiler.source().cleanName(name),compiler.getScope());
        ScopeSnapshot ss =lic.getIncludeSnapshot(compiler.source(),name);
        ...
        if (ss==null) {
            ...
            Scope scope = compiler.getScope();
            
            ScopeSnapshot old_ss=scope.getSnapshot();

            lic.enterInclude();
            
            WarningCollator.get().pushLexer(l);
            try {
            	...
            	WarningCollator.get().popLexer();
            }
            
            if (compiler.getScope()!=scope) error("Change in Scope on include!");

            ss=scope.getSnapshot();
            ss.remove(old_ss);
            
            for ( String names : lic.getNestedIncludes().keySet() ) {
            	compiler.collator().associate(compiler.source().cleanName(name),compiler.getScope());            	
                ScopeSnapshot nss = lic.getIncludeSnapshot(compiler.source(),names);
                if (nss!=null) {
                    ss.add(nss);
                }
            }
            ...
            if (ss.isProcedureOnly()) {
            	compiler.main().getLocalIncludeCache(true).setIncludeSnapshot(compiler.source(),name, ss);
            } else {
            	lic.setIncludeSnapshot(compiler.source(),name,ss);
            }
        } else {
            lic.alsoInclude(compiler,name);            
            if (compiler.getScope().isIncludedAlready(name,true)) {
            	boolean skip=true;
            	if (ss.isProcedureOnly()) {
            		skip=false;
            	}
            ...
            compiler.getScope().mergeinSnapshot(ss);
        }
    }
    ...
    {
    	compiler.importer().process(clazzName);
    }
