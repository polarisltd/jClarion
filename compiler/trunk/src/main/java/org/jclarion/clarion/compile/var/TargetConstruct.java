...
public class TargetConstruct extends Scope 
{
    ...
    public TargetConstruct(ScopeStack stack,String label,ExprType basetype)
    {
    	super(stack);
        this.basetype=basetype;
        
        clazz=new TargetJavaClass(stack.getScope().getPackage(),this);
        stack.compiler().repository().add(clazz,stack.getScope().getPackage(),label);
        
        type = new TargetExprType(clazz.getName(),basetype,this); 
        
        var=new ClassedVariable(label,type,clazz,false,false,false,null);
