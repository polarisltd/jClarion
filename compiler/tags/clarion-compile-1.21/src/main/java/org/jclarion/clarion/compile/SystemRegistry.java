...
public class SystemRegistry {
    ...
    protected void init()
    {
        ...
        ,PatternRewriter.create(null,"notify","CWin.notify(@)",ExprType.rawint).call().range(1,3).rt("runtime.CWin")
        ,PatternRewriter.create(ExprType.rawboolean,"notification","CWin.notification(@)",ExprType.number).call().range(1,3).rt("runtime.CWin")
...
//      ,PatternRewriter.create(ExprType.any,"choose","CRun.choose($,new ClarionObject[] { @ })",ExprType.rawint,ExprType.any).call().rt("runtime.CRun","ClarionObject").min(3)
        ...
        register(
         PatternRewriter.create(JavaPrec.CREATE,JavaExprTypeMapper.importJava(IMutex.class),"newmutex","new IMutex()").call().rt("runtime.concurrent.IMutex")
        ,PatternRewriter.create(JavaPrec.CREATE,JavaExprTypeMapper.importJava(ISemaphore.class),"newsemaphore","new ISemaphore($,$)",ExprType.rawint).range(0,2).call().rt("runtime.concurrent.ISemaphore")
        ,PatternRewriter.create(JavaPrec.CREATE,JavaExprTypeMapper.importJava(ICriticalSection.class),"newcriticalsection","new ICriticalSection()").call().rt("runtime.concurrent.ICriticalSection")
        ,PatternRewriter.create(JavaPrec.CREATE,JavaExprTypeMapper.importJava(IReaderWriterLock.class),"newreaderwriterlock","new IReaderWriterLock($)",ExprType.rawboolean).range(0,1).call().rt("runtime.concurrent.IReaderWriterLock")
        );
    }
