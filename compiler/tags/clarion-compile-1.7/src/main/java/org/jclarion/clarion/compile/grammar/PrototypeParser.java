...
{
    public PrototypeParser(Parser parser, Lexer input) {
        super(parser, input);
    }
    ...
    public ExprType getType(Lex tlex)
    {
        ...
        if (type==null) type=ScopeStack.getScope().getType(tlex.value);
        
        // create placeholder type and hope it gets resolved 
        // in the near future
        if (type==null) {
            type=DanglingExprType.get(tlex.value);
            ScopeStack.getScope().addType(type);
