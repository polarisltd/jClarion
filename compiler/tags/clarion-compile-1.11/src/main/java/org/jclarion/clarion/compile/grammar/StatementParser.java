...
public class StatementParser extends AbstractParser {
    ...
    public StatementParser(Parser parser, Lexer input) {
        super(parser, input);
    }
        ...
        if (result == null //&& la().type == LexType.label
            ...
            if (e != null) {
                ...
                if (e instanceof PotentialAssignmentExpr) {
                    commit(pos);
                    try {
                        result = new ExprJavaCode(((PotentialAssignmentExpr) e)
                            .getAssignExpr());
                    } catch (IllegalStateException ex) {
                        
                        PotentialAssignmentExpr pae = (PotentialAssignmentExpr)e;

                        Expr right = pae.getRightExpr();
                        Expr left  = pae.getLeftExpr();
                        
                        if (right.type().isa(ExprType.rawint) || right.type().isa(ExprType.number)) {
                            
                            ExprBuffer eb = new ExprBuffer(JavaPrec.ASSIGNMENT,null);
                            eb.add(left);
                            eb.add("=");
                            eb.add("(");
                            eb.add(new ExprTypeExpr(left.type()));
                            eb.add(")");
                            eb.add("CMemory.resolveAddress(");
                            eb.add(ExprType.rawint.cast(right));
                            eb.add(");");
                            result=new ExprJavaCode(new DependentExpr(eb,ClarionCompiler.CLARION+".runtime.CMemory"));
                        }

                        if (result==null && 
                            (left.type().isa(ExprType.group) || left.type().isa(ExprType.any)) &&
                            (right.type().isa(ExprType.group) || right.type().isa(ExprType.any)) ) 
                        {
                            LinearJavaBlock ljb = new LinearJavaBlock();
                            
                            ExprBuffer eb;
                            
                            eb = new ExprBuffer(JavaPrec.ASSIGNMENT,null);
                            eb.add(left);
                            eb.add("=");
                            eb.add("new ");
                            eb.add(new ExprTypeExpr(left.type()));
                        ...
                        if (right.type().isa(ExprType.any)) {
    ...
    private JavaCode loopStatement() {
        ...
        if (from != null) {
            ...
            Expr forLoop;
            forLoop = new JoinExpr(JavaPrec.LABEL, null, "for (", init, null,
                    test);
            forLoop = new JoinExpr(JavaPrec.LABEL, null, forLoop, ";",
                    increment, ")");
        ...
        return new LoopJavaCode(new BlockJavaCode(openExpr,block),infinite);
    }
