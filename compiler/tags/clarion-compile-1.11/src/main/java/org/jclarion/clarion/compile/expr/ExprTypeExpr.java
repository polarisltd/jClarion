...
public class ExprTypeExpr extends Expr {

    public ExprTypeExpr(ExprType output) {
        super(JavaPrec.LABEL, output);
    }

    @Override
    public void toJavaString(StringBuilder target) {
        type().generateDefinition(target);
    }

    @Override
    public void collate(JavaDependencyCollector collector) {
        type().collate(collector);
    }
