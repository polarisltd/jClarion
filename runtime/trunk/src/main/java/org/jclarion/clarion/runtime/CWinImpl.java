...
import org.jclarion.clarion.control.CustomMnemonic;
...
import org.jclarion.clarion.swing.ChooserImagePreview;
...
public class CWinImpl implements RemoteWidget {
    ...
    public AbstractWindowTarget getTarget(int id)
    {
    	return getTarget(CRun.getThread(id));
    }
    ...
    public boolean waitWindowClose(Thread t,AbstractWindowTarget awt,int i) {
    	long until = System.currentTimeMillis()+i;
    	synchronized(stack) {
    ...
    private int    	   hookRunning;
    private List<CWinHook> newHooks=null;
    private List<CWinHook> hooks=new ArrayList<CWinHook>();
    
    public void addWinHook(CWinHook hook)
    {
    	synchronized(hooks) {
    		if (newHooks!=null) {
    			newHooks.add(hook);
    ...
    public void removeWinHook(CWinHook hook)
    {
    	synchronized(hooks) {
    		if (hookRunning>0 && newHooks==null) {
    			newHooks=new ArrayList<CWinHook>(hooks);
    		}
    		if (newHooks!=null) {
    ...
    private Iterable<CWinHook> invokeHooks()
    {
    	synchronized(hooks) {
    		hookRunning++;
    	}
    ...
    private boolean addButton(JRootPane pane, JPanel panel, int button, int def,
            int code, String name, Action a, MessageKeyListener listener,MessageFocusTraversalPolicy policy,int count,int tcount) {
        if ((button & code) != code)
            return false;
        final JButton b;
        b=new JButton(a);
    ...
    public void setDefaultRatios(AbstractWindowTarget target) 
    {
    	if (AbstractWindowTarget.suppressDefaultRatio) return;
    	int list_sum_x=0,list_sum_y=0,list_count=0;
    	
    	int mid_x=0,mid_y=0;
    	
    	String ratio = target.getProperty(Prop.RATIO).toString();
    	if (!ratio.equals("default")) {
        	if (!ratio.equals("")) return;
        	if (target.isProperty(Prop.RESIZE)) return;
        	if (!target.isProperty(Prop.MDI)) return;
        	if (this.getServerApp()==null) return;
    	}
    	
    	List<AbstractControl> listControls=new ArrayList<AbstractControl>();
    	
    	collateListControls(target.getControls(),listControls);
    	
    	for (AbstractControl ac : listControls) {
   			Rectangle r = getRect(ac);
   			list_sum_x +=r.x+r.width/2;
   			list_sum_y +=r.y+r.height/2;
   			list_count++;
    	}
    	
    	if (list_count==0) {
    		if (ratio.equals("default")) {
    			mid_x = target.getRawProperty(Prop.WIDTH).intValue()/2;
    			mid_y = target.getRawProperty(Prop.HEIGHT).intValue()/2;
    		} else {
        		return;    			
    		}
    	} else {
    		mid_x=list_sum_x/list_count;
    		mid_y=list_sum_y/list_count;
    	}
    	
    	for (AbstractControl ac : listControls) {
   			Rectangle r = getRect(ac);
   			if (r.x+r.width<mid_x) return;
   			if (r.x>mid_x) return;
   			if (r.y+r.height<mid_y) return;
   			if (r.y>mid_y) return;
    	}

		if (!ratio.equals("default") && !isEditorMode()) {
			target.setProperty(Prop.RESIZE,true);
			target.setProperty(Prop.MAX,true);
		}
    	
    	setDefaultRatios(mid_x,mid_y,"",target.getControls(),1);
	}

	private void collateListControls(Collection<? extends AbstractControl> controls,List<AbstractControl> listControls) {
		for (AbstractControl s : controls) {
			if (s instanceof ListControl && !s.isProperty(Prop.DROP)) {
				listControls.add(s);
			}
			collateListControls(s.getChildren(),listControls);
		}
	}

	private void setDefaultRatios(int mid_x, int mid_y, String parent,Collection<? extends AbstractControl> controls,int depth) {
    	for (AbstractControl ac : controls) 
    	{
    		if (ac.getRawProperty(Prop.RATIO,false)!=null) continue;
    		
    		Rectangle r = getRect(ac);
    		
    		boolean spread=false;
    		boolean skip=ac instanceof TabControl;
    		
    		if (ac instanceof ListControl && !ac.isProperty(Prop.DROP)) {
    			spread=true;
    		}
    		if (ac instanceof SheetControl) {
    			if (!ac.isProperty(Prop.WIZARD)) {
    				spread=true;
    			} else {
    				skip=true;
    			}
    		...
    		if (spread && r.x<=mid_x && r.y<=mid_y && r.x+r.width>=mid_x && r.y+r.height>=mid_y && (parent.equals("") || parent.equals("spread"))) {
    			spread=true;
    			rat="spread";
    		} else {
    			spread=false;
    		...
    		if (rat==null) rat="";
    		if (!spread && !parent.equals("spread") && depth>1) {
    			rat=parent;
    		}
    		...
    		if (skip) {
    			...
    			if (isEditorMode()) {
    				ac.setDefaultRatio(rat);
    			} else {
    				if (!rat.equals(parent)) {
    					ac.setProperty(Prop.RATIO,rat);
    				}
    			}
    		}
    		
    		
    		setDefaultRatios(mid_x,mid_y,rat,ac.getChildren(),depth+1);
    	}		
	}

	private Rectangle getRect(AbstractControl ac) {
		return new Rectangle(
    			ac.getProperty(Prop.XPOS).intValue(),
    			ac.getProperty(Prop.YPOS).intValue(),
    			ac.getProperty(Prop.WIDTH).intValue(),
    			ac.getProperty(Prop.HEIGHT).intValue());
	}
    ...
    private boolean isActionKey(KeyEvent e)
    {
        ...
        if (e.getKeyChar()!=KeyEvent.CHAR_UNDEFINED) {
            if (e.getKeyChar()=='\n') return true;
            if (e.getKeyChar()==KeyEvent.VK_ESCAPE) return true;
            return false;
        } else {
    ...
    private static Set<AWTKeyStroke> TAB;
    private static Set<AWTKeyStroke> SHIFTTAB;
    
    static {
    	TAB=new HashSet<AWTKeyStroke>();
    	TAB.add(KeyStroke.getAWTKeyStroke(9,0,false));
    	SHIFTTAB=new HashSet<AWTKeyStroke>();
    	SHIFTTAB.add(KeyStroke.getAWTKeyStroke(9,KeyEvent.SHIFT_DOWN_MASK,false));
    };
            ...
            final AbstractControl start) 
    {
    	...
    	System.out.println(comp.getFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS));
    	System.out.println(SHIFTTAB);
    	System.out.println(comp.getFocusTraversalKeys(KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS));
    	*/
    	comp.setFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS,TAB);
    	comp.setFocusTraversalKeys(KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS,SHIFTTAB);
        ...
        comp.addKeyListener(new KeyListener() {
            ...
            public void handleEvent(KeyEvent e) {
               ...
               // check for other specials
                ...
                if (key == Constants.TABKEY+ 0x200) {
                	if (traverseSheet(ctl,start,1)) {
                		e.consume();
                        return;
                	}
                }
                if (key == Constants.TABKEY+ 0x300) {
                	if (traverseSheet(ctl,start,-1)) {
                		e.consume();
                        return;
                	}
            ...
            private boolean handleMnemonic(KeyEvent e) {
                ...
                if (start != null) {
                	...
                	if (start instanceof CustomMnemonic) {
                		if (((CustomMnemonic)start).handleMnemonic(mkey)) {
                			e.consume();
                			return true;
                		}
                	}
                ...
                while (ci.hasNext()) {
                    ...
                    if (entry != null) {
                        ...
                        if (candidate != null) {
                        	...
                        	if (candidate instanceof CustomMnemonic) {
                        		if (((CustomMnemonic)candidate).handleMnemonic(mkey)) {
                        			e.consume();
                        			return true;
                        		}
                        		continue;
    ...
    private boolean traverseSheet(AbstractWindowTarget target,AbstractControl ac,int direction)
    {
    	while (ac!=null) {
    		if (ac instanceof SheetControl && !ac.getProperty(Prop.WIZARD).boolValue()) {
    			// candidate
    			SheetControl sc = (SheetControl)ac;    			
        		return traverseSheet(sc,direction);
    		}
    		ac=ac.getParent();
    	}
    ...
    private boolean traverseSheet(SheetControl sc, int direction) 
    {
		int pos = sc.getProperty(Prop.SELSTART).intValue()-1;
		
		int attempt=sc.getChildren().size();
		for (int scan=0;scan<attempt;scan++) {
			pos+=direction;
			if (pos<0) pos=attempt-1;
			if (pos>=attempt) pos=0;
			
			AbstractControl tc = sc.getChild(pos);
			if (!tc.getProperty(Prop.DISABLE).boolValue() && !tc.getProperty(Prop.HIDE).boolValue()) {
				sc.notifyChange(pos+1);
				//sc.setProperty(Prop.SELSTART,pos+1);
				return true;
			}
		}
		return false;
    ...
    {
    	return getImage(name,prefx,prefy,true);
    }

    private static class StdImageReceiver implements ImageReceiver
    {
    	private AWTPause pause;
    	private Image img;
    	private boolean imgSet;
    	
    	public StdImageReceiver()
    	{    		
    	}
    	...
    	{
    		synchronized(this) {
    			if (imgSet) return img;
    		}
    		
    		if (SwingUtilities.isEventDispatchThread()) {
    			AWTPause t = new AWTPause();
    			Image i;
    			boolean set=false;
    			synchronized(this) {
    				set=imgSet;
    ...
    public java.awt.Image getImage(String name, int prefx, int prefy,boolean usecache) 
    {
    	StdImageReceiver ir = new StdImageReceiver();
    	getImage(name,prefx,prefy,usecache,ir);
    	return ir.getImage();
    }

	private Image getImage(InputStream is, String name, String cache,int prefx, int prefy, boolean usecache) {
		if (is == null && !GUIModel.getServer().isLocal() && SwingUtilities.isEventDispatchThread()) {
			
			// Last attempt : look for image locally on client. 
		...
		Image i = getMicrosoftIcon(is, name, prefx, prefy);
		if (i != null) {
			if (usecache) addImagetoCache(cache, i);
			return i;
		}
    ...
    public void getImage(final String f_name, final int prefx,final int prefy,final boolean usecache,final ImageReceiver ir) 
    {
        if (f_name.startsWith("print:/")) {

        	String name=f_name.trim();
            int scalePos = name.indexOf("#");
            int t_scale=100;
            if (scalePos>-1) {
                t_scale=Integer.parseInt(name.trim().substring(scalePos+1));
                name=name.substring(0,scalePos);
            }
            
            final int f_scale=t_scale;
            
            ResponseRunnable nextTask=new ResponseRunnable()
            {
				@Override
            ...
            GUIModel.getServer().send(this,nextTask,GET_PAGE,name);
            return;
        }
        
        String cache = getImageURL(f_name)+"_p"+prefx+"_p"+prefy;
        Image i=getImageFromCache(cache);
        if (i!=null) {
        	ir.setImage(i);
        	return;
        ...
        if (f_name.equals(Icon.NONE)) {
        	ir.setImage(null);
        	return;
        }

        ClassLoader cl = getClass().getClassLoader();
        InputStream is = cl.getResourceAsStream(getImageURL(f_name));
        
        if (is == null) {
        	final String name=f_name.trim();
        	GUIModel.getServer().send(this,new ResponseRunnable() {
				@Override
				public void run(Object result) {
					final String cache=(String)result;
					if (cache!=null) {
		             	Image i=getImageFromCache(cache);
		               	if (i!=null) {
		               		ir.setImage(i);
					...
					GUIModel.getServer().send(CWinImpl.this,new ResponseRunnable() {
						@Override
						public void run(final Object result) {
							if (result==null && !GUIModel.getServer().isLocal() && !SwingUtilities.isEventDispatchThread()) {
								
								// there is no image from the server, the server is remote and we are not the event dispatcher. Switch to event 
								// dispatcher so the dispatcher can try to source the image locally
								
								AWTBlocker.getInstance().runAsSoonAsPossibleOnSwing(new Runnable() {
									@Override
									public void run() {
										Image x= getImage((InputStream)result,name,cache,prefx,prefy,usecache);
										ir.setImage(x);
									} },false,false);
							} else {
								Image x= getImage((InputStream)result,name,cache,prefx,prefy,usecache);
        	...
        	},GET_CACHE_FILE_NAME,f_name);
        } else {
        	ir.setImage(getImage(is,f_name,cache,prefx,prefy,usecache));
        }
    }
    ...
    static {
        ...
        clarionIcons.put(Icon.DOWN, "resources/images/clarion/down.png");
        clarionIcons.put(Icon.BULLET, "resources/images/clarion/bullet.png");
    ...
    public static final int CHECK_RESTORE=15;
	...
	public Object command(int command, Object... params) {
    	switch(command) {
    		case CHECK_RESTORE: {
    			svrCheckRestore((AbstractWindowTarget)params[0]);
    			return null;
    		}
    		case GET_PAGE: {
    		...
    		case GET_CACHE_FILE_NAME: {
    	        ...
    	        Long lastModified;
				if (!GUIModel.getClient().isLocal()) {
					lastModified=ClarionFileFactory.getInstance().getServerOnly().lastModified(name);
				} else {
					lastModified=ClarionFileFactory.getInstance().lastModified(name);
				}
    		...
    		case GET_FILE: {
    			...
    			try {
    				...
    				if (!GUIModel.getClient().isLocal()) {
    					f = ClarionFileFactory.getInstance().getServerOnly().getRandomAccessFile(name.trim());
    				} else {
    					f = ClarionFileFactory.getInstance().getRandomAccessFile(name.trim());
    				}
	...
	private int doMessageBox(String message, String header, String icon,
		...
		while (len < msg.length()) {
			...
			int end_pipe = msg.indexOf('|', len);
			if (end_pipe==-1) end_pipe= msg.length();
				
			if (end_pipe<end_n && end_pipe<end_r) {
				end_r=end_pipe;
				end_n=msg.length();
			}
		...
		MessageFocusTraversalPolicy policy=  new MessageFocusTraversalPolicy();
		...
		int count=0;
		if (addButton(root, buttons, button, defButton, Button.OK, "OK", a, mkl,policy,count,tcount)) count++;
		if (addButton(root, buttons, button, defButton, Button.YES, "Yes", a, mkl,policy,count,tcount)) count++;
		if (addButton(root, buttons, button, defButton, Button.NO, "No", a, mkl,policy,count,tcount)) count++;
		if (addButton(root, buttons, button, defButton, Button.ABORT, "Abort", a,mkl,policy,count,tcount)) count++;
		if (addButton(root, buttons, button, defButton, Button.RETRY, "Retry", a,mkl,policy,count,tcount)) count++;
		if (addButton(root, buttons, button, defButton, Button.CANCEL, "Cancel", a,mkl,policy,count,tcount)) count++;
		if (addButton(root, buttons, button, defButton, Button.IGNORE, "Ignore", a,mkl,policy,count,tcount)) count++;
		if (addButton(root, buttons, button, defButton, Button.HELP, "Help", a, mkl,policy,count,tcount)) count++;
	...
	private boolean doFileDialog(String path,String title,ClarionString target,String ifiles,int options)
	{
        ...
        if ((title!=null && title.toLowerCase().contains("image")) || (ifiles!=null && ifiles.toLowerCase().contains("image"))  
        	|| ifiles.toLowerCase().contains("jpg") || ((options&128)==128)) {  
	...
	private void doOpen(final AbstractWindowTarget target,final AbstractWindowTarget parent,final OpenMonitor monitor) 
	{
		...
		setFontDimensions(target,win,f);
	...
	public void setFontDimensions(AbstractTarget target, Container win,Font f) {
		Dimension fs;
	...
	private AbstractWindowTarget lastCheckRestore;
	private Thread lastRestore;
	
	public void svrCheckRestore(AbstractWindowTarget awt) 
	{
		if (awt.getClarionThread()==null) {
			return;
		}
		if (lastRestore==null) {
			lastRestore=awt.getClarionThread();
			return;
		}
		
		if (awt.getClarionThread()==lastRestore) {
			return;
		}
		
		AbstractWindowTarget prior = getTarget(lastRestore);
		if (prior!=null && !prior.isClosed()) {
			prior.post(Event.SUSPEND);
			stack.get(lastRestore).suspend=true;
		}
		lastRestore=awt.getClarionThread();
		
		WindowStack n = stack.get(awt.getClarionThread()); 
		
		if (n.suspend) {
			n.suspend=false;
			awt.post(Event.RESUME);
		}
	}
	
	public void checkRestore(AbstractWindowTarget awt) 
	{
		if (lastCheckRestore==awt) {
			return;
		}
		lastCheckRestore=awt;
