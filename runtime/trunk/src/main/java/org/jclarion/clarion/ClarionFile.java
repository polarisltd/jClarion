...
/**
 *  Provide abstract model of a File entityf in clarion.
 *  
 * @author barney
 *
 */
public abstract class ClarionFile extends ClarionGroup
{
    ...
    protected void disposeFileState()
    {
        ...
        if (fs.fields!=null) {
            for (int scan=0;scan<fs.fields.length;scan++) {
                fs.fields[scan].removeChangeListener(fs.listeners[scan]);
            }
        }
    ...
    public static boolean isPositionMatches(ClarionString string,ClarionObject[] scanFields) 
    {
        ...
        sos.readByte(); // inclusive
        int count = sos.readByte();
    ...
    {
        FileState fs = getFileState();
        if (fs.isnull==null) return false;
        int indx = fs.index(clarionObject);        
        boolean r = fs.isnull[indx];
        return r;
    }

    
    public void alterNull(ClarionObject clarionObject, boolean nullValue) {
        FileState fs = getFileState();
        if (fs.isnull==null) return;
        int scan = fs.index(clarionObject);
        fs.isnull[scan]=nullValue;
        fs.changed[scan]=true;
    }

    private List<Object> clearFields=null;

	@Override
	public void addVariable(String name, Object object) 
	{
		if (object instanceof ClarionObject) {
			super.addVariable(name, object);			
		} else {
			if (clearFields==null) { 
				clearFields=new ArrayList<Object>();
			}
			clearFields.add(object);
			if (object instanceof ClarionMemoryModel && state.isThreaded()) {
				((ClarionMemoryModel)object).initThread();
			}
		}
	}
	...
	public void clear(int method) {
		super.clear(method);
		if (clearFields!=null) {
			for (Object m : clearFields) {
				if (m instanceof ClarionCloneable) {					
					((ClarionCloneable)m).clear(method);
				}
			}
		}
        FileState fs = getFileState();
    ...
    {
        if (!state.isThreaded()) {
        	state=new ThreadStateGetter<FileState>(factory,state);
        }
    	if (clearFields!=null) {
			for (Object m : clearFields) {
				if (m instanceof ClarionMemoryModel) {
					((ClarionMemoryModel)m).initThread();
				}
			}        		
    	}
        super.initThread();
