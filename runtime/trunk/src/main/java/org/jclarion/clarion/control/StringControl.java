...
public class StringControl extends AbstractControl 
{
    ...
    private void resize()
    {
        ...
        boolean editorMode = CWin.getInstance().isEditorMode();
        ...
        if (!editorMode) {
        	...
        	if (getRawProperty(Prop.WIDTH,false)==null) {
        		ns.width=ps.width;
        	}
        	if (getRawProperty(Prop.HEIGHT,false)==null) {
        		ns.height=ps.height;
        	}
        	if (s.height!=ns.height || s.width!=ns.width) {
        		_label.setSize(ns);
        	}
        }
    }
    ...
    private void justify() {
    	...
    	if (label==null) return;
    	int constant=SwingConstants.LEFT;    	
        if (isProperty(Prop.CENTER)) constant=SwingConstants.CENTER;
        if (isProperty(Prop.RIGHT)) constant=SwingConstants.RIGHT;
        if (isProperty(Prop.DECIMAL)) constant=SwingConstants.RIGHT;
    ...
    protected void doNotifyLocalChange(final int indx,final ClarionObject value) {
        ...
        if (indx == Prop.USE && listener!=null) {
            ...
            if (CWin.getInstance().isEditorMode()) {
            	Formatter f = getPicture();
            	if (f==null) {
            		CWinImpl.run(this,SET_TEXT,"#ERROR");
            	} else {
            		CWinImpl.run(this,SET_TEXT,f.getPictureRepresentation());
            	}
            } else {
    ...
    public Dimension getPreferredSize() 
    {
        ...
        if (getUseObject()!=null) {
            ...
            if (getPicture()!=null) {
                ...
                if (g==null) {
                	g = (new BufferedImage(1,1,BufferedImage.TYPE_INT_RGB)).getGraphics();
                }
                FontMetrics fm = g.getFontMetrics(getComponent().getFont());
