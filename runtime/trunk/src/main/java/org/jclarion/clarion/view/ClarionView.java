...
public class ClarionView extends PropertyObject implements ViewField
{
    ...
    List<ViewField>     orderedFields=new ArrayList<ViewField>();
    ...
    public void set(int sortOffset)
    {
        ...
        if (sortOffset==0) {
			...
			for (int scan = 0; scan < sortOffset; scan++) {
				sos.writeByte(file.isNull(bits[scan].object) ? 1 : 0);
    ...
    public ClarionString getPosition()
    {
        ...
        for (ClarionFile file : allFiles) {
            ...
            for (ClarionKey.Order o : ck.getFields()) {
            	if (!this.file.isNull(o.object)) {
                	allNull=false;
                	break;
                }
            }
            
            sos.writeByte(allNull ? 1 : 0 );
            if (!allNull) { 
            	for (ClarionKey.Order o : ck.getFields()) {
        ...
        if (order!=null) {
            ...
            for (int scan=0;scan<o.length;scan++) {
            	sos.writeByte(file.isNull(o[scan].object) ? 1 : 0 );
        ...
        if (limit>0) {
            ...
            if (!position_string.startsWith(cs_string)) {
                if (log.isLoggable(Level.FINE)) {
                    log.fine("Calling Reget: ["+ClarionString.toBinaryString(position_string)+"] != ["+ClarionString.toBinaryString(cs_string)+"]");
                }
                reget(position);
            } else {
                if (log.isLoggable(Level.FINE)) {
                    log.fine("Skipping Reget: ["+ClarionString.toBinaryString(position_string)+"] != ["+ClarionString.toBinaryString(cs_string)+"]");
                }
            }
        } else {
            reget(position);
    ...
    private boolean regenOrderAndFilter() {
        ...
        while (tok.hasMoreTokens()) {
           ...
           FieldEntry fe = null;
           try {
        	   LabelExprResult bp = CExprImpl.getInstance().resolveBind(s,false);
    ...
    */
    ...
    private List<ClarionObject> getOrderedColumns()
    {
        if (orderedColumns==null) {
            orderedColumns=new ArrayList<ClarionObject>();
            addOrderedColumns(orderedColumns,this);
        }
        return orderedColumns;
    }
    
    private void addOrderedColumns(List<ClarionObject> order,ViewField field)
    {
        ClarionObject o = field.getField();
        if (o!=null) order.add(o);
        Iterable<ViewField> subs = field.getSubFields();
        if (subs!=null) {
	...
	public Object debug(ClarionObject refcode) 
	{
		return file.isNull(refcode);
	}
}
