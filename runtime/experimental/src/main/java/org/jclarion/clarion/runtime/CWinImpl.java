...
public class CWinImpl implements RemoteWidget {
                    ...
                    if (last != null) {
                        ...
                        if (cpos != pos) {
                            ...
                            sc.changeValue(pos,new Runnable() {
                                @Override
                                public void run() {
                                    if (scan==null || scan.getProperty(Prop.SELSTART).intValue() != pos) return;
                                    last = scan;
                                    scan = scan.getParent();
                                    doScanLoop();
                                } });
    ...
    public java.awt.Image getImage(String name, int prefx, int prefy) 
    {
        if (name.startsWith("print:/")) {
        	...
        	Page p = (Page)GUIModel.getServer().sendRecv(this,GET_PAGE,name);
        ...
        if (is == null) {
           	cache = (String)GUIModel.getServer().sendRecv(this,GET_CACHE_FILE_NAME,name);
            ...
            is = (InputStream)GUIModel.getServer().sendRecv(this,GET_FILE,name);
        }
	...
	public Object command(int command, Object... params) {
    	switch(command) {
    		...
    		case GET_CACHE_FILE_NAME: {
    			...
    			File bf = ClarionFileFactory.getInstance().getFile(name);
    			long lastModified=bf!=null ? bf.lastModified() : 0;
    			return lastModified==0 ? null : name+"_"+lastModified;
    		}
    		case GET_FILE: {
    			String name = (String)params[0];
    			...
    			try {
    				ClarionRandomAccessFile f = ClarionFileFactory.getInstance().getRandomAccessFile(name.trim(),false);
			...
			case FILE_DIALOG: {
				boolean result=doFileDialog((String)params[0],(ClarionString)params[1],(String)params[2],(Integer)params[3]);
				return new Object[] { result,(ClarionString)params[1] }; 
			}
			case MESSAGE: {
    		...
    		case LAZY_OPEN: {
                ...
                c.addComponentListener(new ComponentListener() {
                    ...
                    public void componentMoved(ComponentEvent e) {
                        ...
                        target.notifyWinPropertyChange();
	...
	private boolean doFileDialog(final String title,final ClarionString target,final String ifiles,final int options)
	{
        File sf = CFile.pwd;
        if (!target.equals("")) {
        ...
        JFileChooser chooser = new JFileChooser(d, CWin.getView());
		...
		if (target instanceof ClarionApplication) {
			...
			jif.addInternalFrameListener(new InternalFrameListener() {
				...
				public void internalFrameOpened(InternalFrameEvent e) {
                    target.notifyWinPropertyChange();
                    GUIModel.getServer().send(target,AbstractWindowTarget.NOP);
					monitor.setOpened();
					getGuiApp().getStatusPane().setChild(target);
				}
			});
