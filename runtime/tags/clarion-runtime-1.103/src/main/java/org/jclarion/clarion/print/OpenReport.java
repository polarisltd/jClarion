...
public class OpenReport implements PropertyObjectListener
{
    ...
    public Object getResource(String src)
    {
    	return resources.get(src);
    }

    public void setResource(String src,Object obj)
    {
    	resources.put(src,obj);
    }

    public void initReportPreview()
    {
        if (report!=null && report.getPreview()!=null) {
            init();
        }    	
    }
    ...
    */
    ...
    private Page doAddElement(Page currentPage, PrintObject current) 
    {
    	while (true) {
    		...
    		current.getSize(currentPage,graphics);
    		PrintObject next=null;
    		if (current.getHeight()!=current.getOriginalHeight()) {
    			
    			int remain = currentPage.getHeight();
				PrintObject last = currentPage.getLastMoveable();
    			if (last!=null) {
    				int bottom = last.getPositionedY()-currentPage.getY();
    				
    				if (currentPage.getWidth()-(last.getPositionedX()+last.getX()+last.getWidth()-currentPage.getX())<
    					current.getWidth()+current.getX()) {
    					bottom=bottom+last.getY()+last.getHeight();
    				}
    				
    				int gap=currentPage.getHeight()-bottom;
    				
    				/** 
    				 * Split on the gap at bottom of the page but only if 
    				 * the gap is at least 30% of the page size 
    				 */
    				if (gap>currentPage.getHeight()*3/10) { 
   						remain=gap;
    				}
    			}
    			if (current.getHeight()+current.getY()>remain && current.containsResizableComponents()) {
        			int oheight =current.getHeight(); 
    				next=current.split(currentPage, graphics,remain);
    				if (next.getHeight()>=oheight) {
    					next=null;
    				}
    			}
    		}
    		
    		Page p = doAddOneElement(currentPage,current); 
    ...
    private Page doAddOneElement(Page currentPage, PrintObject current) 
    {
    ...
    {
        Page currentPage=ReportFactory.getInstance().newPage(this);
