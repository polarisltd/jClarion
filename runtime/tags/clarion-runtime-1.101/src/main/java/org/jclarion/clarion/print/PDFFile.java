...
public class PDFFile extends ClarionRandomAccessFile 
{
    ...
    public static void save(ClarionReport r,String out)
    {
        ...
        try {
        	ClarionFileFactory.getInstance().create(out);        	
        	OutputStream fos = new OutputStreamWrapper(ClarionFileFactory.getInstance().getRandomAccessFile(out));
            fos.write(f.pdf);
            fos.close();
    ...
    public void generate(OpenReport or, PageBook book)
    {
        ...
        try {
            ...
            document.open();
            PDFContext context=new PDFContext(document,writer);
            ...
            for (int scan=0;scan<book.getPageCount();scan++) {
                ...
                if (!any) {
                    if (or!=null && !or.isInit()) {
                        or.init(new PdfPrintContext(context));
                    }
                }
                p.print(new PdfPrintContext(context));
                any=true;
            }
            document.close();
