...
public class ClarionStringTest extends TestCase {
    ...
    public void testUnboundedStringMemoryConsumption()
    {
    	ClarionString cs = new ClarionString();
    	assertEquals(-1,cs.getCharArraySize());
    	
    	assertConsumption(cs,10,10);
    	assertConsumption(cs,5,10);
    	assertConsumption(cs,15,15);
    	assertConsumption(cs,100000,100000);
    	assertConsumption(cs,50000,100000);
    	assertConsumption(cs,100,328);
    	assertConsumption(cs,0,328);
    }
    
    private void assertConsumption(ClarionString cs,int len,int mem)
    {
    	char c[]=new char[len];
    	Random r= new Random();
    	for (int scan=0;scan<len;scan++) {
    		c[scan]=(char)(r.nextInt(127-32)+32);
    	}
    	String s = new  String(c);
    	
    	cs.setValue(s);
    	assertEquals(mem,cs.getCharArraySize());
    ...
    public void testChangeBug()
    {
    	...
    	ClarionString cs = new ClarionString(5);
    	cs.addChangeListener(new ClarionMemoryChangeListener() { 
			@Override
			public void objectChanged(ClarionMemoryModel model) {
				count[0]++;
			}
    	...
    	cs.setValue(new ClarionString("xx"));
    	assertEquals(2,count[0]);
    	cs.setValue("x");
    	assertEquals(3,count[0]);
    	cs.setValue(new ClarionString(""));
    	assertEquals(4,count[0]);
    	cs.setValue(new ClarionString(""));
    	assertEquals(4,count[0]);
    	cs.setValue(new ClarionString("x"));
    ...
    public void testOverloadTriggersUpdate()
    {
        ...
        s.setValue("片仮名平仮名");
        assertEquals("片仮名平仮名    ",s.toString());
        assertEquals("片仮名平仮名    ",s2.toString());
    ...
    public void testDimInt() {
        ...
        ClarionArray<ClarionString> l = (new ClarionString(20)).dim(5);
        
        for (int scan=1;scan<6;scan++) {
            assertEquals("                    ",l.get(scan).toString());
        }
    }

    public void testDimInt2() {
        
        ClarionArray<ClarionArray<ClarionString>> l = (new ClarionString(20)).dim(6).dim(5);
        
        for (int s1=1;s1<=5;s1++) {
            for (int s2=1;s2<=6;s2++) {
                assertEquals("                    ",l.get(s1).get(s2).toString());
            }
        }
    }
    ...
    public void testDimInt3() {
        ...
        ClarionArray<ClarionArray<ClarionArray<ClarionString>>> l = (new ClarionString(20)).dim(4).dim(5).dim(6);
        
        for (int s1=1;s1<=6;s1++) {
            for (int s2=1;s2<=5;s2++) {
                for (int s3=1;s2<=4;s2++) {
                    assertEquals("                    ",l.get(s1).get(s2).get(s3).toString());
                }
            }
        }
    }
    ...
    public void testSetOverArray() {
        ClarionString c1=new ClarionString(20); 
        ClarionArray<ClarionString> c2=new ClarionString(2).dim(10).setOver(c1);

        assertEquals("                    ",c1.toString());
        for (int scan=1;scan<10;scan++) {
            assertEquals("  ",c2.get(scan).toString());
        }
        
        c1.setValue("xxxxxxxxxxxxxxxxxxxx");
        assertEquals("xxxxxxxxxxxxxxxxxxxx",c1.toString());
        for (int scan=1;scan<10;scan++) {
            assertEquals("xx",c2.get(scan).toString());
        }
    ...
    private void testArray(ClarionArray<? extends ClarionObject> bits,String[] array) {
        for (int scan=1;scan<bits.length();scan++) {
            assertEquals(array[scan],bits.get(scan).toString());
        }
    }
    ...
    public void testQuote()
    {
        ...
        quoteAssert("<65>","<65>",false);
        quoteAssert("A","<65>",true);
