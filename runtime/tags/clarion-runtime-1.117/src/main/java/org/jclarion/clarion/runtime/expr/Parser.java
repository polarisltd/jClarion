...
public class Parser {
    ...
    {
    	return expr(LexType.eof);
    }

    public CExpr expr(LexType end) throws ParseException
    {
        CExpr result = expr1_or();
        if (end!=null) {
        	enforceNext(end,"Expected "+end);
        }
        return result;
    }
    
    private CExpr expr1_or() throws ParseException
    {
        CExpr result = expr1_and();
    ...
    private CExpr expr1_and() throws ParseException
    {
        CExpr result = expr2();
        if (result==null) return null;
    ...
    {
        Lex l = stream.next();
        if (l.type!=type) error(error+" got "+l.type.toString()+" "+l.value);
        return l;
    }
