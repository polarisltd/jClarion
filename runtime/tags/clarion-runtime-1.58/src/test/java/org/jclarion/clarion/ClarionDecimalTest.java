...
public class ClarionDecimalTest extends TestCase {
    ...
    public void testDimInt() {
        ...
        ClarionArray<ClarionDecimal> l = (new ClarionDecimal(10,2,"123235.3435")).dim(5);
        
        for (int scan=1;scan<6;scan++) {
            assertEquals("123235.34",l.get(scan).toString());
        }
    }

    public void testDimInt2() {
        
        ClarionArray<ClarionArray<ClarionDecimal>> l = (new ClarionDecimal(10,2,"-13.456")).dim(6).dim(5);
       
        for (int s1=1;s1<=5;s1++) {
            for (int s2=1;s2<=6;s2++) {
                assertEquals("-13.46",l.get(s1).get(s2).toString());
            }
        }
    }
    ...
    */
    ...
    public void testGCD()
    {
        assertEquals("3",Clarion.newDecimal("15").gcd(Clarion.newDecimal("6")).toString());
        assertEquals("3",Clarion.newDecimal("15").gcd(Clarion.newDecimal("-6")).toString());
        assertEquals("3",Clarion.newDecimal("-15").gcd(Clarion.newDecimal("-6")).toString());
        assertEquals("1.5",Clarion.newDecimal("-1.5").gcd(Clarion.newDecimal("-6")).toString());
        assertEquals("3.2",Clarion.newDecimal("9.6").gcd(Clarion.newDecimal("12.8")).toString());
        assertEquals("1",Clarion.newDecimal("11.5").gcd(Clarion.newDecimal("13.5")).toString());
    }
}
