...
public class PrintObject 
{
    ...
    public void draw(OpenReport report,CWinImpl impl,AbstractControl ac) 
    {
        ...
        if (ac instanceof StringControl) {
            ...
            if (ac.getUseObject()==null) {
                ...
                if (rs==null) {
                    text = format.format(ac.getUseObject().toString()).trim();
                } else {
                    text = format.format(rs.get().toString()).trim();
                }
            }
            ...
            Text t= drawText(ac,text,x1,y1,x2,y2,justify,justifyOffset,c,f,trans);
        ...
        if (ac instanceof TextControl) {
            ...
            drawTextArea(ac,text,x1,y1,x2,y2,c,f,ac.isProperty(Prop.RESIZE));
            return;
        }
    ...
    public void drawImage(AbstractControl control,int x1,int y1,int x2,int y2)
    {
        ...
        if (name.length()>0) {
            i=CWin.getInstance().getImage(name,0,0);
        } else {
            i=CWin.getInstance().getImageFromBinaryData(control.getProperty(Prop.IMAGEBITS));
        }
        if (i!=null) {
            add(new Bitmap(control,x1,y1,x2,y2,i));
        }
    }

    public Text drawText(AbstractControl control,String text,int x1,int y1,int x2,int y2,
            Text.Justify justify,int justifyOffset,
            Color color,Font font,boolean transparent)
    {
        Text t=new Text(control,text,x1,y1,x2,y2,justify,justifyOffset,color,font,transparent);
        add(t);
        return t;
    }

    public void drawTextArea(AbstractControl control,String text,int x1,int y1,int x2,int y2,Color color,Font font,boolean resize)
    {
        add(new TextArea(control,text,x1,y1,x2,y2,color,font,resize));
    }
