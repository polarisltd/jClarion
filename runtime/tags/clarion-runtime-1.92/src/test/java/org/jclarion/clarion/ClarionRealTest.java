...
public class ClarionRealTest extends TestCase 
{
    ...
    public void testSerializeDeserialize() throws IOException 
    {
        ...
        sos.reset();
        c_a.serialize(sos);
        byte[] s_a = CMem.toByteArray(sos);

        sos.reset();
        c_b.serialize(sos);
        byte[] s_b = CMem.toByteArray(sos);
        
        sos.reset();
        c_c.serialize(sos);
        byte[] s_c = CMem.toByteArray(sos);
        ...
        sos.reset();
        CMem.fromByteArray(sos,s_a);
        cr.deserialize(sos);
        assertEquals(a,cr.getDouble());

        sos.reset();
        CMem.fromByteArray(sos,s_b);
        cr.deserialize(sos);
        assertEquals(b,cr.getDouble());

        sos.reset();
        CMem.fromByteArray(sos,s_c);
        cr.deserialize(sos);
