...
public class ClarionSQLDateTest  extends TestCase 
{
    ...
    public void testOptimizedNulls()
    {
        ...
        assertEquals("SELECT id,partnum,created FROM testdate WHERE (created<='2038-01-20' AND (created<'2038-01-20' OR (partnum<'PARTE'))) ORDER BY created DESC,partnum DESC",testFile.getProperty(Prop.SQL).toString());
        testFile.datekey.set();
        assertScan(1,new int[] { 5,6,7,9,8,10 });        
        assertEquals("SELECT id,partnum,created FROM testdate WHERE (created>='1969-11-14' AND (created>'1969-11-14' OR (partnum>='PARTA'))) ORDER BY created,partnum",testFile.getProperty(Prop.SQL).toString());
    }
