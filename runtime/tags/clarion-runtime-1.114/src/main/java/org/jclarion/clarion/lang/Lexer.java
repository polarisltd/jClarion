...
public class Lexer 
{
    ...
    private boolean				  decodeInteger=true;
    ...
    public Lexer(AbstractLexStream stream) {
    	this.reader=stream;
    	this.reader.setName("unk");
    	buffer=new Lex[1];
    }
    ...
    public void setDecodeInteger(boolean mode)
    {
        decodeInteger=mode;
    }
    ...
    public void setTemplateLexer(boolean template)
    {
        this.template=true;
    }
     ...
     */
    Lex _next() {
    	int lineNumber = reader.getLineCount();
    	int charOffset = reader.getCharOffset();
    	Lex l = __next();
    	l.lineNumber=lineNumber;
    	l.charStart=charOffset;
    	l.charEnd=reader.getCharOffset();
    	return l;
    }
    ...
    Lex __next() {
        ...
        if ((c>='a' && c<='z') || (c>='A' && c<='Z') || (c=='_') || 
        		(template && c=='%' && ((reader.peek(1)>='A' && reader.peek(1)<='Z') || (reader.peek(1)>='a' && reader.peek(1)<='z')))
        ) {
        	
        	boolean isTemplate=(template && c=='%');
            ...
            while ( true ) {
                ...
                while ( true ) {
                    ...
                    if (c>='A' && c<='Z') break;
                    if (c>='0' && c<='9') break;
                    if (!isTemplate && c=='_') break;
                    if (!isTemplate && c==':') break;
        ...
        if (c=='@' && reader.peek(1)=='@' && reader.peek(2)=='@') {
            ...
            while (true ) {
                if (reader.eof()) break;
                if (reader.peek(0)=='@' && reader.peek(1)=='@' && reader.peek(2)=='@') {
                	reader.skip(3);
                	break;
                }
                char ch = reader.read();
                if (ch=='\'') {
                	reader.appendToToken(ch);
                }
            }        	
        ...
        if (c>='0' && c<='9') {
            ...
            try {
            	return new Lex(decimal?LexType.decimal:LexType.integer,decodeInteger ? Constant.number(token,javaMode) : token );
            } catch (NumberFormatException ex) {
            	return new Lex(LexType.other,token);
            }
        }
        ...
        switch(c) {
            ...
            case '.': {
                ...
                if (token.length()==1) {
                	...
                	try {
                    	return new Lex(LexType.decimal,Constant.number(token,javaMode));
                	} catch (NumberFormatException ex) {
                		return new Lex(LexType.other,token);
                	}
                }
