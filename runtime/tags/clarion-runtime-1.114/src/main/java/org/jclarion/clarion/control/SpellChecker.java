...
public class SpellChecker 
{
	...
	public void spellCheck(final JTextComponent jta)
	{
		...
		Configuration c =sc.getConfiguration(); 
		c.setBoolean(Configuration.SPELL_IGNOREUPPERCASE,false);
		c.setBoolean(Configuration.SPELL_IGNOREDIGITWORDS,true);
					...
					public void mouseClicked(MouseEvent e) {
						if (e.getButton()==3) {
							...
							for (Highlight h : jta.getHighlighter().getHighlights()) {
								...
								if (from<=pos && to>=pos) {
									try {
										String t_word= jta.getText(h.getStartOffset(),h.getEndOffset()-h.getStartOffset());
								        ...
								        for (int scan=0;scan<t_word.length();scan++) {
								        	char c = t_word.charAt(scan);
								        	if (c>='a' && c<='z') {
								        		if (mode==2) {
								        			mode=0;
								        		}
								        ...
								        if (mode==2) {
								        	t_word=t_word.toLowerCase();
								        }
								        final String word=t_word;
								        ...
								        for (final Word s : l ) {
								        	...
								        	String wrd = s.getWord();
								        	if (mode==2) {
								        		wrd=wrd.toUpperCase();
							        			s.setWord(wrd);
								        	} else if (!wrd.equals(wrd.toLowerCase())) {
								        		// as is
								        	} else {
								        		if (mode==1) {
								        	...
								        	JMenuItem jmi=new JMenuItem(wrd);
								        	jpm.add(jmi);
								        	final int f_pos=pos;
