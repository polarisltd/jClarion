...
public class ClarionFocusTraversalPolicy extends FocusTraversalPolicy  
{
    ...
    public boolean accept(Component scan,Component original,boolean obeySkip) {
        ...
        if (scan.getClass().getName().startsWith("javax.swing.plaf.nimbus.TableScrollPaneCorner")) return false; 
        ...
        if (scan instanceof JToggleButton) {
        	AbstractControl ac = (AbstractControl)(((JComponent)scan).getClientProperty("clarion"));
        	
        	if (ac instanceof TreeMenuControl) {
           		TreeMenuControl tmc = (TreeMenuControl)ac;
           		AbstractButton ab = tmc.getSelectedButton();
           		if (ab!=null && ab!=scan) {
           			return false;
        		}
        	}
