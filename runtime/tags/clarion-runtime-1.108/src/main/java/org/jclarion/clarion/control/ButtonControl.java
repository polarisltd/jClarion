...
public class ButtonControl extends AbstractButtonControl implements KeyableControl, SimpleMnemonicAllowed
{
    ...
    public int getKey()
    {
        return getProperty(Prop.KEY).intValue();
    }
    ...
    public void constructSwingComponent(Container parent) 
    {
        ...
        if (isProperty(Prop.ICON)) {
        	CWin.getInstance().getImage(getProperty(Prop.ICON).toString(),0,0,true,new SwingButtonIconReceiver(button));
        }
    ...
    protected void doNotifyLocalChange(int indx, ClarionObject value) {
        ...
        if (indx==Prop.TEXT && isOpened()) {
            configureButton();
        }
        if (indx==Prop.DEFAULT) {
        	AbstractWindowTarget awt = getWindowOwner();
        	if (awt!=null) {
        		awt.setDefaultButton(this);
        	}
        }
    }
