...
public class ClarionQueue extends ClarionGroup implements ClarionQueueReader
{
	...
	protected static class QueueEntry implements Comparable<QueueEntry>
    {
        ...
        public int compareTo(QueueEntry qe) {
        	return compareTo(qe,false);
        }
        
        @SuppressWarnings("unchecked")
		public int compareTo(QueueEntry qe,boolean insensitive) {
            ...
            if (insensitive) {
            	return v.toString().compareToIgnoreCase(v2.toString());
            }
    ...
    public static class DefaultOrder extends Order
    {
    	private static class O {
			...
			public O(int index, boolean descend, boolean insensitive) {
				super();
				this.index = index;
				this.descend = descend;
				this.insensitive = insensitive;
				
				cmp=index*4 + (descend ? 2 : 0 ) + (insensitive ? 1 : 0 ); 
			}
        ...
        private ArrayList<O> order=new ArrayList<O>();
        ...
        public DefaultOrder(ClarionQueue base,String s) {
            ...
            while ( !decoder.end() ) {
                ...
                if (field.length()>0) {
                    ...
                    order.add(new O(p,descend,insensitive));
                }
                decoder.pop(',');
            }
        ...
        public boolean equalsPartial(Object o) {
            ...
            for (int scan=0;scan<order.size();scan++) {
                if (order.get(scan).cmp!=oo.order.get(scan).cmp) return false;
            }
            return true;
        }
        ...
        public int compare(Record o1, Record o2) {
            ...
            for ( O scan : order ) {
                ...
                int pos=scan.index;
                int diff=o1.fields[pos-1].compareTo(o2.fields[pos-1],scan.insensitive);
                if (scan.descend) {
                	diff=-diff;
                }
		...
		public void clear(Record r, int pos) {			
			...
			O scan = order.get(pos);
			int cv=-1;
			if (scan.descend) cv=1;
			Object o = r.fields[scan.index-1].value;
    ...
    public void toggle(int row, int column) {
        ...
        notifyChange(new ClarionQueueEvent(this,EventType.TOGGLE,row,column));
    }
    ...
    public void doSetValueAt(int row, int column,ClarionObject value) {
        synchronized(data) {
            if (row<=0 || row>records()-1) return;
            Record r = getList().get(row-1);
