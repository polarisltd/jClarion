...
public abstract class CExpr 
{
     ...
     * @return
     */
    public abstract CExprType getType(CExprScope scope);
     ...
     * @return
     */
    public abstract void cast(CExprScope scope,CExprType type);
    
    /**
     * Return whether or not expression type is fixed. i.e. cannot be easily recast
     */
    public abstract boolean isRecastableType(CExprScope scope);
    
    /**
     * Generate result
     * @return
     */
    public abstract ClarionObject eval(CExprScope scope);
    
    public final ClarionObject eval()
    {
    	return eval(CExprScope.DEFAULT);
    }
     ...
     *  not strict SQL
     */
    public abstract boolean generateString(CExprScope scope,StringBuilder out,boolean strict);
    
    public final boolean generateString(StringBuilder out,boolean strict)
    {
    	return generateString(CExprScope.DEFAULT,out,strict);
    }    
    ...
    protected CExprType resolveCasts(CExprScope scope,Iterable<CExpr> in)
    {
        CExprType type=null;
        boolean   force=false;
        ...
        for ( CExpr scan : in ) {
            if (type==null || !force) {
                type=scan.getType(scope);
                force=!scan.isRecastableType(scope);
                continue;
            }
        }
        return type;
    ...
    protected boolean isRecastableType(CExprScope scope,Iterable<CExpr> in)
    {
        for ( CExpr scan : in ) {
            if (!scan.isRecastableType(scope)) return false; 
        }
        return true;
    }
    
    protected void cast(CExprScope scope,Iterable<CExpr> in,CExprType type)
    {
        for ( CExpr scan : in ) {
            scan.cast(scope,type);
        }
    }
