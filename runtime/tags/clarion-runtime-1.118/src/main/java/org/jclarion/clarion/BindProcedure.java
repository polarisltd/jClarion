 ...
 *
 */
public abstract class BindProcedure implements LabelExprResult
{
    public abstract ClarionObject execute(ClarionString p[]);
    
    @Override
	public ClarionObject execute(CExprScope scope, CExpr... params) 
	{        
		ClarionString[] r=null;
        if (params==null) {
        	r=new ClarionString[0];
        } else {
            r=new ClarionString[params.length];
            int scan=0;
            for ( CExpr param : params ) {
                r[scan++]=param.eval(scope).getString();
            }
        }
