...
public class CExpressionTest extends TestCase {
    ...
    public void testEvaluate()
    {
        ...
        assertEquals("1",CExpression.evaluate("1=1").toString());
        assertEquals(0,CError.errorCode());

        assertEquals("0",CExpression.evaluate("1=2").toString());
        assertEquals(0,CError.errorCode());
        ...
        CExpression.bind("die",new BindProcedure() {
			@Override
			public ClarionObject execute(ClarionString[] p) {
				throw new IllegalStateException("Whargarbl");
			}
		});
        
        assertEquals("0",CExpression.evaluate("0 AND die").toString());
        assertEquals(0,CError.errorCode());

        assertEquals("1",CExpression.evaluate("1 OR die").toString());
        assertEquals(0,CError.errorCode());
        
        try {
        	CExpression.evaluate("1 AND die");
        	fail();
        } catch (IllegalStateException ex) {         	
        }

        try {
        	CExpression.evaluate("0 OR die");
        	fail();
        } catch (IllegalStateException ex) {         	
        }
        ...
        assertEquals("1",CExpression.evaluate("1 OR 1 AND 0").toString());
        assertEquals(0,CError.errorCode());
        
        assertEquals("0",CExpression.evaluate("(1 OR 1) AND 0").toString());
        assertEquals(0,CError.errorCode());

        assertEquals("0",CExpression.evaluate("1 and 0 OR 1 AND 0").toString());
        assertEquals(0,CError.errorCode());

        assertEquals("0",CExpression.evaluate("1 AND 0 OR 0 AND 1").toString());
        assertEquals(0,CError.errorCode());

        assertEquals("0",CExpression.evaluate("1 and (0 OR 1 AND 0)").toString());
        assertEquals(0,CError.errorCode());

        assertEquals("1",CExpression.evaluate("1 and (0 OR 1 AND 1)").toString());
        assertEquals(0,CError.errorCode());
        ...
        assertEquals("hello",CExpression.evaluate("'hello'").toString());
        assertEquals(0,CError.errorCode());

        assertEquals("hello world",CExpression.evaluate("'hello'&' world'").toString());
        assertEquals(0,CError.errorCode());
    }
	...
	public class ExprThread extends Thread
	{
		...
		public void run() 
		{
			...
			try {
				...
				ClarionNumber n2=new ClarionNumber();
				CExpression.bind("n1",n1);
				CExpression.bind("n2",n2);
				for (int scan=0;scan<200000;scan++) {
					int i1 = r.nextInt(100)+1;
					int i2 = r.nextInt(100)+1;
					n1.setValue(i1);
					n2.setValue(i2);
					if (r.nextInt(2)==0) {
						assertEquals(i1+i2,CExpression.evaluate("n1+n2").intValue());
					} else {
						assertEquals(i1-i2,CExpression.evaluate("n1-n2").intValue());						
					}
				}
	...
	{
		ExprThread et[] = new ExprThread[10];
		for (int scan=0;scan<10;scan++) {
			et[scan]=new ExprThread();
		}
		for (int scan=0;scan<10;scan++) {
			et[scan].start();
		}
