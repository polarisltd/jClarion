...
public class NumberFormat extends Formatter
{
    ...
    public NumberFormat(String format) {
        ...
        boolean groupingExplicitlySet=false; 
        
        if (decoder.pop('0')) { fill=Fill.ZERO; grouping=Group.NONE; groupingExplicitlySet=true;}
        else if (decoder.pop('_')) { fill=Fill.SPACE; grouping=Group.NONE; groupingExplicitlySet=true;}
        else if (decoder.pop('*')) { fill=Fill.STAR; grouping=Group.NONE; groupingExplicitlySet=true;}
        ...
        if (decoder.peekChar(0)=='.') {
            ...
            if (c<'0' || c>'9') {
                ...
                groupingExplicitlySet=true;
               }
        } 
        else if (decoder.pop('-')) { grouping=Group.HYPHEN; groupingExplicitlySet=true; }
        else if (decoder.pop('_')) { grouping=Group.SPACE; groupingExplicitlySet=true; }
        ...
        else if (decoder.pop('\'') || decoder.pop('`')) {
            decimal=Decimal.COMMA;
            Integer s = decoder.popNumeric();
            if (s==null) throw new IllegalArgumentException("Invalid Picture");
            decimalSize=s;
            if (!groupingExplicitlySet) {
            	grouping=Group.PERIOD;
            }
        } 
    ...
    public String deformat(String input) {
    	return deformat(input,true);
    }

    private String deformat(String input,boolean strictDecimal) {
        ...
        for (int scan=bits.length-1;scan>=bits.length-(iend-istart);scan--) {
            ...
            if (!keep) {
                if (!decimal) {
                    ...
                    if ((c==',' || (c=='.' && !strictDecimal)) && this.decimal==Decimal.COMMA) {
                        decimal=true;
                        keep=true;
                        c='.';
                    }
    ...
    public String format(String input) {
        ...
        input=input.trim();
        ClarionString cs = new ClarionString(input);
        if (!cs.isNumeric()) {
        	input=deformat(input,false);
        ...
        // remove leadng zeros
        int leadZero=0;
        while (leadZero<main.length() && main.charAt(leadZero)=='0') {
        	leadZero++;
        }
        if (leadZero>=main.length()) {
        	leadZero=main.length()-1;
        }
        if (leadZero>0) main=main.substring(leadZero);
        
        if (fraction.length()>decimalSize) {
        	int end = fraction.length()-1;
        	while (end>=0 && fraction.charAt(end)=='0') {
       			end--;
        	}
        	fraction=fraction.substring(0,end+1);
        }
