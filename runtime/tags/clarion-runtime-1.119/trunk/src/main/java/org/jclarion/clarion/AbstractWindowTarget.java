...
public abstract class AbstractWindowTarget extends AbstractTarget implements RemoteWidget {
    ...
    public static boolean suppressDefaultRatio=false;
	...
	public void handleOpen() {
		Container window = this.rootPane;
		if (window==null) return;
		handleOpen(window.getSize());
	}
	
	public void handleOpen(Dimension size) {
		for (AbstractControl ac : getControls()) {
			ac.capturePosition(size.width,size.height);
		}
	}
	...
	public void handleResize(Dimension size)
	{
		for (AbstractControl ac : getControls()) {
			ac.resize(size.width,size.height);
		}		
	}
	
	public List<PropertyChange> collateResizeChanges()
	{
		List<PropertyChange> store=new ArrayList<PropertyChange>();
		for (AbstractControl ac : getControls()) {
			ac.collateResizeChanges(store);
		}		
		return store;
	}
