...
public class QueueComboModel implements ComboBoxModel, ClarionQueueListener
{
    ...
    public QueueComboModel(AbstractListControl control,int ofs)
    {
    ...
    public int getIndex(String anItem)
    {
        ...
        for (int scan=1;scan<=queue.records();scan++) {
            if (anItem.equals(queue.getRecord(scan)[ofs].toString().trim())) {
                return scan;
            }
        }
        return 0;
    ...
    public Object getElementAt(int index) 
    {
        ...
        return new ClarionQueueTableCell(queue,index+1,columns[0],co,ofs);
    }
