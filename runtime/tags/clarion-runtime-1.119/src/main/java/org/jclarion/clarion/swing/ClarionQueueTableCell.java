...
public class ClarionQueueTableCell 
{
    ...
    {
    	this(queue,record,column,row,0);
    }

    public ClarionQueueTableCell(ClarionQueueReader queue,int record,ListColumn column,ClarionObject row[],int ofs)
    {
        this.column=column;
        this.row=row;
        this.queue=queue;
    ...
    {
        int pos = column.getFieldNumber()-1+ofs;
    ...
    public ClarionObject getValue(int offset) {
        int pos = column.getFieldNumber()-1+offset+this.ofs;
    ...
    {
        return queue.hasChildren(record,column.getFieldNumber()+pos+ofs);
    }
    ...
    {
        return queue.getSiblingTree(record,column.getFieldNumber()+pos+ofs);
    }
    ...
    public void handleMouseEvent(MouseEvent e,int x, int y,int width,int height) {
        ...
        if (column.isTree()) {
            ...
            int ofs = 1 + (column.isIcon() | column.isTransparantIcon() ? 1 : 0)+this.ofs;
