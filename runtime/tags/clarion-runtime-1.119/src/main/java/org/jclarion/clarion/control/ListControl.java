...
public class ListControl extends AbstractListControl {
	...
	private boolean column_scale=false;
	private boolean resize_changed=true;
    ...
    private class CustomJTable extends JTable
    {
		...
		protected void paintComponent(Graphics g) {
			...
			if (hasFocus() && getRowCount()==0) {
				g.setColor(java.awt.Color.GRAY);
				Graphics2D g2d = (Graphics2D)g;
				Stroke s = g2d.getStroke();
				g2d.setStroke(new BasicStroke(1,BasicStroke.CAP_BUTT,BasicStroke.JOIN_MITER,10,new float[] { 2,3},0));
				g.drawRect(0,0,getWidth()-1,getRowHeight());
				g2d.setStroke(s);
			}
		}
		...
		public void doLayout() 
        {
            ...
            for (scan=0;scan<columns.length;scan++) {
                TableColumn last = getColumnModel().getColumn(scan);
                columns[scan].establish(last.getWidth());
            }
            
            testColumnResize();
        }

        private void testColumnResize() {
            ListColumn columns[] = getColumns();
            if (columns==null) return;
            if (columns.length!=table.getColumnCount()) return;

            boolean notify=false;
            
			for (int scan=0;scan<columns.length;scan++) {
				TableColumn tc = table.getColumnModel().getColumn(scan);
				ListColumn lc = columns[scan];
            	if (lc.changeWidth(tc.getWidth())) {
            		AbstractWindowTarget awt = getWindowOwner();
            		if (awt!=null) {
            			lc.setWidth(awt.widthPixelsToDialog(tc.getWidth()));
            			notify=true;
            		}
            	}
			}
			if (notify) {
				String newFormat=ListColumn.deconstruct(columns);
				if (newFormat.equals(lastFormat)) return;
				lastFormat=newFormat;
				notifyFormatChange(Event.COLUMNRESIZE,newFormat);
			}
        }
    ...
    {
    	this.column_scale=false;
    	this.resize_changed=true;
    	this.items=null;
        this.base=null;
        this.hscroll=false;
        this.table=null;
    ...
    private void notifyNewSelection()
    {
        ...
        if (sr>=qtm.getAssumedRowCount()) {
        	return;
        }
        
        sr = qtm.getQueue().convertScreenIndexToQueueIndex(sr)+1;
        if (sr>getFrom().records()) {
        	return;
        }
        
        if (qtm.getRowCount()==0) return;
	...
	protected void handleRefresh(Object... params) {
		if (qtm!=null) {
			qtm.processModelChange();
		}
	}

	public void notifyTreeToggle(int record, int column) 
	{
		GUIModel.getServer().send(this,TREE_TOGGLE,record,column);
	}    
    ...
    public void constructSwingComponent(Container parent) 
    {
        ...
        table.addFocusListener(new FocusListener() {
            ...
            public void focusGained(FocusEvent e) {
                ...
                if (isProperty(Prop.NOBAR) || tab.getRowCount()==0) {
                    tab.repaint();
                }
            }
            ...
            public void focusLost(FocusEvent e) {
                ...
                if (tab!=null) {
                    tableFocus=false;
                    if (isProperty(Prop.NOBAR) || tab.getRowCount()==0) tab.repaint();
                }
            }
        });
	...
	protected boolean isAWTChange(int indx) {
        switch(indx) {
        	...
        	case Prop.FORMAT:
        		...
        		if (queue!=null) queue.checkFormatChange();
        		return true;
            case Prop.VSCROLLPOS: 
            case Prop.FONTNAME:
            case Prop.LINEHEIGHT:
	...
	protected void applyRatio(int width, int height, String t) {
		super.applyRatio(width, height, t);
		if (t.equals("listscale")) {
			column_scale=true;
			resize_changed=true;
			resized=false;
		}
    ...
    {    
		...
		if (base!=null && column_scale && resize_changed) {
			base_width=this.base.getWidth();
			for (ListColumn lc : getColumns()) {
				lc.initBaseWidth();
			}
		...
		float size =0;
		if (table!=null) {
			size=table.getFont().getSize2D();
		}
		
		super.resize(_width,_height);
		
		if (table!=null && size!=table.getFont().getSize2D()) {
			table.setRowHeight(table.getFontMetrics(table.getFont()).getHeight());
		}
		
		if (table!=null && column_scale) {
			int current_width=this.base.getWidth();
			resized=true;
			
			for (ListColumn lc : getColumns()) {
				int new_width = lc.getBaseWidth()*current_width/base_width;
				lc.setSwingWidth(new_width);
			}
			table.doLayout();
		}
    }
	...
	public void captureThisPosition(int width, int height) {
		super.captureThisPosition(width, height);
		resize_changed=true;
	}

	@Override
	public void collateResizeChanges(List<PropertyChange> store) {
		super.collateResizeChanges(store);
		if (resized) {
			String newFormat=ListColumn.deconstruct(getColumns());
			PropertyChange pc = new PropertyChange(this,Prop.FORMAT,new ClarionString(newFormat));
			store.add(pc);
		}
	}
