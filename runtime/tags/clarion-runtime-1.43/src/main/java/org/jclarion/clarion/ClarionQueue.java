...
public class ClarionQueue extends ClarionGroup implements ClarionQueueReader
{
    ...
    protected static class QueueEntry implements Comparable<QueueEntry>
    {
        ...
        public void copyFromGroup(GroupEntry base) 
        {
            ...
            if (value!=null) {
                if (value instanceof ReferenceVariable) {
                    ReferenceVariable rv = (ReferenceVariable)value;
                    try {
                        value=rv.field.get(rv.clazz);
                    } catch (IllegalArgumentException e) {
                        log.log(Level.SEVERE,"Got Exception trying to clone group entry",e);
                ...
                if (value instanceof ClarionObject) {
                    value=((ClarionObject)value).genericLike();
                    return;
                }
                if (value instanceof ClarionGroup) {
                    value=((ClarionGroup)value).genericLike();
                    return;
                }
            }
            if (array!=null) {
                Object narray[]=new Object[array.length];
                for (int scan=1;scan<narray.length;scan++) {
                    if ((array[scan] instanceof ClarionObject)) {
                        narray[scan]=((ClarionObject)array[scan]).genericLike();
                    } else {
                        narray[scan]=array[scan];
                    }
                }
                array=narray;
            }
        ...
        {
            if (base.value!=null) {
                if (base.value instanceof ReferenceVariable) {
                    ReferenceVariable rv = (ReferenceVariable)base.value;
                    try {
                        rv.field.set(rv.clazz,value);
                    } catch (IllegalArgumentException e) {
                        log.log(Level.SEVERE,"Got Exception trying to clone group entry",e);
                    } catch (IllegalAccessException e) {
                        log.log(Level.SEVERE,"Got Exception trying to clone group entry",e);
                ...
                if (base.value instanceof ClarionObject) {
                    ((ClarionObject)base.value).setValue(value);
                    return;
                }

                if (base.value instanceof ClarionGroup) {
                    ((ClarionGroup)base.value).merge((ClarionGroup)value);
                    return;
                }
            }
            if (base.array!=null) {
                for (int scan=1;scan<base.array.length;scan++) {
                    if ((base.array[scan] instanceof ClarionObject)) {
                        ((ClarionObject)base.array[scan]).setValue(
                                array[scan]);
                    }
                }
            }
        ...
        public int compareTo(QueueEntry qe) {
            ...
            if (value==null && qe.value==null) {
                return 0;
            }
            if (value==null) return -1;
            if (qe.value==null) return 1;
            
            if (value instanceof ClarionObject) {
                return ((ClarionObject) value).compareTo(qe.value);
            }
            
            if (value instanceof Comparable) {
                return ((Comparable<Object>) value).compareTo(qe.value);
            }
            
            return value.toString().compareTo(qe.value.toString());
        }
    }
    ...
    public class Order implements Comparator<Record>
    {
        ...
        public Order(String s) {
            ...
            while ( !decoder.end() ) {
                ...
                if (field.length()>0) {
                    ...
                    int p =getGroupParamPosition(field.substring(pos+1));
        ...
        public Order ascend(ClarionObject object)
        {
            if (object==null) throw new RuntimeException("Looking for null");
            int r = flatWhere(object);
            if (r==0) {
        ...
        public Order descend(ClarionObject object)
        {
            int r = flatWhere(object);
            if (r==0) {
                throw new RuntimeException("Sort field indeterminite");
        ...
        public boolean equals(Object o) {
            return order.equals(((Order)o).order);
        }
    ...
    {
        if (activeSort!=null && indexedSort==null) {
            indexedSort=new ArrayList<Record>(activeSort);
        }
        if (indexedSort==null) {
            indexedSort=new ArrayList<Record>();
        }
        return indexedSort;
    }
    ...
    {
        if (r.pos==-1 || r.posCalculated!=lastMutate) {
            int pos=1;
            for ( Record scan : getList() ) {
                scan.pos=pos++;
                scan.posCalculated=lastMutate;
            }
        }
        return r.pos;
    }
    ...
    public void add()
    {
        ...
        synchronized(this) { 
            ...
            getList().add(current);
            currentPos=indexedSort.size();
            if (sorts!=null) {
                for (TreeSet<Record> s : sorts.values()) {
                    s.add(current);
                }
            }
            lastMutate++;
        ...
        notifyChange(new ClarionQueueEvent(this,
                ClarionQueueEvent.EventType.ADD,currentPos));
    }
    ...
    public void add(int position)
    {
        ...
        synchronized(this) { 
            ...
            if (position-1<=getList().size()) {
                getList().add(position-1,current);
                currentPos=position;
            } else {
                getList().add(current);
                currentPos=getList().size();
            }
            if (sorts!=null) {
                for (TreeSet<Record> s : sorts.values()) {
                    s.add(current);
    ...
    private TreeSet<Record> getSortOrder(Order order)
    {
        TreeSet<Record> s;
        if (this.sorts==null) {
            this.sorts=new HashMap<Order, TreeSet<Record>>();
        }
        s=sorts.get(order);
        if (s==null) {
            ...
            if (activeSort!=null) {
                s.addAll(activeSort);
            } else if (this.indexedSort!=null) {
                s.addAll(indexedSort);
            }
            sorts.put(order,s);
        }
    ...
    public void put(boolean keepIndexedSort)
    {
        ...
        synchronized(this) {
            if (sorts!=null) {
                ...
                Record r = createRecord(current.id);
                for ( TreeSet<Record> sort : sorts.values() ) {
                
                    if (sort.contains(r)) continue;
                    SortedSet<Record> ss;
                    ss = sort.headSet(current);
                    if (!ss.isEmpty()) {
        ...
        notifyChange(new ClarionQueueEvent(this,
                ClarionQueueEvent.EventType.PUT,currentPos));
    }
    ...
    public void delete()
    {
        synchronized(this) {
            ...
            if (indexedSort!=null) {
                if (currentPos>0) {
                    indexedSort.remove(currentPos-1);
                } else {
                    indexedSort.remove(current);
                }
            }
            current=null;
            lastMutate++;
        ...
        notifyChange(new ClarionQueueEvent(this,
                ClarionQueueEvent.EventType.DELETE,currentPos));
    }
    ...
    public void free()
    {
        ...
        notifyChange(new ClarionQueueEvent(this,
                ClarionQueueEvent.EventType.FREE,currentPos));
    }
    ...
    public void sort(Order order)
    {
        ...
        notifyChange(new ClarionQueueEvent(this,
                ClarionQueueEvent.EventType.SORT,currentPos));
    }

    public void sort(ClarionString order)
    {
        synchronized(this) {
            changeActiveSortOrder(new Order(order.toString()));
        }
    ...
    {
        synchronized(this) {
            if (this.indexedSort!=null) {
                return this.indexedSort.size();
            } 
            if (this.activeSort!=null) {
                return this.activeSort.size();
            }
        }
    ...
    public void get(Order order)
    {
        synchronized(this) {
            ...
            if (s==activeSort) {
                currentPos=records()+1-tail.size();
            } else {
                currentPos=-1;
            }
    ...
    {
        synchronized(this) {
            if (this.activeSort==null) return getPointer();
        
            Record scan = createRecord(0);
            SortedSet<Record> tail = activeSort.tailSet(scan);
    ...
    {
        synchronized(this) {
            if (currentPos==-1 && current!=null) {
                currentPos=getPosition(current);
            }
            return currentPos;
        }
    }
