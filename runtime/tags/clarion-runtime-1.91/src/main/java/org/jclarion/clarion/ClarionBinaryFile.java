...
import org.jclarion.clarion.memory.ByteBufferCMem;
...
public class ClarionBinaryFile extends ClarionFile 
{
    ...
    public void get(int position, Integer len) 
    {
        ...
        try {
            ...
            while(true) {
                ...
                if (read<=0) break;
                stream.writeBytes(get_buffer,0,read);
            }
    ...
    public void next() {
        ...
        try {
            ...
            while ( true ) {
                ...
                stream.writeBytes(buffer,buffer_offset,avail);
                buffer_offset+=avail;
    ...
    private void write(long pos,int len) 
    {
        try {
            ...
            stream.reset();
            this.serialize(stream);
            while (stream.getSize()<len) stream.writeChar(' ');
	...
	public void copyTo(String target) {
		CFile.copyFile(getName().toString(),target);
	}
}
