...
import org.jclarion.clarion.runtime.ThreadCleanupTasks;
...
public class PgSource implements AbstractJDBCSource
{
    ...
    private MyConnection getMyConnection() throws SQLException
    {
        ...
        if (tConnection==null || tConnection.connection==null) {
            ...
            ThreadCleanupTasks.getInstance().add(new Cleanup() {
                    @Override
                    public void cleanup() {
                        MyConnection mc = connection.get(Thread.currentThread());
                        if (mc!=null) {
                            mc.cleanup();
                        }
                    } 
