...
public class ButtonTest extends SwingTC {
    ...
    public void testResizeAndRelocate()
    {
        ...
        */
        ...
        assertEquals(30*w.getFontHeight()/8,b.getComponent().getHeight());

        b.setProperty(Prop.WIDTH,40);
        waitForEventQueueToCatchup();

        assertEquals(40*w.getFontWidth()/4,b.getComponent().getWidth());

        b.setProperty(Prop.XPOS,25);
        waitForEventQueueToCatchup();

        assertEquals(25*w.getFontWidth()/4,b.getComponent().getX());
        
        b.setProperty(Prop.YPOS,58); 
        waitForEventQueueToCatchup();

        assertEquals(58*w.getFontHeight()/8,b.getComponent().getY());

        b.setProperty(Prop.TEXT,"New Text");
        waitForEventQueueToCatchup();
        assertEquals("New Text",b.getButton().getText());
