...
public class PrintObject 
{
    ...
    public PrintObject split(Page report,PrintContext g,int maxHeight)
    {
    	PrintObject po = new PrintObject();
    	
    	po.control=this.control;
    	po.fieldValues=this.fieldValues;
    	po.height=this.height;
    	po.moveable=this.moveable;
    	po.oheight=this.oheight;
    	po.positionedX=this.positionedX;
    	po.positionedY=this.positionedY;
    	po.width=this.width;
    	po.x=this.x;
    	po.y=this.y;
    	
    	for (PrintElement pe : this.components) {
    		po.components.add(pe.split(report,g,maxHeight-this.y));
    	}
    	
    	po.getSize(report,g);
    	this.getSize(report,g);
    	return po;
    }
    ...
    public boolean containsResizableComponents() {
        for ( PrintElement e : components ) {
        	if (e.isResize()) return true;
        }
        return false;
    }
