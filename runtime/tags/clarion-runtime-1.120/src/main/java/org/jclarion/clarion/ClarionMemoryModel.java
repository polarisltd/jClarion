...
public abstract class ClarionMemoryModel implements Threaded 
{
    ...
    public ClarionMemoryModel getFirstOverPeer()
    {
        State s = state.get();
        synchronized(s) {
            if (s.listeners==null) return null;
            for (MyReference r : s.listeners) {
            	ClarionMemoryChangeListener cmcl = r.get();
            	if (cmcl==null) continue;
            	if (!(cmcl instanceof OverGlue)) continue;
            	OverGlue og = (OverGlue)cmcl;
            	ClarionMemoryModel candidate = og.getPeer(this);
