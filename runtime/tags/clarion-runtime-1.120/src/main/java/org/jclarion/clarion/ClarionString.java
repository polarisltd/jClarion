...
public class ClarionString extends ClarionObject 
{
    ...
    {
        State lState = state.get();
        if (lState.chararray==null) return -1;
        return lState.chararray.length;    	
    }

    private void shrinkCharArray()
    {
        State lState = state.get();
        if (lState.chararray==null) return;
        
        int optimalSize=lState.len*2+128;
        if (optimalSize<lState.chararray.length/4) {
        	char n[] = new char[optimalSize];
        	System.arraycopy(lState.chararray,0,n,0,lState.len);
        	lState.chararray=n;
        }
    }
	...
	public String toBinaryString() {
		return toBinaryString(toString());
	}
	...
	public static String toBinaryString(String s) {
		...
		for (int scan = 0; scan < len; scan++) {
			...
			sb.append(hexits[c >> 4]);
			sb.append(hexits[c & 15]);
			sb.append(' ');
		}
