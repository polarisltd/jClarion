...
public class ClarionIcon implements Icon, ClarionIconData
{
	private Image 	image;
	...
	public ClarionIcon(ImageControl img)
	{
        ...
        if (text.length()>0) {
            image  = CWin.getInstance().getImage(text,0,0);
        } else {
            ClarionObject payload = img.getRawProperty(Prop.IMAGEBITS);
            if (payload!=null) {	
                image = CWin.getInstance().getImageFromBinaryData(payload);
            }
        }
        ...
        if (!img.isProperty(Prop.CENTER) && (mw==null || mh==null)) {
