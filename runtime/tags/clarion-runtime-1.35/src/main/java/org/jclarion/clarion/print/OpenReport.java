    ...
    */
    ...
    private Page finalizePage(Page currentPage,boolean force)
    {
        ...
        if (force || currentPage.getMoveableCount()>0) {
            ...
            currentPage.setPageNo(pages.size());
    ...
    private Page doNewPage() 
    {
        Page currentPage=new Page(this,
                report.isProperty(Prop.LANDSCAPE) ? PageFormat.LANDSCAPE : PageFormat.PORTRAIT );
    ...
    public void print()
    {
        if (job!=null) {
            getBook().init();
            job.setPrintable(getBook());
            try {
                job.print(printAttribute);
            ...
            CWin.message(
                    Clarion.newString("No Printer Defined"),
                    Clarion.newString("Print Error"),
                    Icon.HAND);
        }
    }
