...
public class ClarionBasicFile extends ClarionFile 
{
    ...
    private SharedOutputStream         write_stream=new SharedOutputStream();

    private byte    endOfRecord[]       = new byte[] { (byte)'\r', (byte)'\n' };
    private byte    fieldDelimiter[]    = new byte[] { (byte)',' };
    ...
    private boolean matches(byte b,byte in[])
    {
        for (byte test : in ) {
    ...
    private void write(long pos) 
    {
        try {
            ...
            for (int scan=1;scan<=getVariableCount();scan++) {
                ...
                if (o instanceof ClarionString) {
                    ...
                    if (!doQuote) {
                        for (int s_scan=0;s_scan<len;s_scan++) {
                            byte b = (byte)s.charAt(s_scan);
                            if (matches(b,endOfRecord) || matches(b,this.fieldDelimiter) || b==27 | b==this.quote) {
                                doQuote=true;
                                break;
                            }
                        }
                ...
                for (int s_scan=0;s_scan<len;s_scan++) {
                    byte b = (byte)s.charAt(s_scan);
                    if (!this.endOfRecordInQuote && matches(b,this.endOfRecord)) continue;
                    if (!this.eofINQuote && b==27) continue;
                    if (b==this.quote) {
            ...
            byte[] buffer = write_stream.getBytes();
            int size = write_stream.getSize();
        ...
        try {
            if (controlZIsEOF) {
                ...
                if (buffer_offset<buffer_size) {
                    if (buffer[buffer_offset]==27) return true;
                }
            }
    ...
    private SharedOutputStream  read_stream=new SharedOutputStream();
    ...
    private boolean appendToStream(int next_byte)
    {
        ...
        read_stream.write(next_byte);
        if (quoteMode==QM_QUOTE && !endOfRecordInQuote && read_stream.endsWith(endOfRecord[endOfRecord.length-1])) {
            quoteMode=QM_NONE;
        } 
        if (quoteMode==QM_QUOTE && !endOfRecordInQuote && read_stream.endsWith((byte)27)) {
            quoteMode=QM_NONE;
        }

        if (quoteMode==QM_NONE) {
            if (next_byte==fieldDelimiter[fieldDelimiter.length-1] && read_stream.endsWith(fieldDelimiter)) {
                read_stream.skip(-fieldDelimiter.length);
                streamToField();
                return false;
            }
            
            if (next_byte==endOfRecord[endOfRecord.length-1]) {
                read_stream.shrink(endOfRecord);
                streamToField();
                return true;
            }
            
            if (next_byte==27 && this.controlZIsEOF) {
                read_stream.shrink();
    ...
    private void streamToField() 
    {
        ...
        byte buffer[] = read_stream.getBytes();
        for (int scan=0;scan<len;scan++) {
            read_param.append((char)buffer[scan]);
        }
    ...
    public void send(String operator) {
        ...
        if (key.equalsIgnoreCase("comma")) {
            fieldDelimiter=new byte[] { Byte.parseByte(value) };
            return;
        }

        if (key.equalsIgnoreCase("fielddelimiter")) {
            fieldDelimiter=decodeByteArraySend(value);
            return;
        }
        ...
        if (key.equalsIgnoreCase("endofrecord")) {
            endOfRecord=decodeByteArraySend(value);
            return;
        }
