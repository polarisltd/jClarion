...
public abstract class AbstractListControl extends AbstractControl
{
    ...
    public static final int TREE_TOGGLE=104;
    ...
    protected void doNotifyLocalChange(int indx,ClarionObject value) 
    {
        ...
        if (indx==Prop.FROM){
        	from=(ClarionQueue)value.getOwner();
        	if (qcm!=null) {
        		int pos =from.where(value)-1;
        		if (pos<0) pos=0;
                qcm = new QueueComboModel(this,pos);
                combo.setModel(qcm);
        	}
        }
    ...
    public void createComboControl(Container parent,boolean edit) 
    {
        ...
        int indx = 0;
        ClarionObject co = getRawProperty(Prop.SELSTART,false);
