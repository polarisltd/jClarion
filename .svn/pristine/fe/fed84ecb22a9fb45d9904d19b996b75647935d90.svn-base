...
public class StatementParser extends AbstractParser {
    ...
    public StatementParser(ClarionCompiler compiler,Parser parser) {
    ...
    private JavaCode statement() {
        int startDepth = lexer().getSavePointDepth();
        try {
        	return unsafeStatement();
        } catch (AccumulatingClarionCompileError ex) {
        	
        	while (lexer().getSavePointDepth()>startDepth) {
        		lexer().commit();
        	}
        	while (true) {
        		Lex l = next();
        		if (l.type==LexType.eof || l.type==LexType.nl) break;
        	}
        	return new SimpleJavaCode("#COMPILATION ERROR");
        }
    }    
    
    private JavaCode unsafeStatement() {
        int pos = begin();
        ...
        if (result == null && !implicit  && la().value.equalsIgnoreCase("exit")) {
            commit(pos);
            next();
            if (!(compiler.getScope() instanceof RoutineScope))
        ...
        if (result == null //&& la().type == LexType.label
            ...
            if (e != null) {
                ...
                if (e instanceof CallExpr) {
                    commit(pos);
                    result = new ExprJavaCode(new DecoratedExpr(JavaPrec.LABEL,e, ";"));
                }
                ...
                if ((e instanceof VariableExpr) && (la().type == LexType.assign)) {
                    commit(pos);
                ...
                if (result!=null) {
                	JavaDependencyCollector collector = new JavaDependencyCollector();
                	e.collate(collector);
                	if (!collector.getCode().isEmpty()) {
                		LinearJavaBlock ljb = new LinearJavaBlock();
                		ljb.add(new SimpleJavaCode("{"));
                		for (JavaCode jc : collector.getCode()) {
                			ljb.add(jc);
                		}
                		ljb.add(result);
                		ljb.add(new SimpleJavaCode("}"));
    ...
    private JavaCode assignComplexScope(Expr left, Expr right) {
        ...
        if (left.type().getArrayDimSize()>0 && right.type().getArrayDimSize()>0) {
            String loop = compiler.getScope().createTemporaryLabel("_arr_scan");
        ...
        if (left.type().isa(ExprType.group)
            ...
            for ( Variable v : list ) {
                ...
                Expr i_left = left.type().field(left,v.getName(),compiler.getScope());
                if (i_left==null) error("Did not expect this to happen");
                Expr i_right = right.type().field(right,v.getName(),compiler.getScope());
    ...
    private JavaCode caseStatement() {
        ...
        String label = compiler.getScope().createTemporaryLabel("case_");
    ...
    private JavaCode loopStatement(String breakLabel) {
        ...
        if (from != null) {
            ...
            String label = compiler.getScope().createTemporaryLabel("loop_");            
    ...
    private JavaCode executeStatement() {
        ...
        String var = compiler.getScope().createTemporaryLabel("execute_");
    ...
    public JavaCode routineStatement() {
        ...
        ReturningScope s = (ReturningScope) compiler.getScope();

        RoutineScope rs = s.getProcedure().getRoutine(compiler,name.value);

        RoutineScope caller = null;
        if (compiler.getScope() instanceof RoutineScope) {
            caller = (RoutineScope) compiler.getScope();
    ...
    public JavaCode returnStatement() {
        ...
        if (compiler.getScope() instanceof ReturningScope) {
            expectedType = ((ReturningScope) compiler.getScope())
                    .getReturnValue();
        }
    ...
    public JavaCode returnStatement(Expr result, ReturningExpr expectedType) {
        ...
        if (compiler.getScope() instanceof RoutineScope) {
            ...
            ((RoutineScope)compiler.getScope()).setMayReturnToProcedure();
