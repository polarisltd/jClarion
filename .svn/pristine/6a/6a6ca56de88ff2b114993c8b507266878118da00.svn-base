...
public abstract class AbstractControl extends PropertyObject implements RemoteWidget
{
    ...
    private String			drag[];
    private String			drop[];
    private ClarionObject	resetX; 	
    private ClarionObject	resetY;
    ...
    public AbstractControl setRatio(String region)
    {
        setProperty(Prop.RATIO,region);
        return this;    	
    }

    public AbstractControl setRatio(Integer x,Integer y,Integer width,Integer height)
    {
        setProperty(Prop.RATIO_X,x);
        setProperty(Prop.RATIO_Y,y);
        setProperty(Prop.RATIO_WIDTH,width);
        setProperty(Prop.RATIO_HEIGHT,height);
        return this;    	
    }
    ...
    public boolean isSettable(int indx,ClarionObject value)
    {
    	switch(indx) {
    		...
    		case Prop.VSCROLLPOS:
    			...
    			if (value==null && co==null) return false;
    			if (value==null || co==null) return true;
    			return co.intValue()!=value.intValue();
    	}
        return true;
    }
    ...
    public void notifyActualSize()
    {
    	...
    	GUIModel.getServer().send(this,ACTUAL_SIZE_NOTIFY,
    		getWindowOwner().widthPixelsToDialog(r.width- (i == null ? 0 : i.left + i.right),true),
    		getWindowOwner().heightPixelsToDialog(r.height - (i == null ? 0 : i.top + i.bottom),true)
    		);
    	
    	Container container = getWindowOwner().getRootPane();
    	if (container!=null) {
    		captureThisPosition(container.getWidth(),container.getHeight());
    	}
    }

    public void notifyActualResize()
    {
    	if (CWin.getInstance().isEditorMode()) return;
    	...
    	Rectangle rect = new Rectangle(
        		getWindowOwner().widthPixelsToDialog(r.x+ (i == null ? 0 : i.left),false),
        		getWindowOwner().heightPixelsToDialog(r.y+ (i == null ? 0 : i.top),false),
        		getWindowOwner().widthPixelsToDialog(r.width- (i == null ? 0 : i.left + i.right),true),
        		getWindowOwner().heightPixelsToDialog(r.height - (i == null ? 0 : i.top + i.bottom),true)
        		);
    	
    	if (getProperty(Prop.WIDTH).intValue()<0) {
    		rect.x=rect.x+rect.width;
    		rect.width=-rect.width;
    	}
    	if (getProperty(Prop.HEIGHT).intValue()<0) {
    	...
    	switch (command) {
    		...
    		case ACTUAL_SIZE_NOTIFY: {
    			try {
    				...
    				if (params.length>2) {
        				setProperty(Prop.XPOS,params[2]);
        				setProperty(Prop.YPOS,params[3]);    					
    				}
    			} finally {
    				forcedUpdate=false;
    			}
    ...
    public void clearMetaData() 
    {
        ...
        if (!CWin.getInstance().isEditorMode()) {
        	try {
        		...
        		setProperty(Prop.XPOS,resetX);
        		setProperty(Prop.YPOS,resetY);
        	} finally {
        		forcedUpdate=false;
        	}
        }
    ...
    */
	...
	public void opened() 
	{
		...
		resetX=getRawProperty(Prop.XPOS,false);
		resetY=getRawProperty(Prop.YPOS,false);
	}
	
	private Point 		window_base;
	private Rectangle	control_rect;
	private Rectangle	control_adjust;
	private boolean		font_adjust;
	private float		font_size;
	private String		default_ratio;

	protected ClarionObject getDeepProperty(int index)
	{
		PropertyObject scan=this;
		while ( true ) {
			ClarionObject co = scan.getRawProperty(index,false);
			if (co!=null) return co;
			scan=scan.getParentPropertyObject();
			if (scan!=null) {
				co = scan.getRawProperty(index+0x100,false);
				if (co!=null) return co;
			} else {
				return null;
			}
	...
	protected int getDeepInt(int index,int def)
	{
		ClarionObject co = getDeepProperty(index);
		if (co==null) return def;
		return co.intValue();
	}
	
    public void capturePosition(final int width,final int height)
    {
    	captureThisPosition(width,height);
        for (AbstractControl kid : getChildren()) {
        	kid.capturePosition(width,height);
        }
    }
    
    public void captureThisPosition(int width, int height) {    	
        Component c=  getComponent();
        if (c!=null) {
        	window_base=new Point(width,height);
        	control_rect=c.getBounds();

        	font_adjust=false;
        	font_size=0;
        	control_adjust=new Rectangle();
        
        	String ratio  =default_ratio;
        	if (ratio==null) {
        		ClarionObject co = getDeepProperty(Prop.RATIO);
        		if (co!=null) ratio=co.toString();
        	}
        	if (ratio!=null) {
        		String s = ratio.toLowerCase();
        		for (String t : s.split("[^a-z]")) {        			
        			applyRatio(width,height,t);
        		}
        	}
        	
        	control_adjust.x=getDeepInt(Prop.RATIO_X,control_adjust.x);
        	control_adjust.y=getDeepInt(Prop.RATIO_Y,control_adjust.y);
        	control_adjust.width=getDeepInt(Prop.RATIO_WIDTH,control_adjust.width);
        	control_adjust.height=getDeepInt(Prop.RATIO_HEIGHT,control_adjust.height);

        	
        	if (control_adjust.x==0 && control_adjust.y==0 && control_adjust.width==0 && control_adjust.height==0 && font_adjust==false) {
        		control_adjust=null;
        		window_base=null;
        		control_rect=null;
        	}
        	...
        	if (font_adjust) {
        		if (fontComponents!=null && !fontComponents.isEmpty()) {
        			font_size = fontComponents.iterator().next().getFont().getSize2D();
        		} else {
        			font_size = c.getFont().getSize2D();
        		}
        	}
        }
	...
	protected void applyRatio(int width, int height, String t) {
		
		if (t.equals("fontscale")) {
			font_adjust=true;
		}
		
		if (t.equals("spread")) {
			control_adjust.width=1000;
			control_adjust.height=1000;
		}
		if (t.equals("spreadx")) {
			control_adjust.width=1000;
		}
		...
		if (t.equals("bottom")) {
			control_adjust.y=1000;
		}
		if (t.equals("right")) {
			control_adjust.x=1000;
		}
		
		if (t.equals("scalex") && width>0) {
			control_adjust.x=control_rect.x*1000/width;
		}
		
		if (t.equals("scaley") && height>0) {
			control_adjust.y=control_rect.y*1000/height;
		}

		if (t.equals("scalewidth") && width>0) {
			control_adjust.width=control_rect.width*1000/width;
		}
		
		if (t.equals("scaleheight") && height>0) {
			control_adjust.height=control_rect.height*1000/height;
		}
		
		if (t.equals("scale") && width>0 && height>0) {
			control_adjust.x=control_rect.x*1000/width;
			control_adjust.y=control_rect.y*1000/height;
			control_adjust.width=control_rect.width*1000/width;
			control_adjust.height=control_rect.height*1000/height;
	...
	public void resize(final int _width,final int _height)
    {    	
        Component c= window_base!=null ?  getComponent() : null;        
        if (c!=null) {
        	int width=_width-window_base.x;
        	int height=_height-window_base.y;
    	
        	Rectangle r = new Rectangle();
        	r.x=control_rect.x+width*control_adjust.x/1000;
        	r.y=control_rect.y+height*control_adjust.y/1000;
        	r.width=control_rect.width+width*control_adjust.width/1000;
        	r.height=control_rect.height+height*control_adjust.height/1000;

        	c.setBounds(r);
        	
        	if (font_adjust) {
        		float ratio_x = control_rect.width>0 ? ((float)r.width)/control_rect.width : 1;
        		float ratio_y = control_rect.height>0 ? ((float)r.height)/control_rect.height : 1;
        		float new_size = font_size*Math.min(ratio_x,ratio_y);
        		
        		if (fontComponents!=null) {
        			for (Component fc : fontComponents ) {
        				fc.setFont(fc.getFont().deriveFont(new_size));
        			}
        		} else {
        			c.setFont(c.getFont().deriveFont(new_size));
        		}
        	}
	...
	public void collateResizeChanges(List<PropertyChange> store)
	{
        Component c= window_base!=null ?  getComponent() : null;        
        if (c!=null) {
        	Insets i = getControlAdjustInsets();
        	...
        	if (control_adjust.x!=0) addResizeChange(store,Prop.XPOS,Clarion.newNumber(rect.x));
        	if (control_adjust.y!=0) addResizeChange(store,Prop.YPOS,Clarion.newNumber(rect.y));
        	if (control_adjust.width!=0) addResizeChange(store,Prop.WIDTH,Clarion.newNumber(rect.width));
        	if (control_adjust.height!=0) addResizeChange(store,Prop.HEIGHT,Clarion.newNumber(rect.height));
        }	
        
    	for (AbstractControl kid : getChildren()) {
    		kid.collateResizeChanges(store);
    	}        
	}

	private void addResizeChange(List<PropertyChange> store, int prop, ClarionObject newValue) 
	{
		PropertyChange pc = new PropertyChange(this,prop,newValue);
		if (pc.changed()) return;
		store.add(pc);
	}
	
	public void clearResizeMetaData()
	{
		window_base=null;
		control_adjust=null;
		font_adjust=false;
		...
		default_ratio=null;
    	for (AbstractControl kid : getChildren()) {
    		kid.clearResizeMetaData();
    	}        		
	}

	public void setDefaultRatio(String rat) {
		default_ratio=rat;
	}
}
