...
public class NetworkQueue implements ClarionQueueListener,NetworkMetaData
{
	...
	private void calcCol()
	{
		...
		if (colCount>queue.getVariableCount()) {
			colCount=queue.getVariableCount();
		}
	}
	...
	public Object getMetaData() {
		...
		for (int scan=1;scan<=rec;scan++) {
			...
			for (int c=0;c<colCount;c++) {
				Object value=null;
				ClarionObject cvalue = c>=co.length ? null : co[c];
				if (cvalue!=null) {
	...
	public static ClarionQueue reconstruct(ClarionQueue base,Object[] val)
	{
		...
		for (int rscan=1;rscan<=new_records;rscan++) {
			...
			for (int c=1;c<=colCount;c++) {
				...
				if (co==null) continue; // can happen because of null parameters
				if (!changed) {
					if (!co.equals(o)) {
						changed=true;
					}
