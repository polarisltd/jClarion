...
public class Record extends GUIModel 
{
	...
	public static void main(String args[])
	{
		...
		try {
			...
			TeeStream tee = new TeeStream(new FileOutputStream(temp));
			...
			if (r!=null) {
				while(true) {
					...
					if (s.startsWith("    public void "+testName)) {
						while (true ) {
							s = r.readLine();
							if (s==null) break;
							if (s.equals("    }")) break;
			...
			p.println("    public void "+testName+"() {");
			p.flush();
			tee.rewrite(fos);
			p.println("    }");
			p.println("}");
	...
	public Record(GUIModel base,String name,PrintStream log,SimpleExecutor remote,SimpleExecutor local)
	{
	...
	private static Map<Integer,Boolean> guiRespond=new HashMap<Integer,Boolean>();
	...
	private int getRespondID()
	{
		...
		while (true) {
			count++;
			Boolean b = guiRespond.get(count);
			if (b==null) {
				log.println("        ServerResponse s"+count+";");
				b=false;
				guiRespond.put(count,b);
			}
			if (!b) {
				guiRespond.put(count,true);
				break;
			}
		}
		return count;
	...
	private int guiLog(RemoteWidget w,boolean willRespond,int command,Object... params)
	{
		...
		if (count>0) {
			sb.append("        s").append(count).append("=");
		} else {
			sb.append("        ");
		}
		int eat=-1;
				...
				if (w instanceof AbstractListControl && item.getKey()==AbstractListControl.MD_QUEUE) {
					...
					for (int scan=1;scan<=rowCount;scan++) {
						...
						for (int col=1;col<=colSize;col++) {
							logJavaObject(sb,o[scan*colSize+col]);
	...
	private synchronized int log(RemoteWidget w,boolean willRespond,int command,Object... params)
	{
	...
	private synchronized void logResult(RemoteWidget w,int count,Object result, int command,Object... params)
	{		
		StringBuilder entry = new StringBuilder();
		if (getSide().equals("gui")) {
			entry.append("        s").append(count).append(".setResult(");
			logJavaObject(entry,result,true);
			entry.append(");");
		} else {
			entry.append("        assertResult(");
			logJavaObject(entry,result);
			entry.append(",s").append(count).append(");");
		}
		guiRespond.put(count,false);
		doLog(entry);
		log.println();
	}
	...
	public void send(final RemoteWidget w, final ResponseRunnable nextTask, final int command,final Object... params) {
		...
		final int count=log(w,true,command,params);
		localReader.deploy(new Runnable() {
			public void run() {
