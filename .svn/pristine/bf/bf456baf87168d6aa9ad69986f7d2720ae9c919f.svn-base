...
public class GlobalDataCmd extends CommandItem implements StatementBlock
{
	@Override
	...
	protected void execute(TemplateParser parser, TemplateItem parent) throws IOException 
	{
		while ( true ) {
			...
			if (ti instanceof EndCmd && ((EndCmd)ti).isEnd("#ENDGLOBALDATA")) {
				break;
			}
		}
		((CodeSection)parent).setGlobalData(statements);
	}	
