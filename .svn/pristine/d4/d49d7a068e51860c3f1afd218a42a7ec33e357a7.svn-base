...
public class VariableParser extends AbstractParser {
    public VariableParser(ClarionCompiler compiler,Parser parser) {
    ...
    public Variable dataDefinition(String label, Lex la) {
        ...
        if (la.type == LexType.label && la.value.equalsIgnoreCase("like")) {
            ...
            if (exp == null) {
                et = compiler.getScope().getType(next().value);
                
                while (la().type == LexType.dot) {
                    next();
                    et_exp = et.field(new SimpleExpr(JavaPrec.LABEL,et,"1"),next().value,compiler.getScope());
    ...
    public boolean getVariable() {
        ...
        if (label == null) {
            label = compiler.getScope().createTemporaryLabel("_anon_");
            anonymous = true;
        }
        ...
        if (result == null && la.type == LexType.label
            ...
            if (e != null) {
                ...
                EquateClass ec = compiler.equates().get(pkgName[0]);
                result = new EquateVariable(label, ec, pkgName[1], e);

                compiler.getScope().addVariable(result);
                emptyAll();
                return true;
            }
            ...
            if (la().type == LexType.label) {
                ...
                ExprType et = compiler.getScope().getType(prototype);

                if (et != null) {
                    compiler.getScope().addAliasedType(label, et);
                } else {
                    ...
                    if (ExprType.get(collapseType(label)) != null) {
                        ...
                        compiler.getScope().addAlias(label, prototype);
                    }
                }
        ...
        if (result.isExternal()) {
            if (compiler.getScope().getParent()==null) error("Cannot Resolve External");
            if (compiler.getScope().getParent().getVariable(result.getName())==null) error("Cannot resolve external");
            return true;
        }
    ...
    private void getItemize(String label) {
        ...
        while (true) {
            ...
            compiler.stack().getScope().addVariable(i);
    ...
    private static class ImplementsSettingParser extends
        ...
        protected InterfaceExprType getValue(Parser p) {
            InterfaceExprType ret = (InterfaceExprType) p.compiler.getScope().getType(p.next().value);
            return ret;
        }
    }
    ...
    private void getClassDefinition(String label) {
        ...
        if (la().type == LexType.lparam) {
            ...
            if (la().type == LexType.label) {
                ...
                base = compiler.getScope().getType(baseName.value);
                if (base == null)
        ...
        Scope typeScope=compiler.getScope();
        Scope implemScope=typeScope;
        		
        ClassConstruct cc = new ClassConstruct(compiler.stack(),compiler.getScope().getPackage(),label, base);
        compiler.stack().pushScope(cc);
        ...
        for (int scan = 0; scan < settings.length; scan++) {
            ...
            if (name.equals("module")) {
                implemScope = compiler.main().get( ((Lex)value).value );
                continue;
            }
        ...
        if (!isIgnoreWhiteSpace()) {
            ...
            while (true) {
                ...
                if (p != null) {
                	...
                	// try constraining name of procedure to fit supers
              
                	if (methodNames==null) {
                		methodNames=new HashMap<String,String>();
                		if (base!=null) {
                			if (base instanceof ClassedExprType) {
                				ClassedExprType cet=(ClassedExprType)base;
                				Scope s = cet.getDefinitionScope();
                				while (s!=null) {
                					for (Procedure m : s.getProcedures()) {
                						if (m.isNoRelabel()) {
                							methodNames.put(m.getName().toLowerCase(),m.getName());
                						}
                					}
                					s=(Scope)((PolymorphicScope)s).getBase();
                				}
                			}
                		}
                	}
                	String fixedlabel = methodNames.get(p.getName().toLowerCase());
                	if (fixedlabel!=null) {
                		p.fixLabel(fixedlabel);
                	}
    ...
    private void getInterfaceDefinition(String label) {
        ...
        InterfaceConstruct cc = new InterfaceConstruct(compiler.stack(),compiler.getScope().getPackage(),label, base);
        compiler.stack().pushScope(cc);
    ...
    private void getGroupDefinition(boolean anonymous, String label,
        ...
        GroupConstruct gs = new GroupConstruct(compiler.stack(),label, base);
        gs.setPre(pre);
        compiler.stack().pushScope(gs);
        ...
        if (!type && anonymous) {
            ...
            for (Variable v : gs.getAllFields()) {
                compiler.getScope().addVariable(v.clone());
            }
        }
    ...
    private void getViewDefinition(String label) {
        ViewConstruct vs = new ViewConstruct(compiler.stack(),compiler.getScope().getPackage(),label);

        compiler.stack().pushScope(vs);
    ...
    private void getFileDefinition(String label) {
        ...
        FileConstruct fs = new FileConstruct(compiler.stack(),compiler.getScope().getPackage(),label);
        compiler.stack().pushScope(fs);
