...
public class ClarionSQLFileTest extends TestCase {
    ...
    public void testManualBegin() throws SQLException
    {
        ...
        for (String test : new String[] { " Begin;  ", "begin","BEGIN","BEGIN;","beGIN  ; ",
            ...
            assertFalse(testFile.getFileState().source.getConnection().getAutoCommit());
            ...
            assertTrue(testFile.getFileState().source.getConnection().getAutoCommit());
    ...
    public void testWatch() {
        ...
        for (int scan=0;scan<types.length;scan++) {
            ...
            try {
                testFile.getFileState().source.getConnection().createStatement().execute(
                    types[scan]);
            } catch (SQLException e) {
                fail(e.getMessage());
            }
    ...
    {
        try {
            ResultSet rs = testFile.getFileState().source.getConnection().createStatement().executeQuery(sql);
            assertTrue(rs.next());
    ...
    public void testCursorOptimiseEvenInManualTxnFrames() throws SQLException
    {
        ...
        if (getClass()==ClarionSQLFileTest.class) {
            try {
                testFile.next();
