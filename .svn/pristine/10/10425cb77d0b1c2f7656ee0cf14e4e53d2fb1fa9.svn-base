...
public class CompileTestHelper extends TestCase 
{
    ...
    public void setUp()
    {
        ...
        compiler.setSource(new MemoryLexerSource());
        System.setProperty("__clarion.unittest","true");
    }
    
    public MemoryLexerSource getSource()
    {
        return (MemoryLexerSource)compiler.source();
    }
    ...
    public void incrementalCompile(Lexer l)
    {
    ...
    public ClassLoader compile(Lexer l)
    {
        System.setProperty("clarion.compile.forceHardAssert","1");
        try {
    ...
    {
        return getOtherVariable(cl,ClarionCompiler.RAW_BASE+".Main",name);
    }

    public Object getMainObject(ClassLoader cl,String name)
    {
        try {
            Class<?> c = cl.loadClass(ClarionCompiler.RAW_BASE+".Main");
            assertNotNull(c);
    ...
    public void initProgram(ClassLoader cl)
    {
        try {
            Class<?> c = cl.loadClass("clarion.Main");
            assertNotNull(c);
            Method m = c.getMethod("__static_init");
            m.invoke(null);
        } catch (ClassNotFoundException e) {
            fail(e.getMessage());
        } catch (SecurityException e) {
