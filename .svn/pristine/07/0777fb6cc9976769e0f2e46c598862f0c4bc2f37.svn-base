...
//        }
    ...
    public Expr expr_primitive()
    {
        ...
        if (v.type==LexType.label) {
            ...
            if (colonPos>0) {
            	...
            	Expr e = ScopeStack.getScope().resolveAliasVariable(prefix,Labeller.get(field,false),ScopeStack.getScope());
            	if (e!=null) {
                    next();
                    return e;
            	}
            	
                var = ScopeStack.getScope().getAliasVariable(prefix);
                if (var==null) {
                    var = ScopeStack.getScope().getVariable(prefix);
                }
                if (var!=null) {
                    e = var.getExpr(ScopeStack.getScope());
                    e=e.type().field(e,field);
                    if (e!=null) {
                        next();
                        return e;
                    }
        	...
        	Expr empty_pre = ScopeStack.getScope().resolveAliasVariable("",v.value,ScopeStack.getScope());
        	if (empty_pre!=null) {
                next();
                return empty_pre;
        	}
    ...
    private Expr queueCommandF()
    {
        ...
        String name = MainScope.main.registerPrototype(match);
    ...
    public Expr callProcedure(String name,Expr[] params,boolean reportError)
    {
        ...
        if (name.equalsIgnoreCase("omitted") && params.length==1) {
        	...
        	try {
        		...
        		ReturningScope rs = (ReturningScope)ScopeStack.getScope();
        		e = rs.getParameter(id).getExpr(ScopeStack.getScope());
