...
public class VariableParser extends AbstractParser {
    public VariableParser(Parser parser, Lexer input) {
        super(parser, input);
    }
    ...
    private static SettingParser<?> dataSettings = new JoinedSettingParser(
            ...
            new ExprSettingParser("dim"), new ExprSettingParser("dll"),
    ...
    private static SettingParser<?> fileSettings = new JoinedSettingParser(
            ...
            new ExprSettingParser("driver"), new ExprSettingParser("owner"),
    ...
    public Variable dataDefinition(String label, Lex la) {
        ...
        if (la.type == LexType.label && la.value.equalsIgnoreCase("like")) {
            ...
            if (exp != null) {
                if (exp instanceof VariableExpr) {
                    return new LikeVariable(label, (VariableExpr) exp,false,Static);
                }
            }
        ...
        if (la.type == LexType.label
            ...
            if (et != null) {
                ...
                if (et instanceof DanglingExprType) {
                    ...
                    ClassedVariable cv = new ClassedVariable(label, cet, false,Static);
                    return cv;
                }
            }
            error("Unknown type");
        ...
        if (la.type == LexType.reference) {
            ...
            if (dim!=null) {
                return new ReferenceVariable(label, type,Static,dim.toArray(new Expr[dim.size()]));
            } else {
                return new ReferenceVariable(label, type,Static,null);
            }
        }
    ...
    private static Set<String> ignoreableAttributes = GrammarHelper.list(
            "private", "protected", "public", "auto", "link", "dll", "thread");
    ...
    private void getInterfaceDefinition(String label) {
        ...
        SettingParser<Boolean> settingParse = new SimpleSettingParser("type");
    ...
    private void getGroupDefinition(boolean anonymous, String label,
        ...
        if (!type && !anonymous) {
            ClassedVariable result = new ClassedVariable(label, gs.getType(),
                    gs.getJavaClass(), false,Static);
            result.setOver(over);
