	...
	public void writeParameter(OutputStream os, Object parameter)
		...
		switch(type.intValue()) {
			...
			case ARRAY: { 
				...
				if (len>65535) throw new IOException("Cannot serialize large array");
				writeNumber(os,len,2);
				for (int scan=0;scan<len;scan++) {
			...
			case MAP: {
				...
				Map<?,?> c  = (Map<?,?>)parameter;
				int len = c.size();
				if (len>65535) throw new IOException("Cannot serialize large map");
				writeNumber(os,len,2);
