...
public class LikeVariable extends Variable 
{
    ...
    public LikeVariable(String name,VariableExpr base,boolean forceConstruction,boolean Static)
    {
        this.base=base;
    ...
    public Expr[] makeConstructionExpr() {
        if (!forceConstruction && base.type().isa(ExprType.any)) {
            return new Expr[] { new DecoratedExpr(JavaPrec.POSTFIX,base,".like()") };
        } else {
            ...
            return v.makeConstructionExpr();
        }
    }

    @Override
    public Variable clone() {
        return new LikeVariable(getName(),base,forceConstruction,isStatic());
    }
}
