...
{
    public ExprParser(ClarionCompiler compiler,Parser p)
    {
        super(compiler, p);
    }
...
//        }
    ...
    private boolean allowMethods=true;
        ...
        while ( true ) {
            ...
            if (l.type==LexType.lbrack) {
                ...
                while ( true ) {
                	...
                	if (la(0).type==LexType.param) {
                		next();
                		param1=expression();
                		param2=null;
                		if (param1==null) {
                            error("Invalid Array Expr");
                		}
                	} else {
                		break;
                	}
            ...
            if (l.type==LexType.dot && la(1).type==LexType.label) {
                ...
                if (params==null) {
                    ne=e.type().field(e,label.value,compiler.getScope());
                    if (ne==null && allowMethods) {
                ...
                if (ne==null && allowPrototypes && !allowMethods) {
    ...
    public Expr expr_primitive()
    {
        ...
        if (v.type==LexType.integer) {
            ...
            if (v.value.length()>9) {
                ...
                if (v.value.startsWith("0x")) {
                    l = Long.parseLong(v.value.substring(2),16);
                } else {
                    l = Long.parseLong(v.value);
                }
                if (l<Integer.MIN_VALUE || l>Integer.MAX_VALUE) {
                    v.value="(int)"+v.value+"l";
                }
            }
        ...
        if (v.type==LexType.label && la(1).type==LexType.implicit) {
            ...
            if (la(1).value.equals("$")) {
                Variable tvar = compiler.getScope().getVariable(v.value);
                if (tvar!=null) {
        ...
        // start expression
        if (v.type==LexType.label && v.value.equalsIgnoreCase("start") && la(1).type==LexType.lparam) {
            int spos=begin();
            next();
            next();
            ...
            if (next().type==LexType.rparam) {
                ...
                if (e!=null) {
                    ...
                    ExprBuffer bcall = new ExprBuffer(JavaPrec.LABEL,ExprType.rawint);
                    bcall.add("CRun.start(new Runnable() { public void run() { ");
                    bcall.add(e);
                    bcall.add("; } } ).getId()");
                    Expr call=bcall;
                    call=new DependentExpr(call,ClarionCompiler.CLARION+".runtime.CRun");
                    call=new DependentExpr(call,ljb);
                    
                    
                    
                    // TODO final parameters above to be passed as a special type of dependency to be embedded in java code.
                    
                    commit(spos);
                    
                    return new SystemCallExpr(call);
                } else {
                }
            }
            rollback(spos);
        ...
        if (v.type==LexType.label && v.value.equalsIgnoreCase("bind") && la(1).type==LexType.lparam) {
            ...
            if (name==null) {
            	...
            	name=name.cast(ExprType.rawstring);
            }
            
            if ( ok && la().type!=LexType.param) {
            	ok=false;
            } else {
            	next();
            }
            ...
            String syscall=null;
        	if (la().type==LexType.label && la(1).type==LexType.rparam) {
        		syscall=la().value;
        	}
            
        	Expr rhs=null;
            if (ok && !allowPrototypes && allowMethods) {
            ...
            if (rhs==null && syscall!=null) {
            	next();
            	next();
            }
            
            if (rhs!=null && la().type!=LexType.rparam) {
            	rhs=null;
            } else {
            	next();
            }
            
            if (rhs!=null && rhs instanceof VariableExpr) {
                ExprBuffer bind = new ExprBuffer(JavaPrec.POSTFIX,null);
                bind.add("CExpression.bind(");
                bind.add(name);
                bind.add(",");
                
                boolean ref = rhs.utilisesReferenceVariables();
                if (ref) {
                    bind.add("new ReferenceBindProcedure() { public ClarionObject get() { return ");
                }
                bind.add(rhs); // potential problem here, with finalization etc
                if (ref) {
                    bind.add("; } }");
                }
            ...
            if (rhs instanceof PrototypeExpr) {
            	PrototypeExpr pe=(PrototypeExpr)rhs;
                
            	tloop: while ( true ) {
            		ExprType et = pe.type().getReal();
            		if (!(et instanceof ProcedurePrototypeExprType)) break;
                	Procedure procedure=((ProcedurePrototypeExprType)et).getProcedure();
            		
                	Expr object=pe.getBase();
                    if (procedure.getResult()==null) break;
                    if (!procedure.getResult().getType().isa(ExprType.any)) break;
                    Param param[] = procedure.getParams();
                    for (int pscan=0;pscan<param.length;pscan++) {
                        if (!param[pscan].getType().isa(ExprType.any)) {                        	
                        	break tloop;
                        }
                    }
                    Expr params[] = new Expr[procedure.getParams().length];
                    for (int pscan=0;pscan<params.length;pscan++) {
                        ExprType outType = procedure.getParams()[pscan].getType();
                    ...
                    if (object==null) {
                    	cbuffer.add("(new ");
                    	cbuffer.add(procedure.getScope().getJavaClass().getName());
                    	cbuffer.add("()).");
                    } else {
                    	/*
                    	...
                    	if (s instanceof MethodScope) {
                    		...
                    		if (ms.getClassScope()==object.type().getDefinitionScope()) {
                    			object=null;
                    		}
                    	}
                    	*/
                    	if (object!=null) { 
                        	...
                        	if (s instanceof MethodScope) {
                        		MethodScope ms = (MethodScope)s;
                        		String cname=ms.getClassScope().getName();
                        		object=new RegexExpr(object)
                        			.add("(^|(?!(a-zA-Z0-9)))this\\.",cname+".this.",true)
                        			.add("(^|(?!(a-zA-Z0-9)))this$",cname+".this",false)
                        		;
                        	}
                        	if (xobj instanceof VariableExpr) {
                        		Variable var = ((VariableExpr)xobj).getVariable();
                    ...
                    cbuffer.add(new CallExpr(null,procedure,false,params));                    
                    call = cbuffer; 
                    call = new DependentExpr(call,procedure.getScope().getJavaClass());
                    break;
                }
            }
            ...
            if (call==null && syscall!=null) {
                Iterable<Rewriter> list = SystemRegistry.getInstance().get(syscall);
                if (list!=null) { 
                	for (Rewriter test : list) {
                		if (test.getMax()!=test.getMin()) continue;
                		if (test.getType()==null) continue;
        ...
        if (v.type==LexType.label) {
            // lookup variable
            Variable var = compiler.getScope().getVariable(v.value);
            
            if (var!=null) {
                next();
                return var.getExpr(compiler.getScope());
            }
            ...
            if (colonPos>0) {
            	...
            	String prefix = v.value.substring(0,colonPos);
            	String field = v.value.substring(colonPos+1);
            	
            	Expr e = compiler.getScope().resolveAliasVariable(prefix,Labeller.get(field,false),compiler.getScope());
            	if (e!=null) {
                    next();
                    return e;
            	}
            	
                var = compiler.getScope().getAliasVariable(prefix);
                if (var==null) {
                    var = compiler.getScope().getVariable(prefix);
                }
                if (var!=null) {
                    e = var.getExpr(compiler.getScope());
                    e=e.type().field(e,field,compiler.getScope());
                    if (e!=null) {
                        next();
                        return e;
                    }
            ...
            if (compiler.getScope() instanceof ReturningScope) {

                ReturningScope rs =  (ReturningScope)compiler.getScope();
            ...
            // try for empty pre statements
        	Expr empty_pre = compiler.getScope().resolveAliasVariable("",v.value,compiler.getScope());
        	if (empty_pre!=null) {
                next();
                return empty_pre;
        	}
            ...
            // try for prototype
            if (allowPrototypes) {
                Scope scope_scan = compiler.getScope();
                while (scope_scan!=null) {
                    for (Procedure test : scope_scan.getProcedures(v.value)) {
                        next();
                        return new PrototypeExpr(test,null);
                    }
                    scope_scan=scope_scan.getParent();
                }
                
                ExprType et = compiler.getScope().getType(v.value);
                if (et!=null) {
                    next();
        ...
        if (v.type==LexType.use) {
           ...
           if (label==null) {
               ...
               UseVariable uv = compiler.getScope().getUseVariable(label);
               if (uv==null) error("usevar not defined:"+label);
               e=uv.getExpr(compiler.getScope());
           }
           return e;
        }
    ...
    private Expr queueCommandF()
    {
        if (la().type!=LexType.label) return null;
        if (!la().value.toLowerCase().endsWith("f")) return null;
        if (!queueCommands.contains(la().value.substring(0,la().value.length()-1).toLowerCase())) return null;
		if (la(1).type!=LexType.lparam) return null; 

		int pos = begin();
    	String func = next().value.toLowerCase();
    	next();
        ...
        Scope scan = compiler.getScope();
        while (scan!=null && match==null) {
            for ( Procedure test : scan.getProcedures(prototype)) {
                if (test.getResult()==null) continue;
                if (!test.getResult().getType().isa(ExprType.any)) continue;
                Param param[] = test.getParams();
                if (param.length!=2) continue;
                if (!param[0].getType().same(ExprType.group)) continue;
                if (!param[1].getType().same(ExprType.group)) continue;
                match=test;
                break;
            }
            scan=scan.getParent();
        ...
        String name = compiler.main().registerPrototype(match);
    	...
    	eb.add(queue);
    	eb.add(".");
    	eb.add(func.substring(0,func.length()-1));
    	eb.add("(Main.__");
    	eb.add(name);
    	eb.add(")");
    	return new SystemCallExpr(eb);
    }
    ...
    private Expr implicitVariable() {
        ...
        return v.getExpr(compiler.getScope());
    }

    public Expr callProcedure(String name,Expr[] params,boolean reportError)
    {
    	// clear array
    	if (name.equalsIgnoreCase("clear") && params.length>=1 && params[0].type().getArrayDimSize()>0 && params.length<=2) {
            ExprBuffer eb = new ExprBuffer(JavaPrec.POSTFIX,ExprType.NULL);
            eb.add(params[0]);
            eb.add(".clear(");
            if (params.length==2) {
            	eb.add(params[1].cast(ExprType.rawint));
            }
            eb.add(")");
            return new SystemCallExpr(eb);
    	}
        ...
        if (name.equalsIgnoreCase("choose") && params.length>=3 && (!params[0].type().same(ExprType.rawboolean))) {
            ...
            if (params.length==3) {
                ...
                Variable v = compiler.getScope().getVariableThisScopeOnly("__Choose_hold");
                if (v==null) {
                    v=  new SimpleVariable("__Choose_hold",ExprType.rawint,
                        new SimpleExpr(JavaPrec.LABEL,ExprType.rawint,"0"),
                        null,false,null);
                    compiler.getScope().addVariable(v);
                }
        ...
        if (name.equalsIgnoreCase("omitted") && params.length==1) {
        	...
        	try {
        		...
        		ReturningScope rs = (ReturningScope)compiler.getScope();
        		e = rs.getParameter(id).getExpr(compiler.getScope());
        ...
        Procedure p = compiler.getScope().matchProcedure(name,params);
        if (p!=null) {
