...
public class ClarionWindow extends AbstractWindowTarget
{
    ...
    public ClarionWindow setRatio(String region)
    {
    ...
    public void scale()
    {
    	if (scaled) return;
    	scaled=true;
    	String s_delta = System.getProperty("windowscale");
    	if (s_delta==null) return;
    	int delta = Integer.parseInt(s_delta);
    	
    	if (getRawProperty(Prop.FONTSIZE)!=null) {
    		setProperty(Prop.FONTSIZE,getRawProperty(Prop.FONTSIZE).add(delta));
    	}
    ...
    private void scale(AbstractControl ac,int delta) {
    	if (ac==null) return;
    	if (ac.getRawProperty(Prop.FONTSIZE)!=null) {
    		ac.setProperty(Prop.FONTSIZE,ac.getRawProperty(Prop.FONTSIZE).add(delta));
    	}
    ...
    protected Object getAWTProperty(int i) 
    {
    	...
    	if (i==Prop.CLIENTWIDTH) {
    		return Clarion.newNumber(widthPixelsToDialog(c.getWidth()));
    	}
    	if (i==Prop.CLIENTHEIGHT) {
    		return Clarion.newNumber(heightPixelsToDialog(c.getHeight()));
    	}
		return null;
	}
	...
	public void notifySized()
	{
    	...
    	int cw = widthPixelsToDialog(c.getWidth()); 
    	int ch=heightPixelsToDialog(c.getHeight());
    	...
    	ce.setAdditionalData(w,h,max,cw,ch);
    	post(ce);
	}
    ...
    public ClarionObject getLocalProperty(int index) {
    	if (index==Prop.CLIENTWIDTH || index==Prop.CLIENTHEIGHT) {
    		return getClientSetting(index);
    	}
    	return super.getLocalProperty(index);
    }

	private ClarionObject getClientSetting(int index) 
	{
		ClarionObject x = getRawProperty(index,false);
		if (x!=null) return x;
		if (isOpened()) {
			ClarionObject co = (ClarionObject)CWinImpl.runNow(this,GET_AWT_PROPERTY,index);			
			try {
				forcedUpdate=true;
				setProperty(index, co);
			} finally {
				forcedUpdate=false;
			}
			return co;
