...
public class ClarionEventQueue extends EventQueue 
{
    ...
    public boolean setRecordState(boolean record,final String reason)
    {
    	return setRecordState(record,reason,null);
    }
    
    private Object lock;
    
    public boolean setRecordState(boolean record,final String reason,Object lock)
    {
    	if (record==false && lock!=null) {
    		if (record==false) {
    			this.lock=lock;
    		}
    	}
    	...
    	if (record==true) {
    		if (this.lock!=lock) return false; // lock denied
    		this.lock=null;
    	}
