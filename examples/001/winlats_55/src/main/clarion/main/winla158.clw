...
Update::Reloop  BYTE
Update::Error   BYTE
History::KRA:Record LIKE(KRA:Record),STATIC
SAV::KRA:Record      LIKE(KRA:Record)
ToolBarMode     UNSIGNED,AUTO
WindowXPos      SIGNED,AUTO,STATIC
WindowYPos      SIGNED,AUTO,STATIC
WindowPosInit   BOOL(False),STATIC
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the AUTOKRÂSAS File'),AT(,,162,70),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateAutoMarkas'),SYSTEM,GRAY,RESIZE,MDI
                       SHEET,AT(4,4,151,44),USE(?CurrentTab)
                         TAB('General'),USE(?Tab:1)
                           PROMPT('&Krâsa:'),AT(23,23),USE(?KRA:KRASA:Prompt)
                           ENTRY(@S20),AT(51,23),USE(KRA:KRASA)
                           STRING(@S7),AT(50,36),USE(KRA:KRASA_A)
                         END
                       END
                       BUTTON('&OK'),AT(58,52,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(107,52,45,14),USE(?Cancel)
                     END
  CODE
  PUSHBIND
  ...
  ACCEPT
      ...
      IF EVENT() = Event:Completed
        ...
        OF ChangeRecord
          LOOP
            ...
            RecordChanged=false
            IF SAV::KRA:Record <> KRA:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:AUTOKRA(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
    ...
    OF ?KRA:KRASA
      CASE EVENT()
      OF EVENT:Accepted
        KRA:KRASA_A=INIGEN(KRA:KRASA,7,1)
        DISPLAY
      END
    OF ?OK
      CASE EVENT()
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('UpdateAutoKra','winlats.INI')
  WinResize.Resize
  ?KRA:KRASA{PROP:Alrt,255} = 734
  ?KRA:KRASA_A{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
...
!|
  IF FilesOpened
    AUTOKRA::Used -= 1
    IF AUTOKRA::Used = 0 THEN CLOSE(AUTOKRA).
  END
  IF WindowOpened
    INISaveWindow('UpdateAutoKra','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?KRA:KRASA
      KRA:KRASA = History::KRA:Record.KRASA
    OF ?KRA:KRASA_A
      KRA:KRASA_A = History::KRA:Record.KRASA_A
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
...
BRW1::View:Browse    VIEW(AUTOKRA)
                       PROJECT(KRA:KRASA)
                       PROJECT(KRA:KRASA_A)
                     END

Queue:Browse:1       QUEUE,PRE()                  ! Browsing Queue
BRW1::KRA:KRASA        LIKE(KRA:KRASA)            ! Queue Display field
BRW1::KRA:KRASA_A      LIKE(KRA:KRASA_A)          ! Queue Display field
...
BRW1::Sort1:KeyDistribution LIKE(KRA:KRASA_A),DIM(100)
BRW1::Sort1:LowValue LIKE(KRA:KRASA_A)            ! Queue position of scroll thumb
BRW1::Sort1:HighValue LIKE(KRA:KRASA_A)           ! Queue position of scroll thumb
...
SelectButton           SIGNED                   !Browse select button
                     END
WinResize            WindowResizeType
QuickWindow          WINDOW('Browse the AUTOKRASAS File'),AT(,,166,212),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseAutoMarkas'),SYSTEM,GRAY,RESIZE,MDI
                       LIST,AT(8,20,151,135),USE(?Browse:1),IMM,VSCROLL,MSG('Browsing Records'),FORMAT('80L(2)|M~Krâsa~@S20@'),FROM(Queue:Browse:1)
                       ...
                       SHEET,AT(5,5,158,172),USE(?CurrentTab)
                         TAB('Krâsu secîba'),USE(?Tab:2)
                           ENTRY(@S7),AT(19,159),USE(KRA:KRASA_A)
                         END
                       END
                       BUTTON('&Beigt'),AT(106,196,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
    ...
    OF ?KRA:KRASA_A
      ...
      OF EVENT:Accepted
        ...
        IF KRA:KRASA_A
          ...
          BRW1::Sort1:LocatorValue = KRA:KRASA_A
          BRW1::Sort1:LocatorLength = LEN(CLIP(KRA:KRASA_A))
          SELECT(?Browse:1)
...
PrepareProcedure ROUTINE
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Spread)
  INIRestoreWindow('BrowseAutoKra','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
  IF LocalRequest <> SelectRecord
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('BrowseAutoKra','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
!|
  ...
  CASE BRW1::SortOrder
  OF 1
    KRA:KRASA_A = BRW1::Sort1:LocatorValue
  END
  DISPLAY()
  ForceRefresh = False
!---------------------------------------------------------------------------
...
!|
  ...
  IF ERRORCODE()
    ...
    ELSE
      StandardWarning(Warn:RecordFetchError,'AUTOKRA')
      POST(Event:CloseWindow)
    END
    EXIT
  END
  CASE BRW1::SortOrder
  OF 1
    BRW1::Sort1:HighValue = KRA:KRASA_A
  END
  DO BRW1::Reset
  ...
  CASE BRW1::SortOrder
  OF 1
    BRW1::Sort1:LowValue = KRA:KRASA_A
...
!|
  KRA:KRASA = BRW1::KRA:KRASA
  KRA:KRASA_A = BRW1::KRA:KRASA_A
!----------------------------------------------------------------------
...
!|
  BRW1::KRA:KRASA = KRA:KRASA
  BRW1::KRA:KRASA_A = KRA:KRASA_A
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 1
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort1:KeyDistribution[BRW1::CurrentScroll] => UPPER(KRA:KRASA_A)
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 1
      KRA:KRASA_A = BRW1::Sort1:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(KRA:KRA_Key)
      RESET(KRA:KRA_Key,BRW1::HighlightedPosition)
    ELSE
      SET(KRA:KRA_Key,KRA:KRA_Key)
    END
  END
  IF ERRORCODE()
...
!|
  ...
  IF BRW1::RecordCount
    CASE BRW1::SortOrder
    OF 1; ?KRA:KRASA_A{Prop:Disable} = 0
    END
    IF BRW1::HighlightedPosition
  ...
  ELSE
    CLEAR(KRA:Record)
    CASE BRW1::SortOrder
    OF 1; ?KRA:KRASA_A{Prop:Disable} = 1
    END
...
Update::Reloop  BYTE
Update::Error   BYTE
History::AMA:Record LIKE(AMA:Record),STATIC
SAV::AMA:Record      LIKE(AMA:Record)
Auto::Attempts       LONG,AUTO
Auto::Save:AMA:U_NR     LIKE(AMA:U_NR)
ToolBarMode     UNSIGNED,AUTO
WindowXPos      SIGNED,AUTO,STATIC
WindowYPos      SIGNED,AUTO,STATIC
WindowPosInit   BOOL(False),STATIC
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the AUTOMARKAS File'),AT(,,166,70),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateAutoMarkas'),SYSTEM,GRAY,RESIZE,MDI
                       SHEET,AT(4,4,158,44),USE(?CurrentTab)
                         TAB(' '),USE(?Tab:1)
                           STRING(@n_7),AT(128,6),USE(AMA:U_NR)
                           PROMPT('&Marka:'),AT(8,25),USE(?AMA:MARKA:Prompt)
                           ENTRY(@S30),AT(38,25,119,10),USE(AMA:MARKA)
                         END
                       END
                       BUTTON('&OK'),AT(58,52,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(107,52,45,14),USE(?Cancel)
  ...
  ACCEPT
      ...
      IF EVENT() = Event:Completed
        ...
        OF InsertRecord
          ...
          OF NoError
            ...
            POST(Event:CloseWindow)
          OF DupKeyErr
            IF DUPLICATE(AMA:NR_KEY)
              IF StandardWarning(Warn:DuplicateKey,'AMA:NR_KEY')
                SELECT(?AMA:U_NR)
                VCRRequest = VCRNone
                CYCLE
              END
            END
            IF DUPLICATE(AMA:KeyMARKA)
              IF StandardWarning(Warn:DuplicateKey,'AMA:KeyMARKA')
                SELECT(?AMA:U_NR)
                VCRRequest = VCRNone
                CYCLE
              END
        ...
        OF ChangeRecord
          LOOP
            ...
            SETCURSOR(Cursor:Wait)
            RecordChanged=false
            IF SAV::AMA:Record <> AMA:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:AUTOMARKAS(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
    ...
    OF ?AMA:MARKA
      CASE EVENT()
      OF EVENT:Accepted
        AMA:MARKA=INIGEN(AMA:MARKA,30,1)
        DISPLAY
      END
    OF ?OK
      CASE EVENT()
...
PrepareProcedure ROUTINE
    ...
    !  .
      ...
      IF GNET !GLOBÂLAIS TÎKLS
         ...
         OF 1
            ...
            AMA:GNET_FLAG[2]=''
         OF 2
            IF AMA:GNET_FLAG[1]=1     !JA STÂV ADD1 (NEKUR CITUR VÇL NAV IERAKSTÎTS)
            ELSIF AMA:GNET_FLAG[1]=4  !JA STÂV ADD4 (NE VISUR IR IERAKSTÎTS)
               KLUDA(121,'I/O, kamçr nav pabeigta tîklu sinhronizâcija')
               LOCALREQUEST=0
            ELSIF AMA:GNET_FLAG[1]=2  !JA STÂV PUT2 (NEKUR CITUR VÇL NAV NOMAINÎTS)
            ELSIF AMA:GNET_FLAG[1]=5  !JA STÂV PUT5 (NE VISUR IR NOMAINÎTS)
               AMA:GNET_FLAG[1]=2
               AMA:GNET_FLAG[2]=''    !MAINAM VISUR
            ELSIF AMA:GNET_FLAG[1]=3 OR AMA:GNET_FLAG[1]=6  !JA STÂV DEL 
               KLUDA(121,'I/O, kamçr nav pabeigta tîklu sinhronizâcija')
               LOCALREQUEST=0
            ELSE
               AMA:GNET_FLAG[1]=2
               AMA:GNET_FLAG[2]=''
            .
            SAV::AMA:Record = AMA:Record ! LAI NERAKSTA, JA NEKAS NAV MAINÎTS
         OF 3
    !        STOP(AMA:GNET_FLAG)
            IF AMA:GNET_FLAG[1]=1     !JA STÂV ADD1 (NEKUR CITUR VÇL NAV IERAKSTÎTS)
               AMA:GNET_FLAG=''
            ...
            ELSIF AMA:GNET_FLAG[1]=2  !JA STÂV PUT2 (NEKUR CITUR VÇL NAV NOMAINÎTS)
               ...
               LOCALREQUEST=4
            ELSIF AMA:GNET_FLAG[1]=5  !JA STÂV PUT5 (NE VISUR IR NOMAINÎTS)
               AMA:GNET_FLAG[1]=3     !DZÇÐAM VISUR
               AMA:GNET_FLAG[2]=''
               LOCALREQUEST=4
            ELSIF AMA:GNET_FLAG[1]=3 OR AMA:GNET_FLAG[1]=6  !JA STÂV DEL
               KLUDA(121,'I/O, kamçr nav pabeigta tîklu sinhronizâcija')
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Spread)
  INIRestoreWindow('UpdateAutoMarkas','winlats.INI')
  WinResize.Resize
  ?AMA:U_NR{PROP:Alrt,255} = 734
  ?AMA:MARKA{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
ProcedureReturn ROUTINE
!|
!| This routine provides a common procedure exit point for all template
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('UpdateAutoMarkas','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?AMA:U_NR
      AMA:U_NR = History::AMA:Record.U_NR
    OF ?AMA:MARKA
      AMA:MARKA = History::AMA:Record.MARKA
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
...
!|
  ...
  LOOP
    ...
    END
    IF ERRORCODE()
      Auto::Save:AMA:U_NR = 1
    ELSE
      Auto::Save:AMA:U_NR = AMA:U_NR + 1
    END
    AMA:Record = SAV::AMA:Record
    AMA:U_NR = Auto::Save:AMA:U_NR
    SAV::AMA:Record = AMA:Record
    ADD(AUTOMARKAS)
    IF ERRORCODE()
      Auto::Attempts += 1
