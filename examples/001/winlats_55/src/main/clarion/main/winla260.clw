...
A_PersID             PROCEDURE                    ! Declare Procedure
VUT      STRING(30)
VUT_B      STRING(30)
PERSID   STRING(20)
PERSID_B STRING(20)
...
Report REPORT,AT(0,200,8000,10896),PAPER(PAPER:A4),PRE(RPT),FONT('Arial Baltic',8,,,CHARSET:BALTIC),THOUS
Detail DETAIL,AT(0,,8000,1948),USE(?unnamed)
         STRING(@s30),AT(0,542),USE(VUT),TRN,CENTER,FONT(,16,,,CHARSET:BALTIC)
         STRING(@s20),AT(198,823,3385,458),USE(PERSID),TRN,CENTER,FONT('Code 3 de 9',28,,,CHARSET:SYMBOL)
         STRING('_{58}'),AT(0,1708,3750,208),USE(?String3),TRN,CENTER
       END
     END
 ...
 report{Prop:Preview} = PrintPreviewImage
    ...
    VUT_B=CLIP(VUT)&'/'
!    PERSID='*'&FORMAT(KAD:ID,@N_05)&'-'&FORMAT(KAD:OBJ_NR,@N_02)&'*'
    PERSID='*'&FORMAT(KAD:ID,@N_05)&'*'
    PERSID_B='*'&FORMAT(KAD:ID,@N_05)&'/*'
!    IF KAD:OBJ_NR
!       VIETA='#'&CLIP(KAD:OBJ_NR)
!    .
    PRINT(RPT:DETAIL)
...
GETPARAKSTI          FUNCTION (PARAKSTS_NR,OPC,CTRL) ! Declare Procedure
PARAKSTS        STRING(35)
SEC_VUT         LIKE(SEC:VUT)
SEC_AMA         STRING(25)

  CODE                                            ! Begin processed code
!
! (PARAKSTS_NR,OPC,<CTRL>)
!

IF PARAKSTS_NR=4 !LOGIN
   IF ACC_KODS_N
      CHECKOPEN(PAROLES,1)
      SEC:U_NR=ACC_KODS_N
      GET(PAROLES,SEC:NR_KEY)
      IF ERROR()
         STOP('LASOT PAROLES '&SEC:U_NR&' '&ERROR())
      ELSE
         SEC_VUT=SEC:VUT
         SEC_AMA=SEC:AMATS
      .
   ELSE
      SEC_VUT='Mâris Viesis'
      SEC_AMA='Valdes loceklis'
   .
ELSIF PARAKSTS_NR=5 !JÂMEKLÇ LOGINS PÇC CTRL
   IF CTRL
      CHECKOPEN(PAROLES,1)
      SEC:U_NR=CTRL
      GET(PAROLES,SEC:NR_KEY)
...
CASE OPC
OF 1
  EXECUTE PARAKSTS_NR+1!TUKÐS IR 0
     PARAKSTS=''
     PARAKSTS=SYS:PARAKSTS1
     PARAKSTS=SYS:PARAKSTS2
     PARAKSTS=SYS:PARAKSTS3
     PARAKSTS=SEC_VUT  !LOGIN
     PARAKSTS=SEC_VUT  !LOGIN
     PARAKSTS=''       !E-ME
  .
OF 2
  EXECUTE PARAKSTS_NR+1
     PARAKSTS=''
     PARAKSTS=SYS:AMATS1
     PARAKSTS=SYS:AMATS2
     PARAKSTS='Kasieris'
     PARAKSTS=SEC_AMA  !LOGIN
     PARAKSTS=SEC_AMA  !LOGIN
     PARAKSTS=''       !E-ME
  .
OF 3  !FORMAT CODE39
  EXECUTE PARAKSTS_NR+1
     ...
     PARAKSTS=''       !E-ME
  .
  LEN#=LEN(CLIP(PARAKSTS))
  PARAKSTS=UPPER(INIGEN(PARAKSTS,LEN#,1))
  PARAKSTS='*'&CLIP(PARAKSTS)&CLIP(CTRL)&'*'
.
RETURN(PARAKSTS)



BrowseTex1 PROCEDURE
...
BRW1::View:Browse    VIEW(TEKSTI1)
                       PROJECT(TEX1:NR)
                       PROJECT(TEX1:TEKSTS1)
                       PROJECT(TEX1:TEX_A)
                     END

Queue:Browse:1       QUEUE,PRE()                  ! Browsing Queue
BRW1::TEX1:NR          LIKE(TEX1:NR)              ! Queue Display field
BRW1::TEX1:TEKSTS1     LIKE(TEX1:TEKSTS1)         ! Queue Display field
BRW1::TEX:RIK_TIPS     LIKE(TEX:RIK_TIPS)         ! Queue Display field
BRW1::RIK:TIPS         LIKE(RIK:TIPS)             ! Queue Display field
BRW1::TEX1:TEX_A       LIKE(TEX1:TEX_A)           ! Queue Display field
...
BRW1::Sort2:KeyDistribution LIKE(TEX1:TEX_A),DIM(100)
BRW1::Sort2:LowValue LIKE(TEX1:TEX_A)             ! Queue position of scroll thumb
BRW1::Sort2:HighValue LIKE(TEX1:TEX_A)            ! Queue position of scroll thumb
...
QuickWindow          WINDOW('Tekstu plâns'),AT(0,0,440,255),FONT('MS Sans Serif',8,,FONT:bold),IMM,HLP('BrowseTEK'),SYSTEM,GRAY,MDI
                       LIST,AT(8,20,423,209),USE(?Browse:1),IMM,HVSCROLL,FONT(,8,,),MSG('Browsing Records'),FORMAT('52R(1)|~Nr~C(0)@n_7@240L(1)|@s60@'),FROM(Queue:Browse:1),DRAGID('''no TEX1''')
                       STRING('Kopçðanai lietojiet Drag and Drop'),AT(61,236,121,10),USE(?String1),FONT(,,COLOR:Red,,CHARSET:ANSI)
                       SHEET,AT(5,4,430,228),USE(?CurrentTab)
                         TAB('Nosaukumu secîba'),USE(?Tab1)
                         END
                         TAB('Nr secîba'),USE(?Tab2)
                         END
                       END
                       BUTTON('&Dzçst'),AT(337,235,45,14),USE(?Delete:2),DISABLE,HIDE
                       BUTTON('&Beigt'),AT(384,235,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
  ...
  END
  DO PrepareProcedure
  QuickWindow{PROP:TEXT}='Tekstu plâns '&FILENAME1
  ACCEPT
    CASE EVENT()
    OF EVENT:CloseDown
      WinResize.Destroy
...
!|
  IF FilesOpened
    ...
    TEKSTI1::Used -= 1
    IF TEKSTI1::Used = 0 THEN CLOSE(TEKSTI1).
  END
  IF WindowOpened
    CLOSE(QuickWindow)
  END
...
!|
  ...
  ELSE
    ...
    OF 2
      ...
      TEX1:TEX_A = BRW1::Sort2:LocatorValue
    END
  END
...
!|
  ...
  IF ERRORCODE()
    ...
    ELSE
      StandardWarning(Warn:RecordFetchError,'TEKSTI1')
      POST(Event:CloseWindow)
    END
    EXIT
  END
  CASE BRW1::SortOrder
  OF 2
    BRW1::Sort2:HighValue = TEX1:TEX_A
  END
  DO BRW1::Reset
  ...
  CASE BRW1::SortOrder
  OF 2
    BRW1::Sort2:LowValue = TEX1:TEX_A
...
!|
  TEX1:NR = BRW1::TEX1:NR
  TEX1:TEKSTS1 = BRW1::TEX1:TEKSTS1
  TEX:RIK_TIPS = BRW1::TEX:RIK_TIPS
  RIK:TIPS = BRW1::RIK:TIPS
  TEX1:TEX_A = BRW1::TEX1:TEX_A
!----------------------------------------------------------------------
...
!|
  BRW1::TEX1:NR = TEX1:NR
  BRW1::TEX1:TEKSTS1 = TEX1:TEKSTS1
  BRW1::TEX:RIK_TIPS = TEX:RIK_TIPS
  BRW1::RIK:TIPS = RIK:TIPS
  BRW1::TEX1:TEX_A = TEX1:TEX_A
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 2
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort2:KeyDistribution[BRW1::CurrentScroll] => UPPER(TEX1:TEX_A)
...
!|
  IF BRW1::RecordCount
    ...
    ELSE                                          ! ELSE (What keycode was hit)
      ...
      OF 1
        IF CHR(KEYCHAR())
          IF UPPER(SUB(TEX1:NR,1,1)) = UPPER(CHR(KEYCHAR()))
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 2
      TEX1:TEX_A = BRW1::Sort2:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(TEX1:NR_KEY)
      RESET(TEX1:NR_KEY,BRW1::HighlightedPosition)
    ELSE
      SET(TEX1:NR_KEY,TEX1:NR_KEY)
    END
  OF 2
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(TEX1:TEX_KEY)
      RESET(TEX1:TEX_KEY,BRW1::HighlightedPosition)
    ELSE
      SET(TEX1:TEX_KEY,TEX1:TEX_KEY)
    END
  END
  IF ERRORCODE()
