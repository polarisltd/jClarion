...
BRW1::KUR:VAL          LIKE(KUR:VAL)              ! Queue Display field
BRW1::KUR:DATUMS       LIKE(KUR:DATUMS)           ! Queue Display field
BRW1::KUR:KURSS        LIKE(KUR:KURSS)            ! Queue Display field
BRW1::KUR:TIPS         LIKE(KUR:TIPS)             ! Queue Display field
BRW1::KUR:LB           LIKE(KUR:LB)               ! Queue Display field
...
BRW1::Sort1:KeyDistribution LIKE(KUR:DATUMS),DIM(100)
BRW1::Sort1:LowValue LIKE(KUR:DATUMS)             ! Queue position of scroll thumb
BRW1::Sort1:HighValue LIKE(KUR:DATUMS)            ! Queue position of scroll thumb
BRW1::Sort2:LocatorValue STRING(30)               ! Flag for Range/Filter test
BRW1::Sort2:LocatorLength BYTE                    ! Flag for Range/Filter test
BRW1::Sort2:KeyDistribution LIKE(KUR:VAL),DIM(100)
BRW1::Sort2:LowValue LIKE(KUR:VAL)                ! Queue position of scroll thumb
BRW1::Sort2:HighValue LIKE(KUR:VAL)               ! Queue position of scroll thumb
...
QuickWindow          WINDOW('Kursi_k.tps'),AT(,,200,248),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseKURSI_K'),SYSTEM,GRAY,RESIZE
                       LIST,AT(9,20,178,182),USE(?Browse:1),IMM,VSCROLL,MSG('Browsing Records'),FORMAT('20R(1)|M~Val~C(0)@s3@44R(1)|M~Datums~C(0)@d06.@62R(1)|M~Kurss~C(0)@n-15.7@17C|M~' &|
   'Tips~@s1@12C|M~LB~@s1b@'),FROM(Queue:Browse:1)
                       BUTTON('&Ievadît'),AT(2,228,45,14),USE(?Insert:2)
                       BUTTON('&Mainît'),AT(50,228,45,14),USE(?Change:2),DEFAULT
                       BUTTON('&Dzçst'),AT(97,228,45,14),USE(?Delete:2)
                       SHEET,AT(6,4,187,222),USE(?CurrentTab)
                         TAB('&Valûtu secîba'),USE(?Tab:2)
                           ENTRY(@s3),AT(22,209),USE(KUR:VAL)
                           STRING('- pçc valûtas'),AT(49,211),USE(?String1)
                         END
                         TAB('D&atumu secîba'),USE(?Tab:3)
                           ENTRY(@D6),AT(13,207),USE(KUR:DATUMS)
                           STRING('- pçc datuma'),AT(68,209),USE(?String2)
                         END
                       END
                       BUTTON('&Beigt'),AT(145,228,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
    ...
    OF ?KUR:VAL
      ...
      OF EVENT:Accepted
        ...
        IF KUR:VAL
          ...
          BRW1::Sort2:LocatorValue = KUR:VAL
          BRW1::Sort2:LocatorLength = LEN(CLIP(KUR:VAL))
          SELECT(?Browse:1)
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('BrowseKURSI_K','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('BrowseKURSI_K','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
...
!|
  ...
  CASE BRW1::SortOrder
  OF 1
    BRW1::Sort1:LocatorValue = KUR:DATUMS
    CLEAR(KUR:DATUMS)
  OF 2
    KUR:VAL = BRW1::Sort2:LocatorValue
  END
  DISPLAY()
  ForceRefresh = False
!---------------------------------------------------------------------------
...
!|
  ...
  IF ERRORCODE()
    ...
    ELSE
      StandardWarning(Warn:RecordFetchError,'KURSI_K')
      POST(Event:CloseWindow)
    END
    EXIT
  END
  CASE BRW1::SortOrder
  OF 1
    BRW1::Sort1:HighValue = KUR:DATUMS
  OF 2
    BRW1::Sort2:HighValue = KUR:VAL
  END
  DO BRW1::Reset
  ...
  CASE BRW1::SortOrder
  OF 1
    BRW1::Sort1:LowValue = KUR:DATUMS
    ...
    END
  OF 2
    BRW1::Sort2:LowValue = KUR:VAL
    SetupStringStops(BRW1::Sort2:LowValue,BRW1::Sort2:HighValue,SIZE(BRW1::Sort2:LowValue),ScrollSort:AllowAlpha+ScrollSort:CaseSensitive)
    LOOP BRW1::ScrollRecordCount = 1 TO 100
...
!|
  KUR:VAL = BRW1::KUR:VAL
  KUR:DATUMS = BRW1::KUR:DATUMS
  KUR:KURSS = BRW1::KUR:KURSS
  KUR:TIPS = BRW1::KUR:TIPS
  KUR:LB = BRW1::KUR:LB
!----------------------------------------------------------------------
...
!|
  BRW1::KUR:VAL = KUR:VAL
  BRW1::KUR:DATUMS = KUR:DATUMS
  BRW1::KUR:KURSS = KUR:KURSS
  BRW1::KUR:TIPS = KUR:TIPS
  BRW1::KUR:LB = KUR:LB
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 1
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort1:KeyDistribution[BRW1::CurrentScroll] => KUR:DATUMS
      ...
      OF 2
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort2:KeyDistribution[BRW1::CurrentScroll] => KUR:VAL
            IF BRW1::CurrentScroll <= 1
              BRW1::CurrentScroll = 0
            ELSIF BRW1::CurrentScroll = 100
              BRW1::CurrentScroll = 100
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 1
      KUR:DATUMS = BRW1::Sort1:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    OF 2
      KUR:VAL = BRW1::Sort2:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(KUR:DAT_KEY)
      RESET(KUR:DAT_KEY,BRW1::HighlightedPosition)
    ELSE
      SET(KUR:DAT_KEY,KUR:DAT_KEY)
    END
  OF 2
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(KUR:NOS_KEY)
      RESET(KUR:NOS_KEY,BRW1::HighlightedPosition)
    ELSE
      SET(KUR:NOS_KEY,KUR:NOS_KEY)
    END
  END
  IF ERRORCODE()
...
!|
  ...
  IF BRW1::RecordCount
    CASE BRW1::SortOrder
    OF 1; ?KUR:DATUMS{Prop:Disable} = 0
    OF 2; ?KUR:VAL{Prop:Disable} = 0
    END
    IF BRW1::HighlightedPosition
  ...
  ELSE
    CLEAR(KUR:Record)
    CASE BRW1::SortOrder
    OF 1; ?KUR:DATUMS{Prop:Disable} = 1
    OF 2; ?KUR:VAL{Prop:Disable} = 1
    END
...
Update::Reloop  BYTE
Update::Error   BYTE
History::VAL:Record LIKE(VAL:Record),STATIC
SAV::VAL:Record      LIKE(VAL:Record)
ToolBarMode     UNSIGNED,AUTO
WindowXPos      SIGNED,AUTO,STATIC
WindowYPos      SIGNED,AUTO,STATIC
WindowPosInit   BOOL(False),STATIC
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the VAL_K File'),AT(,,301,203),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateVAL_K'),SYSTEM,GRAY,RESIZE
                       SHEET,AT(3,6,294,178),USE(?CurrentTab)
                         TAB('Valûta un Valsts kods'),USE(?Tab:1)
                           PROMPT('&Valûta (SWIFT):'),AT(7,23,59,10),USE(?VAL:NOS:Prompt)
                           ENTRY(@s3),AT(98,20,29,14),USE(VAL:VAL),CENTER,FONT(,10,,FONT:bold,CHARSET:ANSI)
                           PROMPT('&Nosaukums(daudzsk.):'),AT(7,35,74,10),USE(?VAL:NOSAUKUMS:Prompt)
                           ENTRY(@s20),AT(98,35,83,10),USE(VAL:RUBLI)
                           STRING('ja jâievada tikai valsts'),AT(186,32),USE(?String1:2),FONT(,,COLOR:Gray,,CHARSET:ANSI)
                           PROMPT('&Nosaukums(daudzsk.angl.):'),AT(7,46,90,10),USE(?VAL:NOSAUKUMS_A:Prompt)
                           ENTRY(@s20),AT(98,47,83,10),USE(VAL:RUBLI_A)
                           PROMPT('Sîkna&uda(daudzsk.):'),AT(7,57),USE(?VAL:KAPIKI:Prompt)
                           ENTRY(@s7),AT(98,58,55,10),USE(VAL:KAPIKI)
                           PROMPT('Sîkna&uda(daudzsk. angl.):'),AT(7,69),USE(?VAL:KAPIKI_A:Prompt)
                           ENTRY(@s7),AT(98,69,55,10),USE(VAL:KAPIKI_A)
                           STRING('kods un nosaukums'),AT(186,40),USE(?String1:3),FONT(,,COLOR:Gray,,CHARSET:ANSI)
                           OPTION('&Dzimte'),AT(7,79,54,22),USE(VAL:DZIMTE),BOXED
                             RADIO('V'),AT(13,89),USE(?VAL:DZIMTE:Radio1)
                             RADIO('S'),AT(30,89),USE(?VAL:DZIMTE:Radio2)
                           END
                           STRING('eirozonas valstîm'),AT(186,48),USE(?String1:4),FONT(,,COLOR:Gray,,CHARSET:ANSI)
                           STRING('Ðos 5 atstâjiet tukðus,'),AT(186,23),USE(?String1),FONT(,,COLOR:Gray,,CHARSET:ANSI)
                           PROMPT('Valsts &kods:'),AT(7,104),USE(?VAL:KODS:Prompt)
                           ENTRY(@s2),AT(62,102,17,12),USE(VAL:V_KODS),UPR
                           PROMPT('Va&lsts:'),AT(7,116,53,10),USE(?VAL:VALSTS:Prompt)
                           ENTRY(@s20),AT(62,116,83,10),USE(VAL:VALSTS),REQ
                           PROMPT('Valsts &angliksi:'),AT(7,128,53,10),USE(?VAL:VALSTS_A:Prompt)
                           ENTRY(@s20),AT(62,128,83,10),USE(VAL:VALSTS_A)
                           PROMPT('T&esta kurss:'),AT(7,141,55,10),USE(?VAL:TEST:Prompt)
                           ENTRY(@n-17.10),AT(62,140,63,10),USE(VAL:TEST),LEFT(1)
                           PROMPT('Noklusçtais &BKK 262..'),AT(7,156),USE(?VAL:BKK:Prompt)
                           ENTRY(@s5),AT(92,156,40,10),USE(VAL:BKK)
                           PROMPT('Noklusçtais BKK 2&61..'),AT(7,168,79,10),USE(?VAL:BKKK:Prompt)
                           ENTRY(@s5),AT(92,168,40,10),USE(VAL:BKKK)
                           OPTION('Noklusçtais kursa &Tips:'),AT(158,121,131,56),USE(VAL:Tips),BOXED,MSG('Kursa tips')
                             RADIO('1-EUR/naudas vienîbu'),AT(170,132),USE(?VAL:Tips:Radio1)
                             RADIO('2-EUR/100 naudas vienîbâm'),AT(170,142),USE(?VAL:Tips:Radio2)
                             RADIO('3-EUR/1000 naudas vienîbâm'),AT(170,153),USE(?VAL:Tips:Radio3)
                             RADIO('4-naudas vienîbas/EUR'),AT(170,164),USE(?VAL:Tips:Radio4)
                           END
                         END
                       END
                       BUTTON('&OK'),AT(202,187,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(252,187,45,14),USE(?Cancel)
                       STRING(@s8),AT(5,191),USE(VAL:ACC_KODS),CENTER,FONT(,,COLOR:Gray,,CHARSET:ANSI)
                       STRING(@D06.),AT(44,191),USE(VAL:ACC_DATUMS),FONT(,,COLOR:Gray,,CHARSET:ANSI)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
      ...
      IF EVENT() = Event:Completed
        ...
        OF InsertRecord
          ...
          OF NoError
            ...
            POST(Event:CloseWindow)
          OF DupKeyErr
            IF DUPLICATE(VAL:NOS_KEY)
              IF StandardWarning(Warn:DuplicateKey,'VAL:NOS_KEY')
                SELECT(?VAL:NOS:Prompt)
                VCRRequest = VCRNone
                CYCLE
              END
            END
            IF DUPLICATE(VAL:KODS_KEY)
              IF StandardWarning(Warn:DuplicateKey,'VAL:KODS_KEY')
                SELECT(?VAL:NOS:Prompt)
                VCRRequest = VCRNone
                CYCLE
              END
        ...
        OF ChangeRecord
          LOOP
            ...
            SETCURSOR(Cursor:Wait)
            RecordChanged=false
            IF SAV::VAL:Record <> VAL:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:VAL_K(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
    ...
    OF ?OK
      ...
      OF EVENT:Accepted
        ...
        ELSE
          POST(EVENT:Completed)
        END
         IF ~VAL:VAL AND ~VAL:V_KODS
            KLUDA(87,'ne valsts kods, ne valûta')
            SELECT(VAL:VAL)
            CYCLE
         .
         IF VAL_LV#=TRUE
            IF ~(VAL:VAL='Ls' OR VAL:VAL='LVL')
               KLUDA(0,'Latvijas nacionâlâ valûta ir Ls vai LVL ...')
               SELECT(VAL:VAL)
               CYCLE
            ELSE
               VAL_LV=VAL:VAL
            ...
            .
         .
         VAL:ACC_KODS=ACC_kods
         VAL:ACC_DATUMS=today()
      END
    OF ?Cancel
      CASE EVENT()
      OF EVENT:Accepted
...
PrepareProcedure ROUTINE
    ...
    !  .
      IF GNET !GLOBÂLAIS TÎKLS
         ...
         OF 1
            ...
            VAL:GNET_FLAG[2]=''
         OF 2
            IF VAL:GNET_FLAG[1]=1     !JA STÂV ADD1 (NEKUR CITUR VÇL NAV IERAKSTÎTS)
            ELSIF VAL:GNET_FLAG[1]=4  !JA STÂV ADD4 (NE VISUR IR IERAKSTÎTS)
               KLUDA(121,'I/O, kamçr nav pabeigta tîklu sinhronizâcija')
               LOCALREQUEST=0
            ELSIF VAL:GNET_FLAG[1]=2  !JA STÂV PUT2 (NEKUR CITUR VÇL NAV NOMAINÎTS)
            ELSIF VAL:GNET_FLAG[1]=5  !JA STÂV PUT5 (NE VISUR IR NOMAINÎTS)
               VAL:GNET_FLAG[1]=2
               VAL:GNET_FLAG[2]=''    !MAINAM VISUR
            ELSIF VAL:GNET_FLAG[1]=3 OR VAL:GNET_FLAG[1]=6  !JA STÂV DEL 
               KLUDA(121,'I/O, kamçr nav pabeigta tîklu sinhronizâcija')
               LOCALREQUEST=0
            ELSE
               VAL:GNET_FLAG[1]=2
               VAL:GNET_FLAG[2]=''
            .
            SAV::VAL:Record = VAL:Record ! LAI NERAKSTA, JA NEKAS NAV MAINÎTS
         OF 3
    !        STOP(VAL:GNET_FLAG)
            IF VAL:GNET_FLAG[1]=1     !JA STÂV ADD1 (NEKUR CITUR VÇL NAV IERAKSTÎTS)
               VAL:GNET_FLAG=''
            ...
            ELSIF VAL:GNET_FLAG[1]=2  !JA STÂV PUT2 (NEKUR CITUR VÇL NAV NOMAINÎTS)
               ...
               LOCALREQUEST=4
            ELSIF VAL:GNET_FLAG[1]=5  !JA STÂV PUT5 (NE VISUR IR NOMAINÎTS)
               VAL:GNET_FLAG[1]=3     !DZÇÐAM VISUR
               VAL:GNET_FLAG[2]=''
               LOCALREQUEST=4
            ELSIF VAL:GNET_FLAG[1]=3 OR VAL:GNET_FLAG[1]=6  !JA STÂV DEL
               KLUDA(121,'I/O, kamçr nav pabeigta tîklu sinhronizâcija')
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Spread)
  INIRestoreWindow('UpdateVAL_K','winlats.INI')
  WinResize.Resize
  ?VAL:VAL{PROP:Alrt,255} = 734
  ?VAL:RUBLI{PROP:Alrt,255} = 734
  ?VAL:RUBLI_A{PROP:Alrt,255} = 734
  ?VAL:KAPIKI{PROP:Alrt,255} = 734
  ?VAL:KAPIKI_A{PROP:Alrt,255} = 734
  ?VAL:DZIMTE{PROP:Alrt,255} = 734
  ?VAL:V_KODS{PROP:Alrt,255} = 734
  ?VAL:VALSTS{PROP:Alrt,255} = 734
  ?VAL:VALSTS_A{PROP:Alrt,255} = 734
  ?VAL:TEST{PROP:Alrt,255} = 734
  ?VAL:BKK{PROP:Alrt,255} = 734
  ?VAL:BKKK{PROP:Alrt,255} = 734
  ?VAL:Tips{PROP:Alrt,255} = 734
  ?VAL:ACC_KODS{PROP:Alrt,255} = 734
  ?VAL:ACC_DATUMS{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
ProcedureReturn ROUTINE
!|
!| This routine provides a common procedure exit point for all template
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('UpdateVAL_K','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?VAL:VAL
      VAL:VAL = History::VAL:Record.VAL
    OF ?VAL:RUBLI
      VAL:RUBLI = History::VAL:Record.RUBLI
    OF ?VAL:RUBLI_A
      VAL:RUBLI_A = History::VAL:Record.RUBLI_A
    OF ?VAL:KAPIKI
      VAL:KAPIKI = History::VAL:Record.KAPIKI
    OF ?VAL:KAPIKI_A
      VAL:KAPIKI_A = History::VAL:Record.KAPIKI_A
    OF ?VAL:DZIMTE
      VAL:DZIMTE = History::VAL:Record.DZIMTE
    OF ?VAL:V_KODS
      VAL:V_KODS = History::VAL:Record.V_KODS
    OF ?VAL:VALSTS
      VAL:VALSTS = History::VAL:Record.VALSTS
    OF ?VAL:VALSTS_A
      VAL:VALSTS_A = History::VAL:Record.VALSTS_A
    OF ?VAL:TEST
      VAL:TEST = History::VAL:Record.TEST
    OF ?VAL:BKK
      VAL:BKK = History::VAL:Record.BKK
    OF ?VAL:BKKK
      VAL:BKKK = History::VAL:Record.BKKK
    OF ?VAL:Tips
      VAL:Tips = History::VAL:Record.Tips
    OF ?VAL:ACC_KODS
      VAL:ACC_KODS = History::VAL:Record.ACC_KODS
    OF ?VAL:ACC_DATUMS
      VAL:ACC_DATUMS = History::VAL:Record.ACC_DATUMS
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
...
Queue:Browse:1       QUEUE,PRE()                  ! Browsing Queue
BRW1::VAL:VAL          LIKE(VAL:VAL)              ! Queue Display field
BRW1::VAL:RUBLI        LIKE(VAL:RUBLI)            ! Queue Display field
BRW1::VAL:V_KODS       LIKE(VAL:V_KODS)           ! Queue Display field
BRW1::VAL:VALSTS       LIKE(VAL:VALSTS)           ! Queue Display field
BRW1::VAL:BKK          LIKE(VAL:BKK)              ! Queue Display field
BRW1::VAL:BKKK         LIKE(VAL:BKKK)             ! Queue Display field
BRW1::Mark             BYTE                       ! Queue POSITION information
BRW1::Position         STRING(512)                ! Queue POSITION information
                     END                          ! END (Browsing Queue)
BRW1::CurrentScroll  BYTE                         ! Queue position of scroll thumb
BRW1::ScrollRecordCount LONG                      ! Queue position of scroll thumb
BRW1::SkipFirst      BYTE                         ! Skip first retrieved record in page fill
BRW1::Sort2:KeyDistribution LIKE(VAL:VAL),DIM(100)
BRW1::Sort2:LowValue LIKE(VAL:VAL)                ! Queue position of scroll thumb
BRW1::Sort2:HighValue LIKE(VAL:VAL)               ! Queue position of scroll thumb
BRW1::CurrentEvent   LONG                         !
BRW1::CurrentChoice  LONG                         !
BRW1::RecordCount    LONG                         !
BRW1::SortOrder      BYTE                         !
...
SelectButton           SIGNED                   !Browse select button
                     END
WinResize            WindowResizeType
QuickWindow          WINDOW('Valûtu saraksts'),AT(,,280,236),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseVAL_K'),SYSTEM,GRAY
                       LIST,AT(8,20,261,189),USE(?Browse:1),IMM,VSCROLL,FONT(,9,,FONT:bold),MSG('Browsing Records'),FORMAT('19C|M~VAL~@s3@82L|M~Valûta~L(2)@s20@13C|M~VK~@s2@80L(1)|M~Valsts~@s20@26C|M~Bank' &|
   'a~@s5@20C|M~Kase~@s5@'),FROM(Queue:Browse:1)
                       BUTTON('&Ievadît'),AT(12,218,45,14),USE(?Insert:2)
                       BUTTON('&Mainît'),AT(60,218,45,14),USE(?Change:2),DEFAULT
                       BUTTON('&Dzçst'),AT(110,218,45,14),USE(?Delete:2)
                       SHEET,AT(2,4,273,210),USE(?CurrentTab)
                         TAB('Valûtas'),USE(?Tab:2)
                         END
                         TAB('Valstis'),USE(?Tab:3)
                         END
                       END
                       BUTTON('Iz&vçlçties'),AT(174,218),USE(?Select),FONT(,,COLOR:Navy,,CHARSET:ANSI),KEY(EnterKey)
                       BUTTON('&Beigt'),AT(222,218,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
...
PrepareProcedure ROUTINE
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Spread)
  INIRestoreWindow('BrowseVAL_K','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
  IF LocalRequest <> SelectRecord
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('BrowseVAL_K','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
!| that the record is "fresh" from the disk, without the disk access required.
!|
  VAL:VAL = BRW1::VAL:VAL
  VAL:RUBLI = BRW1::VAL:RUBLI
  VAL:V_KODS = BRW1::VAL:V_KODS
  VAL:VALSTS = BRW1::VAL:VALSTS
  VAL:BKK = BRW1::VAL:BKK
  VAL:BKKK = BRW1::VAL:BKKK
!----------------------------------------------------------------------
BRW1::FillQueue ROUTINE
!|
!| This routine is used to fill the BrowseBox QUEUE from several sources.
...
!| Finally, the POSITION of the current VIEW record is added to the QUEUE
!|
  BRW1::VAL:VAL = VAL:VAL
  BRW1::VAL:RUBLI = VAL:RUBLI
  BRW1::VAL:V_KODS = VAL:V_KODS
  BRW1::VAL:VALSTS = VAL:VALSTS
  BRW1::VAL:BKK = VAL:BKK
  BRW1::VAL:BKKK = VAL:BKKK
  BRW1::Position = POSITION(BRW1::View:Browse)
!----------------------------------------------------------------------
BRW1::PostNewSelection ROUTINE
!|
...
!|
  ...
  ELSIF BRW1::RecordCount
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 2
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort2:KeyDistribution[BRW1::CurrentScroll] => VAL:VAL
            IF BRW1::CurrentScroll <= 1
              BRW1::CurrentScroll = 0
            ELSIF BRW1::CurrentScroll = 100
              BRW1::CurrentScroll = 100
...
!|
  IF BRW1::RecordCount
    ...
    ELSE                                          ! ELSE (What keycode was hit)
      ...
      OF 1
        IF CHR(KEYCHAR())
          IF UPPER(SUB(VAL:V_KODS,1,1)) = UPPER(CHR(KEYCHAR()))
            BRW1::CurrentEvent = EVENT:ScrollDown
            DO BRW1::ScrollOne
            GET(Queue:Browse:1,BRW1::CurrentChoice)
            DO BRW1::FillBuffer
      ...
      OF 2
        IF CHR(KEYCHAR())
          IF SUB(VAL:VAL,1,1) = CHR(KEYCHAR())
            BRW1::CurrentEvent = EVENT:ScrollDown
            DO BRW1::ScrollOne
            GET(Queue:Browse:1,BRW1::CurrentChoice)
            DO BRW1::FillBuffer
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 2
      VAL:VAL = BRW1::Sort2:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(VAL:KODS_KEY)
      RESET(VAL:KODS_KEY,BRW1::HighlightedPosition)
    ELSE
      SET(VAL:KODS_KEY,VAL:KODS_KEY)
    END
  OF 2
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(VAL:NOS_KEY)
      RESET(VAL:NOS_KEY,BRW1::HighlightedPosition)
    ELSE
      SET(VAL:NOS_KEY,VAL:NOS_KEY)
    END
  END
  IF ERRORCODE()
    StandardWarning(Warn:ViewOpenError)
