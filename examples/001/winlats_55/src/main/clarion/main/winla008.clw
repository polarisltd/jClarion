...
QuickWindow          WINDOW('PAROLES.TPS'),AT(0,0,458,293),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('Paroles'),SYSTEM,GRAY
   ...
   '@31L|M~Faili~C@s8@41L|M~B�ze~C@s15@79L|M~Noliktavas~C@s25@76L|M~Fisk�l� druka~C@' &|
   's25@35L|M~Algas~C@s15@35L|M~P/L~C@s15@35L|M~Laika-u~C@s25@'),FROM(Queue:Browse:1)
                         BUTTON('&C-Kop<275>t'),AT(127,271,45,14),USE(?Button6)
                         BUTTON('&Ievad<299>t'),AT(235,271,45,14),USE(?Insert:2)
                       BUTTON('&Main�t'),AT(284,271,45,14),USE(?Change:2),DEFAULT
                       BUTTON('&Dz�st'),AT(333,271,45,14),USE(?Delete:2)
                       SHEET,AT(4,4,450,263),USE(?CurrentTab)
                         TAB('Se&cure_key'),USE(?Tab:2)
                         END
                           TAB('Iek<353><275>jo &Nr sec<299>ba'),USE(?Tab:3)
                           END
                         TAB('&Public_key'),USE(?Tab:4)
                         END
                       END
                       BUTTON('&Beigt'),AT(393,271,45,14),USE(?Close)
                         BUTTON('Iz&v<275>l<275>ties'),AT(181,271),USE(?Select)
                     END
  CODE
  PUSHBIND
...
!|
  ...
  END
  IF Super_sk=0
     KLUDA(88,'neviena Supervizora parole, OBLIG�TI atjaunojiet...')
  .
  SETCURSOR()
!----------------------------------------------------------------------
...
QuickWindow          WINDOW('Update the GLOBAL File'),AT(,,413,343),FONT('MS Sans Serif',9,,FONT:bold),CENTER,IMM,HLP('PAROLES'),SYSTEM,GRAY,NOFRAME
                       SHEET,AT(-4,4,410,315),USE(?CurrentTab)
                         TAB('Sist�mas GLOB�LIE dati'),USE(?Tab2)
                           STRING('P�d�jie dokumentu Nr (9999999 - nenumur�t):'),AT(254,5),USE(?String10),FONT(,,COLOR:Gray,,CHARSET:ANSI)
                           PROMPT('DB Gads(v�rds) :'),AT(18,24),USE(?GL:DB_GADS:Prompt)
                           PROMPT('Re�istr�cijas &Nr:'),AT(13,58,77,10),USE(?GL:REG_NR:Prompt)
                           ...
                           PROMPT('&PVN re�istr�cijas Nr:'),AT(13,74,91,10),USE(?GL:VID_NR:Prompt)
                           ENTRY(@s13),AT(110,71,57,12),USE(GL:VID_NR)
                           PROMPT('&VID teritori�l� noda�a:'),AT(13,87,91,10),USE(?GL:VID_NOS:Prompt)
                           ENTRY(@s25),AT(110,86,100,12),USE(GL:VID_NOS)
                           PROMPT('&Soci�l�s apdro�in��anas Nr:'),AT(13,100,94,10),USE(?GL:SOC_NR:Prompt)
                           ...
                           PROMPT('Pamatdarb�bas veids NACE:'),AT(13,115,97,10),USE(?GL:NACE:Prompt)
                           ENTRY(@N_4B),AT(111,114,36,12),USE(GL:NACE),RIGHT(1),MSG('Pamatdarb�bas veids'),TIP('Pamatdarb�bas veids')
                           PROMPT('VK-PVN U_Nr WinLat� (Partneros):'),AT(149,116),USE(?GL:VIDPVN_U_NR:Prompt)
                           ENTRY(@n_6B),AT(268,114,36,12),USE(GL:VIDPVN_U_NR)
                           PROMPT('M�su U_Nr WinLat� (Partneros):'),AT(156,101,110,10),USE(?GL:CLIENT_U_NR:Prompt)
                           ENTRY(@n_6B),AT(268,100,36,12),USE(GL:CLIENT_U_NR)
                           PROMPT('&Juridisk� adrese:'),AT(13,129,57,10),USE(?GL:ADRESE:Prompt)
                           ENTRY(@s60),AT(76,128,205,12),USE(GL:ADRESE)
                           PROMPT('&R�kojums:'),AT(331,137,34,10),USE(?GL:RIK_NR:Prompt),RIGHT
                           ...
                           PROMPT('&Maks�juma Uzdevums:'),AT(283,24,82,10),USE(?GL:MAU_NR:Prompt),RIGHT
                           ...
                           BUTTON('Main�t finansu Gadu'),AT(110,22,78,14),USE(?ButtonDBGADS),DISABLE
                           STRING(@N_4),AT(78,22),USE(GL:DB_GADS),FONT(,12,,FONT:bold,CHARSET:ANSI)
                           PROMPT('Kases i&e��mumu orderis:'),AT(279,38,86,10),USE(?GL:KIE_NR:Prompt),RIGHT
                           ...
                           PROMPT('&R��ins:'),AT(331,66,34,10),USE(?GL:REK_NR:Prompt),RIGHT
                           ...
                           OPTION('Pa�izmaksas apr.'),AT(13,142,70,22),USE(GL:FMI_TIPS),BOXED
                             RADIO('VS'),AT(25,153),USE(?GL:FMI_TIPS:Radio1)
                             RADIO('FIFO'),AT(46,153),USE(?GL:FMI_TIPS:Radio2)
                           END
                           OPTION('Sv�trkodu autonumer�ciju veikt no:'),AT(86,142,137,24),USE(?Option2),BOXED
                           END
                           ENTRY(@n_13),AT(91,153,71,9),USE(GL:EAN_NR)
                           STRING('(0 - nenumur�t)'),AT(164,153),USE(?String1:2),FONT(,,COLOR:Gray,)
                           BUTTON('�enet�t 13.z�mi'),AT(225,150,77,14),USE(?Button13z)
                           ...
                           PROMPT('Koresp/r��.Nr :'),AT(307,165,48,9),USE(?GL:KOR:Prompt)
                       ...
                       BUTTON('Pal�dz�ba'),AT(351,321,45,14),USE(?Help),HIDE,STD(STD:Help)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
...
!    IF ~GL:DB_B_DAT THEN GL:DB_B_DAT=DATE(12,31,GL:DB_GADS).
    MENESI=AGE(GL:DB_S_DAT,GL:DB_B_DAT+1) !VI�� R��INA, NEIESKAITOT
    ...
    LOOP I#=1 TO 10
       ...
       .
    .
    IF BAND(GL:BAITS,00000001b)  !�ENER�T SV_KODA 13 Z�MI
       UNHIDE(?IMAGE13Z)
    .
  ...
  ACCEPT
    ...
    OF ?ButtonSM
      ...
      OF EVENT:Accepted
        ...
        IF ~INRANGE(DB_MENESI,6,18)
           KLUDA(0,'Neat�auts finansu gads: '&DB_MENESI&' m�ne�i')
           HIDE(?OK)
        ELSE
           UNHIDE(?OK)
        .
    ...
    OF ?ButtonDBGADS
      ...
      OF EVENT:Accepted
        DO SyncWindow
         KLUDA(0,'tagad nor�d�tais periods b�s pamats visam UGP, tie��m main�sim?',11,1)
         IF ~KLU_DARBIBA !OTR�DI
      ...
      OF EVENT:Accepted
        ...
        ELSE
           IF INRANGE(GL:EAN_NR,200000000000,209999999998)
              ...
              UNHIDE(?IMAGE13Z)
           ELSE
              KLUDA(0,'Sv�trkodam j�b�t apgabal� 200000000000-209999999998 (12 z�mes)')
           .
        .
        DISPLAY
      END
    ...
    OF ?OK
      ...
      OF EVENT:Accepted
          ...
          IF ~(YEAR(DB_S_DAT)=YEAR(DB_B_DAT))  !CIT�DS FINANSU GADS
...
!  .
  IF ~VAL:RUBLI_A OR ~VAL:KAPIKI_A
     KLUDA(44,'val�tas nosaukums angliski Val�t�s un Valst�s')
...
! Pagaid�m cirvis
  IF (VAL:VAL='Ls' OR VAL:VAL='LVL') AND CHAR_KAPIKI[2]='1' AND ~(CHAR_KAPIKI[1]='1')
     RINDA=CLIP(VAL:RUBLI) & ' ' & FORMAT(KAPIKI,@N02) & ' sant�ms.'
  ELSE
     RINDA=CLIP(VAL:RUBLI) & ' ' & FORMAT(KAPIKI,@N02) & ' ' & CLIP(VAL:KAPIKI) & '.'
  .
  IF RUBLI=0
...
PADS         ROUTINE
    ...
    .
    KK[I#-1] = ''
    IF I# = 5                              ! Desmit vai padsmit t�ksto�i
      KK[I#] = CLIP(KK[I#])&' t�ksto�i'
    .
