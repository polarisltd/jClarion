...
QuickWindow          WINDOW('Update the NODALAS File'),AT(,,362,116),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateNodalas'),SYSTEM,GRAY,RESIZE
                       SHEET,AT(4,4,357,95),USE(?CurrentTab)
                         TAB('Noda�a'),USE(?Tab:1)
                           PROMPT('No&da�a:'),AT(7,31),USE(?NOD:U_NR:Prompt)
                           ENTRY(@S2),AT(48,31,15,10),USE(NOD:U_NR),LEFT,REQ
                           STRING('(aizpildiet abas z�mes; virsrakstiem-pirmo)'),AT(67,32),USE(?String1)
  ...
  IF KEYCODE() = MouseRight
    SETKEYCODE(0)
  END
  @java-code 'System.out.println("update nodalas");'
...
QuickWindow          WINDOW('NODALAS.tps'),AT(,,361,278),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseNodalas'),SYSTEM,GRAY,RESIZE
                       ...
                       BUTTON('Iz&v�l�ties'),AT(217,262,45,14),USE(?Select:2),FONT(,,COLOR:Navy,,CHARSET:ANSI)
                       SHEET,AT(4,4,353,256),USE(?CurrentTab)
                         TAB('Numuru sec�ba'),USE(?Tab:2)
                           BUTTON('&Ievad�t'),AT(210,243,45,14),USE(?Insert:3)
                           BUTTON('&Main�t'),AT(259,243,45,14),USE(?Change:3),DEFAULT
                           BUTTON('&Dz�st'),AT(308,243,45,14),USE(?Delete:3)
                         END
                         TAB('Kodu sec�ba'),USE(?Tab:3)
                         END
                       END
  ...
  IF KEYCODE() = MouseRight
    SETKEYCODE(0)
  END
  @java-code 'System.out.println("browse nodalas");'
  DO PrepareProcedure
  ACCEPT
    CASE EVENT()
    OF EVENT:CloseDown
...
AppFrame             APPLICATION(' '),AT(,,453,325),FONT('MS Sans Serif',9,,FONT:bold),ICON('CLARION.ICO'),SYSTEM,MAX,MAXIMIZE,RESIZE,IMM
                       MENUBAR
                         MENU('&1-Seanss'),USE(?Seanss)
                           ...
                           ITEM,SEPARATOR
                           ITEM('&2-Beigt darbu'),USE(?Exit),MSG('Beigt darbu ar �o aplik�ciju'),STD(STD:Close)
                           ITEM('&3-ParMani'),USE(?SeanssParMani),HIDE,MSG('Par mani')
                           ! ITEM('&4-FTP:WinLats lejupl�de no www.assako.lv'),USE(?OneSeanss4FTP),HIDE ! didnt support "HIDE"
                           ITEM('&4-FTP:WinLats lejuplde no www.assako.lv'),USE(?OneSeanss4FTP),HIDE
                           
                         END
                         MENU('&2-Serviss'),USE(?Serviss)
                           ITEM('&1-Kalkulātors'),USE(?ServissKalkulators),MSG('Kalkulātors')
                         END
                         MENU('&3-Sistēmas dati'),USE(?System)
                           ITEM('&1-Globālie dati'),USE(?Systemglob),DISABLE,MSG('Konkrētā gada datubāzes Globālie dati')
                           ITEM('&2-Paroles'),USE(?SystemParoles),DISABLE,MSG('Paroles, tikai DB adminstratora pieeja')
                         END
                         MENU('&4-Faili'),USE(?Faili)
                           ITEM('&2-Kontu plāns'),USE(?BrowseKON_K),MSG('Browse KON_K')
                           ITEM('&3-Banku saraksts'),USE(?BrowseBANKAS_K),MSG('Browse BANKAS_K')
                           ITEM('&4-Valūtas un valstis'),USE(?BrowseVAL_K),MSG('Browse VAL_K')
                           ITEM('&5-Valūtu kursi'),USE(?BrowseKURSI_K),MSG('Browse KURSI_K')
                           ITEM('&6-Nodalas'),USE(?FourFaili6Nodalas)
                           ITEM('&7-Projekti'),USE(?FailiProjekti)
                           ITEM('&8-Atzīmes Partneriem'),USE(?FailiAtzimesPartneriem)
                           ITEM,SEPARATOR
                           MENU('&Z-Izziņas no failiem'),USE(?FailiIzzinasnofailiem2) !,LAST
                             ITEM('&1-Partneru saraksts'),USE(?FourFaili9Izzinasnofailiem1Partnerusaraksts)
                             ITEM('&2-Kontu plāns'),USE(?IzzinasnoFailiemKontuplans)
                             ITEM('&3-Banku saraksts'),USE(?FourFaili9Izzinasnofailiem2Bankusarakstsplans)
                             ITEM('&4-Atlaižu lapas'),USE(?FailiIzzinasnofailiemAtlaizulapas)
                             ITEM('&5-Piešķirtās atlaides'),USE(?FailiIzzinasnofailiemPieskirtasatlaides)
                             ITEM('&6-Atlaides klientiem'),USE(?FourFaili9Izzinasnofailiem6Atlaidesklientiem)
                             ITEM('&7-Atzīmes partneriem'),USE(?FourFailiZIzz7AtzPartneriem)
                           END
                         END
                       END
...
FTP_Window WINDOW('Jaun�ko versiju lejupl�de'),AT(,,318,102),FONT('MS Sans Serif',9,,FONT:bold,CHARSET:BALTIC), |
         CENTER,GRAY
       STRING('Labdien!'),AT(32,7),USE(?String1),FONT(,10,COLOR:Navy,FONT:bold,CHARSET:ANSI)
       STRING('Tikai garantijas vai p�cgarantijas noteikumu darb�bas laik�:'),AT(26,20),USE(?String10)
       STRING('www.assako.lv'),AT(26,29),USE(?String2),FONT(,,COLOR:Navy,FONT:underline,CHARSET:ANSI)
       STRING(' ir pieejama WinLats versija 2.11 ar p�d�j�m izmai��m'),AT(69,29),USE(?String4)
       STRING(@D06.B),AT(246,29),USE(FTPDATUMS)
       STRING('Lai sekm�gi veiktu lejupl�di un aizvietotu J�su winlats.exe  no'),AT(26,38),USE(?String5)
       STRING(@D06.B),AT(227,38),USE(WLDATUMS),CENTER
       STRING(','),AT(273,38),USE(?String11)
       STRING('visiem p�r�jiem lietot�jiem nepiecie�ams uz br�di beigt darbu ar WinLatu.'),AT(26,48), |
           USE(?String6)
       STRING('Vecais exe fails tiks saglab�ts k� winlats_old.exe'),AT(26,58),USE(?String7)
       STRING('P�c veiksm�gas atjauno�anas WinLats b�s j�palai� v�lreiz.'),AT(26,67),USE(?String12)
       BUTTON('Veikt lejupl�di'),AT(251,78,62,14),USE(?OkButton),DEFAULT
...
STR           STRING(200)
  ...
  IF KEYCODE() = MouseRight
    SETKEYCODE(0)
  END
  @java-code 'System.out.println("winl001.main() before prepare");'
  DO PrepareProcedure
  !WinLats v2.09.
  @java-code 'System.out.println("winl001.main() before checkopen");'
  ...
  .
!  FING=GETFING(1,DB_GADS,DB_S_DAT,DB_B_DAT)    ! %%%%%% disabled function
 ...
 OMIT('SKIP000040')
  ...
  .
 SKIP000040
   @java-code 'System.out.println("winl001.main() 001");'
  ...
  NOMNAME='NOM_K'  !05.09.09, PAGAIDÂM IZMANTOTS TPSFIX-ATJAUNO�?ANÂ
  ...
  IF GL:DB_GADS > 2013
     ...
     @java-code 'System.out.println("GETVAL_K "+$.toString());'(Val_uzsk)
...
!     STOP('Nav atrasta uzskaites valuta. Val. = Ls')
 ...
 OMIT('SKIP000050')  
    OF ?OSeanss4FTP
         ...
         ACCEPT
            ...
            OF ?OkButton
               IF EVENT()=Event:ACCEPTED
                  IF ~OPENANSI('TEMP.TXT',1)   !TEKO�?Â FOLDERÎ
                     STOP('FTP')
                     BREAK
                  .
                  OUTA:LINE='assako'
                  ...
                  ELSE
                     ...
                     COPY('WINLATS.ARJ',CLIP(FILENAME1)&'\WINLATS.ARJ')   !NO TEKO�?Â UZ \BIN
                     IF ERROR()
      ...
      .
 SKIP000050  
    OF ?ServissKalkulators
      ! START(kalkis,25000) %%%%% disabled
    OF ?Systemglob
      ! UpdateGlobal ! %%%% disabled
    OF ?SystemParoles
      ! Paroles  ! %%%%  disabled
    OF ?BrowseKON_K
      ! BrowseKON_K ! %%%%  disabled
    OF ?BrowseBANKAS_K
      BrowseBANKAS_K
    OF ?BrowseVAL_K
      ! BrowseVAL_K ! %%%%  disabled
    OF ?BrowseKURSI_K
      ! BrowseKURSI_K ! %%%%  disabled
    OF ?FourFaili6Nodalas
      ! BrowseNodalas ! %%%%  disabled
    OF ?FailiProjekti
      ! BrowseProjekti ! %%%%  disabled
    OF ?FailiAtzimesPartneriem
      ! BrowsePAR_Z ! %%%%  disabled
    OF ?FourFaili9Izzinasnofailiem1Partnerusaraksts
       !�?ITEN MDI BÇRNUS NEDRÎKST SAUKT
       ! START(F_PrintPar_K,50000) ! %%%%  disabled
      
    OF ?IzzinasnoFailiemKontuplans
       !�?ITEN MDI BÇRNUS NEDRÎKST SAUKT
       ! START(F_PrintKON_K,50000)  ! %%%%  disabled
    OF ?FourFaili9Izzinasnofailiem2Bankusarakstsplans
      START(F_BankasReport,50000) 
       !�?ITEN MDI BÇRNUS NEDRÎKST SAUKT
    OF ?FailiIzzinasnofailiemAtlaizulapas
       !�?ITEN MDI BÇRNUS NEDRÎKST SAUKT
       ! START(F_AtlaizuLapas,50000) ! %%%%  disabled
    OF ?FailiIzzinasnofailiemPieskirtasatlaides
       !�?ITEN MDI BÇRNUS NEDRÎKST SAUKT
       ! START(F_AtlaidesReport,50000) ! %%%%  disabled
    OF ?FourFaili9Izzinasnofailiem6Atlaidesklientiem
       !�?ITEN MDI BÇRNUS NEDRÎKST SAUKT
       ! START(F_ATLAIDESKLIENTIEM,50000) ! %%%%  disabled
    OF ?FourFailiZIzz7AtzPartneriem
       !OPCIJA='1'
       !IZZFILTF
       !IF GLOBALRESPONSE=REQUESTCOMPLETED
       !�?ITEN MDI BÇRNUS NEDRÎKST SAUKT
       ! START(F_ATZ_K,50000) ! %%%%  disabled
...
!---------------------------------------------------------------------------
PrepareProcedure ROUTINE
  @java-code 'System.out.println("Hello PrepareProcedure");'
  FilesOpened = True
  OPEN(AppFrame)
  WindowOpened=True
  !WinResize.Initialize(AppStrat:Spread)
