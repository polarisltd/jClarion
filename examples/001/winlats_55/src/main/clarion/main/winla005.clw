...
Update::Reloop  BYTE
Update::Error   BYTE
History::ATZ:Record LIKE(ATZ:Record),STATIC
SAV::ATZ:Record      LIKE(ATZ:Record)
Auto::Attempts       LONG,AUTO
Auto::Save:ATZ:NR       LIKE(ATZ:NR)
ToolBarMode     UNSIGNED,AUTO
WindowXPos      SIGNED,AUTO,STATIC
WindowYPos      SIGNED,AUTO,STATIC
WindowPosInit   BOOL(False),STATIC
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the ATZ_K File'),AT(,,194,142),FONT('MS Sans Serif',9,COLOR:MENUTEXT,FONT:bold,CHARSET:BALTIC),IMM,HLP(' '),SYSTEM,GRAY,RESIZE
                       SHEET,AT(2,4,192,120),USE(?CurrentTab)
                         TAB('Atzîme'),USE(?Tab:1)
                           ENTRY(@n3),AT(5,18,27,10),USE(ATZ:NR),RIGHT(1)
                           PROMPT('Teksts:'),AT(6,34),USE(?TEX:TEKSTS:Prompt)
                           ENTRY(@s40),AT(34,33,157,10),USE(ATZ:TEKSTS)
                           OPTION('Brîdinâjuma lîmenis :'),AT(11,46,179,72),USE(ATZ:WARLEVEL),BOXED
                             RADIO('0-zinâðanai'),AT(18,58),USE(?TEX:WARLEVEL:Radio0)
                             RADIO('1-jâbrîdina,sastâdot dokumentu'),AT(18,69),USE(?TEX:WARLEVEL:Radio1 ),VALUE('1')
                             RADIO('2-aizliegts izrakstît P/Z'),AT(18,79),USE(?TEX:WARLEVEL:Radio2),VALUE('2')
                             RADIO('3-aizliegts tirgot caur kasi'),AT(18,88),USE(?TEX:WARLEVEL:Radio3 ),VALUE('3')
                             RADIO('4-speciâli noteikumi'),AT(18,98),USE(?TEX:WARLEVEL:Radio4),VALUE('4')
                           END
                         END
                       END
                       BUTTON('&OK'),AT(98,126,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(147,126,45,14),USE(?Cancel)
                     END
  CODE
  PUSHBIND
  ...
  IF ATLAUTS[22]='1' AND LOCALREQUEST=2 !Aizliegts mainît par:atzîmi
     LOCALREQUEST=0
  .
  CASE LocalRequest
  OF InsertRecord
    ...
    ELSE
      ...
      IF EVENT() = Event:Completed
        ...
        OF InsertRecord
          ...
          OF DupKeyErr
            IF DUPLICATE(ATZ:NR_KEY)
              IF StandardWarning(Warn:DuplicateKey,'ATZ:NR_KEY')
                SELECT(?ATZ:NR)
                VCRRequest = VCRNone
                CYCLE
              END
        ...
        OF ChangeRecord
          LOOP
            ...
            RecordChanged=false
            IF SAV::ATZ:Record <> ATZ:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:PAR_Z(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('UpdatePAR_Z','winlats.INI')
  WinResize.Resize
  ?ATZ:NR{PROP:Alrt,255} = 734
  ?ATZ:TEKSTS{PROP:Alrt,255} = 734
  ?ATZ:WARLEVEL{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('UpdatePAR_Z','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?ATZ:NR
      ATZ:NR = History::ATZ:Record.NR
    OF ?ATZ:TEKSTS
      ATZ:TEKSTS = History::ATZ:Record.TEKSTS
    OF ?ATZ:WARLEVEL
      ATZ:WARLEVEL = History::ATZ:Record.WARLEVEL
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
...
!|
  ...
  LOOP
    ...
    IF ERRORCODE() AND ERRORCODE() <> BadRecErr
      StandardWarning(Warn:RecordFetchError,'PAR_Z')
      POST(Event:CloseWindow)
      EXIT
    END
    IF ERRORCODE()
      Auto::Save:ATZ:NR = 1
    ELSE
      Auto::Save:ATZ:NR = ATZ:NR + 1
    END
    ATZ:Record = SAV::ATZ:Record
    ATZ:NR = Auto::Save:ATZ:NR
    SAV::ATZ:Record = ATZ:Record
    ADD(PAR_Z)
    IF ERRORCODE()
      Auto::Attempts += 1
...
BRW1::View:Browse    VIEW(PAR_Z)
                       PROJECT(ATZ:NR)
                       PROJECT(ATZ:TEKSTS)
                       PROJECT(ATZ:WARLEVEL)
                     END

Queue:Browse:1       QUEUE,PRE()                  ! Browsing Queue
BRW1::ATZ:NR           LIKE(ATZ:NR)               ! Queue Display field
BRW1::ATZ:TEKSTS       LIKE(ATZ:TEKSTS)           ! Queue Display field
BRW1::ATZ:WARLEVEL     LIKE(ATZ:WARLEVEL)         ! Queue Display field
BRW1::ATZ:WARLEVEL:NormalFG LONG                  ! Normal Foreground
BRW1::ATZ:WARLEVEL:NormalBG LONG                  ! Normal Background
BRW1::ATZ:WARLEVEL:SelectedFG LONG                ! Selected Foreground
BRW1::ATZ:WARLEVEL:SelectedBG LONG                ! Selected Background
BRW1::WARTEXT          LIKE(WARTEXT)              ! Queue Display field
BRW1::WARTEXT:NormalFG LONG                       ! Normal Foreground
BRW1::WARTEXT:NormalBG LONG                       ! Normal Background
BRW1::WARTEXT:SelectedFG LONG                     ! Selected Foreground
BRW1::WARTEXT:SelectedBG LONG                     ! Selected Background
...
BRW1::Sort1:KeyDistribution LIKE(ATZ:NR),DIM(100)
BRW1::Sort1:LowValue LIKE(ATZ:NR)                 ! Queue position of scroll thumb
BRW1::Sort1:HighValue LIKE(ATZ:NR)                ! Queue position of scroll thumb
...
SelectButton           SIGNED                   !Browse select button
                     END
WinResize            WindowResizeType
QuickWindow          WINDOW('atz_k.tps'),AT(,,350,280),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseTeksti'),SYSTEM,GRAY,RESIZE
                       LIST,AT(8,20,333,215),USE(?Browse:1),IMM,VSCROLL,MSG('Browsing Records'),FORMAT('16R(2)|M~Nr~L(0)@n3@163L(1)|M~Teksts~L(5)@s40@15L(1)M*~Nr~C(0)@s1@80L(2)|M*~Leve' &|
   'l~@s30@'),FROM(Queue:Browse:1)
                       SHEET,AT(4,4,341,253),USE(?CurrentTab)
                         TAB('Atzîmes partneriem'),USE(?Tab:2)
                           BUTTON('&Ievadît'),AT(191,239,45,14),USE(?Insert:2)
                           BUTTON('&Mainît'),AT(240,239,45,14),USE(?Change),DEFAULT
                           BUTTON('&Dzçst'),AT(288,239,45,14),USE(?Delete:2)
                         END
                       END
                       BUTTON('Iz&vçlçties'),AT(249,260),USE(?Select),FONT(,,COLOR:Navy,,CHARSET:ANSI)
                       BUTTON('&Beigt'),AT(294,260,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
...
PrepareProcedure ROUTINE
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Spread)
  INIRestoreWindow('BrowsePAR_Z','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
  IF LocalRequest <> SelectRecord
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('BrowsePAR_Z','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
!|
  ATZ:NR = BRW1::ATZ:NR
  ATZ:TEKSTS = BRW1::ATZ:TEKSTS
  ATZ:WARLEVEL = BRW1::ATZ:WARLEVEL
  WARTEXT = BRW1::WARTEXT
!----------------------------------------------------------------------
...
!|
   WARTEXT=GETPAR_ATZIME(ATZ:NR,3)
  BRW1::ATZ:NR = ATZ:NR
  BRW1::ATZ:TEKSTS = ATZ:TEKSTS
  BRW1::ATZ:WARLEVEL = ATZ:WARLEVEL
  IF (ATZ:WARLEVEL=2)
    BRW1::ATZ:WARLEVEL:NormalFG = 255
    BRW1::ATZ:WARLEVEL:NormalBG = -1
    BRW1::ATZ:WARLEVEL:SelectedFG = 255
    BRW1::ATZ:WARLEVEL:SelectedBG = -1
  ELSIF (ATZ:WARLEVEL=4)
    BRW1::ATZ:WARLEVEL:NormalFG = 4227327
    BRW1::ATZ:WARLEVEL:NormalBG = -1
    BRW1::ATZ:WARLEVEL:SelectedFG = 4227327
    BRW1::ATZ:WARLEVEL:SelectedBG = -1
  ELSE
    BRW1::ATZ:WARLEVEL:NormalFG = -1
    BRW1::ATZ:WARLEVEL:NormalBG = -1
    BRW1::ATZ:WARLEVEL:SelectedFG = -1
    BRW1::ATZ:WARLEVEL:SelectedBG = -1
  END
  BRW1::WARTEXT = WARTEXT
    BRW1::WARTEXT:NormalFG = -1
    BRW1::WARTEXT:NormalBG = -1
    BRW1::WARTEXT:SelectedFG = -1
    BRW1::WARTEXT:SelectedBG = -1
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 1
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort1:KeyDistribution[BRW1::CurrentScroll] => ATZ:NR
...
!|
  IF BRW1::RecordCount
    ...
    ELSE                                          ! ELSE (What keycode was hit)
      ...
      OF 1
        IF CHR(KEYCHAR())
          IF UPPER(SUB(ATZ:NR,1,1)) = UPPER(CHR(KEYCHAR()))
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 1
      ATZ:NR = BRW1::Sort1:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(ATZ:NR_KEY)
      RESET(ATZ:NR_KEY,BRW1::HighlightedPosition)
    ELSE
      SET(ATZ:NR_KEY,ATZ:NR_KEY)
    END
  END
  IF ERRORCODE()
...
Update::Reloop  BYTE
Update::Error   BYTE
History::PRP:Record LIKE(PRP:Record),STATIC
SAV::PRP:Record      LIKE(PRP:Record)
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the PROJ_P File'),AT(,,233,98),FONT('MS Sans Serif',9,,FONT:bold,CHARSET:BALTIC),IMM,HLP('UpdatePROJ_P'),SYSTEM,GRAY,RESIZE,MDI
                       SHEET,AT(4,4,225,72),USE(?CurrentTab)
                         TAB,USE(?Tab:1)
                           PROMPT('&Datums:'),AT(8,30),USE(?PROP:DATUMS:Prompt)
                           ENTRY(@D06.b),AT(61,30,53,10),USE(PRP:DATUMS),RIGHT(1)
                           PROMPT('&Plâns:'),AT(8,44),USE(?PROP:PLANS:Prompt)
                           ENTRY(@n-14.2),AT(61,44,64,10),USE(PRP:PLANS),DECIMAL(12)
                           PROMPT('&Komentârs:'),AT(8,58),USE(?PROP:Komentars:Prompt)
                           ENTRY(@s40),AT(61,58,164,10),USE(PRP:Komentars)
                         END
                       END
                       BUTTON('&OK'),AT(134,80,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(183,80,45,14),USE(?Cancel)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
  ...
  CASE LocalRequest
  OF InsertRecord
    ActionMessage = 'Adding a PROJ_P Record'
  OF ChangeRecord
    ActionMessage = 'Changing a PROJ_P Record'
  OF DeleteRecord
  END
  QuickWindow{Prop:Text} = ActionMessage
  ENABLE(TBarBrwHistory)
      ...
      IF EVENT() = Event:Completed
        ...
        OF InsertRecord
          ...
          OF NoError
            ...
            POST(Event:CloseWindow)
          OF DupKeyErr
            IF DUPLICATE(PRP:Nr_Key)
              IF StandardWarning(Warn:DuplicateKey,'PRP:Nr_Key')
                SELECT(?PROP:DATUMS:Prompt)
                CYCLE
              END
            END
        ...
        OF ChangeRecord
          LOOP
            ...
            SETCURSOR(Cursor:Wait)
            RecordChanged=false
            IF SAV::PRP:Record <> PRP:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:PROJ_P(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('UpdatePROJ_P','winlats.INI')
  WinResize.Resize
  ?PRP:DATUMS{PROP:Alrt,255} = 734
  ?PRP:PLANS{PROP:Alrt,255} = 734
  ?PRP:Komentars{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
ProcedureReturn ROUTINE
!|
!| This routine provides a common procedure exit point for all template
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('UpdatePROJ_P','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?PRP:DATUMS
      PRP:DATUMS = History::PRP:Record.DATUMS
    OF ?PRP:PLANS
      PRP:PLANS = History::PRP:Record.PLANS
    OF ?PRP:Komentars
      PRP:Komentars = History::PRP:Record.Komentars
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
