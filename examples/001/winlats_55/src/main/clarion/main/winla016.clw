...
BRW1::PAR_NOS_S        LIKE(PAR_NOS_S)            ! Queue Display field
BRW1::VES:DATUMS       LIKE(VES:DATUMS)           ! Queue Display field
BRW1::VES:KAM          LIKE(VES:KAM)              ! Queue Display field
BRW1::VES:PROCESS      LIKE(VES:PROCESS)          ! Queue Display field
...
BRW1::Sort3:KeyDistribution LIKE(VES:CRM),DIM(100)
BRW1::Sort3:LowValue LIKE(VES:CRM)                ! Queue position of scroll thumb
BRW1::Sort3:HighValue LIKE(VES:CRM)               ! Queue position of scroll thumb
...
QuickWindow          WINDOW('VESTURE.tps'),AT(-1,-1,452,325),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseVesture'),SYSTEM,GRAY,RESIZE,MDI
                       BUTTON('I&zdruka'),AT(7,306,45,14),USE(?Button5),DISABLE
                       LIST,AT(2,20,446,282),USE(?Browse:1),IMM,VSCROLL,MSG('Browsing Records'),FORMAT('15R(1)|M~H~C(0)@n3@60L(1)|M~Partneris~C(0)@s15@43C|M~Datums~@D06.B@19C|M~Kam~@n_' &|
   '3B@8C|M~P~@n1B@540L(2)|M~Saturs~C(0)@s135@'),FROM(Queue:Browse:1)
                       SHEET,AT(-2,2,455,321),USE(?CurrentTab)
                         TAB('CRM:Manu darbu saraksts'),USE(?Tab:1)
                         END
                         TAB('CRM:Visu darbu saraksts'),USE(?Tab:2),DISABLE
                         END
                       END
                       BUTTON('&Ievadît'),AT(94,306,45,14),USE(?Insert:2)
                       BUTTON('&Mainît'),AT(142,306,45,14),USE(?Change:2),DEFAULT
                       BUTTON('&Dzçst'),AT(189,306,45,14),USE(?Delete:2)
                       BUTTON('&Beigt'),AT(403,306,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('BrowseVestureA','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('BrowseVestureA','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
...
!|
  ...
  PAR_NOS_S = BRW1::PAR_NOS_S
  VES:DATUMS = BRW1::VES:DATUMS
  VES:KAM = BRW1::VES:KAM
  VES:PROCESS = BRW1::VES:PROCESS
  SATURS = BRW1::SATURS
...
!|
  ...
  SATURS=CLIP(VES:SATURS)&' '&CLIP(VES:SATURS2)&' '&CLIP(VES:SATURS3)
  BRW1::VES:CRM = VES:CRM
  BRW1::PAR_NOS_S = PAR_NOS_S
  BRW1::VES:DATUMS = VES:DATUMS
  BRW1::VES:KAM = VES:KAM
  BRW1::VES:PROCESS = VES:PROCESS
  BRW1::SATURS = SATURS
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 3
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort3:KeyDistribution[BRW1::CurrentScroll] => VES:CRM
...
!|
  IF BRW1::RecordCount
    ...
    ELSE                                          ! ELSE (What keycode was hit)
      ...
      OF 1
        IF CHR(KEYCHAR())
          IF UPPER(SUB(VES:CRM,1,1)) = UPPER(CHR(KEYCHAR()))
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 3
      VES:CRM = BRW1::Sort3:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(VES:CRM_KEY)
      RESET(VES:CRM_KEY,BRW1::HighlightedPosition)
    ELSE
      SET(VES:CRM_KEY,VES:CRM_KEY)
    END
    BRW1::View:Browse{Prop:Filter} = |
    '(VES:CRM>0 AND  VES:KAM=ACC_KODS_N)'
  OF 2
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(VES:CRM_KEY)
      RESET(VES:CRM_KEY,BRW1::HighlightedPosition)
...
Update::Reloop  BYTE
Update::Error   BYTE
History::KONR:Record LIKE(KONR:Record),STATIC
SAV::KONR:Record     LIKE(KONR:Record)
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the KRI_K File'),AT(,,368,193),FONT('MS Sans Serif',8,,FONT:bold,CHARSET:BALTIC),IMM,HLP('UpdateKRI_K'),GRAY,RESIZE,MDI
                       SHEET,AT(2,4,364,164),USE(?CurrentTab)
                         TAB('Mainît atïauts tikai lietotâja definçtâs rindas un angliskos nosaukumus'),USE(?Tab:1)
                           OPTION('&UGP dokuments'),AT(10,24,157,65),USE(KONR:UGP),DISABLE,BOXED
                             RADIO('Bilance'),AT(18,37),USE(?KRI:UGP:Radio1),VALUE('B')
                             RADIO('Peïòas/zaudçjumu aprçíins (PZA2)'),AT(18,48),USE(?KRI:UGP:Radio2),VALUE('P')
                             RADIO('Naudas plûsmas pârskats (NPP2)'),AT(18,60),USE(?KRI:UGP:Radio3),VALUE('N')
                             RADIO('Paðu kapitâla izmaiòu pârskats (PKIP)'),AT(18,72),USE(?KRI:UGP:Radio4),VALUE('K')
                           END
                           PROMPT('Rindas &Kods saskaòâ ar EDS v.5.12.'),AT(6,94,123,10),USE(?KRI:KODS:Prompt)
                           ENTRY(@n4B),AT(130,94,40,10),USE(KONR:KODS),DISABLE,REQ
                           PROMPT('&Rindiòas nosaukums'),AT(6,116,71,10),USE(?KRI:TEKSTS:Prompt)
                           ENTRY(@S100),AT(78,116,285,10),USE(KONR:Nosaukums),DISABLE
                           PROMPT('Ang&liski'),AT(6,130),USE(?KRI:TEKSTS1:Prompt)
                           ENTRY(@S100),AT(78,130,285,10),USE(KONR:NosaukumsA)
                         END
                       END
                       BUTTON('OK'),AT(270,175,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(319,175,45,14),USE(?Cancel)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
  ...
  ACCEPT
    ...
    OF EVENT:OpenWindow
      IF NOT WindowInitialized
        ...
        WindowInitialized = True
      END
      SELECT(?KRI:UGP:Radio1)
         IF LOCALREQUEST=1
            SELECT(?KONR:KODS)
         ELSIF LOCALREQUEST=2
            SELECT(?KONR:NOSAUKUMS)
         ELSE
            SELECT(?CANCEL)
         .
    OF EVENT:GainFocus
    ...
    ELSE
      ...
      IF EVENT() = Event:Completed
        History::KONR:Record = KONR:Record
        CASE LocalRequest
        OF InsertRecord
          ADD(KON_R)
          CASE ERRORCODE()
          OF NoError
            ...
            POST(Event:CloseWindow)
          OF DupKeyErr
            IF DUPLICATE(KONR:UGP_KEY)
              IF StandardWarning(Warn:DuplicateKey,'KONR:UGP_KEY')
                SELECT(?KRI:UGP:Radio1)
                CYCLE
              END
            END
        ...
        OF ChangeRecord
          LOOP
            ...
            SETCURSOR(Cursor:Wait)
            RecordChanged=false
            IF SAV::KONR:Record <> KONR:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:KON_R(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('UpdateKon_R','winlats.INI')
  WinResize.Resize
  ?KONR:UGP{PROP:Alrt,255} = 734
  ?KONR:KODS{PROP:Alrt,255} = 734
  ?KONR:Nosaukums{PROP:Alrt,255} = 734
  ?KONR:NosaukumsA{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
ProcedureReturn ROUTINE
!|
!| This routine provides a common procedure exit point for all template
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('UpdateKon_R','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?KONR:UGP
      KONR:UGP = History::KONR:Record.UGP
    OF ?KONR:KODS
      KONR:KODS = History::KONR:Record.KODS
    OF ?KONR:Nosaukums
      KONR:Nosaukums = History::KONR:Record.Nosaukums
    OF ?KONR:NosaukumsA
      KONR:NosaukumsA = History::KONR:Record.NosaukumsA
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
...
BRW1::View:Browse    VIEW(KON_R)
                       PROJECT(KONR:KODS)
                       PROJECT(KONR:Nosaukums)
                       PROJECT(KONR:UGP)
                     END

Queue:Browse:1       QUEUE,PRE()                  ! Browsing Queue
BRW1::KONR:KODS        LIKE(KONR:KODS)            ! Queue Display field
BRW1::KONR:Nosaukums   LIKE(KONR:Nosaukums)       ! Queue Display field
BRW1::KONR:UGP         LIKE(KONR:UGP)             ! Queue Display field
BRW1::Mark             BYTE                       ! Queue POSITION information
BRW1::Position         STRING(512)                ! Queue POSITION information
                     END                          ! END (Browsing Queue)
BRW1::CurrentScroll  BYTE                         ! Queue position of scroll thumb
BRW1::ScrollRecordCount LONG                      ! Queue position of scroll thumb
BRW1::SkipFirst      BYTE                         ! Skip first retrieved record in page fill
BRW1::Sort1:Reset:P  LIKE(P)
BRW1::Sort2:Reset:N  LIKE(N)
BRW1::Sort3:Reset:K1 LIKE(K1)
BRW1::Sort4:KeyDistribution LIKE(KONR:KODS),DIM(100)
BRW1::Sort4:LowValue LIKE(KONR:KODS)              ! Queue position of scroll thumb
BRW1::Sort4:HighValue LIKE(KONR:KODS)             ! Queue position of scroll thumb
BRW1::Sort4:Reset:B  LIKE(B)
BRW1::QuickScan      BYTE                         ! Flag for Range/Filter test
BRW1::CurrentEvent   LONG                         !
BRW1::CurrentChoice  LONG                         !
BRW1::RecordCount    LONG                         !
...
BRW1::PopupText      STRING(128)                  !
WinResize            WindowResizeType
QuickWindow          WINDOW('KON_R.TPS'),AT(,,390,317),FONT('MS Sans Serif',9,,FONT:bold,CHARSET:BALTIC),CENTER,IMM,HLP('BrowseKRI_K'),SYSTEM,GRAY,RESIZE,MDI
                       LIST,AT(8,20,369,274),USE(?Browse:1),IMM,VSCROLL,MSG('Browsing Records'),FORMAT('20R(2)|M~Kods~C(0)@n4@80L(2)|M~Rindiòas nosaukums~@S100@'),FROM(Queue:Browse:1)
                       BUTTON('&Mainît '),AT(289,298,45,14),USE(?Change:2),DEFAULT
                       SHEET,AT(4,4,381,312),USE(?CurrentTab)
                         TAB('Bilance'),USE(?Tab:2)
                         END
                         TAB('PZA2'),USE(?Tab:3)
                         END
                         TAB('NPP2'),USE(?Tab:4)
                         ...
                         END
                       END
                       BUTTON('&Beigt'),AT(337,298,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
  ...
  ACCEPT
    ...
    OF EVENT:OpenWindow
      ...
      END
      SELECT(?Browse:1)
      QUICKWINDOW{PROP:TEXT}=CLIP(LONGPATH())&'\'&KONRNAME&'.TPS'
    OF EVENT:GainFocus
      ForceRefresh = True
      IF NOT WindowInitialized
        DO InitializeWindow
...
PrepareProcedure ROUTINE
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Spread)
  INIRestoreWindow('BrowseKON_R','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
  BIND('B',B)
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('BrowseKON_R','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
!|
  ...
  IF BRW1::SortOrder = BRW1::LastSortOrder
    CASE BRW1::SortOrder
    OF 1
      IF BRW1::Sort1:Reset:P <> P
        BRW1::Changed = True
      END
    OF 2
      IF BRW1::Sort2:Reset:N <> N
        BRW1::Changed = True
      END
    OF 3
      IF BRW1::Sort3:Reset:K1 <> K1
        BRW1::Changed = True
      END
    OF 4
      IF BRW1::Sort4:Reset:B <> B
        BRW1::Changed = True
      END
    END
  ELSE
  ...
  IF BRW1::SortOrder <> BRW1::LastSortOrder OR BRW1::Changed OR ForceRefresh
    CASE BRW1::SortOrder
    OF 1
      BRW1::Sort1:Reset:P = P
    OF 2
      BRW1::Sort2:Reset:N = N
    OF 3
      BRW1::Sort3:Reset:K1 = K1
    OF 4
      BRW1::Sort4:Reset:B = B
    END
    DO BRW1::GetRecord
    DO BRW1::Reset
    IF BRW1::LastSortOrder = 0
...
!|
  ...
  CASE BRW1::SortOrder
  OF 4
    BRW1::Sort4:HighValue = KONR:KODS
  END
  DO BRW1::Reset
  NEXT(BRW1::View:Browse)
  IF ERRORCODE()
  ...
  CASE BRW1::SortOrder
  OF 4
    BRW1::Sort4:LowValue = KONR:KODS
    SetupRealStops(BRW1::Sort4:LowValue,BRW1::Sort4:HighValue)
    LOOP BRW1::ScrollRecordCount = 1 TO 100
      BRW1::Sort4:KeyDistribution[BRW1::ScrollRecordCount] = NextRealStop()
    END
  END
...
!| that the record is "fresh" from the disk, without the disk access required.
!|
  KONR:KODS = BRW1::KONR:KODS
  KONR:Nosaukums = BRW1::KONR:Nosaukums
  KONR:UGP = BRW1::KONR:UGP
!----------------------------------------------------------------------
BRW1::FillQueue ROUTINE
!|
!| This routine is used to fill the BrowseBox QUEUE from several sources.
...
!| Finally, the POSITION of the current VIEW record is added to the QUEUE
!|
  BRW1::KONR:KODS = KONR:KODS
  BRW1::KONR:Nosaukums = KONR:Nosaukums
  BRW1::KONR:UGP = KONR:UGP
  BRW1::Position = POSITION(BRW1::View:Browse)
!----------------------------------------------------------------------
BRW1::PostNewSelection ROUTINE
!|
...
!|
  ...
  IF KEYCODE() = MouseRight
    ...
    IF BRW1::RecordCount
      IF BRW1::PopupText
        BRW1::PopupText = '&Mainît |-|' & BRW1::PopupText
      ELSE
        BRW1::PopupText = '&Mainît '
      END
    ELSE
      IF BRW1::PopupText
        BRW1::PopupText = '~&Mainît |-|' & BRW1::PopupText
  ...
  ELSIF BRW1::RecordCount
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 4
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort4:KeyDistribution[BRW1::CurrentScroll] => KONR:KODS
            IF BRW1::CurrentScroll <= 1
              BRW1::CurrentScroll = 0
            ELSIF BRW1::CurrentScroll = 100
              BRW1::CurrentScroll = 100
...
!|
  IF BRW1::RecordCount
    ...
    ELSE                                          ! ELSE (What keycode was hit)
      ...
      OF 1
        IF CHR(KEYCHAR())
          IF UPPER(SUB(KONR:KODS,1,1)) = UPPER(CHR(KEYCHAR()))
            BRW1::CurrentEvent = EVENT:ScrollDown
            DO BRW1::ScrollOne
            GET(Queue:Browse:1,BRW1::CurrentChoice)
            DO BRW1::FillBuffer
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 4
      KONR:KODS = BRW1::Sort4:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(KONR:UGP_KEY)
      RESET(KONR:UGP_KEY,BRW1::HighlightedPosition)
    ELSE
      KONR:UGP = P
      SET(KONR:UGP_KEY,KONR:UGP_KEY)
    END
    BRW1::View:Browse{Prop:Filter} = |
    'UPPER(KONR:UGP) = UPPER(P)'
  OF 2
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(KONR:UGP_KEY)
      RESET(KONR:UGP_KEY,BRW1::HighlightedPosition)
...
!|
  CASE BRW1::SortOrder
  OF 1
    P = BRW1::Sort1:Reset:P
  OF 2
    N = BRW1::Sort2:Reset:N
  OF 3
    K1 = BRW1::Sort3:Reset:K1
  OF 4
    B = BRW1::Sort4:Reset:B
  END
...
!|
!| Next, GlobalRequest is set the the value of LocalRequest, and the update procedure
!| (UpdateKon_R) is called.
!|
!| Upon return from the update, the routine BRW1::Reset is called to reset the VIEW
!| and reopen it.
!|
  !XECUTE(CHOICE(?CURRENTTAB)-1)
  !  KRI:UGP='B'
  !  KRI:UGP='P'
  !  KRI:UGP='N'
  !  KRI:UGP='K'
  !
  !TOP(CHOICE(?CURRENTTAB)&KRI:UGP)
