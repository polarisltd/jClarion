...
! <T_S_DAT>: DB S_DAT ????????????????????????VAI MAX M�N_SK
! <T_B_DAT>: DB B_DAT
! OPC    : 1-ATGRIE� TEKSTU SDAT-BDAT MM MENESI
!          2-GL:DB_B_DAT
!          3-T_GD(DATUMS) VAI S/B_DAT JA AT��IR�S
!          4-M�NE�U SKAITS �AJ� FIN_G
!          5-S_DAT VAI DB_S_DAT
!          6-T_GD(DATUMS) VAI DB_B_DAT JA LIEL�KS
!
 IF INRANGE(OPC,1,6)
    IF T_GD=DB_GADS OR ~T_GD OR OPC=3 OR OPC=6
       IF ~T_S_DAT THEN T_S_DAT=DB_S_DAT.
       IF ~T_B_DAT THEN T_B_DAT=DB_B_DAT.
       MENESI=AGE(T_S_DAT,T_B_DAT+1) !VI�� R��INA, NEIESKAITOT
       DB_MENESI=MENESI[1:2]         !LOCAL
       TEXT='FG: '&FORMAT(T_S_DAT,@D06.)&'-'&FORMAT(T_B_DAT,@D06.)&' '&DB_MENESI&' m�ne�i'
    ELSE
       DB_MENESI=12
       TEXT=''
    .
    EXECUTE OPC
       ...
       BEGIN               !3
          IF INRANGE(T_GD,T_S_DAT,T_B_DAT)
             RETURN(T_GD)
          ELSIF T_GD > T_B_DAT
             KLUDA(0,'DB paredz�ta dokumentiem '&FORMAT(T_S_DAT,@D06.)&'-'&FORMAT(T_B_DAT,@D06.)&|
             '.Prasiet sys_admin. izveidot jaunu DB...',,1)
             RETURN(T_B_DAT)
          ELSE ! T_GD < T_S_DAT
             KLUDA(0,'DB paredz�ta dokumentiem '&FORMAT(T_S_DAT,@D06.)&'-'&FORMAT(T_B_DAT,@D06.),,1)
             RETURN(T_S_DAT)
          .
       .
       RETURN(DB_MENESI)   !4
...
!
 ...
 IF OPC=1 !NO ATLIKUMI(),SELFTESTA
    !ANSIFILENAME=CLIP(LONGPATH())&'\INET\NOM_K.TXT'  %%%%% malformed expression
 ...
 IF ~BYTES(OUTFILEANSI)
    IF CL_NR=1493 !R�GAS B�VMATERI�LI
       OUTA:LINE='Nomenklat�ra'&CHR(9)&'M�rvien�ba'&CHR(9)&'Nosaukums'&CHR(9)&'Cena (5)'&CHR(9)&|
       'P�d�j� iep.cena'&CHR(9)&'Atlikums'
    ELSIF CL_NR=1479 OR| !SANTEKO
          CL_NR=1033 OR| !SAIMNIEKS-SERVISS
          CL_NR=1734     !INSBERGS
       OUTA:LINE='Kods'&CHR(9)&'Nosaukums'&CHR(9)&'NosaukumsA'&CHR(9)&|
       'Nomenklat�ra'&CHR(9)&'Grupa'&CHR(9)&'Gr.nos.'&CHR(9)&'Apak�gr.'&CHR(9)&'Agr.nos.'&CHR(9)&|
       'Cena('&NOKL_CP&')'&CHR(9)&'Atlikums'&CHR(9)&'M�rvien�ba'
    ELSIF CL_NR=1464 OR|  !AUTO �LE
          CL_NR=1102      !AD REM
!          ACC_KODS_N=0    !ASSAKO
       OUTA:LINE='Ra�ot�ja kods'&CHR(9)&'Nomenklat�ra'&CHR(9)&'Nosaukums'&CHR(9)&'Cena('&NOKL_CP&')'&|
 ...
 IF (NOM:REDZAMIBA=0 OR NOM:REDZAMIBA=2 OR NOM:REDZAMIBA=3) AND| !AKT�VAS & N�KOTNES & LIKVID�JAMAS
 ~CYCLENOM(NOM:NOMENKLAT)
    IF CL_NR=1493         !R�GAS B�VMATERI�LI
       ATLIKUMS=GETNOM_A(NOM_NOMENKLAT,5,0) !5-ATLIKUMS KOP�
