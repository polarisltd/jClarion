...
PrintSkipDetails BOOL,AUTO
  ...
  OPCIJA='0'
!  izzfiltF                     !NO MAINa NEDR�KST SAUKT.... ! %%%%%%%%%% winla262.clw not included 
  IF GlobalResponse=RequestCancelled
     @java-code 'System.out.println("F_BankasReport RequestCancelled");'
     DO PROCEDURERETURN
  .
  ...
  SEND(BANKAS_K,'QUICKSCAN=on')
   @java-code 'System.out.println("F_BankasReport Accept");'
  ...
  ACCEPT
    ...
    OF Event:OpenWindow
      ...
      IF LocalResponse = RequestCancelled
        ...
        CYCLE
      END
      PrintPreviewImage='report001.wmf'
      @java-code 'System.out.println("F_BankasReport opening report "+$.toString());'(PrintPreviewImage)
      OPEN(report)
    ...
    OF Event:Timer
      LOOP RecordsPerCycle TIMES
        ...
        IF ~PrintSkipDetails THEN
          @java-code 'System.out.println("F_BankasReport print detail");'          
          PRINT(RPT:detail)
        END
        LOOP
          DO GetNextRecord
    ...
    END
  END
   @java-code 'System.out.println("F_BankasReport out of screen loop");'

!  IF SEND(BANKAS_K,'QUICKSCAN=off').    ! %%%%%% why not it like this?
  IF LocalResponse = RequestCompleted
    ! F:DBF='W'   ! disable ANSIJOB %%%
    !IF F:DBF='W'   !WMF
       PRINT(RPT:DETAIL1)
       ENDPAGE(report)
       CLOSE(ProgressWindow)
       @java-code 'System.out.println("F_BankasReport progresswindow closed");'
       
!       RP     ! winla050.clw not yet included %%%%%%%%%%%%%%
  ...
  !IF F:DBF='W'   !disable ANSIJOB
  
   @java-load 'org.jclarion.clarion.compile.ext.Print'
   pdfOut = GETPDF( report)  !, STRING Print report to a virtual file as PDF
   @java-code 'System.out.println($.toString());'(pdfOut)
   writePDF(report)
   previewPDF(report) 
     ...
     @java-code 'System.out.println("F_BankasReport closing report");'          
     CLOSE(report)
     
     FREE(PrintPreviewQueue)
     FREE(PrintPreviewQueue1)
  !ELSE
  !   @java-code 'System.out.println("F_BankasReport closing outputansi");'          
  !   CLOSE(OUTFILEANSI)
  !   ANSIFILENAME=''
  !.
  @java-code 'System.out.println("F_BankasReport report closed, Do procedureReturn");'
  DO ProcedureReturn
...
WinResize            WindowResizeType ! %%%%% WinResize
QuickWindow          WINDOW('Update the BANKAS_K File'),AT(,,274,168),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateBANKAS_K'),SYSTEM,GRAY,RESIZE
                       SHEET,AT(5,4,265,143),USE(?CurrentTab),FONT(,9,,FONT:bold)
                         TAB('Bankas rekvizīti'),USE(?Tab:1)
                           ...
                           PROMPT('Korespondējošais konts:'),AT(10,66,83,10),USE(?BAN:KOR_K:Prompt)
                           ...
                           PROMPT('Speciāls teksts:'),AT(9,111),USE(?BAN:ADRESE3:Prompt)
                           ENTRY(@s31),AT(62,111,128,10),USE(BAN:SPEC)
                           PROMPT('&EL Maksājuma taka:'),AT(9,125,72,10),USE(?BAN:MAKSAJUMA_TAKA:Prompt)
  ...
  END
  DO PrepareProcedure
  ActionMessage='Aplūkošanas režīms'   !ja ir localrequest, sistēma pati pārrakstīs pāri
  CASE LocalRequest
  OF InsertRecord
    ActionMessage = 'Ieraksts tiks pievienots'
  OF ChangeRecord
    ActionMessage = 'Ieraksts tiks mainīts'
  OF DeleteRecord
  END
  ...
  ACCEPT
    ...
    OF EVENT:OpenWindow
      ...
      IF LOCALREQUEST=3
         ...
         disable(1,?cancel-2)
         enable(?Tab:1)  !ATV�RT CURRENTTAB NEDR�KST
         SELECT(?cancel)
...
PrepareProcedure ROUTINE
  ...
  IF LocalRequest = InsertRecord
    ...
    !  IF NOM:NOMENKLAT='' AND LOCALREQUEST=3  !NESAPROTAMA CLARA K��DA
    !     LocalResponse = RequestCancelled
    !     DO PROCEDURERETURN
    !  .
    
      IF GNET !GLOB�LAIS T�KLS
         CASE LOCALREQUEST
         OF 1
            BAN:GNET_FLAG[1]=1
            BAN:GNET_FLAG[2]=''
         OF 2
            IF BAN:GNET_FLAG[1]=1     !JA ST�V ADD1 (NEKUR CITUR V�L NAV IERAKST�TS)
            ELSIF BAN:GNET_FLAG[1]=4  !JA ST�V ADD4 (NE VISUR IR IERAKST�TS)
               KLUDA(121,'I/O, kam�r nav pabeigta t�klu sinhroniz�cija')
               LOCALREQUEST=0
            ELSIF BAN:GNET_FLAG[1]=2  !JA ST�V PUT2 (NEKUR CITUR V�L NAV NOMAIN�TS)
            ELSIF BAN:GNET_FLAG[1]=5  !JA ST�V PUT5 (NE VISUR IR NOMAIN�TS)
               BAN:GNET_FLAG[1]=2
               BAN:GNET_FLAG[2]=''    !MAINAM VISUR
            ELSIF BAN:GNET_FLAG[1]=3 OR BAN:GNET_FLAG[1]=6  !JA ST�V DEL 
               KLUDA(121,'I/O, kam�r nav pabeigta t�klu sinhroniz�cija')
               LOCALREQUEST=0
            ELSE
               BAN:GNET_FLAG[1]=2
               BAN:GNET_FLAG[2]=''
            .
            SAV::BAN:Record = BAN:Record ! LAI NERAKSTA, JA NEKAS NAV MAIN�TS
         OF 3
    !        STOP(BAN:GNET_FLAG)
            IF BAN:GNET_FLAG[1]=1     !JA ST�V ADD1 (NEKUR CITUR V�L NAV IERAKST�TS)
               BAN:GNET_FLAG=''
            ELSIF BAN:GNET_FLAG[1]=4  !JA ST�V ADD4 (NE VISUR IR IERAKST�TS)
               KLUDA(121,'dz�st, kam�r nav pabeigta t�klu sinhroniz�cija')
               LOCALREQUEST=0
            ELSIF BAN:GNET_FLAG[1]=2  !JA ST�V PUT2 (NEKUR CITUR V�L NAV NOMAIN�TS)
               BAN:GNET_FLAG[1]=3
               LOCALREQUEST=4
            ELSIF BAN:GNET_FLAG[1]=5  !JA ST�V PUT5 (NE VISUR IR NOMAIN�TS)
               BAN:GNET_FLAG[1]=3     !DZ��AM VISUR
               BAN:GNET_FLAG[2]=''
               LOCALREQUEST=4
            ELSIF BAN:GNET_FLAG[1]=3 OR BAN:GNET_FLAG[1]=6  !JA ST�V DEL
               KLUDA(121,'I/O, kam�r nav pabeigta t�klu sinhroniz�cija')
...
!|
  IF FilesOpened
    IF ~(BAN:KODS=OLD_KODS) AND ~(OLD_KODS='') AND LOCALREQUEST=2 !Main�ts bankas kods
       KLUDA(109,'')
       IF klu_darbiba   !nav atteikuma
          ...
          OPEN(ProgressWindow)
          Progress:Thermometer = 0
          ?Progress:PctText{Prop:Text} = '0% Izpild�ti'
    ...
    !           STOP('SAPRATA'&BAN:KODS)
             ...
             .
          .
          reset(par_k,par_position)  !noliekam viet�
          next(par_k)
...
BRW1::Sort1:KeyDistribution STRING(4),DIM(100)   ! %%%%%% did not recognize DIM and LIKE at same time, LIKE(BAN:NOS_A)->STRING(4)
...
BRW1::Sort2:KeyDistribution STRING(15),DIM(100) ! %%%%%% did not recognize DIM and LIKE at same time, LIKE(BAN:KODS)->STRING(15)
...
QuickWindow          WINDOW('Browse the BANKAS_K File'),AT(,,268,238),FONT('MS Sans Serif',9,,FONT:bold),IMM,VSCROLL,HLP('BrowseBANKAS_K'),SYSTEM,GRAY,RESIZE
                       LIST,AT(5,20,257,178),USE(?Browse:1),IMM,VSCROLL,MSG('Browsing Records'),FORMAT('49L|M~Bankas kods~L(2)@s11@64L|M~Saīsinātais nosaukums~L(1)@s15@126L|M~Pilnais n' &|
   'osaukums~L(2)@s31@'),FROM(Queue:Browse:1)
                       BUTTON('&Ievadīt'),AT(110,201,45,14),USE(?Insert:2)
                       BUTTON('&Mainīt'),AT(158,201,45,14),USE(?Change),DEFAULT
                       BUTTON('&Dzēst'),AT(207,201,45,14),USE(?Delete:2)
                       BUTTON('&C-Kopēt'),AT(111,222,45,14),USE(?kopet)
                       SHEET,AT(2,4,263,215),USE(?CurrentTab)
                         TAB('&Kodu secība'),USE(?Tab:3)
                           ENTRY(@s11),AT(7,201),USE(BAN:KODS),MSG('Bankas kods')
                           STRING('-pēc koda'),AT(63,202),USE(?String2)
                         END
                         TAB('&Nosaukumu sec�ba'),USE(?Tab:2)
                           ENTRY(@s4),AT(7,202),USE(BAN:NOS_A)
                           STRING('-pēc nosaukuma'),AT(36,203),USE(?String1)
                         END
                       END
                       BUTTON('Iz&vēlēties'),AT(159,222),USE(?Select),FONT(,,COLOR:Navy,,CHARSET:ANSI),KEY(EnterKey)
  ...
  END
  DO PrepareProcedure
  ! ?BROWSE:1{PROP:FORMAT}=GETINI('BrowseBANKAS_K','?BROWSE:1 Format',?BROWSE:1{PROP:FORMAT},'WinLats.ini') ! %%%% dont restore format!
...
!|
  IF FilesOpened
    ...
    IF BANKAS_K::Used = 0 THEN CLOSE(BANKAS_K).
  END
  if ?BROWSE:1{PROP:FORMAT} <> ''    ! %%%% write INI entry only if format is not blank, otherwise grid stops working! 
     PUTINI('BrowseBANKAS_K','?BROWSE:1 Format',?BROWSE:1{PROP:FORMAT},'WinLats.ini') ! %%%%% moved line inside if
  .                                                       
  IF WindowOpened
...
!|
  @java-code 'System.out.println("BRW1::ScrollOne ======== ");'
...
!|
  @java-code 'System.out.println("BRW1::ScrollPage ======== ");'
...
!|
  @java-code 'System.out.println("BRW1::ScrollEnd ======== ");'
  FREE(Queue:Browse:1)
  BRW1::RecordCount = 0
  DO BRW1::Reset
  BRW1::ItemsToFill = ?Browse:1{Prop:Items}
...
!|
  @java-code 'System.out.println("BRW1::FillRecord ======== ");'    !zzz
  IF BRW1::RecordCount
    ...
    END
    @java-code 'System.out.println("BRW1::FillRecord RESET "+$.toString());'(BRW1::Position)
  ...
  LOOP WHILE BRW1::ItemsToFill
    IF BRW1::FillDirection = FillForward
      @java-code 'System.out.println("BRW1::FillRecord NEXT ");'
      NEXT(BRW1::View:Browse)
    ELSE
      @java-code 'System.out.println("BRW1::FillRecord PREVIOUS ");'
      PREVIOUS(BRW1::View:Browse)
    END
    IF ERRORCODE()
      @java-code 'System.out.println("BRW1::FillRecord Errorcode "+$);'(ERRORCODE())
      IF ERRORCODE() = BadRecErr
        DO BRW1::RestoreResetValues
        BREAK
      ELSE
    ...
    IF BRW1::AddQueue
      IF BRW1::RecordCount = ?Browse:1{Prop:Items}
        ...
        ELSE
          GET(Queue:Browse:1,BRW1::RecordCount)   ! Get the first queue item
        END
        @java-code 'System.out.println("BRW1::FillRecord DELETE Queue:Browse ");'
        DELETE(Queue:Browse:1)
        BRW1::RecordCount -= 1
      END
      DO BRW1::FillQueue
      @java-code 'System.out.println("BRW1::FillRecord ADD Queue:Browse ");'
      IF BRW1::FillDirection = FillForward
        ADD(Queue:Browse:1)
      ELSE
...
!|
  @java-code 'System.out.println("BRW1::LocateRecord ======== ");'  
...
!|
  @java-code 'System.out.println("BRW1::RefreshPage ======== ");'  
  SETCURSOR(Cursor:Wait)
  ...
  END
  DO BRW1::FillRecord 
  SETCURSOR() ! %%%%% TEMP_ADDON                            ! Fill with next read(s)
  EXIT        ! %%%%% TEMP_ADDON quit refreshpage after Fillrecord as queue is populated.
