...
BRW1::View:Browse    VIEW(NOM_P)
                       PROJECT(NOP:NOMENKLAT)
                       PROJECT(NOP:NOL_NR)
                       PROJECT(NOP:PLAUKTS)
                       PROJECT(NOP:KOMENTARS)
                     END

Queue:Browse:1       QUEUE,PRE()                  ! Browsing Queue
BRW1::NOP:NOMENKLAT    LIKE(NOP:NOMENKLAT)        ! Queue Display field
BRW1::NOP:NOL_NR       LIKE(NOP:NOL_NR)           ! Queue Display field
BRW1::NOP:PLAUKTS      LIKE(NOP:PLAUKTS)          ! Queue Display field
BRW1::NOP:KOMENTARS    LIKE(NOP:KOMENTARS)        ! Queue Display field
...
BRW1::Sort1:KeyDistribution LIKE(NOP:NOMENKLAT),DIM(100)
BRW1::Sort1:LowValue LIKE(NOP:NOMENKLAT)          ! Queue position of scroll thumb
BRW1::Sort1:HighValue LIKE(NOP:NOMENKLAT)         ! Queue position of scroll thumb
...
QuickWindow          WINDOW('Browse the NOM_P File'),AT(,,315,220),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseNom_P'),SYSTEM,GRAY,RESIZE,MDI
                       LIST,AT(8,20,289,168),USE(?Browse:1),IMM,HVSCROLL,MSG('Browsing Records'),FORMAT('80L(2)|M~NOMENKLAT~@S21@12L(2)|M~N~C(0)@n2@64L(2)|M~Plaukts~@S15@80L(2)|M~Koment' &|
   'ârs~@S25@'),FROM(Queue:Browse:1)
                       BUTTON('&Ievadît'),AT(112,198,45,14),USE(?Insert:2)
                       BUTTON('&Mainît'),AT(161,198,45,14),USE(?Change:2),DEFAULT
                       BUTTON('&Dzçst'),AT(210,198,45,14),USE(?Delete:2),HIDE
                       SHEET,AT(4,4,299,190),USE(?CurrentTab)
                         TAB('Atraðanâs vieta'),USE(?Tab:2)
                         END
                       END
                       BUTTON('Beigt'),AT(258,199,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('BrowseNom_P','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('BrowseNom_P','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
...
!|
  ...
  IF ERRORCODE()
    ...
    ELSE
      StandardWarning(Warn:RecordFetchError,'NOM_P')
      POST(Event:CloseWindow)
    END
    EXIT
  END
  CASE BRW1::SortOrder
  OF 1
    BRW1::Sort1:HighValue = NOP:NOMENKLAT
  END
  DO BRW1::Reset
  ...
  CASE BRW1::SortOrder
  OF 1
    BRW1::Sort1:LowValue = NOP:NOMENKLAT
...
!|
  NOP:NOMENKLAT = BRW1::NOP:NOMENKLAT
  NOP:NOL_NR = BRW1::NOP:NOL_NR
  NOP:PLAUKTS = BRW1::NOP:PLAUKTS
  NOP:KOMENTARS = BRW1::NOP:KOMENTARS
!----------------------------------------------------------------------
...
!|
  BRW1::NOP:NOMENKLAT = NOP:NOMENKLAT
  BRW1::NOP:NOL_NR = NOP:NOL_NR
  BRW1::NOP:PLAUKTS = NOP:PLAUKTS
  BRW1::NOP:KOMENTARS = NOP:KOMENTARS
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 1
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort1:KeyDistribution[BRW1::CurrentScroll] => UPPER(NOP:NOMENKLAT)
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 1
      NOP:NOMENKLAT = BRW1::Sort1:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(NOP:NOM_KEY)
      RESET(NOP:NOM_KEY,BRW1::HighlightedPosition)
    ELSE
      NOP:NOMENKLAT = NOMENKLAT
      SET(NOP:NOM_KEY,NOP:NOM_KEY)
    END
    BRW1::View:Browse{Prop:Filter} = |
    'UPPER(NOP:NOMENKLAT) = UPPER(NOMENKLAT)'
  END
  IF ERRORCODE()
...
!|
  ...
  CASE BRW1::SortOrder
  OF 1
    NOP:NOMENKLAT = BRW1::Sort1:Reset:NOMENKLAT
  END
...
UpdateNOM_C          PROCEDURE                    ! Declare Procedure
CurrentTab           STRING(80)
LocalRequest         LONG
OriginalRequest      LONG
LocalResponse        LONG
...
SAV::NOC:Record      LIKE(NOC:Record)
ToolBarMode     UNSIGNED,AUTO
...
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the NOM_C File'),AT(,,309,107),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateNOM_C'),SYSTEM,GRAY,RESIZE,MDI
                       SHEET,AT(4,4,299,78),USE(?CurrentTab)
                         TAB('General'),USE(?Tab:1)
                           STRING(@s21),AT(8,21),USE(NOC:NOMENKLAT)
                           PROMPT('Nosaukums &Krieviski:'),AT(8,34,71,10),USE(?NOC:NOSAUKUMS_C:Prompt)
                           ENTRY(@S50),AT(84,34,204,10),USE(NOC:NOSAUKUMS_C),font(,,,,CHARSET:CYRILLIC)
                           STRING('Rakstot krieviski, jâpârslçdz klaviatûra uz Ru !!!!!'),AT(10,46),FONT(,,0808080H,),USE(?String2)
                           PROMPT('&Nosaukums Angïiski:'),AT(8,58,70,10),USE(?NOC:NOSAUKUMS_A:Prompt)
                           ENTRY(@S50),AT(84,58,204,10),USE(NOC:NOSAUKUMS_A)
                         END
                       END
                       BUTTON('&OK'),AT(209,86,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(257,86,45,14),USE(?Cancel)
                       STRING(@S8),AT(5,89),FONT(,,0808080H,),USE(NOC:ACC_KODS)
                     END
  ...
  BIND(NOC:RECORD)
  FilesOpened = True
  IF ~GETNOM_VALODA(NOC:NOMENKLAT,4)
  ...
  IF LOCALREQUEST=INSERTRECORD
     NOC:NOMENKLAT=NOM:NOMENKLAT
  .
  CASE LocalRequest
  OF InsertRecord
    ActionMessage = 'Adding a NOM_C Record'
  OF ChangeRecord
    ActionMessage = 'Changing a NOM_C Record'
  OF DeleteRecord
  END
  ...
  ACCEPT
    ...
    OF EVENT:CloseDown
      ...
      WinResize.Destroy
    OF EVENT:OpenWindow
      ?NOC:NOSAUKUMS_C{PROP:font,1}='ARIAL CYR'
      IF NOT WindowInitialized
        DO InitializeWindow
        WindowInitialized = True
      END
      SELECT(?NOC:NOMENKLAT)
    OF EVENT:GainFocus
      ForceRefresh = True
      IF NOT WindowInitialized
        DO InitializeWindow
      ...
      IF EVENT() = Event:Completed
        ...
        OF ChangeRecord
          LOOP
            ...
            SETCURSOR(Cursor:Wait)
            RecordChanged=false
            IF SAV::NOC:Record <> NOC:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:NOM_C(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
...
PrepareProcedure ROUTINE
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Spread)
  INIRestoreWindow('UpdateNOM_C','WINLATS.INI')
  WinResize.Resize
  ?NOC:NOMENKLAT{PROP:Alrt,255} = 734
  ?NOC:NOSAUKUMS_C{PROP:Alrt,255} = 734
  ?NOC:NOSAUKUMS_A{PROP:Alrt,255} = 734
  ?NOC:ACC_KODS{PROP:Alrt,255} = 734
...
!|
  IF FilesOpened
    NOM_C::Used -= 1
    IF NOM_C::Used = 0 THEN CLOSE(NOM_C).
  END
  IF WindowOpened
    INISaveWindow('UpdateNOM_C','WINLATS.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?NOC:NOMENKLAT
      NOC:NOMENKLAT = History::NOC:Record.NOMENKLAT
    OF ?NOC:NOSAUKUMS_C
      NOC:NOSAUKUMS_C = History::NOC:Record.NOSAUKUMS_C
    OF ?NOC:NOSAUKUMS_A
      NOC:NOSAUKUMS_A = History::NOC:Record.NOSAUKUMS_A
    OF ?NOC:ACC_KODS
      NOC:ACC_KODS = History::NOC:Record.ACC_KODS
  END
  DISPLAY()
...
!|
  NOC:Record = SAV::NOC:Record
  SAV::NOC:Record = NOC:Record
UpdateCrossref PROCEDURE
...
Update::Reloop  BYTE
Update::Error   BYTE
History::CRO:Record LIKE(CRO:Record),STATIC
SAV::CRO:Record      LIKE(CRO:Record)
ToolBarMode     UNSIGNED,AUTO
WindowXPos      SIGNED,AUTO,STATIC
WindowYPos      SIGNED,AUTO,STATIC
WindowPosInit   BOOL(False),STATIC
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the CROSSREF File'),AT(,,198,126),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateCrossref'),SYSTEM,GRAY,RESIZE,MDI
                       SHEET,AT(4,4,187,100),USE(?CurrentTab)
                         TAB('General'),USE(?Tab:1)
                           PROMPT('&Kataloga Nr:'),AT(8,31,48,10),USE(?CRO:KATALOGA_NR:Prompt)
                           ENTRY(@S21),AT(81,31,88,10),USE(CRO:KATALOGA_NR)
                           PROMPT('&Raþotâja kods:'),AT(8,44),USE(?CRO:RAZ_K:Prompt)
                           ENTRY(@S3),AT(81,44,40,10),USE(CRO:RAZ_K)
                           PROMPT('&Nosaukums:'),AT(8,58),USE(?CRO:NOS_S:Prompt)
                           ENTRY(@S16),AT(81,58,68,10),USE(CRO:NOS_S)
                           PROMPT('&Nomenklatûra:'),AT(8,72),USE(?CRO:NOMENKLAT:Prompt)
                           ENTRY(@S21),AT(81,72,88,10),USE(CRO:NOMENKLAT)
                         END
                       END
                       BUTTON('&OK'),AT(94,108,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(143,108,45,14),USE(?Cancel)
                       STRING(@S8),AT(5,111),USE(CRO:ACC_KODS),FONT(,,COLOR:Gray,)
                       STRING(@D6),AT(39,111),USE(CRO:ACC_DATUMS),FONT(,,COLOR:Gray,)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
      ...
      IF EVENT() = Event:Completed
        ...
        OF ChangeRecord
          LOOP
            ...
            SETCURSOR(Cursor:Wait)
            RecordChanged=false
            IF SAV::CRO:Record <> CRO:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:CROSSREF(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
...
PrepareProcedure ROUTINE
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Spread)
  INIRestoreWindow('UpdateCrossref','winlats.INI')
  WinResize.Resize
  ?CRO:KATALOGA_NR{PROP:Alrt,255} = 734
  ?CRO:RAZ_K{PROP:Alrt,255} = 734
  ?CRO:NOS_S{PROP:Alrt,255} = 734
  ?CRO:NOMENKLAT{PROP:Alrt,255} = 734
  ?CRO:ACC_KODS{PROP:Alrt,255} = 734
  ?CRO:ACC_DATUMS{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
ProcedureReturn ROUTINE
!|
!| This routine provides a common procedure exit point for all template
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('UpdateCrossref','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?CRO:KATALOGA_NR
      CRO:KATALOGA_NR = History::CRO:Record.KATALOGA_NR
    OF ?CRO:RAZ_K
      CRO:RAZ_K = History::CRO:Record.RAZ_K
    OF ?CRO:NOS_S
      CRO:NOS_S = History::CRO:Record.NOS_S
    OF ?CRO:NOMENKLAT
      CRO:NOMENKLAT = History::CRO:Record.NOMENKLAT
    OF ?CRO:ACC_KODS
      CRO:ACC_KODS = History::CRO:Record.ACC_KODS
    OF ?CRO:ACC_DATUMS
      CRO:ACC_DATUMS = History::CRO:Record.ACC_DATUMS
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
