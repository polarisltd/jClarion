...
GETVAL_K             FUNCTION (VALVAL,REQ)        ! Declare Procedure
 ...
 !
 !  VALVAL - PIEPRAS�T� VAL�TA
 !  REQ - 0 ATGRIE� TUK�UMU UN NOT�RA RECORD
 !        1 IZSAUC BROWSE
 !        2 IZSAUC K��DU
 !
 IF VALVAL='' AND ~REQ   !SPEC
      @java-code 'System.out.println("GETVAL_K empty args_1");'
    RETURN('')
 ELSIF VALVAL OR REQ
    IF VAL_K::Used = 0
       CheckOpen(VAL_K,1)
    ...
    IF ERRORCODE()<>0
       @java-code 'System.out.println("GETVAL_K error_1 "+$+$.toString());'(ERRORCODE(),ERROR())   ! %%%%%%% cool stuff
        ! USE MESSAGE TO REPORT ERRORS IN GUI WINDOW    message(errorcode()) ! %%%%%%%%%%
        !      message(error())
       IF REQ = 2
          KLUDA(14,VALVAL)
          CLEAR(VAL:RECORD)
 ...
 ELSE
      @java-code 'System.out.println("GETVAL_K empty args_2");'   
    RETURN('')
 .
 @java-code 'System.out.println("GETVAL_K normal return "+$.toString());'(VAL:VAL)
 RETURN(VAL:VAL)
...
!             STOP(FILENAME2)
             ! RENAME(FILENAME1,FILENAME2)  ! %%%%% disabled functionality
             IF ERROR()
                RETURN(1)
             .
          .
...
GetBankas_k          FUNCTION (ban_kods,REQ,RET)  ! Declare Procedure
 ...
 !        1 IZSAUC BROWSE
 !        2 IZSAUC K��DU
 !  RET - 1 ATGRIE� BAN:NOS_P
 !        2 ATGRIE� MAKSAJUMA_TAKA
 !
