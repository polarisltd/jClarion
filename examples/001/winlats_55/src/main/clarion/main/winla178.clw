...
report REPORT,AT(198,1750,8000,9396),PAPER(PAPER:A4),PRE(RPT),FONT('Arial Baltic',10,,,CHARSET:BALTIC), |
       ...
       HEADER,AT(198,198,8000,1552),USE(?unnamed:2)
         ...
         STRING('Pre�u summa'),AT(3792,1094,938,208),USE(?String11:7),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('Pre�u summa'),AT(4781,1094,1250,208),USE(?String11:9),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
...
detail DETAIL,AT(,,,177),USE(?unnamed:3)
         ...
         LINE,AT(677,-10,0,198),USE(?Line8:2),COLOR(COLOR:Black)
           STRING(@s50),AT(729,0,2937,156),USE(NOS_P,,?NOS_P:2),FONT(,8,,,CHARSET:BALTIC)
...
RPT_FOOT3 DETAIL,AT(,,,260),USE(?unnamed)
         ...
         STRING('Sast�d�ja :'),AT(260,83,479,156),USE(?String32),FONT(,7,,,CHARSET:BALTIC)
...
!** Pirc�ju atskaite par visiem
  ...
  ACCEPT
    ...
    OF Event:OpenWindow
      ...
      IF D_K='K'
         VIRSRAKSTS='Atskaite par liel�kajiem pre�u sa��m�jiem '&FORMAT(S_DAT,@D06.)&'-'&FORMAT(B_DAT,@D06.)
         FORMA='FORMA IZ8'
         TAB_2_TEXT='Sa��m�js'
      ELSE
         VIRSRAKSTS='Atskaite par liel�kajiem pre�u pieg�d�t�jiem '&FORMAT(S_DAT,@D06.)&'-'&FORMAT(B_DAT,@D06.)
         FORMA='FORMA IE7'
         TAB_2_TEXT='Pieg�d�t�js'
      .
      IF F:LIELMAZ='M' !NOR�DIET MAZ�KO S, KAS....
         !18/12/2013 FILTRS_TEXT = 'Summas, kas >= Ls '&CLIP(MINMAXSUMMA)
         FILTRS_TEXT = 'Summas, kas >= '&val_uzsk&' '&CLIP(MINMAXSUMMA)
      ELSE
         IF F:IEKLAUTPK
            !18/12/2013 FILTRS_TEXT = 'Summas, kas <= Ls '&CLIP(MINMAXSUMMA)&' ar�,kas nav Noliktav�'
            FILTRS_TEXT = 'Summas, kas <= '&val_uzsk&' '&CLIP(MINMAXSUMMA)&' ar�,kas nav Noliktav�'
         ELSE
            !18/12/2013 FILTRS_TEXT = 'Summas, kas <= Ls '&CLIP(MINMAXSUMMA)&' tikai,kas ir Noliktav�'
            FILTRS_TEXT = 'Summas, kas <= '&val_uzsk&' '&CLIP(MINMAXSUMMA)&' tikai,kas ir Noliktav�'
         .
      .
      IF PAR_NR = 999999999 !VISI
         FILTRS_TEXT=CLIP(FILTRS_TEXT)&' '&GETFILTRS_TEXT('101100000') !1-OBJ,2-NOD,3-PART,4-PARG,5-NOM,6-NOMT,7-DN,8-(1:par�di),9-ID
!                                                          123456789
      ELSE                  !KONKR�TS
         FILTRS_TEXT=CLIP(FILTRS_TEXT)&' '&GETFILTRS_TEXT('100000000') !1-OBJ,2-NOD,3-PART,4-PARG,5-NOM,6-NOMT,7-DN,8-(1:par�di),9-ID
!                                                          123456789
...
!          123456
      ...
      ELSE !WORD/EXCEL
        ...
        ADD(OUTFILEANSI)
        IF F:dbf='E'
           OUTA:LINE='NPK'&CHR(9)&TAB_2_TEXT&CHR(9)&'Pre�u summa'&CHR(9)&'Pre�u summa'&CHR(9)&CHR(9)&'t.s. Atlaide'
           ADD(OUTFILEANSI)
           OUTA:LINE=CHR(9)&CHR(9)&'bez PVN'&CHR(9)&'ar PVN'
           ADD(OUTFILEANSI)
        ELSE   !WORD
           OUTA:LINE='NPK'&CHR(9)&TAB_2_TEXT&CHR(9)&'Pre�u summa bez PVN'&CHR(9)&'Pre�u summa ar PVN'&CHR(9)&CHR(9)&|
           't.s. Atlaide'
           ADD(OUTFILEANSI)
        .
      .
    OF Event:Timer
      LOOP RecordsPerCycle TIMES
        ...
        LOOP
          ...
          IF NOL:U_NR=1 THEN CYCLE.    !SALDO IGNOR�JAM
...
!        NOS_P=DOK_SENR
!     .
     IF RECORDS(P_TABLE) !DA��DAS VAL�TAS 1 PARTNERIM
...
report REPORT,AT(198,1698,8000,9396),PAPER(PAPER:A4),PRE(RPT),FONT('Arial Baltic',10,,,CHARSET:BALTIC), |
       ...
       HEADER,AT(198,302,8000,1396),USE(?unnamed:2)
         ...
         STRING('ar PVN, val�t�'),AT(6594,1146,1094,208),USE(?String18:8),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
...
RPT_FOOT3 DETAIL,AT(,,,240),USE(?unnamed)
         ...
         STRING('Sast�d�ja :'),AT(167,73),USE(?String42),FONT(,7,,,CHARSET:BALTIC)
...
!
  ...
  Progress:Thermometer = 0
  ?Progress:PctText{Prop:Text} = '0%'
  ProgressWindow{Prop:Text} = 'Izg�ju��s preces, S_PAR'
  ?Progress:UserString{Prop:Text}=''
  ...
  ACCEPT
    ...
    OF Event:OpenWindow
      ...
      VIRSRAKSTS='Izzi�a par izg�ju�aj�m ('&D_K&') prec�m (S_Pirc�jiem) no '&format(S_DAT,@d06.)&' l�dz '&|
      format(B_DAT,@d06.)&' Noliktava: '&clip(loc_nr)&' '&SYS:AVOTS
      IF PAR_NR = 999999999 !VISI
         FILTRS_TEXT=GETFILTRS_TEXT('101110000') !1-OBJ,2-NOD,3-PART,4-PARG,5-NOM,6-NOMT,7-DN,8-(1:par�di),9-ID
!                                    123456789
       ELSE   !KONKR�TS
         FILTRS_TEXT=GETFILTRS_TEXT('100010000') !1-OBJ,2-NOD,3-PART,4-PARG,5-NOM,6-NOMT,7-DN,8-(1:par�di),9-ID
!                                    123456789
         FILTRS_TEXT=CLIP(FILTRS_TEXT)&' Sa��m�js: '&GETPAR_K(PAR_NR,2,2)
      .
...
!             CHR(9)&'Summa ar'
             ...
             CHR(9)&'Summa ar'
             ADD(OUTFILEANSI)
!             OUTA:LINE=CHR(9)&CHR(9)&CHR(9)&CHR(9)&'PVN, Ls'&CHR(9)&CHR(9)&'PVN, val�t�'
             OUTA:LINE=CHR(9)&CHR(9)&CHR(9)&CHR(9)&'PVN, '&val_uzsk&CHR(9)&CHR(9)&'PVN, val�t�'
             ADD(OUTFILEANSI)
          ELSE
!             OUTA:LINE='Npk'&CHR(9)&'U_Nr'&CHR(9)&'Pilns nosaukums'&CHR(9)&'Daudzums'&CHR(9)&'Summa bez PVN, Ls'&CHR(9)&|
!             'PVN,Ls'&CHR(9)&'Summa ar PVN, val�t�'
             OUTA:LINE='Npk'&CHR(9)&'U_Nr'&CHR(9)&'Pilns nosaukums'&CHR(9)&'Daudzums'&CHR(9)&'Summa bez PVN, '&val_uzsk&CHR(9)&|
             'PVN,'&val_uzsk&CHR(9)&'Summa ar PVN, val�t�'
             ADD(OUTFILEANSI)
          .
      .
    OF Event:Timer
...
report REPORT,AT(400,1375,12000,6500),PAPER(PAPER:A4),PRE(RPT),FONT('Arial Baltic',8,,,CHARSET:BALTIC), |
       ...
       HEADER,AT(400,198,12000,1177),USE(?unnamed)
         ...
         STRING('Nomenklat�ra'),AT(135,729,1615,208),USE(?String4:5),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         ...
         STRING('V�rt�ba'),AT(9688,729,1094,208),USE(?String4:3),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
...
RPT_FOOT3 DETAIL,AT(,,12000,271),USE(?unnamed:4)
         ...
         STRING('Sast�d�ja :'),AT(156,83),USE(?String30),LEFT,FONT(,7,,,CHARSET:BALTIC)
...
PrintSkipDetails BOOL,AUTO
  CODE                                            ! Begin processed code
! TIEK SAUKTS NO N_IZZI��M UN FAILIEM UN MultiN_IZZI��M
!
  PUSHBIND
  ...
  Progress:Thermometer = 0
  ?Progress:PctText{Prop:Text} = '0%'
  ProgressWindow{Prop:Text} = 'Pre�u atlikumi noliktav�'
  ?Progress:UserString{Prop:Text}=''
  ...
  ACCEPT
    ...
    OF Event:OpenWindow
      ...
      IF F:NOA      !sauc no Failiem vai M-atskait�m
         B_DAT=TODAY()
         Stringplaukts='Noliktava'
         FORMAPN1='FORMA FPN1'
         VIRSRAKSTS='Pre�u atlikumi uz '&FORMAT(B_DAT,@D06.)&' visas noliktavas'
     ELSE          !SAUC NO NOL_IZZI��M
         ...
         .
         FORMAPN1='FORMA PN1'
         VIRSRAKSTS='Pre�u atlikumi uz '&FORMAT(B_DAT,@D06.)&' Noliktava: '&clip(loc_nr)&' '&SYS:AVOTS
      .
      FILTRS_TEXT=GETFILTRS_TEXT('000011000') !1-OBJ,2-NOD,3-PART,4-PARG,5-NOM,6-NOMT,7-DN,8-(1:par�di),9-ID
!                                 123456789
      IF F:ATL THEN FILTRS_TEXT=CLIP(FILTRS_TEXT)&' P�c atlikumiem: '.
      ...
      ELSE           !WORD,EXCEL
        ...
        ADD(OUTFILEANSI)
        IF F:DBF='E'
           OUTA:LINE='Nomenklat�ra'&CHR(9)&'Kataloga Numurs'&CHR(9)&'Kods'&CHR(9)&'Nosaukums'&CHR(9)&'Atlikums'&CHR(9)&|
           'K-Projekts'&CHR(9)&'Cena('&nokl_cp&')'&CHR(9)&'Cena('&nokl_cp&')'&CHR(9)&'V�rt�ba ar PVN'&CHR(9)&|
           CHR(9)&'Plaukts'
           ADD(OUTFILEANSI)
        ...
        ELSE   !WORD
           OUTA:LINE='Nomenklat�ra'&CHR(9)&'Kataloga Numurs'&CHR(9)&'Kods'&CHR(9)&'Nosaukums'&CHR(9)&'Atlikums'&CHR(9)&|
           'K-Projekts'&CHR(9)&'Cena('&nokl_cp&') bez PVN'&CHR(9)&'Cena('&nokl_cp&') bez PVN'&CHR(9)&'V�rt�ba ar PVN'&|
           CHR(9)&CHR(9)&'Plaukts'
           ADD(OUTFILEANSI)
        .
      .
    OF Event:Timer
      LOOP RecordsPerCycle TIMES
        IF ~CYCLENOM(NOM:NOMENKLAT) AND INSTRING(NOM:TIPS,NOM_TIPS7) 
           ...
           DISPLAY(?Progress:UserString)
!           DAUDZUMS=GETNOM_A(NOM:NOMENKLAT,1,2)     ! P�RR��INA ATLIKUMUS KONKR�T� NOLIKTAV�
!           DAUDZUMS=LOOKATL(2)
           IF F:NOA              !VISAS NOLIKTAVAS NO FAILIEM
              DAUDZUMS=GETNOM_A(NOM:NOMENKLAT,5,0)
              K_PROJEKTS=GETNOM_A(NOM:NOMENKLAT,8,0)   !K-Projekts
           ELSE
!              IF (B_DAT=TODAY() OR B_DAT=DATE(12,31,DB_GADS)) AND RS='A' !NAV J�GAS P�RR��IN�T  16.07.2007.-IR J�GA
!                 DAUDZUMS=GETNOM_A(NOM:NOMENKLAT,1,0)
!                 K_PROJEKTS=GETNOM_A(NOM:NOMENKLAT,4,0)
!                 IF  CL_NR=1033 THEN  K_PROJEKTS=NOA:K_PROJEKTS[1]+NOA:K_PROJEKTS[2].   !��PSALA
!              ELSE
                 DAUDZUMS=LOOKATL(3) !K� F-JA NO RST
                 K_PROJEKTS=SUMMA
!              .
           .
           IF ~(NOM:REDZAMIBA=1 AND DAUDZUMS=0) !~ARH�VS AR ATLIKUMU
              IF (DAUDZUMS>0 AND F:ATL[1]='1') OR (DAUDZUMS=0 AND F:ATL[2]='1') OR (DAUDZUMS<0 AND F:ATL[3]='1')
                ...
                SUMMA_K+=SUMMA*BANKURS(NOM_VAL,B_DAT,' Nomenklat�rai:'&NOM:NOMENKLAT)
                IF ~F:DTK
                   IF F:DBF='W'              !tikai WMF t.s. noliktavas
                      IF F:NOA               !VISAS NOLIKTAVAS
                         VAIRAK_KA_1#=0
                         LOOP N#= 1 TO NOL_SK
                             IF GETNOM_A('POZICION�TS',1,0,N#)
                                VAIRAK_KA_1#+=1
                                FOUND#=N#
                             .
                         .
                         IF VAIRAK_KA_1#>1
                            ...
                            LOOP N#= 1 TO NOL_SK
                                ...
                                IF DAUDZUMSN AND ~(CL_NR=1033)     !��PSALA
