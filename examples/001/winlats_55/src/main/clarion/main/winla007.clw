...
window               WINDOW,AT(,,204,94),FONT('MS Sans Serif',8,,FONT:regular),CENTER,GRAY,NOFRAME
                       ...
                       STRING('Izstr�d�t�js: Inform�cijas servisa firma "ASSAKO"'),AT(13,12,182,10),USE(?String2),CENTER
                       STRING('Kokneses pr. 25, R�ga, LV-1014, 67518058,29234246'),AT(13,22,182,10),USE(?String2:2),CENTER
...
Calculator WINDOW('Kalkul�tors'),AT(,,116,192),FONT('Arial',8,,),CENTER,ICON('G_CALC.ICO'),SYSTEM,GRAY, |
         DOUBLE,AUTO
       ...
       BUTTON('�'),AT(67,56,16,14),FONT('Symbol',8,,),TIP('Square Root'),USE(?Root)
       ...
       STRING('Atmi��:'),AT(4,22,30,10),LEFT
...
QuickWindow          WINDOW('Update the PAROLES File'),AT(,,250,298),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('Upateparoles'),SYSTEM,GRAY,RESIZE
                       SHEET,AT(4,4,241,274),USE(?CurrentTab)
                         TAB('Lietot�ju pieejas l�me�a kontrole '),USE(?Tab:1)
                           BUTTON('Atv�rt U-NR -->'),AT(9,18,56,14),USE(?AUNR)
                           ENTRY(@n_6),AT(69,19,31,11),USE(SEC:U_NR),DISABLE,FONT(,,,FONT:bold),REQ,OVR,UPR
                           PROMPT('&Sl�pt� da�a'),AT(8,33,54,10),USE(?SEC:SECURE:Prompt)
                           ENTRY(@s8),AT(69,32,58,11),USE(SEC:SECURE),FONT(,,,FONT:bold),REQ,OVR,UPR
                           PROMPT('&Publisk� da�a'),AT(8,46,58,10),USE(?SEC:PUBLISH:Prompt)
                           ENTRY(@s8),AT(69,45,58,11),USE(SEC:PUBLISH),FONT(,,,FONT:bold),REQ,OVR,UPR
                           PROMPT('&V�rds Uzv�rds:'),AT(9,59,57,10),USE(?SEC:VUT:Prompt)
                           ...
                           BUTTON('Pieeja B�zei DB:1-15'),AT(9,99,98,14),USE(?base)
                           ...
                           BUTTON('Pieeja Fisk�lai Dr DB:41-65'),AT(9,130,98,14),USE(?FP)
                           ...
                           BUTTON('Aizliegt datu apmai�u un importa interfeisu '),AT(40,218),USE(?AizliegtDatuApmainu)
                           IMAGE('CANCEL4.ICO'),AT(201,234),USE(?ImageA_self),HIDE
                           BUTTON('Aizliegt  selftestu'),AT(94,238,98,14),USE(?AizliegtSelftestu)
                           BUTTON('Aizliegt pieeju neapstiprin�tiem rakstiem'),AT(39,257,153,14),USE(?AizliegtNEAP)
...
screen:files WINDOW('Pieeja failiem'),AT(,,283,145),GRAY
       OPTION('&Sist�mas kop�jie faili :'),AT(12,14,123,117),USE(?option1),BOXED
         RADIO('Partneri, atz�mes,'),AT(18,26),USE(?Option1:Radio1)
         RADIO('Kontu pl�ns'),AT(18,43,51,10),USE(?Option1:Radio2)
         RADIO('Bankas'),AT(18,53,37,10),USE(?option1:Radio3)
         RADIO('Val�tas, val�tu kursi'),AT(18,62,82,10),USE(?option1:Radio4)
         RADIO('Nomenklat�ras, grupas,'),AT(18,71,87,10),USE(?option1:Radio5)
         RADIO('Kadri'),AT(18,99,29,10),USE(?option1:Radio6)
         RADIO('Auto'),AT(18,109,29,10),USE(?option1:Radio7)
       END
       OPTION('&Pieejas l�menis :'),AT(138,17,135,63),USE(access),BOXED
         RADIO('0-Pilna pieeja'),AT(145,29),USE(?access:Radio1),VALUE('0')
         RADIO('1-Aizliegts dz�st'),AT(145,38),USE(?access:Radio2),VALUE('1')
         RADIO('2-Aizliegts dz�st un main�t'),AT(145,47),USE(?access:Radio3),VALUE('2')
         RADIO('3-Aizliegts dz�st,main�t un ievad�t'),AT(145,56),USE(?access:Radio4),VALUE('3')
         RADIO('N-Nav pieejas'),AT(145,65),USE(?access:Radio5),VALUE('N')
       END
       STRING('projekti, atlaides,adreses'),AT(25,35),USE(?String9)
       STRING('apak�grupas, m�rvien�bas,'),AT(25,80),USE(?String7)
       ...
       BUTTON('&Aizliegts main�t Par Grupas un Karti'),AT(139,81,122,15),USE(?grupasaizl)
       IMAGE('CANCEL4.ICO'),AT(263,81,16,15),USE(?Imagegrupas),HIDE
       BUTTON('&Aizliegts main�t Par Atz�mi un Kred.L'),AT(140,97,122,15),USE(?atzimeaizl)
       IMAGE('CANCEL4.ICO'),AT(263,97,16,15),USE(?Imageatzime),HIDE
       STRING('tekstu pl�ns, cenu v�sture'),AT(25,89),USE(?String8)
       BUTTON('&OK'),AT(240,124,35,15),USE(?OkFiles),DEFAULT
     END
screen:base WINDOW('Pieeja B�zei'),AT(,,265,176),GRAY
       ...
       END
       OPTION('Pieejas l�menis :'),AT(121,17,135,63),USE(access,,?access11),BOXED
         RADIO('0-Pilna pieeja'),AT(128,29),USE(?access11:Radio1),VALUE('0')
         RADIO('1-Aizliegts dz�st'),AT(128,38),USE(?access11:Radio2),VALUE('1')
         RADIO('2-Aizliegts dz�st un main�t'),AT(128,47),USE(?access11:Radio3),VALUE('2')
         RADIO('3-Aizliegts dz�st,main�t un ievad�t'),AT(128,56),USE(?access11:Radio4),VALUE('3')
...
screen:nolik WINDOW('Pieeja Noliktavai'),AT(,,386,230),GRAY
       ...
       END
       OPTION('Pieejas l�menis :'),AT(163,7,135,61),USE(access,,?access21),BOXED
         RADIO('0-Pilna pieeja'),AT(170,19),USE(?access21:Radio1),VALUE('0')
         RADIO('1-Aizliegts dz�st'),AT(170,28),USE(?access21:Radio2),VALUE('1')
         RADIO('2-Aizliegts dz�st un main�t'),AT(170,37),USE(?access21:Radio3),VALUE('2')
         RADIO('3-Aizliegts dz�st,main�t un ievad�t'),AT(170,46),USE(?access21:Radio4),VALUE('3')
       ...
       BUTTON('Aizliegt apskat�t D P/Z un jebkuru pieeju iepirkuma cen�m'),AT(163,71,197,13),USE(?BNOL1)
       ...
       BUTTON('Aizliegt apvienot un sadal�t gabalos P/Z'),AT(163,113,197,13),USE(?BNOL4)
       IMAGE('CANCEL4.ICO'),AT(363,112,14,14),USE(?ImageNOL4),HIDE
       BUTTON('Aizliegt pieeju pas�t�jumiem'),AT(163,127,197,13),USE(?BNOL5)
       IMAGE('CANCEL4.ICO'),AT(363,126,14,14),USE(?ImageNOL5),HIDE
       BUTTON('Aizliegts atv�rt/sl�gt P/Z (S)'),AT(163,141,197,13),USE(?BNOL6)
       ...
       BUTTON('Aizliegt pieeju Atlaidei, Summai un Teko�ai cenai (ar� FP)'),AT(163,169,197,13),USE(?BNOLatlaide)
       IMAGE('CANCEL4.ICO'),AT(363,168,14,14),USE(?ImageNOLatlaide),HIDE
       BUTTON('Aizliegt p�rdot, ja veidojas negat�vi atlikumi (ar� FP)'),AT(163,183,197,13),USE(?NOTMINUSI)
       IMAGE('CANCEL4.ICO'),AT(363,182,14,14),USE(?ImageNOTMINUSI),HIDE
       BUTTON('Aizliegt p�rdot zem minim�l�s realiz�cijas cenas (ar� FP)'),AT(163,197,197,13),USE(?NOTMINRC)
     ...
     END
screen:fp WINDOW('Pieeja Fisk�lai Drukai'),AT(,,340,176),GRAY
       OPTION('Fisk�la Druka'),AT(10,14,177,152),USE(?option51),BOXED
       ...
       END
       OPTION('Pieejas l�menis :'),AT(193,17,135,65),USE(access,,?access51),BOXED
         RADIO('0-Pilna pieeja'),AT(200,29),USE(?access51:Radio1),VALUE('0')
         RADIO('1-Aizliegts dz�st'),AT(200,38),USE(?access51:Radio2),VALUE('1')
         RADIO('2-Aizliegts dz�st un main�t'),AT(200,47),USE(?access51:Radio3),VALUE('2')
         RADIO('3-Aizliegts dz�st,main�t un ievad�t'),AT(200,56),USE(?access51:Radio4),VALUE('3')
...
screen:alga WINDOW('Pieeja Algai'),AT(,,265,176),GRAY
       ...
       END
       OPTION('Pieejas l�menis :'),AT(121,17,135,63),USE(access,,?access31),BOXED
         RADIO('0-Pilna pieeja'),AT(128,29),USE(?access31:Radio1),VALUE('0')
         RADIO('1-Aizliegts dz�st'),AT(128,38),USE(?access31:Radio2),VALUE('1')
         RADIO('2-Aizliegts dz�st un main�t'),AT(128,47),USE(?access31:Radio3),VALUE('2')
         RADIO('3-Aizliegts dz�st,main�t un ievad�t'),AT(128,56),USE(?access31:Radio4),VALUE('3')
     ...
     END
screen:pam WINDOW('Pieeja Pamatl�dzek�iem'),AT(,,265,176),GRAY
       OPTION('Pamatl�dzek�i'),AT(10,14,88,152),USE(?option41),BOXED
       ...
       END
       OPTION('Pieejas l�menis :'),AT(121,17,135,63),USE(access,,?access41),BOXED
         RADIO('0-Pilna pieeja'),AT(128,29),USE(?access41:Radio1),VALUE('0')
         RADIO('1-Aizliegts dz�st'),AT(128,38),USE(?access41:Radio2),VALUE('1')
         RADIO('2-Aizliegts dz�st un main�t'),AT(128,47),USE(?access41:Radio3),VALUE('2')
         RADIO('3-Aizliegts dz�st,main�t un ievad�t'),AT(128,56),USE(?access41:Radio4),VALUE('3')
...
screen:LM WINDOW('Pieeja Laika uzskaites sist�mai'),AT(,,340,176),GRAY
       ...
       END
       OPTION('Pieejas l�menis :'),AT(193,17,135,65),USE(access,,?access61),BOXED
         RADIO('0-Pilna pieeja'),AT(200,29),USE(?access61:Radio1),VALUE('0')
         RADIO('1-Aizliegts dz�st'),AT(200,38),USE(?access61:Radio2),VALUE('1')
         RADIO('2-Aizliegts dz�st un main�t'),AT(200,47),USE(?access61:Radio3),VALUE('2')
         RADIO('3-Aizliegts dz�st,main�t un ievad�t'),AT(200,56),USE(?access61:Radio4),VALUE('3')
  ...
  IF SEC:SUPER_ACC='1'
     UNHIDE(?IMAGESUPER)
  .
  IF SEC:SPEC_ACC[1]='1'  !DATU APMAI�A UN IMP_INT
     UNHIDE(?ImageA_DAII)
  .
    ...
    OF ?nolik
      ...
      OF EVENT:Accepted
        ...
        ELSE
           HIDE(?IMAGENOL4)
        .
        IF SEC:SPEC_ACC[6]='1' !PAS�T�JUMI
           UNHIDE(?IMAGENOL5)
        ELSIF SEC:SPEC_ACC[6]='2'
           UNHIDE(?IMAGENOL5)
           ?BNOL5{PROP:TEXT}='Ierobe�ota pieeja pas�t�jumiem'
        ELSE
           HIDE(?IMAGENOL5)
        .
        ...
        ELSIF SEC:SPEC_ACC[11]='3'
           UNHIDE(?IMAGENOLSERVISS)
           ?BNOLserviss{PROP:TEXT}='Aizliegt pieeju meistaru atskait�m'
        ELSE
          ...
          OF ?BNOL5      !PIEEJA PAS�T�JUMIEM
             ...
             OF EVENT:Accepted
                ...
                ELSIF SEC:SPEC_ACC[6]='2'
                   SEC:SPEC_ACC[6]=''
                   HIDE(?IMAGENOL5)
                   ?BNOL5{PROP:TEXT}='Aizliegt pieeju pas�t�jumiem'
                ELSE
                   SEC:SPEC_ACC[6]='1'
                   UNHIDE(?IMAGENOL5)
                   ?BNOL5{PROP:TEXT}='Aizliegt pieeju pas�t�jumiem'
