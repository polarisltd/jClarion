   ...
   INCLUDE('abwindow.inc'),ONCE
                     ...
                     MAP
                       INCLUDE('COOKB002.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('COOKB013.INC'),ONCE        !Req'd for module callout resolution
                     END


Browseingredients PROCEDURE                           !Generated from procedure template - Window
...
ViewPosition           STRING(1024)                   !Entry's view position
                     END
QuickWindow          WINDOW('Browse the ingredients File'),AT(,,220,188),FONT('MS Sans Serif',8,,),IMM,HLP('Browseingredients'),SYSTEM,GRAY,RESIZE,MDI
   ...
   '~cost~C(0)@n10.2@'),FROM(Queue:Browse:1)
                       BUTTON('&Insert'),AT(69,148,45,14),USE(?Insert:2)
                       BUTTON('&Change'),AT(118,148,45,14),USE(?Change:2),DEFAULT
                       BUTTON('&Delete'),AT(167,148,45,14),USE(?Delete:2)
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('Browseingredients')
