...
public class Abreport
{
	...
	// added idx2percentile(Printpreviewclass_2  %%%%%%%%%%
	public static ClarionNumber idx2percentile(Printpreviewclass_2 self,ClarionNumber index)
	{
		if (index.boolValue()) {
			self.zoomqueue.get(index);
			CRun._assert(!(CError.errorCode()!=0));
	...
	// added percentile2idx(Printpreviewclass_2  %%%%%%%%%%

	public static ClarionNumber percentile2idx(Printpreviewclass_2 self,ClarionNumber percentile)
	{
		ClarionNumber c=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
		final int loop_1=self.zoomqueue.records();for (c.setValue(1);c.compareTo(loop_1)<=0;c.increment(1)) {
			self.zoomqueue.get(c);
