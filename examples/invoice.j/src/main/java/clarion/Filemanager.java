...
public class Filemanager
{
	...
	public ClarionNumber used;
	public ClarionNumber opened;
	public ClarionNumber atEOF;
	public ClarionNumber autoIncDone;
	public ClarionNumber lastError;
	...
	public ClarionNumber blobChanged;
	public ClarionString previousBuffer;
	public ClarionNumber savePreviousBuffer;
	...
	private static class _Filecallbackinterface_Impl extends Filecallbackinterface
	{
		...
		public ClarionNumber functionCalled(ClarionNumber opCode,Params parameters,ClarionString errCode,ClarionString errMsg)
		{
			ClarionNumber retVal=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
			{
				...
				case_1_match=false;
				if (case_1.equals(Driverop.ADD)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.APPEND)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.ADDLEN)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.APPENDLEN)) {
					case_1_break=true;
				}
				case_1_match=false;
				if (!case_1_break && case_1.equals(Driverop.CLOSE)) {
					if (_owner.inRestore.boolValue() && _owner.opened.boolValue()) {
						_owner.opened.setValue(0);
						_owner.used.setValue(Constants.FALSE);
						_owner.close();
					}
					case_1_break=true;
				}
				case_1_match=false;
				if (!case_1_break && case_1.equals(Driverop.DESTROY)) {
					_owner.inCallBack.increment(1);
					if (!parameters.index.boolValue()) {
						Abfile.cleanUp(_owner);
					}
					else if (_owner.used.boolValue()) {
					}
					_owner.inCallBack.decrement(1);
					case_1_break=true;
				}
			}
			retVal.setValue(_owner.functionCalled(opCode.like(),parameters,errCode,errMsg));
			return retVal.like();
		}
		...
		{
			{
				...
				boolean case_1_match=false;
				case_1_match=false;
				if (case_1.equals(Driverop.SETBLOBPROPERTY)) {
					if (parameters.property.get().equals(Prop.HANDLE)) {
						_owner.blobChanged.setValue(Constants.TRUE);
					}
					case_1_break=true;
				}
				case_1_match=false;
				if (!case_1_break && case_1.equals(Driverop.PUTBLOBDATA)) {
					_owner.blobChanged.setValue(Constants.TRUE);
					case_1_break=true;
				}
				case_1_match=false;
				if (!case_1_break && case_1.equals(Driverop.DELETE)) {
					_owner.blobChanged.setValue(Constants.FALSE);
					case_1_break=true;
				}
				case_1_match=false;
				if (!case_1_break && case_1.equals(Driverop.NEXT)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.PREVIOUS)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.GETFILEKEY)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.GETFILEPTR)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.GETFILEPTRLEN)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.GETKEYPTR)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.REGETFILE)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.REGETKEY)) {
					if (_owner.savePreviousBuffer.boolValue()) {
						if (!(_owner.previousBuffer==null)) {
							//_owner.previousBuffer;
						}
						_owner.previousBuffer=Abfile.dupString(_owner.buffer.getString());
					}
					case_1_break=true;
				}
				case_1_match=false;
				if (!case_1_break && case_1.equals(Driverop.ADD)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.APPEND)) {
					case_1_match=true;
				...
				case_1_match=false;
				if (!case_1_break && case_1.equals(Driverop.PUT)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.PUTFILEPTR)) {
					case_1_match=true;
				}
				if (case_1_match || case_1.equals(Driverop.PUTFILEPTRLEN)) {
					_owner.blobChanged.setValue(Constants.FALSE);
					case_1_break=true;
				}
			}
			return _owner.functionDone(opCode.like(),parameters,errCode,errMsg);
		}
	}
	...
	public Filemanager()
	{
		...
		used=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
		opened=Clarion.newNumber().setEncoding(ClarionNumber.UNSIGNED);
		atEOF=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
		autoIncDone=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
		lastError=Clarion.newNumber().setEncoding(ClarionNumber.UNSIGNED);
		...
		keys=null;
		blobChanged=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
		previousBuffer=null;
		savePreviousBuffer=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
	}
	...
	public ClarionNumber cancelAutoInc(Relationmanager r)
	{
		...
		if (!this.useFile().boolValue()) {
			if (this.hasAutoInc.boolValue() && this.autoIncDone.boolValue()) {
				if (r==null) {
					retVal.setValue(this.deleteRecord(Clarion.newNumber(Constants.FALSE)));
				}
				else {
					retVal.setValue(r.delete(Clarion.newNumber(0)));
				}
				if (!retVal.boolValue()) {
					this.autoIncDone.setValue(0);
				}
			}
		}
		return retVal.like();
	...
	public ClarionNumber close()
	{
		...
		if (this.opened.boolValue()) {
			this.opened.decrement(1);
			if (!this.opened.boolValue()) {
				this.file.close();
				this.used.setValue(Constants.FALSE);
			}
		}
	...
	{
		return this.atEOF.like();
	}
	public ClarionNumber getError()
	{
		return this.lastError.like();
	}
	...
	public ClarionNumber getFieldAscending(ClarionKey k,ClarionNumber b)
	{
		ClarionNumber retVal=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
		this.setKey(k);
		this.keys.fields.get().get(b);
		retVal.setValue(this.keys.fields.get().ascend);
		return retVal.like();
	}
				...
				if (handleError.boolValue()) {
					...
					for (i.setValue(1);i.compareTo(this.keys.records())<=0;i.increment(1)) {
						...
						if (this.keys.key.get().duplicateCheck()) {
							this.errors.setKey((!this.keys.description.clip().equals("") ? this.keys.description.clip() : this.keys.key.get().getProperty(Prop.NAME)).getString());
		...
		for (int loop_1=2;loop_1>0;loop_1--) {
			...
			}
			{
				...
				if (case_1_match || case_1==Constants.ISOPENERR) {
					this.used.setValue(Constants.TRUE);
					this.atEOF.setValue(0);
					this.autoIncDone.setValue(0);
					return Clarion.newNumber(Level.BENIGN);
					// UNREACHABLE! :case_1_break=true;
	...
	public ClarionNumber openServer(ClarionNumber handleError,ClarionNumber forceOpen)
	{
		...
		this.bindFields();
		if (forceOpen.boolValue() || !this.lazyOpen.boolValue() && !this.opened.boolValue()) {
			rVal.setValue(this.openFile(handleError.like()));
			if (rVal.boolValue()) {
				return rVal.like();
			}
		}
		this.opened.increment(1);
		return Clarion.newNumber(Level.BENIGN);
	}
	...
	public void restoreFile(ClarionNumber id,Filemanager frm,ClarionNumber doRestore)
	{
		...
		if (!this.useFile().boolValue()) {
			...
			if (doRestore.boolValue()) {
				this.autoIncDone.setValue(frm.saved.autoIncDone);
			}
	...
	public ClarionNumber saveFile()
	{
		...
		this.saved.autoIncDone.setValue(this.autoIncDone);
		this.saved.add();
		return id.like();
	}
	public void setError(ClarionNumber err)
	{
		...
		this.lastError.setValue(err);
	}
	public void setErrors(Errorclass e)
	{
		this.errors=e;
	}
	...
	public ClarionNumber _throw()
	{
		...
		return this.errors.takeError(this.lastError.like());
	}
	...
	public ClarionNumber useFile(ClarionNumber usage)
	{
		...
		if (this.lazyOpen.boolValue()) {
			if (!this.used.boolValue()) {
				if (this.opened.compareTo(1)<0) {
	...
	{
		if (query.boolValue() && !this._throw(Clarion.newNumber(Msg.CONFIRMDELETE)).equals(Level.BENIGN)) {
			return Clarion.newNumber(Level.CANCEL);
		}
		this.file.delete();
		if (CError.errorCode()!=0) {
			this.setError(Clarion.newNumber(Msg.DELETEFAILED));
			return Clarion.newNumber(Level.NOTIFY);
		}
		else {
			return Clarion.newNumber(Level.BENIGN);
	...
	{
		{
			...
			boolean case_1_match=false;
			case_1_match=false;
			if (case_1.equals(Driverop.DELETE)) {
				return this.preDelete(errCode,errMsg);
				// UNREACHABLE! :case_1_break=true;
			}
			case_1_match=false;
			if (!case_1_break && case_1.equals(Driverop.ADD)) {
			...
			if (case_1_match || case_1.equals(Driverop.APPENDLEN)) {
				return this.preInsert(opCode.like(),parameters.len.like(),errCode,errMsg);
				// UNREACHABLE! :case_1_break=true;
			}
			case_1_match=false;
			if (!case_1_break && case_1.equals(Driverop.PUT)) {
			...
			if (case_1_match || case_1.equals(Driverop.PUTFILEPTRLEN)) {
				return this.preUpdate(parameters.pointer.like(),parameters.len.like(),errCode,errMsg);
				// UNREACHABLE! :case_1_break=true;
			}
		}
	...
	{
		{
			...
			if (case_1_match || case_1.equals(Driverop.APPENDLEN)) {
				return this.postInsert(errCode,errMsg);
				// UNREACHABLE! :case_1_break=true;
			}
			case_1_match=false;
			if (!case_1_break && case_1.equals(Driverop.PUT)) {
			...
			if (case_1_match || case_1.equals(Driverop.PUTFILEPTRLEN)) {
				return this.postUpdate(errCode,errMsg);
				// UNREACHABLE! :case_1_break=true;
			}
			case_1_match=false;
			if (!case_1_break && case_1.equals(Driverop.DELETE)) {
				return this.postDelete(errCode,errMsg);
				// UNREACHABLE! :case_1_break=true;
			}
		}
		return Clarion.newNumber(Constants.TRUE);
	}
	public ClarionNumber preDelete(ClarionString errCode,ClarionString errMsg)
	{
		return Clarion.newNumber(Constants.TRUE);
	}
	public ClarionNumber postDelete(ClarionString errCode,ClarionString errMsg)
	{
		return Clarion.newNumber(Constants.TRUE);
	}
	public ClarionNumber preInsert(ClarionNumber opCode,ClarionNumber addLen,ClarionString errCode,ClarionString errMsg)
	{
		return Clarion.newNumber(Constants.TRUE);
	}
	public ClarionNumber postInsert(ClarionString errCode,ClarionString errMsg)
	{
		return Clarion.newNumber(Constants.TRUE);
	}
	public ClarionNumber preUpdate(ClarionNumber pntr,ClarionNumber putLen,ClarionString errCode,ClarionString errMsg)
	{
		return Clarion.newNumber(Constants.TRUE);
	}
	public ClarionNumber postUpdate(ClarionString errCode,ClarionString errMsg)
	{
		return Clarion.newNumber(Constants.TRUE);
	}
}
