...
public class ProgressWindow_6 extends ClarionWindow
{
	...
	public ProgressWindow_6(ClarionNumber progressThermometer)
	{
		this.setText("Report Progress...").setAt(null,null,142,59).setCenter().setTimer(1).setGray().setDouble();
		this.setId("printselectedproduct.progresswindow");
		ProgressControl _C1=new ProgressControl();
		_C1.setRange(0,100).setAt(15,15,111,12);
		this._progressThermometer=this.register(_C1.use(progressThermometer),"printselectedproduct.progresswindow.progress:thermometer");
		this.add(_C1);
		StringControl _C2=new StringControl();
		_C2.setText("").setAt(0,3,141,10).setCenter(null);
		this._progressUserString=this.register(_C2,"printselectedproduct.progresswindow.progress:userstring");
		this.add(_C2);
		StringControl _C3=new StringControl();
		_C3.setText("").setAt(0,30,141,10).setCenter(null);
		this._progressPctText=this.register(_C3,"printselectedproduct.progresswindow.progress:pcttext");
		this.add(_C3);
		...
		this._progressCancel=this.register(_C4,"printselectedproduct.progresswindow.progress:cancel");
		this.add(_C4);
	}
}
