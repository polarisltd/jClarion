...
import clarion.equates.Listzone;
...
public class Browseclass extends Viewmanager
{
	...
	public ClarionNumber savedSort;
	public ClarionString savedSortString;
	public Locatorclass savedLocator;
	public Stepclass savedThumb;
	public ClarionAny savedLocatorField;
	...
	public Browseclass()
	{
		...
		savedSort=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
		savedSortString=Clarion.newString(512).setEncoding(ClarionString.CSTRING);
		savedLocator=null;
		savedThumb=null;
		savedLocatorField=Clarion.newAny();
	...
	public void setLocatorFromSort()
	{
		CRun._assert(!(this.sort.locator.get()==null));
		if (this.sort.order.get().boolValue()) {
			this.sort.locator.get().setLocatorField(this.getFirstSortField());
	...
	public void checkEIP()
	{
		...
		this.eip.deleteAction.setValue(this.deleteAction);
		this.eip.bc=this;
	...
	{
		this.primary.me._throw(Clarion.newNumber(Msg.CONCURRENCYFAILEDFROMBROWSE));
	...
	public void replaceSort(ClarionString p0,Locatorclass p1)
	{
		replaceSort(p0,p1,(Stepclass)null);
	}
	public void replaceSort(ClarionString p0)
	{
		replaceSort(p0,(Locatorclass)null);
	}
	public void replaceSort(ClarionString newSortString,Locatorclass newLocator,Stepclass newStepClass)
	{
		if (!this.savedSort.boolValue()) {
			this.saveSort();
		}
		if (!(newLocator==null)) {
			this.sort.locator.set(newLocator);
			this.sort.locator.get().setShadow(Clarion.newString(""));
		}
		else {
			this.sort.locator.set(null);
		}
		if (!(newStepClass==null)) {
			this.sort.thumb.set(newStepClass);
		}
		else {
			this.sort.thumb.set(null);
		}
		this.setOrder(newSortString.like());
		this.setSort(Clarion.newNumber(this.sort.getPointer()),Clarion.newNumber(Constants.TRUE));
	}
	...
	public void resetFromAsk(ClarionNumber request,ClarionNumber response)
	{
		if (response.equals(Constants.REQUESTCOMPLETED)) {
			this.view.flush();
			if (request.equals(Constants.DELETERECORD) || request.equals(Constants.CHANGERECORD) && this.view.getProperty(Prop.FILTER).boolValue() && CExpression.evaluate(this.view.getProperty(Prop.FILTER).toString()).equals(Constants.FALSE)) {
				this.listQueue.delete();
	...
	{
		if (this.savedSort.boolValue()) {
			this.savedSort.setValue(Constants.FALSE);
			this.sort.locator.set(this.savedLocator);
			if (!(this.savedLocator==null)) {
				this.savedLocator.freeElement.setReferenceValue(this.savedLocatorField);
			}
			this.setOrder(this.savedSortString.like());
			this.setSort(Clarion.newNumber(this.sort.getPointer()),Clarion.newNumber(Constants.TRUE));
			this.savedSortString.setValue("");
			this.sort.thumb.set(this.savedThumb);
			if (!(this.sort.locator.get()==null)) {
				this.sort.locator.get().setShadow(Clarion.newString(""));
				this.sort.locator.get().updateWindow();
			}
	...
	{
		if (!this.savedSort.boolValue()) {
			this.savedSort.setValue(Constants.TRUE);
			this.savedLocator=this.sort.locator.get();
			this.savedThumb=this.sort.thumb.get();
			if (!(this.savedLocator==null)) {
				this.savedLocatorField.setReferenceValue(this.savedLocator.freeElement);
			}
			if (!(this.sort.order.get()==null)) {
				if (!this.sort.order.get().equals("")) {
					this.savedSortString.setValue(this.sort.order.get());
				}
				else {
					this.savedSortString.setValue("");
				}
	...
	{
		ClarionNumber li=Clarion.newNumber().setEncoding(ClarionNumber.SIGNED);
		ClarionNumber sel=Clarion.newNumber().setEncoding(ClarionNumber.SIGNED);
		this.currentEvent.setValue(ev);
		li.setValue(this.ilc.getItems());
		if (!this.fileLoaded.boolValue()) {
		...
		if (this.fileLoaded.boolValue()) {
			sel.setValue(this.ilc.choice().subtract(Clarion.getControl(this.ilc.getControl()).getProperty(Prop.YORIGIN)).add(1));
			if (sel.equals(1)) {
				this.ilc.setChoice(this.currentChoice.add(1).getNumber());
				this.ilc.setChoice(this.currentChoice.like());
			}
			else {
				this.ilc.setChoice(this.currentChoice.like());
				this.ilc.setChoice(this.currentChoice.subtract(sel).add(1).getNumber());
				this.ilc.setChoice(this.currentChoice.like());
			}
		}
	}
	...
	{
		ClarionNumber rVal=Clarion.newNumber(0).setEncoding(ClarionNumber.BYTE);
		ClarionNumber rangeChanged=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
		rVal.setValue(this.initSort(b.like()));
		rangeChanged.setValue(this.applyRange());
		if (rangeChanged.boolValue() || rVal.boolValue() || force.boolValue() || !this.loaded.boolValue() || this.loadPending.boolValue()) {
			this.resetResets();
			this.applyOrder();
			this.applyFilter();
	...
	{
		ClarionReal vsp=Clarion.newReal();
		ClarionNumber oldVsp=Clarion.newNumber().setEncoding(ClarionNumber.LONG);
		ClarionNumber sel=Clarion.newNumber().setEncoding(ClarionNumber.LONG);
		{
			...
			if (!case_1_break && Clarion.newNumber(case_1).equals(this.ilc.getControl())) {
				{
					...
					if (!case_2_break && case_2==Event.SCROLLDRAG) {
						...
						if (vsp.compareTo(1)<=0) {
							...
							if (this.listQueue.records().compareTo(this.ilc.getItems())>0) {
								vsp.setValue(Clarion.getControl(this.ilc.getControl()).getProperty(Prop.VSCROLLPOS).multiply(100).divide(this.listQueue.records().subtract(this.ilc.getItems())));
							}
						}
						if (vsp.compareTo(100)>=0) {
							vsp.setValue(100);
							...
							if (this.fileLoaded.boolValue()) {
								sel.setValue(this.ilc.choice().subtract(Clarion.getControl(this.ilc.getControl()).getProperty(Prop.YORIGIN)).add(1));
								this.currentChoice.setValue(Clarion.getControl(this.ilc.getControl()).getProperty(Prop.VSCROLLPOS).divide(this.listQueue.records().subtract(this.ilc.getItems())).multiply(this.listQueue.records()));
								...
								Clarion.getControl(this.ilc.getControl()).setClonedProperty(Prop.VSCROLLPOS,oldVsp);
							}
							else {
								if (!(this.sort.freeElement.getValue()==null) && !(this.sort.thumb.get()==null)) {
									this.sort.freeElement.setValue(this.sort.thumb.get().getValue(vsp.getNumber()));
									this.resetFromBuffer();
								}
								else {
									if (vsp.compareTo(50)<0) {
			...
			if (this.listQueue.records().boolValue()) {
				{
					...
					if (!case_1_break && case_1==Constants.DELETEKEY) {
						if (this.deleteControl.boolValue() && !Clarion.getControl(this.deleteControl).getProperty(Prop.DISABLE).boolValue()) {
					...
					if (!case_1_break && case_1==Constants.MOUSELEFT2) {
						if (this.selecting.boolValue()) {
							if (this.selectControl.boolValue() && !Clarion.getControl(this.selectControl).getProperty(Prop.DISABLE).boolValue()) {
							...
							if (!Clarion.getControl(this.ilc.getControl()).getProperty(Proplist.MOUSEDOWNZONE).equals(Listzone.HEADER)) {
								try {
									takeKey_CheckChange();
								} catch (ClarionRoutineResult _crr) {
									return (ClarionNumber)_crr.getResult();
	...
	{
		if (this.changeControl.boolValue() && !Clarion.getControl(this.changeControl).getProperty(Prop.DISABLE).boolValue()) {
	...
	{
		if (this.insertControl.boolValue() && !Clarion.getControl(this.insertControl).getProperty(Prop.DISABLE).boolValue()) {
			if (CWin.keyCode()==Constants.INSERTKEY) {
	...
	{
		if (this.viewControl.boolValue() && !Clarion.getControl(this.viewControl).getProperty(Prop.DISABLE).boolValue()) {
	...
	public void updateQuery(Queryclass qc,ClarionNumber caseless)
	{
		...
		ClarionNumber found=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
		ClarionNumber field=Clarion.newNumber(1).setEncoding(ClarionNumber.UNSIGNED);
		ClarionFile f=null;
		...
		while (Clarion.getControl(this.ilc.getControl()).getProperty(Proplist.EXISTS,i).boolValue()) {
			...
			if (fn.boolValue()) {
				found.setValue(Constants.FALSE);
				for (field.setValue(1);field.compareTo(this.view.getProperty(Prop.FIELDS))<=0;field.increment(1)) {
					f=this.view.getFileForField(field.intValue());
					record=(ClarionGroup)CMemory.resolveAddress(f.getProperty(Prop.RECORD).intValue());
					if (record.who(this.view.getFileIndexForField(field.intValue())).equals(fn)) {
						found.setValue(Constants.TRUE);
						break;
					}
				}
				if (found.boolValue()) {
	...
	public void updateThumb()
	{
		if (this.hasThumb.boolValue() && (this.listQueue.records().compareTo(this.ilc.getItems())>=0 || this.allowUnfilled.boolValue())) {
			...
			if (this.sort.thumb.get()==null || this.sort.freeElement.getValue()==null) {
				...
				if (this.listQueue.records().equals(this.ilc.getItems())) {
					{
						ClarionObject case_1=Clarion.getControl(this.ilc.getControl()).getProperty(Prop.VSCROLLPOS);
						boolean case_1_break=false;
						if (case_1.equals(1)) {
							Clarion.getControl(this.ilc.getControl()).setProperty(Prop.VSCROLLPOS,0);
							case_1_break=true;
						}
						if (!case_1_break && case_1.equals(99)) {
							Clarion.getControl(this.ilc.getControl()).setProperty(Prop.VSCROLLPOS,100);
							case_1_break=true;
						}
					}
				}
	...
	{
		ClarionNumber pos=Clarion.newNumber(50).setEncoding(ClarionNumber.LONG);
		ClarionNumber recs=Clarion.newNumber().setEncoding(ClarionNumber.LONG);
		if (this.fileLoaded.boolValue() && this.listQueue.records().compareTo(this.ilc.getItems())>0) {
			recs.setValue(this.listQueue.records());
			if (this.currentChoice.compareTo(1)<=0) {
				pos.setValue(0);
			}
			else {
				pos.setValue(this.currentChoice.multiply(this.listQueue.records().subtract(this.ilc.getItems())).divide(this.listQueue.records()).getDecimal().round(1));
			}
		}
		else {
	...
	public void updateViewRecord()
	{
		...
		if (this.listQueue.records().boolValue()) {
			...
			if (rc.equals(Constants.NODRIVERSUPPORT)) {
				pos.setValue(this.view.getPosition());
				this.view.reset(this.listQueue.getViewPosition());
				this.view.watch();
				this.view.next();
				rc.setValue(CError.errorCode());
	...
	public void updateWindow()
	{
		...
		if (this.records().boolValue()) {
			...
			if (this.viewControl.boolValue()) {
				CWin.enable(this.viewControl.intValue());
			}
		}
		else {
			if (this.changeControl.boolValue()) {
			...
			if (this.viewControl.boolValue()) {
				CWin.disable(this.viewControl.intValue());
			}
		}
		if (this.selectControl.boolValue()) {
			if (this.selecting.boolValue()) {
