				...
				if (CWin.event()==Event.ALERTKEY) {
					{
						...
						if (case_2==Constants.MOUSELEFT2) {
							CWin.post(Event.ACCEPTED,(this.vals.records()==0 ? Clarion.newNumber(Feq.INSERT) : Clarion.newNumber(Feq.CHANGE)).intValue());
							case_2_break=true;
						}
						if (!case_2_break && case_2==Constants.CTRLENTER) {
							CWin.post(Event.ACCEPTED,Feq.CHANGE);
							case_2_break=true;
						}
						if (!case_2_break && case_2==Constants.INSERTKEY) {
							CWin.post(Event.ACCEPTED,Feq.INSERT);
							case_2_break=true;
						}
						if (!case_2_break && case_2==Constants.DELETEKEY) {
							if (this.vals.records()!=0) {
								CWin.post(Event.ACCEPTED,Feq.DELETE);
							}
							case_2_break=true;
						}
					}
