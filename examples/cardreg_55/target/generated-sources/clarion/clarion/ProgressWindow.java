...
public class ProgressWindow extends ClarionWindow
{
	...
	public ProgressWindow(ClarionNumber progressThermometer)
	{
		...
		this.setId("printcreditcardregistry.progresswindow");
		ProgressControl _C1=new ProgressControl();
		_C1.setRange(0,100).setAt(15,15,111,12);
		this._progressThermometer=this.register(_C1.use(progressThermometer),"printcreditcardregistry.progresswindow.progress:thermometer");
		this.add(_C1);
		StringControl _C2=new StringControl();
		_C2.setText("").setAt(0,3,141,10).setCenter(null);
		this._progressUserString=this.register(_C2,"printcreditcardregistry.progresswindow.progress:userstring");
		this.add(_C2);
		StringControl _C3=new StringControl();
		_C3.setText("").setAt(0,30,141,10).setCenter(null);
		this._progressPctText=this.register(_C3,"printcreditcardregistry.progresswindow.progress:pcttext");
		this.add(_C3);
		ButtonControl _C4=new ButtonControl();
		_C4.setText("Cancel").setAt(45,42,50,15);
		this._progressCancel=this.register(_C4,"printcreditcardregistry.progresswindow.progress:cancel");
		this.add(_C4);
	}
}
