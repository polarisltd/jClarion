! $Header: /0192-7/XMLSupport/src/XMLExchange.clw 64    6.08.03 17:27 Sergei $
!****************************************************************************
...
rc      BYTE    
  ...
  IF flag=SVMode:IgnoreSchema
    ...
    if not omitted(4)
      rc = DOMToXMLFile(doc, path, newLine, Format)
    else
      rc = DOMToXMLFile(doc, path, NEWLINE_DEFAULT, Format)
    end
    if(not rc)
       return CPXMLErr:XMLWriteFail  
    end         
...
XMLExchange.toXMLTree   procedure(XMLHRManager hrm,signed lvl)
...
rcb     byte
    ...
    loop while(rc=0)
         ...
         if(maxLvl=lvl)
            ...
            chaLvl=SELF.toXMLTree(hrm,lvl+1)
            if(chaLvl<lvl)
               hrElem.close()           
               return chaLvl !go up             
            end
