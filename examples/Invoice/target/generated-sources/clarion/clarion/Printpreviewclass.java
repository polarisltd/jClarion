...
public class Printpreviewclass extends Windowmanager
{
	...
	public ClarionBool mDIWindow;
	public ClarionNumber firstPage;
	public Printpreviewfilequeue imageQueue;
	...
	public ClarionNumber prtPrevMenuSaveAs;
	...
	public ClarionNumber prtPrevLastCtrl;
	public ClarionNumber prtPrevLastDynCtrl;
	public Printpreviewclass()
	{
		...
		prtPrevMenuSaveAs=Clarion.newNumber().setEncoding(ClarionNumber.USHORT);
		...
		prtPrevLastCtrl=Clarion.newNumber().setEncoding(ClarionNumber.USHORT);
		prtPrevLastDynCtrl=Clarion.newNumber().setEncoding(ClarionNumber.USHORT);
	}
	...
	public ClarionNumber askPrintPages()
	{
		...
		Window_4 window=new Window_4(this);
		ClarionNumber rVal=Clarion.newNumber(Constants.FALSE).setEncoding(ClarionNumber.BYTE);
		try {
	...
	public ClarionNumber display(ClarionNumber initZoom,ClarionNumber initCurrentPage,ClarionNumber initPagesAcross,ClarionNumber initPagesDown)
	{
		...
		try {
			...
			if (this.imageQueue.records()!=0) {
				...
				this.setDefaultPages();
				previewWindow.open();
				if (this.mDIWindow.boolValue()) {
					previewWindow.setProperty(Prop.MDI,Constants.TRUE);
				}
				...
				this.prtPrevMenuSaveAs.setValue(previewWindow._saveAs);
				...
				this.prtPrevLastCtrl.setValue(previewWindow._zoomList);
				this.prtPrevLastDynCtrl.setValue(this.prtPrevLastCtrl);
				if (!(this.targetSelector==null)) {
					if (!this.targetSelector.items().boolValue()) {
						CWin.disable(previewWindow._saveAs);
					}
					else if (this.targetSelector.items().equals(1)) {
						Clarion.getControl(previewWindow._saveAs).setProperty(Prop.TEXT,ClarionString.staticConcat("Save As ",this.targetSelector.getReportGeneratorName(Clarion.newNumber(1))));
					}
				}
				else {
					CWin.disable(previewWindow._saveAs);
	...
	public void init(Printpreviewfilequeue imageQueue)
	{
	...
	public void init(Printpreviewfilequeue imageQueue,Reporttargetselectorclass pTargetSelector,Wmfdocumentparser wMFDocParser)
	{
		this.targetSelector=pTargetSelector;
		this.wMFParser=wMFDocParser;
		this.init(imageQueue);
	}
	...
	public void open()
	{
		...
		Clarion.getControl(this.prtPrevZoomList).setClonedProperty(Prop.FROM,this.zoomQueue.menuText);
	...
	public ClarionNumber takeAccepted()
	{
		...
		}
		{
			...
			if (!case_1_break && Clarion.newNumber(case_1).equals(this.prtPrevCurrentPage)) {
				if (Abreport.idx2Percentile(this,this.zoomIndex.like()).equals(Constants.PAGEWIDTH)) {
					this.drawPage();
				}
				else if (!CRun.inRange(this.currentPage.subtract(this.firstPage).add(1),Clarion.newNumber(1),this.pagesAcross.multiply(this.pagesDown))) {
			...
			case_1_match=false;
			if (!case_1_break && Clarion.newNumber(case_1).equals(this.prtPrevMenuSaveAs)) {
				if (!(this.targetSelector==null)) {
					if (this.targetSelector.ask(Clarion.newNumber(Constants.FALSE),Clarion.newNumber(Constants.TRUE)).boolValue()) {
						this.reportTarget=this.targetSelector.getSelected();
						if (this.reportTarget.supportResultQueue().equals(Constants.TRUE)) {
							this.reportTarget.setResultQueue(this.outputFileQueue);
						}
						if (this.reportTarget.askProperties(Clarion.newNumber(Constants.TRUE)).equals(Level.BENIGN)) {
							this.wMFParser.init(this.imageQueue,this.reportTarget,this.errors);
							returnValue.setValue(this.wMFParser.generateReport());
							if (returnValue.equals(Level.BENIGN)) {
								if (this.reportTarget.supportResultQueue().equals(Constants.TRUE)) {
									returnValue.setValue(this.processResultFiles(this.outputFileQueue));
								}
							}
