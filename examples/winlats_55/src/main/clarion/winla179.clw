...
report REPORT,AT(146,1833,8000,9396),PAPER(PAPER:A4),PRE(RPT),FONT('Arial Baltic',8,,,CHARSET:BALTIC), |
       ...
       HEADER,AT(146,160,8000,1698),USE(?unnamed:2)
         ...
         STRING('Apgrozîjums'),AT(6073,1250,729,208),USE(?String4:11),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('Izgâjis'),AT(5396,1250,625,208),USE(?String4:9),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('Ienâcis'),AT(4719,1250,625,208),USE(?String4:4),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         LINE,AT(4688,1198,0,521),USE(?Line2:16),COLOR(COLOR:Black)
         STRING('(rîts)'),AT(4406,1458,260,208),USE(?String4:10),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         ...
         STRING('Preèu apgrozîjuma reìistrâcijas þurnâls'),AT(2250,688,3229,208),USE(?String4),CENTER, |
         ...
         STRING('Nomenklatûra'),AT(135,1250,1615,208),USE(?String4:5),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
...
detail2 DETAIL,AT(,,,250),USE(?unnamed)
         ...
         STRING('Sastâdîja :'),AT(115,73,469,146),USE(?String30),LEFT,FONT(,7,,,CHARSET:BALTIC)
...
PrintSkipDetails BOOL,AUTO
  ...
  ProgressWindow{Prop:Text} = 'Preèu apgrozîjuma reìistrâcijas þurnâls'
  ...
  ACCEPT
    ...
    OF Event:OpenWindow
      ...
      ELSE
        ...
        ADD(OUTFILEANSI)
        OUTA:LINE='Preèu apgrozîjuma reìistrâcijas þurnâls'
        ADD(OUTFILEANSI)
        OUTA:LINE=''
        ADD(OUTFILEANSI)
        OUTA:LINE='NOMENKLATÛRA'&CHR(9)&'NOSAUKUMS'&CHR(9)&'('&NOKL_CP&') Cena'&CHR(9)&'Atlikums  uz'&CHR(9)&'Ienâcis'&CHR(9)&'Izgâjis'&CHR(9)&'Apgrozîjums'&CHR(9)&'Atlikums uz'
        ADD(OUTFILEANSI)
        OUTA:LINE=NOMENKLAT&CHR(9)&CHR(9)&'bez PVN'&CHR(9)&format(S_DAT,@d06.)&'(rîts)'&CHR(9)&CHR(9)&CHR(9)&CHR(9)&format(B_DAT,@d06.)&'(vakars)'
        ADD(OUTFILEANSI)
!        OUTA:LINE=''
!        ADD(OUTFILEANSI)
      .
  ...
  LOOP I#=1 TO RECORDS(AP_TABLE)
     ...
     DISPLAY(?Progress:UserString)
     GET(AP_TABLE,I#)
     CENA = GETNOM_K(A:NOMENKLAT,0,7)    !PIE REIZES ARÎ POZICIONÇ
     IF NOM:TIPS='A' THEN CYCLE.
     APGROZIJUMS=A:IENACIS-A:IZGAJIS
     ATLIKUMSB=A:ATLIKUMSS+A:IENACIS-A:IZGAJIS
     IF ~(A:IENACIS= 0 AND A:IZGAJIS=0 AND F:KRI) ! F:KRI = '1' -tikai, kam ir apgrozîjums
       IF F:DBF='W'       !WMF
         PRINT(RPT:DETAIL)
       ELSE               !WORD,EXCEL
...
PAGE_HEAD DETAIL,AT(,,,500),USE(?unnamed:5)
         ...
         STRING('Nomenklatûra'),AT(156,52,1563,208),USE(?String19),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING(@s35),AT(1823,156,2813,208),USE(KodsKatNos_text),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('Daudz. pçc'),AT(4792,52,625,208),USE(?String19:4),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         ...
         STRING('pçc fakta'),AT(6990,260,625,208),USE(?String19:12),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
...
RPT_FOOT3 DETAIL,AT(,,,3896),USE(?unnamed:4)
         ...
         STRING('Sastâdîja:'),AT(385,3698),USE(?String60),FONT(,7,,,CHARSET:BALTIC)
         STRING(@s8),AT(833,3698),USE(ACC_kods),LEFT,FONT(,7,,,CHARSET:ANSI)
         STRING('/ amats, vârds, uzvârds, paraksts /'),AT(2188,3542,2031,208),USE(?String2:28),LEFT,FONT(,9,,FONT:regular,CHARSET:BALTIC)
         STRING('- _{25}'),AT(2188,2656,2031,208),USE(?String2:24),LEFT,FONT(,10,,FONT:bold,CHARSET:BALTIC)
         STRING('/ amats, vârds, uzvârds, paraksts /'),AT(2188,2865,2031,208),USE(?String2:25),LEFT,FONT(,9,,FONT:regular,CHARSET:BALTIC)
         STRING('- _{25}'),AT(2188,2240,2031,208),USE(?String2:22),LEFT,FONT(,10,,FONT:bold,CHARSET:BALTIC)
         STRING('/ amats, vârds, uzvârds, paraksts /'),AT(2188,2448,2031,208),USE(?String2:23),LEFT,FONT(,9,,FONT:regular,CHARSET:BALTIC)
         ...
         STRING('Komisijas priekðsçdçtâjs - _{25}'),AT(521,1042,3802,156),USE(?String2:16),LEFT,FONT(,10,,FONT:bold,CHARSET:BALTIC)
         STRING('- _{25}'),AT(2188,1823,2031,208),USE(?String2:20),LEFT,FONT(,10,,FONT:bold,CHARSET:BALTIC)
         STRING('/ amats, vârds, uzvârds, paraksts /'),AT(2188,2031,2031,208),USE(?String2:21),LEFT,FONT(,9,,FONT:regular,CHARSET:BALTIC)
         STRING('/ amats, vârds, uzvârds, paraksts /'),AT(2188,1198,2031,208),USE(?String2:17),LEFT,FONT(,9,,FONT:regular,CHARSET:BALTIC)
         STRING('Slçdziens :'),AT(260,156,990,260),USE(?String2:15),LEFT,FONT(,12,,FONT:bold,CHARSET:BALTIC)
         STRING('Komisijas locekïi {13}- _{25}'),AT(521,1406,3802,208),USE(?String2:18),LEFT,FONT(,10,,FONT:bold,CHARSET:BALTIC)
         STRING('Ðî inventarizâcijas akta datus un aprçíinus pârbaudîju'),AT(521,3073,3646,260),USE(?String2:26), |
             LEFT,FONT(,10,,FONT:bold,CHARSET:BALTIC)
         STRING('/ amats, vârds, uzvârds, paraksts /'),AT(2188,1615,2031,208),USE(?String2:19),LEFT,FONT(,9,,FONT:regular,CHARSET:BALTIC)
...
Virsraksts_w WINDOW(' '),AT(,,140,115),FONT('MS Sans Serif',10,,FONT:bold,CHARSET:BALTIC),GRAY
       ...
       OPTION('Preces nosaukums :'),AT(8,31,126,61),USE(KodsKatNos_NR),BOXED
         ...
         RADIO('Kods,Saîsinâtais nosaukums'),AT(17,54,102,10),USE(?KodsKatNos_NR:Radio12),VALUE('2')
  ...
  IF F:IDP='1' !Jâbûvç INVENT fails
    IF LOC_NR < 9
       NOL=LOC_NR
    ELSE
       NOL=CHR(LOC_NR+55)
  ...
  ACCEPT
    ...
    OF Event:OpenWindow
      ...
      EXECUTE KodsKatNos_Nr
         ...
         KodsKatNos_text='Kods,Nosaukums(saîsin.)'
...
!             'Daudzums pçc'&CHR(9)&'Cena ('&nokl_cp&'), Ls'&CHR(9)&'Summa,Ls'&CHR(9)&'Daudzums'
             OUTA:LINE='Nomenklatûra'&CHR(9)&'Kataloga Nr'&CHR(9)&'Kods'&CHR(9)&'Nosaukums'&CHR(9)&'Mçrv.'&CHR(9)&|
             'Daudzums pçc'&CHR(9)&'Cena ('&nokl_cp&'), '&val_uzsk&CHR(9)&'Summa,'&val_uzsk&CHR(9)&'Daudzums'
             ADD(OUTFILEANSI)
    ...
    OF Event:Timer
      LOOP RecordsPerCycle TIMES
        ...
        IF ~cyclenom(nom:nomenklat) AND ~(NOM:TIPS='A')
           ...
           NOS=GETNOM_K('POZICIONÇTS',0,13)
           SUMMA=CENA*DAUDZUMS
           SUMMAK+=SUMMA
           DAUDZUMSK+=DAUDZUMS
           IF DAUDZUMS OR ~F:KRI !JÂDRUKÂ ARÎ NULLES
             IF F:IDP='1'        !Jâbûvç INVENT fails
...
D_DAT                LONG               ! RÇÍINÂMÂ DOK.DATUMS
DAUDZUMS             DECIMAL(12,3)
NSUMMA               DECIMAL(14,4)
...
report REPORT,AT(140,2230,8000,8969),PAPER(PAPER:A4),PRE(RPT),FONT('Arial Baltic',8,,,CHARSET:BALTIC), |
         THOUS                          !Portretam optimâlais lapas garums 2230+8970=11200
       HEADER,AT(140,150,8000,2073),USE(?unnamed) !Nav Reports, visu skaita no lapas sâkuma
         ...
         STRING('Ienâcis'),AT(3854,1615,1458,208),USE(?String12:5),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('Izgâjis'),AT(5469,1615,1458,208),USE(?String12:6),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         ...
         STRING('sçrija un Numurs'),AT(781,1813,1198,208),USE(?String12:13),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         LINE,AT(104,2031,7708,0),USE(?Line1:2),COLOR(COLOR:Black)
         LINE,AT(104,1563,0,521),USE(?Line2),COLOR(COLOR:Black)
         STRING('Nomenklatûra:'),AT(573,1250,990,208),USE(?String2:4),LEFT,FONT(,9,,,CHARSET:BALTIC)
...
RPT_FOOT DETAIL,AT(,,,448),USE(?unnamed:3)
         ...
         STRING('Sastâdîja :'),AT(208,292,490,125),USE(?String45),FONT(,7,,,CHARSET:BALTIC)
         STRING(@s8),AT(719,292,521,125),USE(ACC_kods),LEFT,FONT(,7,,,CHARSET:ANSI)
       END
       FOOTER,AT(140,11200,8000,63)     !Nav Reports, visu skaita no lapas sâkuma
...
!
  ...
  Progress:Thermometer = 0
  ?Progress:PctText{Prop:Text} = '0%'
  ProgressWindow{Prop:Text} = 'Preèu uzskaites kartiòa'
  ?Progress:UserString{Prop:Text}=''
  SEND(NOLIK,'QUICKSCAN=on')
  ACCEPT
    CASE EVENT()
    OF Event:OpenWindow
      VIRSRAKSTS='Preèu un materiâlu daudzuma un summas uzskaites kartîte uz '&format(B_DAT,@D06.)
      ...
      ELSE
          ...
          ADD(OUTFILEANSI)
          IF F:DBF='E'
             OUTA:LINE='Datums'&CHR(9)&'Dokumenta'&CHR(9)&'Partnera'&CHR(9)&'Ienâcis'&CHR(9)&CHR(9)&'Izgâjis'&|
             CHR(9)&CHR(9)&'Atlikums'
             ADD(OUTFILEANSI)
             OUTA:LINE=CHR(9)&'Sçrija un Nr'&CHR(9)&'T,Nr,Nosaukums'&CHR(9)&'Daudzums'&CHR(9)&'Cena bez PVN'&CHR(9)&|
             'Daudzums'&CHR(9)&'Cena bez PVN'&CHR(9)&'Daudzums'
             ADD(OUTFILEANSI)
          ELSE
             OUTA:LINE='Datums'&CHR(9)&'Dokumenta Sçrija un Nr'&CHR(9)&'Partnera T,Nr,Nosaukums'&CHR(9)&'Ienâcis daudzums'&|
             CHR(9)&'Cena bez PVN'&CHR(9)&'Izgâjis daudzums'&CHR(9)&'Cena bez PVN'&CHR(9)&'Atlikums daudzums'
             ADD(OUTFILEANSI)
          .
          OUTA:LINE=''
          ADD(OUTFILEANSI)
      ...
      LOOP RecordsPerCycle TIMES
        ...
        IF NOL:PAR_NR=PAR_NR OR PAR_NR=999999999
           ...
           IF NOL:DATUMS>=S_DAT
             IF NOL:PAR_NR
                IF ~PAV:PAR_NR
                   PAR_NOS_S=GetPar_k(NOL:par_nr,2,25)  !Visdrîzâk no kases...
                   PARTIPS=PAR:TIPS
             ...
             IF ATLAUTS[11]='1' THEN SS0=0. !AIZLIEGTS APSKATÎT D P/Z UN JEBKURU PIEEJU IEP CENÂM
             IF F:DBF='W'
                 PRINT(RPT:DETAIL)
             ELSE
  ...
  IF LocalResponse = RequestCompleted
    ...
    IF F:DBF='W'
        ...
        ENDPAGE(report)
    ELSE
        OUTA:LINE='KOPÂ/Vidçji no gada sâkuma:'&CHR(9)&CHR(9)&CHR(9)&LEFT(format(DSk,@N-_12.3))&CHR(9)&|
