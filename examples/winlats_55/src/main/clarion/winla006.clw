...
BRW1::View:Browse    VIEW(PROJ_P)
                       PROJECT(PRP:DATUMS)
                       PROJECT(PRP:PLANS)
                       PROJECT(PRP:Komentars)
                       PROJECT(PRP:U_NR)
                     END

Queue:Browse:1       QUEUE,PRE()                  ! Browsing Queue
BRW1::PRP:DATUMS       LIKE(PRP:DATUMS)           ! Queue Display field
BRW1::PRP:PLANS        LIKE(PRP:PLANS)            ! Queue Display field
BRW1::PRP:Komentars    LIKE(PRP:Komentars)        ! Queue Display field
BRW1::PRP:U_NR         LIKE(PRP:U_NR)             ! Queue Display field
...
BRW1::Sort1:KeyDistribution LIKE(PRP:DATUMS),DIM(100)
BRW1::Sort1:LowValue LIKE(PRP:DATUMS)             ! Queue position of scroll thumb
BRW1::Sort1:HighValue LIKE(PRP:DATUMS)            ! Queue position of scroll thumb
BRW1::Sort1:Reset:F:OBJ_NR LIKE(F:OBJ_NR)
...
QuickWindow          WINDOW('PROJ_P.tps File'),AT(,,288,188),FONT('MS Sans Serif',8,,FONT:bold,CHARSET:BALTIC),IMM,HLP('BrowsePROJ_P'),SYSTEM,GRAY,RESIZE
                       LIST,AT(8,20,272,142),USE(?Browse:1),IMM,HVSCROLL,MSG('Browsing Records'),FORMAT('46C|M~Datums~@D06.@49R(1)|M~Plâns~C(0)@n-14.2@80L(1)|M~Komentârs~L(2)@s40@'),FROM(Queue:Browse:1)
                       BUTTON('&Ievadît'),AT(93,170,45,14),USE(?Insert:2)
                       BUTTON('&Mainît'),AT(142,170,45,14),USE(?Change:2),DEFAULT
                       BUTTON('&Dzçst'),AT(191,170,45,14),USE(?Delete:2)
                       SHEET,AT(4,4,280,162),USE(?CurrentTab)
                         TAB('Plâns un tâ izmaiòas'),USE(?Tab:2)
                         END
                       END
                       BUTTON('&Beigt'),AT(241,170,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('BrowsePROJ_P','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
  BIND('F:OBJ_NR',F:OBJ_NR)
...
!|
  IF FilesOpened
    PROJ_P::Used -= 1
    IF PROJ_P::Used = 0 THEN CLOSE(PROJ_P).
  END
  IF WindowOpened
    INISaveWindow('BrowsePROJ_P','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
...
!|
  ...
  IF BRW1::SortOrder = BRW1::LastSortOrder
    CASE BRW1::SortOrder
    OF 1
      IF BRW1::Sort1:Reset:F:OBJ_NR <> F:OBJ_NR
        BRW1::Changed = True
      END
    END
  ELSE
  ...
  IF BRW1::SortOrder <> BRW1::LastSortOrder OR BRW1::Changed OR ForceRefresh
    CASE BRW1::SortOrder
    OF 1
      BRW1::Sort1:Reset:F:OBJ_NR = F:OBJ_NR
    END
    DO BRW1::GetRecord
    DO BRW1::Reset
...
!|
  ...
  IF ERRORCODE()
    ...
    ELSE
      StandardWarning(Warn:RecordFetchError,'PROJ_P')
      POST(Event:CloseWindow)
    END
    EXIT
  END
  CASE BRW1::SortOrder
  OF 1
    BRW1::Sort1:HighValue = PRP:DATUMS
  END
  DO BRW1::Reset
  ...
  CASE BRW1::SortOrder
  OF 1
    BRW1::Sort1:LowValue = PRP:DATUMS
...
!|
  PRP:DATUMS = BRW1::PRP:DATUMS
  PRP:PLANS = BRW1::PRP:PLANS
  PRP:Komentars = BRW1::PRP:Komentars
  PRP:U_NR = BRW1::PRP:U_NR
!----------------------------------------------------------------------
...
!|
  BRW1::PRP:DATUMS = PRP:DATUMS
  BRW1::PRP:PLANS = PRP:PLANS
  BRW1::PRP:Komentars = PRP:Komentars
  BRW1::PRP:U_NR = PRP:U_NR
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 1
        LOOP BRW1::CurrentScroll = 100 TO 1 BY -1
          IF BRW1::Sort1:KeyDistribution[BRW1::CurrentScroll] => PRP:DATUMS
...
!|
  IF BRW1::RecordCount
    ...
    ELSE                                          ! ELSE (What keycode was hit)
      ...
      OF 1
        IF CHR(KEYCHAR())
          IF UPPER(SUB(PRP:DATUMS,1,1)) = UPPER(CHR(KEYCHAR()))
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 1
      PRP:DATUMS = BRW1::Sort1:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(PRP:Nr_Key)
      RESET(PRP:Nr_Key,BRW1::HighlightedPosition)
    ELSE
      PRP:U_NR = F:OBJ_NR
...
!|
  CASE BRW1::SortOrder
  OF 1
    F:OBJ_NR = BRW1::Sort1:Reset:F:OBJ_NR
  END
...
!|
  ...
  CASE BRW1::SortOrder
  OF 1
    PRP:U_NR = BRW1::Sort1:Reset:F:OBJ_NR
  END
...
SAV::PRO:Record      LIKE(PRO:Record)
Auto::Attempts       LONG,AUTO
Auto::Save:PRO:U_NR     LIKE(PRO:U_NR)
ToolBarMode     UNSIGNED,AUTO
WindowXPos      SIGNED,AUTO,STATIC
WindowYPos      SIGNED,AUTO,STATIC
WindowPosInit   BOOL(False),STATIC
WinResize            WindowResizeType
QuickWindow          WINDOW('PROJEKTI.tps'),AT(,,361,135),FONT('MS Sans Serif',8,,FONT:bold),IMM,HLP('UpdatePROJEKTI'),SYSTEM,GRAY,RESIZE
                       SHEET,AT(4,4,355,113),USE(?CurrentTab)
                         TAB('Projekts (Objekts) :'),USE(?Tab:1)
                           PROMPT('&Unikâls Nr:'),AT(10,33,41,10),USE(?Prompt2)
                           ENTRY(@n10),AT(55,33,40,10),USE(PRO:U_NR),RIGHT(1)
                           PROMPT('&Kods:'),AT(11,47),USE(?PRO:KODS:Prompt)
                           ENTRY(@s9),AT(55,47,40,10),USE(PRO:KODS)
                           PROMPT('&Nosaukums :'),AT(10,60),USE(?PRO:NOS_P:Prompt)
                           ENTRY(@S105),AT(55,60,302,10),USE(PRO:NOS_P)
                           STRING(@s6),AT(55,71),USE(PRO:NOS_A)
                           BUTTON('&Nav aktîvs'),AT(10,86,45,14),USE(?ButtonNA)
                           BUTTON('&Virsraksts'),AT(86,87,45,14),USE(?ButtonV)
                           BUTTON('&Plâns'),AT(159,86,45,14),USE(?ButtonP)
                           IMAGE('CHECK3.ICO'),AT(206,82,17,20),USE(?ImageP),HIDE
                           IMAGE('CANCEL4.ICO'),AT(57,82,17,20),USE(?ImageNA),HIDE
                           IMAGE('CHECK3.ICO'),AT(133,82,17,20),USE(?ImageV),HIDE
                         END
                       END
                       BUTTON('&OK'),AT(268,120,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(316,120,45,14),USE(?Cancel)
                     END
PRO_BAITS    STRING(8)
  CODE
  PUSHBIND
  ...
  END
  DO PrepareProcedure
  IF BAND(PRO:BAITS,00000001b)    ! Nav aktîvs
     UNHIDE(?IMAGENA)
     PRO_BAITS[1]='1'
  .
  IF BAND(PRO:BAITS,00000010b)    ! Virsraksts
     UNHIDE(?IMAGEV)
     PRO_BAITS[2]='1'
  .
  IF BAND(REG_BASE_ACC,00000010b) ! Budþets
     IF PRO:U_NR < 31000
        ?TAB:1{PROP:TEXT}='Klasifikâcijas kodi'
     ELSE
        ?TAB:1{PROP:TEXT}='Ekonomiskie kodi'
     .
  .
  IF GETproj_P(PRO:U_NR,1)        ! IR DEFINÇTS PLÂNS - arî ~budþetam
     UNHIDE(?ImageP)
  .
  ...
  ACCEPT
      ...
      IF EVENT() = Event:Completed
        ...
        OF InsertRecord
          ...
          OF NoError
            ...
            POST(Event:CloseWindow)
          OF DupKeyErr
            IF DUPLICATE(PRO:NR_Key)
              IF StandardWarning(Warn:DuplicateKey,'PRO:NR_Key')
                SELECT(?Prompt2)
                VCRRequest = VCRNone
                CYCLE
              END
        ...
        OF ChangeRecord
          LOOP
            ...
            IF RecordChanged THEN
              Update::Error = RIUpdate:PROJEKTI(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
    ...
    OF ?PRO:NOS_P
      CASE EVENT()
      OF EVENT:Accepted
        PRO:NOS_A=INIGEN(PRO:NOS_P,6,1)
      END
    OF ?ButtonNA
      CASE EVENT()
      OF EVENT:Accepted
    ...
    OF ?ButtonP
      ...
      OF EVENT:Accepted
        ...
        LocalRequest = OriginalRequest
        DO RefreshWindow
        IF GETPROJ_P(PRO:U_NR,1) !IR DEFINÇTAS PLÂNS
           UNHIDE(?ImageP)
        ELSE
           HIDE(?ImageP)
        .
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('UpdatePROJEKTI','winlats.INI')
  WinResize.Resize
  ?PRO:U_NR{PROP:Alrt,255} = 734
  ?PRO:KODS{PROP:Alrt,255} = 734
  ?PRO:NOS_P{PROP:Alrt,255} = 734
  ?PRO:NOS_A{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
ProcedureReturn ROUTINE
!|
!| This routine provides a common procedure exit point for all template
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('UpdatePROJEKTI','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?PRO:U_NR
      PRO:U_NR = History::PRO:Record.U_NR
    OF ?PRO:KODS
      PRO:KODS = History::PRO:Record.KODS
    OF ?PRO:NOS_P
      PRO:NOS_P = History::PRO:Record.NOS_P
    OF ?PRO:NOS_A
      PRO:NOS_A = History::PRO:Record.NOS_A
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
...
!|
  ...
  LOOP
    ...
    IF ERRORCODE() AND ERRORCODE() <> BadRecErr
      StandardWarning(Warn:RecordFetchError,'PROJEKTI')
      POST(Event:CloseWindow)
      EXIT
    END
    IF ERRORCODE()
      Auto::Save:PRO:U_NR = 1
    ELSE
      Auto::Save:PRO:U_NR = PRO:U_NR + 1
    END
    PRO:Record = SAV::PRO:Record
    PRO:U_NR = Auto::Save:PRO:U_NR
    SAV::PRO:Record = PRO:Record
    ADD(PROJEKTI)
    IF ERRORCODE()
      Auto::Attempts += 1
...
BRW1::View:Browse    VIEW(PROJEKTI)
                       PROJECT(PRO:U_NR)
                       PROJECT(PRO:BAITS)
                       PROJECT(PRO:NOS_A)
                     END
...
Queue:Browse:1       QUEUE,PRE()                  ! Browsing Queue
BRW1::PRO:U_NR         LIKE(PRO:U_NR)             ! Queue Display field
BRW1::PRO:U_NR:NormalFG LONG                      ! Normal Foreground
BRW1::PRO:U_NR:NormalBG LONG                      ! Normal Background
BRW1::PRO:U_NR:SelectedFG LONG                    ! Selected Foreground
BRW1::PRO:U_NR:SelectedBG LONG                    ! Selected Background
...
BRW1::PRO:BAITS        LIKE(PRO:BAITS)            ! Queue Display field
BRW1::PRO:NOS_A        LIKE(PRO:NOS_A)            ! Queue Display field
BRW1::Mark             BYTE                       ! Queue POSITION information
BRW1::Position         STRING(512)                ! Queue POSITION information
                     END                          ! END (Browsing Queue)
BRW1::CurrentScroll  BYTE                         ! Queue position of scroll thumb
...
BRW1::Sort3:KeyDistribution LIKE(PRO:U_NR),DIM(100)
BRW1::Sort3:LowValue LIKE(PRO:U_NR)               ! Queue position of scroll thumb
BRW1::Sort3:HighValue LIKE(PRO:U_NR)              ! Queue position of scroll thumb
...
SelectButton           SIGNED                   !Browse select button
                     END
WinResize            WindowResizeType
QuickWindow          WINDOW('PROJEKTI (Objekti)'),AT(,,361,278),FONT('MS Sans Serif',8,,FONT:bold),IMM,HLP('BrowseProjekti'),SYSTEM,GRAY,RESIZE
                       LIST,AT(8,20,347,220),USE(?Browse:1),IMM,VSCROLL,MSG('Browsing Records'),FORMAT('31R(2)|M*~Nr~C(0)@n7@460L(1)|M*~Kods, Nosaukums~L(5)@s115@'),FROM(Queue:Browse:1)
                       BUTTON('Iz&vçlçties'),AT(251,261,45,14),USE(?Select:2),FONT(,,COLOR:Navy,,CHARSET:ANSI)
                       BUTTON('&Ievadît'),AT(213,242,45,14),USE(?Insert:3)
                       BUTTON('&Mainît'),AT(261,242,45,14),USE(?Change:3),DEFAULT
                       BUTTON('&Dzçst'),AT(309,242,45,14),USE(?Delete:3)
                       SHEET,AT(4,4,355,255),USE(?CurrentTab)
                         TAB('Aktîvie &Nr secîba'),USE(?Tab:2)
                           ENTRY(@n6),AT(8,243),USE(PRO:U_NR)
                           STRING('- pçc Nr'),AT(43,245),USE(?String1:3)
                         END
                         TAB('Akt. No&sauk.sec.'),USE(?Tab:3)
                           ENTRY(@s6),AT(8,243),USE(PRO:NOS_A)
                           STRING('- pçc nosaukuma'),AT(43,246),USE(?String1)
                         END
                         TAB('&Visi Nosauk.sec.'),USE(?Tab3)
                           ENTRY(@s6),AT(8,243),USE(PRO:NOS_A,,?PRO:NOS_A:2)
                           STRING('- pçc nosaukuma'),AT(44,245),USE(?String1:2)
                         END
                       END
                       BUTTON('&Beigt'),AT(313,261,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
  ...
  DO PrepareProcedure
  IF BAND(REG_BASE_ACC,00000010b) ! Budþets
     QUICKWINDOW{PROP:TEXT}='Klasifikâcijas/Ekonomiskie kodi'
  ELSE
     SELECT(?Tab:3) !NOSAUKUMU SECÎBA
  .
  ACCEPT
    CASE EVENT()
    OF EVENT:CloseDown
    ...
    OF ?PRO:NOS_A
      ...
      OF EVENT:Accepted
        ...
        IF PRO:NOS_A
          BRW1::LocateMode = LocateOnValue
          DO BRW1::LocateRecord
          BRW1::Sort1:LocatorValue = PRO:NOS_A
          BRW1::Sort1:LocatorLength = LEN(CLIP(PRO:NOS_A))
          SELECT(?Browse:1)
          DO BRW1::PostNewSelection
        END
      END
...
PrepareProcedure ROUTINE
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Resize)
  INIRestoreWindow('BrowseProjekti','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
  IF LocalRequest <> SelectRecord
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('BrowseProjekti','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
!|
  ...
  CASE BRW1::SortOrder
  OF 1
    PRO:NOS_A = BRW1::Sort1:LocatorValue
  OF 2
    PRO:NOS_A = BRW1::Sort2:LocatorValue
  OF 3
    BRW1::Sort3:LocatorValue = PRO:U_NR
    CLEAR(PRO:U_NR)
  END
  DISPLAY()
  ForceRefresh = False
...
!| that the record is "fresh" from the disk, without the disk access required.
!|
  PRO:U_NR = BRW1::PRO:U_NR
  NOS_P = BRW1::NOS_P
  PRO:BAITS = BRW1::PRO:BAITS
  PRO:NOS_A = BRW1::PRO:NOS_A
!----------------------------------------------------------------------
BRW1::FillQueue ROUTINE
!|
!| This routine is used to fill the BrowseBox QUEUE from several sources.
...
!| Finally, the POSITION of the current VIEW record is added to the QUEUE
!|
  NOS_P=CLIP(PRO:KODS)&' '&PRO:NOS_P
  BRW1::PRO:U_NR = PRO:U_NR
  IF (BAND(PRO:BAITS,00000010b))
    BRW1::PRO:U_NR:NormalFG = 255
    BRW1::PRO:U_NR:NormalBG = -1
    BRW1::PRO:U_NR:SelectedFG = 255
    BRW1::PRO:U_NR:SelectedBG = -1
  ELSE
    BRW1::PRO:U_NR:NormalFG = -1
    BRW1::PRO:U_NR:NormalBG = -1
    BRW1::PRO:U_NR:SelectedFG = -1
    BRW1::PRO:U_NR:SelectedBG = -1
  END
  BRW1::NOS_P = NOS_P
  IF (BAND(PRO:BAITS,00000010b))
  ...
  ELSE
    ...
    BRW1::NOS_P:SelectedBG = -1
  END
  BRW1::PRO:BAITS = PRO:BAITS
  BRW1::PRO:NOS_A = PRO:NOS_A
  BRW1::Position = POSITION(BRW1::View:Browse)
!----------------------------------------------------------------------
BRW1::PostNewSelection ROUTINE
!|
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 3
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort3:KeyDistribution[BRW1::CurrentScroll] => PRO:U_NR
            IF BRW1::CurrentScroll <= 1
              BRW1::CurrentScroll = 0
            ELSIF BRW1::CurrentScroll = 100
              BRW1::CurrentScroll = 100
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 3
      PRO:U_NR = BRW1::Sort3:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(PRO:NOS_KEY)
      RESET(PRO:NOS_KEY,BRW1::HighlightedPosition)
    ELSE
      SET(PRO:NOS_KEY,PRO:NOS_KEY)
    END
    BRW1::View:Browse{Prop:Filter} = |
    '(~BAND(PRO:BAITS,00000001b))'
  OF 2
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(PRO:NOS_KEY)
      RESET(PRO:NOS_KEY,BRW1::HighlightedPosition)
  ...
  OF 3
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(PRO:NR_Key)
      RESET(PRO:NR_Key,BRW1::HighlightedPosition)
    ELSE
      SET(PRO:NR_Key,PRO:NR_Key)
    END
    BRW1::View:Browse{Prop:Filter} = |
    '(~BAND(PRO:BAITS,00000001b))'
  END
...
!|
  ...
  IF BRW1::RecordCount
    CASE BRW1::SortOrder
    OF 1; ?PRO:NOS_A{Prop:Disable} = 0
    OF 2; ?PRO:NOS_A{Prop:Disable} = 0
    OF 3; ?PRO:U_NR{Prop:Disable} = 0
    END
    IF BRW1::HighlightedPosition
      LOOP BRW1::CurrentChoice = 1 TO BRW1::RecordCount
        GET(Queue:Browse:1,BRW1::CurrentChoice)
  ...
  ELSE
    CLEAR(PRO:Record)
    CASE BRW1::SortOrder
    OF 1; ?PRO:NOS_A{Prop:Disable} = 1
    OF 2; ?PRO:NOS_A{Prop:Disable} = 1
    OF 3; ?PRO:U_NR{Prop:Disable} = 1
    END
    BRW1::CurrentChoice = 0
