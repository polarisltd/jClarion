...
report REPORT,AT(200,1608,8000,10000),PAPER(PAPER:A4),PRE(RPT),FONT('Arial Baltic',8,,,CHARSET:BALTIC), |
       ...
       HEADER,AT(200,150,8000,1458),USE(?unnamed:2)
         ...
         STRING('(-) mçs esam'),AT(6750,990,1042,208),USE(?String55:7),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('numurs'),AT(448,1146,885,208),USE(?String8:2),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('(-) mçs esam'),AT(5917,990,781,208),USE(?String55:5),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('parâdâ preci'),AT(5917,1198,781,208),USE(?String55:6),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('(+) mums pa-'),AT(5083,990,781,208),USE(?String55:3),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('râdâ preci'),AT(4250,1198,781,208),USE(?String55:2),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('râdâ naudu'),AT(5083,1198,781,208),USE(?String55:4),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
         STRING('parâdâ naudu'),AT(6750,1198,1042,208),USE(?String55:8),CENTER,FONT(,8,,FONT:bold,CHARSET:BALTIC)
...
PER_FOOT DETAIL,AT(,,,250),USE(?unnamed:4)
         ...
         STRING('KOPÂ pa periodu :'),AT(177,104,,156),USE(?String20)
...
RPT_FOOT DETAIL,AT(,,,177),USE(?unnamed:3)
         ...
         STRING('KOPÂ no gada sâkuma :'),AT(177,10,1510,156),USE(?String65:2),LEFT
       END
...
RPT_FOOT1 DETAIL,AT(,,,250),USE(?unnamed)
         ...
         STRING('Sastâdîja :'),AT(63,73,573,156),USE(?String41),LEFT,FONT(,7,,,CHARSET:BALTIC)
...
PAGE_FOOT DETAIL,AT(,,,323)
         ...
         STRING('KOPÂ pa lapu : '),AT(177,104,885,156),USE(?String48)
...
PrintSkipDetails BOOL,AUTO
  ...
  KOMENT='Norçíini ar : '&CLIP(nos_p)&' '&FORMAT(S_DAT,@D06.)&'-'&FORMAT(B_DAT,@D06.)
  CHECKOPEN(VAL_K)
  CLEAR(VAL:RECORD)
  SET(VAL:NOS_KEY)
  DO NEXTVAL
...
!      GGK:DATUMS = DATE(1,1,GADS)
      ...
      ELSE
          ...
          ADD(OUTFILEANSI)
          IF F:DBF='E'
            OUTA:LINE='NR GG'&CHR(9)&'Dokumenta'&CHR(9)&'Datums'&CHR(9)&'Ieraksta saturs'&CHR(9)&'219** (+)-mums'&CHR(9)&'231** (+)-mums'&CHR(9)&'521** (-)-mçs esam'&CHR(9)&'531** (-)-mçs esam'
            ADD(OUTFILEANSI)
            OUTA:LINE=        CHR(9)&'numurs'   &CHR(9)         &CHR(9)                  &CHR(9)&'parâdâ preci'  &CHR(9)&'parâdâ naudu'  &CHR(9)&'parâdâ preci'      &CHR(9)&'parâdâ naudu'
            ADD(OUTFILEANSI)
          ELSE
            OUTA:LINE='NR GG'&CHR(9)&'Dokumenta numurs'&CHR(9)&'Datums'&CHR(9)&'Ieraksta saturs'&CHR(9)&'219** (+)-mums parâdâ preci'&CHR(9)&'231** (+)-mums parâdâ naudu'&CHR(9)&'521** (-)-mçs esam parâdâ preci'&CHR(9)&'531** (-)-mçs esam parâdâ naudu'
            ADD(OUTFILEANSI)
          END
      end
    OF Event:Timer
      LOOP RecordsPerCycle TIMES
        ...
        .
!************************** 1.SOLIS-SKAITAM NO GADA SâKUMA *****************
        BKK=GGK:BKK[1:3]
        CASE BKK
        OF '219'
           k2190 += ggk:summa
      ...
      !************************** 2.SOLIS SKAITAM PA PERIODU *********************
          ...
          SATURS2 = F_TEKSTS[2]
          SATURS3 = F_TEKSTS[3]
!          IF ~(GGK:VAL='Ls' OR GGK:VAL='LVL') OR F_TEKSTS[2]  !BÛS JÂDRUKÂ VISMAZ 2 RINDAS
...
!             DOK_NR = GETDOK_SENR(2,GG:DOK_SENR,,GG:ATT_DOK)
  ...
  IF LocalResponse = RequestCompleted
    ...
    IF F:DBF = 'W'
        PRINT(RPT:PER_FOOT)
    ELSE
        OUTA:LINE='Kopâ pa periodu:'&CHR(9)&CHR(9)&CHR(9)&CHR(9)&LEFT(FORMAT(P2190,@N-_13.2B))&CHR(9)&|
    ...
    IF F:DBF = 'W'
        PRINT(RPT:RPT_FOOT)
    ELSE
        OUTA:LINE='Kopâ no gada sâkuma:'&CHR(9)&CHR(9)&CHR(9)&CHR(9)&LEFT(FORMAT(K2190,@N-_13.2B))&CHR(9)&|
...
window WINDOW('   Kïûda '),AT(,,358,71),FONT('MS Sans Serif',9,,FONT:bold,CHARSET:BALTIC),CENTER,IMM, |
       ...
       BUTTON('&darît'),AT(293,45,58,14),USE(?darit),DEFAULT
       BUTTON('&nedarît'),AT(240,45,49,14),USE(?nedarit)
     END
  CODE                                            ! Begin processed code
!
!  KRASA=1 ZIÒA ZILÂ KRÂSÂ
!
  ...
  IF OPC
     ...
     IF ERROR()
         ZINA='Kïûdas KODS= ' &CLIP(OPC)& ' TEKSTS= '& NORADE & ' ... paòemiet KLUDAS.TPS no www.assako.lv <BIN> '
     ELSE
        CASE KLU:KARTIBA
        OF 1                                        ! OTRÂDI
  ...
  OF 3
     ...
     ?NEDARIT{prop:text}=' &Atlikt  '
  OF 4
     ?DARIT{prop:text}=' &Pârrçíinât  '
     ?NEDARIT{prop:text}=' &Atlikt  '
  OF 5
     ?DARIT{prop:text}=' Aiz&vietot  '
     ?NEDARIT{prop:text}='&Atstât kâ ir'
  OF 6
  ...
  OF 11
     ...
     ?NEDARIT{prop:text}=' &Mainît'
  OF 12
     ?DARIT{prop:text}=' &Apskatît'
     ?NEDARIT{prop:text}=' &Pârbûvçt'
  .
  IF KRASA=1 !ZILÂ KRÂSÂ
     ZINA1=ZINA
     ZINA=''
     WINDOW{PROP:TEXT}='Ziòa'
  .
...
SUBST        STRING(6),DIM(4)
  ...
  CODE                                            ! Begin processed code
! stop(1)   INIGEN LIETO ARÎ CHECK_CLIENT
 IF ~INRANGE(OPC,1,10)
    STOP('INIGEN: PIEPRASÎTS ATGRIEZT:'&OPC)
    RETURN('')
 .
 ...
 OF 2
     BURTI='ÂÈÇÌÎÍÏÒÐÛÞ'
     LOOP I# = 1 TO gar
        J#=INSTRING(lauks[I#],'âèçìîíïòðûþ')
        IF J#
           LAUKS[I#]=BURTI[J#]
        .
     .
 OF 3
     KLUDA(0,'Tiks mainîti Lielie/mazie burti(CASE) tekoðajam laukam',5,1)
     IF KLU_DARBIBA
        BURTI ='ÂÈÇÌÎÍÏÒÐÛÞ'
        BURTIM='âèçìîíïòðûþ'
        LOOP I# = 1 TO gar
           J#=INSTRING(lauks[I#],'âèçìîíïòðûþ')
           K#=INSTRING(lauks[I#],'ÂÈÇÌÎÍÏÒÐÛÞ')
           IF J#
              LAUKS[I#]=BURTI[J#]
           ELSIF K#
              LAUKS[I#]=BURTIM[K#]
 ...
 OF 5  ! TELEHANSAS BURTI UN SIMBOLI
     ...
     LOOP I# = 1 TO gar
        J#=INSTRING(lauks[I#],'ÂâÈèÇçÌìÎîÍíÏïÒòÐðÛûÞþ&Ôô')
        IF J#
           LAUKS[I#]=BURTI[J#]
        .
     .
...
!        IF ~INRANGE(J#,0020h,007Eh) THEN LAUKS[I#]='.'.
     ...
     IF TUKSS# > 0 THEN
        ...
        .
     .
 OF 10 ! NOVÂCAM BANKAS SUBSTITÛCIJAS---PAGAIDU LÂPÎÐANÂS
     BURTI='""<>'
     SUBST[1]='&quot;'
     SUBST[2]='&apos;'
     SUBST[3]='&lt;'
