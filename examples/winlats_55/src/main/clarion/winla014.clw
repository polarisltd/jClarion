                       ...
                       SHEET,AT(4,6,258,170),USE(?CurrentTab)
                         TAB('Cita adrese '),USE(?Tab:1)
                           ...
                           OPTION('&Tips'),AT(11,68,234,20),USE(ADR:TIPS)
                             RADIO('Fileâles adrese'),AT(18,74,62,10),USE(?ADR:TIPS:Radio1),VALUE('F')
                             RADIO('Pasta adrese'),AT(81,74),USE(?ADR:TIPS:Radio2),VALUE('P')
                             RADIO('Veikala adrese'),AT(141,74,62,10),USE(?ADR:TIPS:Radio3),VALUE('V')
                             RADIO('Arhîvs'),AT(205,74,62,10),USE(?ADR:TIPS:Radio4),VALUE('A')
                           END
                           CHECK('Maksâtâjs'),AT(18,99,100,12),USE(Payer),VALUE('1','0')
                           CHECK('Pircçjs'),AT(18,111,100,12),USE(Buyer),VALUE('1','0')
                           CHECK('Piegâdâtâjs'),AT(18,88,100,12),USE(Seller),VALUE('1','0')
                           OPTION,AT(10,65,234,20),USE(IsKlientsAddress)
                             RADIO('Klienta adrese'),AT(18,72,95,10),USE(?IsKlientsAddress:Radio1),VALUE('1')
                             RADIO('Piegâdes adrese'),AT(119,72),USE(?IsKlientsAddress:Radio2),VALUE('0')
                           END
...
QuickWindow          WINDOW('Browse the ADR_K File'),AT(,,250,193),FONT('MS Sans Serif',9,,FONT:bold),CENTER,IMM,HLP('BrowseADR_K'),SYSTEM,GRAY,RESIZE
                       STRING('Juridiskâ adrese :'),AT(33,1),USE(?String2)
                       ...
                       BUTTON('Iz&vçlçties citu  '),AT(52,174,124,14),USE(?Select:2),FONT(,,COLOR:Navy,,CHARSET:ANSI),DEFAULT
                       BUTTON('&Ievadît'),AT(85,153,45,14),USE(?Insert:3)
                       BUTTON('&Mainît'),AT(134,153,45,14),USE(?Change:3),DEFAULT
                       BUTTON('&Dzçst'),AT(183,153,45,14),USE(?Delete:3)
                       BUTTON('Visa&s'),AT(4,174,46,14),USE(?visas),HIDE,DEFAULT
                       SHEET,AT(4,9,241,162),USE(?CurrentTab)
                         TAB('Citas'),USE(?Tab:2)
                         END
                         TAB('arî Arhîvs'),USE(?Tab:3)
                         END
                       END
  ...
  IF LOCALREQUEST=SELECTRECORD
     IF INRANGE(JOB_NR,1,15)                 !BASE
        ...
        ?Select:2{PROP:TEXT}='Iz&vçlçties citu kâ pasta ardesi'
        ?close{prop:TEXT}='&Atstât juridisko'
     ELSIF INRANGE(JOB_NR,16,40)             !NOL
        ?Change:3{PROP:DEFAULT}=''
        ?Select:2{PROP:DEFAULT}='1'
        ?Select:2{PROP:TEXT}='Iz&vçlçties citu kâ izkrauðanas vietu'
        ?close{prop:TEXT}='&Atstât juridisko'
        IF ADR_NR=999999999
           UNHIDE(?VISAS)
        .
...
QuickWindow          WINDOW('Update the PAR_E File'),AT(,,270,141),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateADR_K'),SYSTEM,GRAY,RESIZE
                       SHEET,AT(4,6,258,110),USE(?CurrentTab)
                         TAB('Cits e-pasts'),USE(?Tab:1)
                           ...
                           PROMPT('&Piezîmes:'),AT(33,64),USE(?EMA:AMATS:Prompt)
