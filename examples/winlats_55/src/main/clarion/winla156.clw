...
QuickWindow          WINDOW('Amati.tps'),AT(,,184,259),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseAmati'),SYSTEM,GRAY,RESIZE
                       ...
                       BUTTON('Iz&vçlçties'),AT(56,242,45,14),USE(?Select:2),FONT(,,COLOR:Navy,,CHARSET:ANSI)
                       SHEET,AT(5,2,174,238),USE(?CurrentTab)
                         TAB('Nosaukumu secîba'),USE(?Tab:2)
                           ENTRY(@s5),AT(54,207),USE(AMS:AMA_A)
                           PROMPT('- âtrâ meklçðana pçc amata'),AT(87,209),USE(?Prompt1)
                           BUTTON('&Ievadît'),AT(28,222,45,14),USE(?Insert:3)
                           BUTTON('&Mainît'),AT(77,222,45,14),USE(?Change:3),DEFAULT
                           BUTTON('&Dzçst'),AT(126,222,45,14),USE(?Delete:3)
                         END
                       END
...
SOCSCREEN WINDOW('Soc.apdr. % maiòa'),AT(,,184,81),GRAY
       STRING(@N6B),AT(66,7),USE(JB#)
       STRING('Mainît Darba Devçja VSA no'),AT(9,26),USE(?String2)
       ENTRY(@N_5.2),AT(104,25,26,12),USE(PR37)
       STRING('uz'),AT(133,26),USE(?String3)
       ENTRY(@N_5.2),AT(145,25,26,12),USE(PR37J)
       STRING('Mainît Darba Òçmçja VSA no'),AT(9,40),USE(?String21)
...
PPFSCREEN WINDOW('Privâto pensiju fonda  % maiòa'),AT(,,157,75),GRAY
       STRING(@N6B),AT(59,7),USE(JB#,,?JB:1)
       STRING('Mainît PPF %  no'),AT(18,26),USE(?String12)
...
QuickWindow          WINDOW('Kadru saraksts'),AT(,,460,330),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseKADRI'),SYSTEM,GRAY,RESIZE,MDI
                       MENUBAR
                         MENU('&2-Serviss'),USE(?2Serviss)
                           ITEM('&2-Selftests Kadriem (Rîkojumi)'),USE(?2Serviss2Selftests)
                         END
                         MENU('&6-Speciâlâs funkcijas'),USE(?6Speciâlâsfunkcijas)
                           ITEM('&1- Mainît VSA %'),USE(?6Speciâlâsfunkcijas1MainîtVSA)
                           ITEM('&2-Minît PPF %'),USE(?6Speciâlâsfunkcijas2MinîtPPF)
                         END
                       END
                       LIST,AT(8,22,441,262),USE(?Browse:1),IMM,HVSCROLL,MSG('Browsing Records'),FORMAT('18R(1)|M~ID~C(0)@n4@114L(1)|M~Vârds Uzvârds~C(0)@s30@56L(2)|M~Pers. kods~@p#####' &|
   '#-#####p@30R(1)|M~Ter. k.~L(2)@n06@48L(1)|M~Grâm./Karte~L(2)@s12@10C|M~ST~@s1@16' &|
   'C|M~Nod~@s2@19R(1)|M~Obj~C(0)61L(1)|M~Amats~C(2)@s25@45L(1)|M~Pieòemts~C(0)@d06.' &|
   '@23R(1)|M~DDS%~C(0)@n5.2@23D(12)|M~DNS%~C(0)@n5.2@24D(12)|M~PPF%~C(0)@n-6.2@40L(' &|
   '1)|M~Atlaists~@d06.B@'),FROM(Queue:Browse:1)
                       BUTTON('Apskatît Kartiòu'),AT(388,286,61,14),USE(?ButtonKartiòa)
                       BUTTON('Iz&vçlçties'),AT(214,303,45,14),USE(?Select),FONT(,,COLOR:Navy,,CHARSET:ANSI)
                       BUTTON('&Ievadît'),AT(262,303,45,14),USE(?Insert:3)
                       BUTTON('&Mainît'),AT(310,303,45,14),USE(?Change),DEFAULT
                       BUTTON('&Dzçst'),AT(358,303,45,14),USE(?Delete:3)
                       SHEET,AT(4,4,449,319),USE(?CurrentTab)
                         TAB('Uzvârdu secîbâ'),USE(?Tab:2)
                           BUTTON('Personas ID (jâbût CODE39.fnt)'),AT(276,286,110,14),USE(?Button7)
                           STRING(@s45),AT(7,289,183,10),USE(STR_KA_DA),LEFT(1),FONT(,,COLOR:Gray,,CHARSET:ANSI)
                           BUTTON('Rîkojumi un Dokumenti'),AT(193,286,82,14),USE(?ButtonRD)
                           ENTRY(@s3),AT(28,305,20,10),USE(KAD:INI)
                           STRING('pçc Uzvârda'),AT(54,305),USE(?String1)
                         END
                         TAB('Nodaïu un Uzvârdu'),USE(?Tab2)
                         END
                         TAB('Iekðçjo ID'),USE(?Tab3)
                         END
                         TAB('Arî arhîvs'),USE(?Tab4)
                         END
                       END
  ...
  IF LOCALREQUEST=SELECTRECORD
     ...
     QUICKWINDOW{PROP:TEXT}='Izvçlaties no kadru saraksta'
     IF INRANGE(JOB_NR,16,40)   !TIEK SAUKTS NO P/Z (SERVISA)
        HIDE(?INSERT:3)
        HIDE(?CHANGE)
        HIDE(?DELETE:3)
        F:NODALA=SYS:NODALA     !NODAÏAS FILTRS MEISTARIEM(SERVISÂ)
     .
  .
  ...
  ACCEPT
    ...
    OF EVENT:OpenWindow
      ...
      !   DO BRW1::InitializeBrowse
         ...
         SELECT(?Browse:1)
         DO RefreshWindow
         ?Browse:1{PROPLIST:HEADER,1}='ID' !Lai piespiestu pârlasît ekrânu
      .
    OF EVENT:GainFocus
      ForceRefresh = True
      IF NOT WindowInitialized
    ...
    OF ?2Serviss2Selftests
      ...
      LocalRequest = OriginalRequest
      DO RefreshWindow
    OF ?6Speciâlâsfunkcijas1MainîtVSA
      DO SyncWindow
      OPEN(SOCSCREEN)
      JB#=0
      DISPLAY
      ...
      IF JB#
         ...
         DO RefreshWindow
      .
    OF ?6Speciâlâsfunkcijas2MinîtPPF
      DO SyncWindow
      OPEN(PPFSCREEN)
      JB#=0
      DISPLAY
...
!|
  ...
  END
  STR_KA_DA='Kopâ definçtas kapitâla daïas '&FORMAT(KAD_KA_DAK,@N6.2)&'% no 100%'
  SETCURSOR()
  DO BRW1::Reset
  PREVIOUS(BRW1::View:Browse)
  IF ERRORCODE()
