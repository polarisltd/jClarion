...
QuickWindow          WINDOW('Update the KAT_K File'),AT(,,358,209),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateKAT_K'),SYSTEM,GRAY,RESIZE,MDI
                       SHEET,AT(4,4,350,178),USE(?CurrentTab)
                         TAB('General'),USE(?Tab:1)
                           ...
                           PROMPT('Likme Ìeometriski degresîvai metodei :'),AT(31,112),USE(?KAT:LIKME:Prompt)
                           ENTRY(@n-4.0),AT(159,112,40,10),USE(KAT:LIKME),RIGHT(1)
                           PROMPT('Kalpoðanas laiks LIN metodei :'),AT(57,124,97,10),USE(?KAT:GADI:Prompt)
                           ENTRY(@n-5.1),AT(159,124,40,10),USE(KAT:GADI),DECIMAL(8)
                           PROMPT('Apliekamâ îpaðuma noteikðamas koef. :'),AT(29,135,125,10),USE(?KAT:APLKOEF:Prompt)
                           ...
                           OPTION('&Ilgtermiòa ieguldîjumu veids :'),AT(18,22,322,64),USE(KAT:IIV),BOXED
                             ...
                             RADIO('22 Çkas,bûves un ilggad.stâdîjumi'),AT(64,48),USE(?KAT:IIV:Radio22),VALUE('22')
                             RADIO('13'),AT(25,59),USE(?KAT:IIV:Radio13),VALUE('13')
                             RADIO('23 Ilgtermiòa ieguldîjumi nomâtajos P/L'),AT(64,59),USE(?KAT:IIV:Radio23),VALUE('23')
                             RADIO('24 Iekârtas un maðînas'),AT(207,37),USE(?KAT:IIV:Radio24),VALUE('24')
                             RADIO('25 Pârçjie P/L un inventârs'),AT(207,48),USE(?KAT:IIV:Radio25),VALUE('25')
                             RADIO('26 P/L izveid.un nep.celtn.obj.izm.'),AT(207,59),USE(?KAT:IIV:Radio26),VALUE('26')
                             RADIO('27 Avansa maksâjumi par P/L'),AT(207,71),USE(?KAT:IIV:Radio27),VALUE('27')
                           END
                         END
                       END
  ...
  IF KAT:KAT[1]='1'
    GADI_PROCENTI='gadi ('&FORMAT((1/kat:gadi)*100,@N_7.3)&'% gadâ)'
  ELSE
    GADI_PROCENTI='gadi'
  .
  CASE LocalRequest
  ...
  ACCEPT
    ...
    OF ?KAT:KAT
      ...
      OF EVENT:Accepted
        IF KAT:KAT[1]='1'
          GADI_PROCENTI='gadi ('&ROUND(100/KAT:GADI,.001)&'% gadâ)'
        ELSE
          GADI_PROCENTI='gadi'
        .
        DISPLAY
...
QuickWindow          WINDOW('Browse the KAT_K File'),AT(,,358,225),FONT('MS Sans Serif',8,,FONT:bold),IMM,HLP('BrowseKAT_K'),SYSTEM,GRAY,RESIZE,MDI
   ...
   'C(0)@n2B@22D(7)|M~LIN~C(0)@n-5.1B@80L(2)|M~Nosaukums~@s72@'),FROM(Queue:Browse:1)
                       BUTTON('&Ievadît'),AT(207,188,45,14),USE(?Insert:2)
                       BUTTON('&Mainît'),AT(255,188,45,14),USE(?Change:2),DEFAULT
                       BUTTON('&Dzçst'),AT(305,188,45,14),USE(?Delete:2)
                       BUTTON('Iz&vçlçties'),AT(259,209,41,14),USE(?Select),FONT(,,COLOR:Navy,,CHARSET:ANSI)
...
QuickWindow          WINDOW('Update the PAM_P File'),AT(,,190,85),FONT('MS Sans Serif',9,,FONT:bold,CHARSET:BALTIC),IMM,HLP('UpdatePAM_P'),SYSTEM,GRAY,RESIZE,MDI
                       SHEET,AT(4,4,179,58),USE(?CurrentTab)
                         TAB('General'),USE(?Tab:1)
                           ...
                           PROMPT('Atraðanâs vieta:'),AT(8,34),USE(?PAP:VIETA:Prompt)
                           ENTRY(@S25),AT(68,34,104,10),USE(PAP:VIETA)
                           PROMPT('&Komentârs:'),AT(8,48),USE(?PAP:KOMENTARS:Prompt)
