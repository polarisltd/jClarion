...
Update::Reloop  BYTE
Update::Error   BYTE
History::EIR:Record LIKE(EIR:Record),STATIC
SAV::EIR:Record      LIKE(EIR:Record)
ToolBarMode     UNSIGNED,AUTO
WindowXPos      SIGNED,AUTO,STATIC
WindowYPos      SIGNED,AUTO,STATIC
WindowPosInit   BOOL(False),STATIC
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the EIROKODI File'),AT(,,273,98),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('UpdateEIROKODI'),SYSTEM,GRAY,RESIZE,MDI
                       SHEET,AT(4,4,265,72),USE(?CurrentTab)
                         TAB('Kods, kam jârçíina papildus mçrvienîba'),USE(?Tab:1)
                           PROMPT('Kods:'),AT(8,27),USE(?EIR:kods:Prompt)
                           ENTRY(@n_9B),AT(61,27,45,10),USE(EIR:kods),RIGHT(1)
                           PROMPT('Nosaukums:'),AT(8,40),USE(?EIR:nos_s:Prompt)
                           ENTRY(@s50),AT(61,40,204,10),USE(EIR:nos_s)
                           PROMPT('Pap.Mçrv.:'),AT(8,52),USE(?EIR:papild:Prompt)
                           ENTRY(@s3),AT(61,52,30,10),USE(EIR:papild)
                         END
                       END
                       BUTTON('&OK'),AT(175,80,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(224,80,45,14),USE(?Cancel)
                     END
  CODE
  PUSHBIND
    ...
    ELSE
      ...
      IF EVENT() = Event:Completed
        ...
        OF InsertRecord
          ...
          OF DupKeyErr
            IF DUPLICATE(EIR:Keykods)
              IF StandardWarning(Warn:DuplicateKey,'EIR:Keykods')
                SELECT(?EIR:kods:Prompt)
                VCRRequest = VCRNone
                CYCLE
              END
        ...
        OF ChangeRecord
          LOOP
            ...
            RecordChanged=false
            IF SAV::EIR:Record <> EIR:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:EIROKODI(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('UpdateEIROKODI','winlats.INI')
  WinResize.Resize
  ?EIR:kods{PROP:Alrt,255} = 734
  ?EIR:nos_s{PROP:Alrt,255} = 734
  ?EIR:papild{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
...
!|
  IF FilesOpened
    EIROKODI::Used -= 1
    IF EIROKODI::Used = 0 THEN CLOSE(EIROKODI).
  END
  IF WindowOpened
    INISaveWindow('UpdateEIROKODI','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?EIR:kods
      EIR:kods = History::EIR:Record.kods
    OF ?EIR:nos_s
      EIR:nos_s = History::EIR:Record.nos_s
    OF ?EIR:papild
      EIR:papild = History::EIR:Record.papild
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
...
BRW1::View:Browse    VIEW(EIROKODI)
                       PROJECT(EIR:kods)
                       PROJECT(EIR:nos_s)
                       PROJECT(EIR:papild)
                     END
...
BRW1::EIR:kods         LIKE(EIR:kods)             ! Queue Display field
BRW1::EIR:nos_s        LIKE(EIR:nos_s)            ! Queue Display field
BRW1::EIR:papild       LIKE(EIR:papild)           ! Queue Display field
...
BRW1::Sort1:KeyDistribution LIKE(EIR:kods),DIM(100)
BRW1::Sort1:LowValue LIKE(EIR:kods)               ! Queue position of scroll thumb
BRW1::Sort1:HighValue LIKE(EIR:kods)              ! Queue position of scroll thumb
...
SelectButton           SIGNED                   !Browse select button
                     END
WinResize            WindowResizeType
QuickWindow          WINDOW('Browse the EIROKODI File'),AT(,,315,205),FONT('MS Sans Serif',9,,FONT:bold),IMM,HLP('BrowseEirokodi'),SYSTEM,GRAY,RESIZE,MDI
                       LIST,AT(0,6,304,174),USE(?Browse:1),IMM,VSCROLL,MSG('Browsing Records'),FORMAT('51R(2)|M~Kods~C(0)@n_12@203L(2)|M~Nosaukums~@s50@28L(2)|M~Pap.Mçrv.~@s3@'),FROM(Queue:Browse:1)
                       ENTRY(@n-12.0),AT(2,185),USE(EIR:kods)
                       BUTTON('Iz&vçlçties'),AT(56,185,45,14),USE(?Select:2),HIDE,FONT(,,COLOR:Navy,,CHARSET:ANSI)
                       BUTTON('&Ievadît'),AT(108,185,45,14),USE(?Insert:3)
                       BUTTON('&Mainît'),AT(156,185,45,14),USE(?Change:3),DEFAULT
                       BUTTON('&Dzçst'),AT(204,185,45,14),USE(?Delete:3)
                       BUTTON('&Beigt'),AT(259,184,45,14),USE(?Close)
                     END
  CODE
  PUSHBIND
  LocalRequest = GlobalRequest
    ...
    OF ?EIR:kods
      ...
      OF EVENT:Accepted
        ...
        IF EIR:kods
          ...
          BRW1::Sort1:LocatorValue = EIR:kods
          BRW1::Sort1:LocatorLength = LEN(CLIP(EIR:kods))
          SELECT(?Browse:1)
...
PrepareProcedure ROUTINE
  ...
  INIRestoreWindow('BrowseEirokodi','winlats.INI')
  WinResize.Resize
  BRW1::AddQueue = True
  BRW1::RecordCount = 0
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('BrowseEirokodi','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
!|
  ...
  CASE BRW1::SortOrder
  OF 1
    EIR:kods = BRW1::Sort1:LocatorValue
  END
  DISPLAY()
  ForceRefresh = False
!---------------------------------------------------------------------------
...
!|
  ...
  IF ERRORCODE()
    ...
    ELSE
      StandardWarning(Warn:RecordFetchError,'EIROKODI')
      POST(Event:CloseWindow)
    END
    EXIT
  END
  ...
  OF 1
    ...
    END
  END
  IF SEND(EIROKODI,'QUICKSCAN=off').
!----------------------------------------------------------------------
...
!|
  EIR:kods = BRW1::EIR:kods
  EIR:nos_s = BRW1::EIR:nos_s
  EIR:papild = BRW1::EIR:papild
!----------------------------------------------------------------------
...
!|
  BRW1::EIR:kods = EIR:kods
  BRW1::EIR:nos_s = EIR:nos_s
  BRW1::EIR:papild = EIR:papild
    ...
    IF BRW1::RecordCount = ?Browse:1{Prop:Items}
      ...
      OF 1
        LOOP BRW1::CurrentScroll = 1 TO 100
          IF BRW1::Sort1:KeyDistribution[BRW1::CurrentScroll] => EIR:kods
...
!|
  ...
  ELSE
    CASE BRW1::SortOrder
    OF 1
      EIR:kods = BRW1::Sort1:KeyDistribution[?Browse:1{Prop:VScrollPos}]
      BRW1::LocateMode = LocateOnValue
      DO BRW1::LocateRecord
    END
  END
...
!|
  ...
  OF 1
    IF BRW1::LocateMode = LocateOnEdit
      BRW1::HighlightedPosition = POSITION(EIR:Keykods)
      RESET(EIR:Keykods,BRW1::HighlightedPosition)
    ELSE
      SET(EIR:Keykods,EIR:Keykods)
    END
  END
  IF ERRORCODE()
...
ActionMessage        CSTRING(40)
RecordChanged        BYTE,AUTO
AUB_AUT_TEXTX        STRING(10)
SAV_AUT_TEXTX        STRING(10)
AUB_STATUSSX         BYTE
AUB_PAV_NRX          ULONG
SOLI                 BYTE
DIENA_V              STRING(11)
Update::Reloop  BYTE
Update::Error   BYTE
History::AUB:Record LIKE(AUB:Record),STATIC
SAV::AUB:Record      LIKE(AUB:Record)
ToolBarMode     UNSIGNED,AUTO
WindowXPos      SIGNED,AUTO,STATIC
WindowYPos      SIGNED,AUTO,STATIC
WindowPosInit   BOOL(False),STATIC
WinResize            WindowResizeType
QuickWindow          WINDOW('Update the AU_BILDE File'),AT(,,158,255),FONT('MS Sans Serif',9,,FONT:bold,CHARSET:BALTIC),IMM,HLP('UpdateAU_BILDE'),SYSTEM,GRAY,RESIZE,MDI
                       SHEET,AT(0,4,154,226),USE(?CurrentTab)
                         TAB(' '),USE(?Tab:1)
                           PROMPT('Datums:'),AT(11,20),USE(?AUB:DATUMS:Prompt)
                           ENTRY(@D06.b),AT(41,20,43,10),USE(AUB:DATUMS),RIGHT(1)
                           PROMPT('Plkst.'),AT(87,20),USE(?AUB:PLKST_P:Prompt)
                           ENTRY(@T1),AT(109,20,35,10),USE(AUB:PLKST_P)
                           STRING(@s11),AT(15,32),USE(DIENA_V),FONT(,,COLOR:Gray,,CHARSET:ANSI)
                           STRING('virsrakstam 00:00'),AT(84,32),USE(?StringVirsrakstam),HIDE,FONT(,,COLOR:Gray,,CHARSET:ANSI)
                           STRING('Vieta Nr '),AT(45,41),USE(?StringVietaNr)
                           STRING(@n2),AT(80,41),USE(VIETA_NR)
                           BUTTON('Atvçrt (mainît) P(R)ojektu :'),AT(11,52,91,14),USE(?ButtonPRojekts)
                           ENTRY(@N_10B),AT(105,54,39,10),USE(AUB_PAV_NRX)
                           PROMPT('A/M identifikators :'),AT(5,74),USE(?AUB_AUT_TEXT:Prompt)
                           ENTRY(@s10),AT(79,72,65,10),USE(AUB_AUT_TEXTX)
                           BUTTON('Papildus informâcija'),AT(72,85,73,14),USE(?ButtonINFO)
                           OPTION('Pasûtîjuma statuss'),AT(11,102,136,110),USE(AUB_STATUSSX),BOXED
                             RADIO('0-Nekas vçl nav plânots'),AT(18,113),USE(?aub_statussX:Radio:0),VALUE('0')
                             RADIO('1-Plânots pasûtîjums'),AT(18,128),USE(?aub_statussX:Radio:1),VALUE('1')
                             RADIO('4-Plânots garantijas pasûtîjums'),AT(18,137),USE(?aub_statussX:Radio:4),VALUE('4')
                             RADIO('5-Plânots atkârtots pasûtîjums'),AT(18,148),USE(?aub_statusXs:Radio:5),VALUE('5')
                             RADIO('6-Plânots iekðçjais pasûtîjums'),AT(19,158),USE(?aub_statussX:Radio:6),VALUE('6')
                             RADIO('2-Pasûtîjums izpildîts'),AT(18,176),USE(?aub_statussX:Radio:2),VALUE('2')
                             RADIO('9-Dienas virsraksts vai nestrâdâ'),AT(18,195),USE(?aub_statussX:Radio:9),VALUE('9')
                           END
                           LINE,AT(11,123,136,0),USE(?Line1),COLOR(COLOR:Silver)
                           LINE,AT(10,169,136,0),USE(?Line1:2),COLOR(COLOR:Silver)
                           LINE,AT(11,187,136,0),USE(?Line1:3),COLOR(COLOR:Silver)
                           STRING('Aizpildît (Mainît) uz leju vçl'),AT(5,215),USE(?StringAizpildit)
                           ENTRY(@N2),AT(95,215,13,10),USE(SOLI)
                           STRING('pusstundas'),AT(111,215),USE(?StringPusstundas)
                         END
                       END
                       BUTTON('OK'),AT(58,234,45,14),USE(?OK),DEFAULT
                       BUTTON('&Atlikt'),AT(107,234,45,14),USE(?Cancel)
                     END
SAV_POSITION    STRING(255)
PLKST           STRING(5)
AUB_STATUSS     BYTE
  CODE
  PUSHBIND
  ...
  IF PAV::U_NR   !TIEK SAUKTS NO P/Z
     DISABLE(?BUTTONPRojekts)
     IF ~AUB:PAV_NR[VIETA_NR] THEN AUB:PAV_NR[VIETA_NR]=PAV::U_NR. !JAUNS PASÛTÎJUMS
     IF ~AUB:STATUSS[VIETA_NR]
        IF GETPAR_K(PAV:PAR_NR,0,20)='R' THEN AUB:STATUSS[VIETA_NR]=6 !IEKÐÇJAIS
        ELSIF INSTRING(PAV:APM_K,'45') THEN AUB:STATUSS[VIETA_NR]=4   !GARANTIJA
        ELSE AUB:STATUSS[VIETA_NR]=1.                                 !KLIENTA
     .
...
!     IF ~AUB:AUT_TEXT2[VIETA_NR] THEN AUB:AUT_TEXT2[VIETA_NR]=GETAUTO(PAV:VED_NR,8)&' '&PAV:NOKA.
  .
  AUB_AUT_TEXTX =AUB:AUT_TEXT[VIETA_NR]  !VAR MAINÎT
  SAV_AUT_TEXTX =AUB_AUT_TEXTX
  AUB_STATUSSX  =AUB:STATUSS[VIETA_NR]   !VAR MAINÎT
  AUB_PAV_NRX   =AUB:PAV_NR[VIETA_NR]    !TIKAI IZSPÎDINÂT

  IF LOCALREQUEST=CHANGERECORD
     IF AUB:PLKST_P=0 !VIRSRAKSTS
        DISABLE(?AUB:DATUMS:Prompt,?ButtonINFO)
        DISABLE(?aub_statussX:Radio:0,?aub_statussX:Radio:2)
        DISABLE(?StringAizpildit,?StringPusstundas)
     ELSE
        DISABLE(?AUB:DATUMS:Prompt,?AUB:PLKST_P)
     .
  ELSIF LOCALREQUEST=INSERTRECORD
     HIDE(?StringVietaNr,?StringPusstundas)
     UNHIDE(?StringVirsrakstam)
  ELSIF LOCALREQUEST=DELETERECORD
     DISABLE(?AUB:DATUMS:Prompt,?StringPusstundas)
  .
  ...
  ACCEPT
    ...
    OF EVENT:OpenWindow
      ...
      IF NOT WindowInitialized
        ...
        WindowInitialized = True
      END
      SELECT(?AUB:DATUMS:Prompt)
      IF LOCALREQUEST=CHANGERECORD
         IF ~AUB_AUT_TEXTX
            SELECT(?AUB_AUT_TEXTX)
         ELSE
            SELECT(?AUB_STATUSSX)
         .
      .
      ...
      IF EVENT() = Event:Completed
        ...
        OF InsertRecord
          ...
          OF DupKeyErr
            IF DUPLICATE(AUB:DAT_KEY)
              IF StandardWarning(Warn:DuplicateKey,'AUB:DAT_KEY')
                SELECT(?AUB:DATUMS:Prompt)
                VCRRequest = VCRNone
                CYCLE
              END
        ...
        OF ChangeRecord
          LOOP
            ...
            SETCURSOR(Cursor:Wait)
            RecordChanged=false
            IF SAV::AUB:Record <> AUB:Record
              RecordChanged = True
            END
            IF RecordChanged THEN
              Update::Error = RIUpdate:AU_BILDE(1)
            ELSE
              Update::Error = 0
            END
            SETCURSOR()
    ...
    OF ?ButtonPRojekts
      CASE EVENT()
      OF EVENT:Accepted
 DO PERFORMPAVAD  !SARAKSTÎS UZ LEJU AUB:PAV_NR[VIETA_NR] VISIEM TÂDIEM PAÐIEM
 AUB_PAV_NRX   =AUB:PAV_NR[VIETA_NR]
 IF ~AUB_AUT_TEXTX THEN AUB_AUT_TEXTX =AUB:AUT_TEXT[VIETA_NR].
! IF ~AUB_AUT_TEXT2 THEN AUB_AUT_TEXT2=AUB:AUT_TEXT2[VIETA_NR].
 IF ~AUB_STATUSSX THEN AUB_STATUSSX  =AUB:STATUSS[VIETA_NR].
 DISPLAY
    ...
    OF ?ButtonINFO
      ...
      OF EVENT:Accepted
        ...
        SET(AUX:AUT_KEY,AUX:AUT_KEY)
        NEXT(AU_TEX)
        IF ERROR() OR ~(AUX:AUT_TEXT=AUB:AUT_TEXT[VIETA_NR])
    ...
    OF ?OK
      ...
      OF EVENT:Accepted
  IF LOCALREQUEST=CHANGERECORD
     AUB:AUT_TEXT[VIETA_NR] =AUB_AUT_TEXTX
     IF SAV_AUT_TEXTX AND ~(SAV_AUT_TEXTX=AUB_AUT_TEXTX) !MAINÎTS IDENTIFIKATORS
        KLUDA(0,'Mainîts a/m identifikators, jâmaina arî pârçjos PLKST-os un Darba vietâs')
        AUX:AUT_TEXT=SAV_AUT_TEXTX !MEKLÇJAM VECO
        GET(AU_TEX,AUX:AUT_KEY)
        IF ~ERROR()
           AUX:AUT_TEXT=AUB_AUT_TEXTX
           IF RIUPDATE:AU_TEX()
              KLUDA(24,AUBTEXNAME)
           .
        .
     .
     AUB:STATUSS[VIETA_NR]  =AUB_STATUSSX
     AUB:pav_nr[VIETA_NR]   =AUB_PAV_NRX
        IF RIUPDATE:AU_BILDE()   !JÂSAGLABÂ, JO SOLI IZKUSTINÂS
        KLUDA(24,AUBNAME)
     .
  
     IF SOLI !CIK SOÏI JÂAIZPILDA
        kluda(0,'Aizpildu uz leju vçl '&clip(SOLI*0.5)&' stundas',2)
        IF KLU_DARBIBA
           SAV_AUT_TEXTX=AUB:AUT_TEXT[VIETA_NR]
     !       AUB_STATUSS =AUB:STATUSS[VIETA_NR]
     !       AUB_PAV_NR  =AUB:PAV_NR[VIETA_NR]
           SAV_POSITION=POSITION(AU_BILDE)
           SET(AUB:DAT_KEY,AUB:DAT_KEY)
           NEXT(AU_BILDE)  !PIRMO IGNORÇJAM
           LOOP SOLI TIMES
              NEXT(AU_BILDE)
              IF ERROR() !JÂCER, BEIGUSIES TABULA
                 KLUDA(0,'Beigusies tabula, jâuzbûvç...')
                 BREAK
              .
              IF ~AUB:PLKST_P  !DIENAS VIRSRAKSTS
                 KLUDA(0,'Nepiecieðama vçl 1 diena...',,1)
                 SOLI+=1
                 CYCLE
              .
              IF AUB:STATUSS[VIETA_NR]=9 !PROGULS
                 KLUDA(0,FORMAT(AUB:DATUMS,@D06.)&' '&FORMAT(AUB:PLKST_P,@T1)&' vietâ '&clip(VIETA_NR)&' nav paredzçts strâdât ')
                 SOLI+=1
                 CYCLE
              .
              IF AUB:AUT_TEXT[VIETA_NR] AND ~(SAV_AUT_TEXTX=AUB:AUT_TEXT[VIETA_NR]) !KÂDS CITS VAI MAINÎTS IDENTIFY
                 CASE AUB:STATUSS[VIETA_NR]
                 OF 1
                 OROF 4
                 OROF 5
                 OROF 6
                     KLUDA(0,FORMAT(AUB:DATUMS,@D06.)&' '&FORMAT(AUB:PLKST_P,@T1)&' vieta '&clip(VIETA_NR)&' jau rezervçta '&|
                     AUB:AUT_TEXT[VIETA_NR],3)
                 OF 2
                    KLUDA(0,FORMAT(AUB:DATUMS,@D06.)&' '&FORMAT(AUB:PLKST_P,@T1)&' vietâ '&clip(VIETA_NR)&' jau FIKSÇTI DARBI '&|
                    AUB:AUT_TEXT[VIETA_NR],4)
                 .
                 IF ~KLU_DARBIBA
                    BREAK
                 .
              ...
              .
           .
           RESET(AU_BILDE,SAV_POSITION)
           NEXT(AU_BILDE)
        .
     .
  ELSIF LOCALREQUEST=INSERTRECORD
     IF AUB:DATUMS%7+1 = 1  AND AUB:PLKST_P=0     !VIRSRAKSTS SVÇTDIENA
        AUB_STATUSS=9
     ELSIF AUB:DATUMS%7+1 = 7 AND AUB:PLKST_P=0   !VIRSRAKSTS SESTDIENAI
        AUB_STATUSS=9
     ELSIF AUB:PLKST_P=0                          !VIRSRAKSTS DARBADIENAI
        AUB_STATUSS=9
     ELSE                                         !JAUNS PLKST
        AUB_STATUSS=0
     .
     LOOP J#=1 TO 20  !VAJADZÇS 22
...
PrepareProcedure ROUTINE
  ...
  QuickWindow{PROP:MinHeight}=QuickWindow{PROP:Height}
  WinResize.Initialize(AppStrat:Spread)
  INIRestoreWindow('UpdateAU_BILDE','winlats.INI')
  WinResize.Resize
  ?AUB:DATUMS{PROP:Alrt,255} = 734
  ?AUB:PLKST_P{PROP:Alrt,255} = 734
!---------------------------------------------------------------------------
ProcedureReturn ROUTINE
!|
!| This routine provides a common procedure exit point for all template
...
!|
  ...
  END
  IF WindowOpened
    INISaveWindow('UpdateAU_BILDE','winlats.INI')
    CLOSE(QuickWindow)
  END
  IF LocalResponse
    GlobalResponse = LocalResponse
...
PERFORMPAVAD     ROUTINE
 IF AUB:STATUSS[VIETA_NR]=9 !DIENAS VIRSRAKSTS VAI VIETA DZELTENA
...
!       IF ~AUB:AUT_TEXT2[VIETA_NR] THEN AUB:AUT_TEXT2[VIETA_NR]=GETAUTO(PAV:VED_NR,8)&' '&PAV:NOKA.
       ...
       ELSE
          ...
          LOOP
             NEXT(AU_BILDE)
             IF ERROR() OR ~AUB:AUT_TEXT[VIETA_NR] OR ~(AUB:AUT_TEXT[VIETA_NR]=SAV_AUT_TEXTX) THEN BREAK.
             AUB:PAV_NR[VIETA_NR]=PAV:U_NR
             IF RIUPDATE:AU_BILDE()
                KLUDA(24,'GRAFIKS')
             .
...
HistoryField  ROUTINE
  CASE FOCUS()
    OF ?AUB:DATUMS
      AUB:DATUMS = History::AUB:Record.DATUMS
    OF ?AUB:PLKST_P
      AUB:PLKST_P = History::AUB:Record.PLKST_P
  END
  DISPLAY()
!---------------------------------------------------------------
PrimeFields ROUTINE
