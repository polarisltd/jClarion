...
import clarion.winla_sf.Cr_group;
import clarion.winla_sf.Cs_group;
...
{
	public static Cr_group cr_group;
	public static Cs_group cs_group;
	static {
	...
	public static void __static_init() {
		cr_group=new Cr_group();
		cs_group=new Cs_group();
	}
	...
	{
		ClarionString inputlowlimit=Clarion.newString(255);
		ClarionString inputhighlimit=Clarion.newString(255);
		ClarionNumber currentstep=Clarion.newNumber().setEncoding(ClarionNumber.LONG);
		ClarionNumber currentval=Clarion.newNumber().setEncoding(ClarionNumber.LONG);
		ClarionString lowcharacter=Clarion.newString(1);
		ClarionString highcharacter=Clarion.newString(1);
		ClarionString nextcharacter=Clarion.newString(1);
		ClarionNumber _this_number=Clarion.newNumber();
		ClarionNumber _that_number=Clarion.newNumber();
		if (listtype==null) {
			listtype.setValue(Scrollsort.ALLOWALPHA);
		}
		Winla_sf.cs_group.sortcharactercount.setValue(0);
		Winla_sf.cs_group.sortcharacters.setValue("");
		for (currentstep.setValue(1);currentstep.compareTo(255)<=0;currentstep.increment(1)) {
			nextcharacter.setValue(ClarionString.chr(currentstep.intValue()));
			if (Winla_sf.cs_group.numericcharacters.inString(nextcharacter.toString(),1,1)!=0) {
				if ((listtype.intValue() & Scrollsort.ALLOWNUMERIC)!=0) {
					setupstringstops_addsortcharacter(nextcharacter);
				}
			}
			else if (Winla_sf.cs_group.overridecharacters.inString(nextcharacter.toString(),1,1)!=0) {
				if ((listtype.intValue() & Scrollsort.ALLOWALT)!=0) {
					setupstringstops_addsortcharacter(nextcharacter);
				}
			}
			else if (nextcharacter.isAlpha()) {
				if ((listtype.intValue() & Scrollsort.ALLOWALPHA)!=0) {
					if (nextcharacter.isUpper()) {
						setupstringstops_addsortcharacter(nextcharacter);
					}
					else if ((listtype.intValue() & Scrollsort.CASESENSITIVE)!=0) {
						setupstringstops_addsortcharacter(nextcharacter);
					}
				}
			}
		...
		inputlowlimit.setValue(processlowlimit.sub(1,inputstringsize.intValue()));
		inputhighlimit.setValue(processhighlimit.sub(1,inputstringsize.intValue()));
		if (Clarion.newNumber(inputhighlimit.clip().len()).compareTo(inputstringsize)<0) {
			nextcharacter.setValue(Winla_sf.cs_group.sortcharacters.sub(Winla_sf.cs_group.sortcharactercount.intValue(),1));
			if (inputstringsize.add(1).subtract(inputhighlimit.len()).compareTo(0)>0) {
				inputhighlimit.setValue(inputhighlimit.clip().concat(nextcharacter.all(inputstringsize.add(1).subtract(inputhighlimit.len()).intValue())));
			}
		}
		final ClarionNumber loop_2=inputstringsize.like();for (currentstep.setValue(1);currentstep.compareTo(loop_2)<=0;currentstep.increment(1)) {
			if (currentstep.compareTo(inputlowlimit.len())>0) {
				break;
			}
			lowcharacter.setValue(inputlowlimit.sub(currentstep.intValue(),1));
			highcharacter.setValue(inputhighlimit.sub(currentstep.intValue(),1));
			if (!lowcharacter.equals(highcharacter)) {
				break;
			}
		}
		Winla_sf.cs_group.stringrootlen.setValue(currentstep.subtract(1));
		Winla_sf.cs_group.stringroot.setValue(inputlowlimit.sub(1,Winla_sf.cs_group.stringrootlen.intValue()));
		Winla_sf.cs_group.lowstring.setValue(inputlowlimit.sub(currentstep.intValue(),4));
		Winla_sf.cs_group.highstring.setValue(inputhighlimit.sub(currentstep.intValue(),4));
		if ((listtype.intValue() & Scrollsort.ALLOWALPHA)!=0 && !((listtype.intValue() & Scrollsort.CASESENSITIVE)!=0)) {
			Winla_sf.cs_group.lowstring.setValue(Winla_sf.cs_group.lowstring.upper());
			Winla_sf.cs_group.highstring.setValue(Winla_sf.cs_group.highstring.upper());
		}
		currentstep.setValue(0);
		Winla_sf.cs_group.lowvalue.setValue(0);
		Winla_sf.cs_group.highvalue.setValue(0);
		while (true) {
			currentstep.increment(1);
			if (currentstep.equals(5)) {
				break;
			}
			Winla_sf.cs_group.lowvalue.setValue(Winla_sf.cs_group.lowvalue.multiply(Winla_sf.cs_group.sortcharactercount.add(1)));
			Winla_sf.cs_group.highvalue.setValue(Winla_sf.cs_group.highvalue.multiply(Winla_sf.cs_group.sortcharactercount.add(1)));
			_this_number.setValue(_that_number);
			if (currentstep.compareTo(Winla_sf.cs_group.lowstring.clip().len())<=0) {
				nextcharacter.setValue(Winla_sf.cs_group.lowstring.sub(currentstep.intValue(),1));
				currentval.setValue(Winla_sf.cs_group.sortcharacters.inString(nextcharacter.toString(),1,1));
				Winla_sf.cs_group.lowvalue.increment(currentval);
			}
			if (currentstep.compareTo(Winla_sf.cs_group.highstring.clip().len())<=0) {
				nextcharacter.setValue(Winla_sf.cs_group.highstring.sub(currentstep.intValue(),1));
				currentval.setValue(Winla_sf.cs_group.sortcharacters.inString(nextcharacter.toString(),1,1));
				Winla_sf.cs_group.highvalue.increment(currentval);
			}
		}
		Winla_sf.cs_group.stepvalue.setValue(Winla_sf.cs_group.highvalue.subtract(Winla_sf.cs_group.lowvalue).divide(99));
		Winla_sf.cs_group.currentstep.setValue(0);
		return;
	}
	public static void setupstringstops_addsortcharacter(ClarionString nextcharacter)
	{
		if (Winla_sf.cs_group.sortcharactercount.boolValue()) {
			Winla_sf.cs_group.sortcharacters.setValue(Winla_sf.cs_group.sortcharacters.sub(1,Winla_sf.cs_group.sortcharactercount.intValue()).concat(nextcharacter));
		}
		else {
			Winla_sf.cs_group.sortcharacters.setValue(nextcharacter);
		}
		Winla_sf.cs_group.sortcharactercount.increment(1);
	}
	public static ClarionString nextstringstop()
	{
		ClarionString stepstring=Clarion.newString(4);
		ClarionNumber currentstop=Clarion.newNumber().setEncoding(ClarionNumber.LONG);
		ClarionNumber currentstep=Clarion.newNumber().setEncoding(ClarionNumber.LONG);
		ClarionNumber currentval=Clarion.newNumber().setEncoding(ClarionNumber.LONG);
		stepstring.setValue("");
		Winla_sf.cs_group.currentstep.increment(1);
		{
			ClarionNumber case_1=Winla_sf.cs_group.currentstep;
			boolean case_1_break=false;
			if (case_1.equals(1)) {
				stepstring.setValue(Winla_sf.cs_group.lowstring);
				case_1_break=true;
			}
			if (!case_1_break && case_1.equals(100)) {
				stepstring.setValue(Winla_sf.cs_group.highstring);
				case_1_break=true;
			}
			if (!case_1_break) {
				currentstop.setValue(Winla_sf.cs_group.lowvalue.add(Winla_sf.cs_group.stepvalue.multiply(Winla_sf.cs_group.currentstep.subtract(1))));
				for (currentstep.setValue(1);currentstep.compareTo(4)<=0;currentstep.increment(1)) {
					currentval.setValue(currentstop.modulus(Winla_sf.cs_group.sortcharactercount.add(1)));
					currentstop.setValue(currentstop.subtract(currentval).divide(Winla_sf.cs_group.sortcharactercount.add(1)));
					if (currentval.compareTo(0)>0 && currentval.compareTo(Winla_sf.cs_group.sortcharacters.len())<=0) {
						stepstring.setValue(Winla_sf.cs_group.sortcharacters.sub(currentval.intValue(),1).concat(stepstring));
					}
				}
			}
		}
		if (Winla_sf.cs_group.stringrootlen.boolValue() && Clarion.newNumber(Winla_sf.cs_group.stringroot.len()).compareTo(Winla_sf.cs_group.stringrootlen)<=0) {
			return Clarion.newString(Winla_sf.cs_group.stringroot.sub(1,Winla_sf.cs_group.stringrootlen.intValue()).concat(stepstring));
		}
		else {
			return stepstring.like();
		}
	}
	public static void setuprealstops(ClarionReal inputlowlimit,ClarionReal inputhighlimit)
	{
		Winla_sf.cr_group.lowlimit.setValue(inputlowlimit);
		Winla_sf.cr_group.highlimit.setValue(inputhighlimit);
		Winla_sf.cr_group.stepvalue.setValue(inputhighlimit.subtract(inputlowlimit).divide(99));
		Winla_sf.cr_group.currentstep.setValue(0);
		return;
	}
	public static ClarionReal nextrealstop()
	{
		Winla_sf.cr_group.currentstep.increment(1);
		{
			ClarionNumber case_1=Winla_sf.cr_group.currentstep;
			boolean case_1_break=false;
			if (case_1.equals(1)) {
				return Winla_sf.cr_group.lowlimit.like();
				// UNREACHABLE! :case_1_break=true;
			}
			if (!case_1_break && case_1.equals(100)) {
				return Winla_sf.cr_group.highlimit.like();
				// UNREACHABLE! :case_1_break=true;
			}
			if (!case_1_break) {
				return Winla_sf.cr_group.lowlimit.add(Winla_sf.cr_group.stepvalue.multiply(Winla_sf.cr_group.currentstep.subtract(1))).getReal();
			}
		}
		return Clarion.newReal();
	}
