...
public class Eipmanager extends Windowmanager
{
	...
	public ClarionNumber mouseLeftIndex;
	public Eipmanager()
	{
		...
		tab=null;
		mouseLeftIndex=Clarion.newNumber().setEncoding(ClarionNumber.BYTE);
	}
	...
	public ClarionNumber kill()
	{
		...
		if (this.mouseLeftIndex.boolValue()) {
			Clarion.getControl(this.listControl).setProperty(Prop.ALRT,this.mouseLeftIndex,Constants.MOUSELEFT);
		}
	...
	{
		if (this.eq.control.get()==null || Clarion.getControl(this.listControl).getProperty(Proplist.WIDTH,this.column).equals(0)) {
	...
	public ClarionNumber run(ClarionNumber req)
	{
		...
		this.mouseLeftIndex.setValue(0);
		for (i.setValue(1);i.compareTo(254)<=0;i.increment(1)) {
			if (Clarion.getControl(this.listControl).getProperty(Prop.ALRT,i).equals(Constants.MOUSELEFT)) {
				this.mouseLeftIndex.setValue(i);
				Clarion.getControl(this.listControl).setProperty(Prop.ALRT,i,0);
				break;
			}
		}
	...
	public ClarionNumber takeAcceptAll()
	{
		ClarionNumber i=Clarion.newNumber().setEncoding(ClarionNumber.UNSIGNED);
		i.setValue(1);
		while (Clarion.getControl(this.listControl).getProperty(Proplist.EXISTS,i).boolValue()) {
			this.eq.get(i);
			if (!(CError.errorCode()!=0)) {
				this.fields.list.get(i);
				if (!(this.eq.control.get()==null)) {
					CWin.update(this.eq.control.get().feq.intValue());
					if (this.eq.control.get().req.boolValue()) {
						if (!this.fields.list.right.boolValue()) {
							this.clearColumn();
							this.column.setValue(i);
							this.again.setValue(1);
							this.resetColumn();
							return Clarion.newNumber(Constants.FALSE);
						}
					}
					if (this.eq.control.get().takeAccepted(Clarion.newNumber(Editaction.FORWARD)).equals(Editaction.CANCEL)) {
						this.clearColumn();
						this.column.setValue(i);
						this.again.setValue(1);
						this.resetColumn();
	...
	public void takeAction(ClarionNumber action)
	{
		...
		if (action.boolValue() && !action.equals(Editaction.CANCEL)) {
			action.setValue(this.eq.control.get().takeAccepted(action.like()));
		}
		{
