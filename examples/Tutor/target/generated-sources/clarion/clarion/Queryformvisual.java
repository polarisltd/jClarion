...
public class Queryformvisual extends Queryvisual
{
	...
	public ClarionNumber init()
	{
		...
		for (i.setValue(1);i.compareTo(this.qfc.fields.records())<=0;i.increment(1)) {
			...
			Clarion.getControl(control).setProperty(Prop.TRN,Constants.TRUE);
			...
			this.setText(Clarion.newNumber(Feq.STARTCONTROL).add(i.multiply(3).subtract(1)).getNumber(),this.qfc.fields.middle.like());
		}
	...
	public void init_Positions(ClarionNumber maxPW,ClarionNumber maxW)
	{
		...
		if (fullWidth.compareTo(minWidth)<0) {
			...
			if (fullWidth.compareTo(lMargin.add(Clarion.getControl(Feq.OK).getProperty(Prop.WIDTH)).add(Constants.HSPACING).add(Clarion.getControl(Feq.CANCEL).getProperty(Prop.WIDTH)).add(Constants.HSPACING).add(Clarion.getControl(Feq.CLEAR).getProperty(Prop.WIDTH)).add(lMargin))<0) {
				fullWidth.setValue(lMargin.add(Clarion.getControl(Feq.OK).getProperty(Prop.WIDTH)).add(Constants.HSPACING).add(Clarion.getControl(Feq.CANCEL).getProperty(Prop.WIDTH)).add(Constants.HSPACING).add(Clarion.getControl(Feq.CLEAR).getProperty(Prop.WIDTH)).add(lMargin));
			}
		}
		for (i.setValue(1);i.compareTo(this.qfc.fields.records())<=0;i.increment(1)) {
	...
	public void setText(ClarionNumber control,ClarionString entryText)
	{
		...
		if (entryText.len()!=0) {
			if (entryText.stringAt(1).equals("^")) {
				entryText.setValue(entryText.stringAt(2,entryText.len()));
			}
		}
	...
	{
		return Clarion.newNumber(Constants.QBEFORMBASED);
	}
	public void updateFields()
	{
		...
		for (i.setValue(1);i.compareTo(this.qfc.fields.records())<=0;i.increment(1)) {
			...
			if (Clarion.getControl(control).getProperty(Prop.SCREENTEXT).boolValue()) {
				this.qfc.fields.middle.setValue(Abquery.makeOperator(Clarion.getControl(control).getProperty(Prop.SCREENTEXT).getString(),Clarion.getControl(control.subtract(1)).getProperty(Prop.VALUE).getString().clip()));
			}
			else {
				this.qfc.fields.middle.clear();
			}
			this.qfc.fields.put();
		}
	}
