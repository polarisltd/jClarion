! $Header: /0192-7/XMLSupport/src/cpxml.clw 46    8/11/03 10:36a Mal $
!****************************************************************************
!  FILE..........: cpxml.clw
!  AUTHOR........: 
...
cc                 byte
    ...
    loop row = 1 to records(que)
       ...
       !if duplicate name found 
           SetLastError(CPXMLErr:DuplicateName)
           return false
        end
        cc=IsCorrectName(que.XMLName)!test the XML name for correctness
        if(not cc)
          SetLastError(CPXMLErr:IllegalName)
          return false
        end
...
doc &Document,auto
  ...
  if(doc&=null)
     SetLastError(CPXMLErr:XMLReadFail)
     return null
  else
     SetLastError(CPXMLErr:NoError)
     return doc
...
cc byte
  ...
  if(cc)
    ...
    SetLastError(CPXMLErr:XMLWriteFail)
  end
  return cc
...
!retVal cstring('row')
  ...
  return 'row'
!===  ====  ========
AppendElement procedure(*Document doc, *Node parent, string ELabel)
...
cs CStringClass
  ...
  return E
!===  ====  ========
AppendElementAndText procedure(*Document doc, *Node parent, string ELabel, string TData, byte base64=false)
...
cs CStringClass
  ...
  code
  E &= AppendElement(doc, parent, ELabel)
  if not E &= null
    if base64
  ...
  return E
!===  ====  ========
AppendElementAndAttribute procedure(*Document doc, *Node parent, string ELabel, string attribute, string value)
